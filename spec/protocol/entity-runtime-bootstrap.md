...
- fail closed instead of rebuilding map-index presence from a surviving directory entry when the opposite kind still owns that runtime entity ID or client-visible actor `VID` in the map index, so directory-to-map repair cannot shadow cross-kind remnants
