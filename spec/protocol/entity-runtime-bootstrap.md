...
- reject cross-kind player/static collisions when the opposite-kind primary entity index survives but the map bucket is missing, so a partial static-actor map-index teardown cannot let player registration or movement claim the same entity ID
