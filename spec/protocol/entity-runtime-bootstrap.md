...
- repair player-directory presence from surviving map-index presence during runtime lookups by entity ID, client-visible `VID`, exact character name, or deterministic connected-player snapshot/scope reads, so a partial teardown or repair that loses the directory entry does not strand whisper/scope/introspection/visibility lookups until the next update path
- fail closed instead of repairing player-directory presence when a directory-only static actor still owns the same runtime entity ID or client-visible static-actor `VID`, so lookup repair cannot shadow opposite-kind remnants after partial map-index teardown
- suppress ambiguous player-directory rows from connected-player snapshot/scope reads and secondary `VID` / exact-name lookups when a static-actor directory survivor owns the same runtime entity ID or projected client-visible `VID`; direct entity-ID lookup remains available for explicit cleanup in entity-ID collision cases
...
- fail closed instead of repairing non-player-directory presence when a directory-only player still owns the same runtime entity ID or client-visible player `VID`, so static-actor lookup repair cannot shadow opposite-kind remnants after partial map-index teardown
- omit directory-only static actors from all-static snapshots and scope visibility composition when a surviving player directory or map-index entry already owns that actor's runtime entity ID or projected client-visible `VID`, leaving the static directory entry intact for explicit cleanup rather than leaking a ghost non-player actor to `/local/static-actors`/visibility-style readers
- fail closed for direct static-actor lookup by entity ID or client-visible `VID` in that same collision shape, so interaction/target readers cannot resolve a directory-only non-player actor over a surviving map-index player `VID`
...
- reject cross-kind player/static collisions when the opposite-kind directory entry survives but map-index presence is missing, so player registration/update and static-actor registration/update cannot silently overwrite a partially torn-down actor that still owns the shared runtime entity ID namespace
...
- when duplicate map-only same-kind buckets survive for the same entity ID and at least one bucket is already canonical for the stored snapshot, lookup/read repair preserves the canonical bucket before pruning the stale source buckets instead of picking the lowest stale source map by iteration order
- when duplicate map-only player/static buckets survive and the map index still remembers the actor's previously committed effective map, lookup/read repair preserves that remembered effective-map bucket before pruning stale source buckets; this prevents canonical-looking older snapshots from overriding the last committed map bucket after primary-index loss
- expose an all-player map-index snapshot that includes surviving map-only player presence, allowing entity-registry `PlayerCharacters()` and scope readers to repair missing player-directory entries before visibility/social/introspection code decides that a live map-index player has disappeared
- repair player/static map-index presence from surviving directory entries before direct map readers (`MapCharacters`, `StaticActors`) and full occupancy snapshots return, so a partial teardown or repair that cleared only the map index does not make `/local/maps`-style introspection drop otherwise directory-live actors
- fail closed instead of rebuilding map-index presence from a surviving primary/directory entry when the opposite kind still owns that runtime entity ID or client-visible actor `VID` in the map index, so directory-to-map and primary-index-to-map repair cannot shadow cross-kind remnants
- reject map-index player registration/update when any surviving static-actor primary entry or map bucket owns the requested player `VID`, and reject static-actor registration/update when any surviving player primary entry or map bucket owns the actor's projected visibility `VID`
- reject player map-index registration/update when the player identity has a malformed runtime name according to the same directory-owned validator, so map occupancy cannot retain a player that exact-name lookup and persisted account/ticket stores would reject
- keep primary player/static entity lookups available in that collision shape, but suppress their automatic map-presence repair until an explicit cleanup resolves the opposite-kind map bucket; map-occupancy snapshots and direct per-map readers therefore preserve the surviving opposite-kind map bucket instead of surfacing both actors under one shared entity ID or one shared client-visible `VID`
- when the primary entry already has a same-kind map bucket and a later opposite-kind map bucket survives for the same client-visible `VID`, direct map readers and map-occupancy snapshots omit the blocked primary bucket instead of returning duplicate visible-carrier occupancy; the primary lookup remains available for explicit cleanup or later repair
- when both sides survive only as map-index buckets and no primary/directory entry remains authoritative, direct map readers, all-player/all-static snapshots, and full occupancy snapshots suppress both ambiguous map-only sides if they collide by runtime entity ID or client-visible static/player `VID`; this fail-closed state avoids inventing a winner for cleanup-order damage that lost every authoritative primary index
...
The loopback operator surface exposes both the deterministic full map list (`GET /local/maps`) and an exact per-map lookup (`GET /local/maps/{map_index}`) that returns the same single-row occupancy shape for occupied maps while failing closed with `404` for currently empty maps.
The tolerant cleanup rules are deliberately narrow: they make reconnect/close/register/read repair idempotent across owned runtime indexes without treating stale map/index remnants as a live session. Same-kind map-bucket repair only follows the actor snapshot's own effective map; directory-to-map repair only restores same-kind directory survivors when no opposite-kind map-index remnant owns that identity; cross-kind primary-index and bucket remnants are still authoritative enough to block conflicting registration because player and static-actor entity IDs share one runtime identity space.
