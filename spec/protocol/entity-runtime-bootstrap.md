...
- omit directory-only static actors from all-static snapshots and scope visibility composition when surviving map-index player presence already owns that actor's projected client-visible `VID`, leaving the static directory entry intact for explicit cleanup rather than leaking a ghost non-player actor to `/local/static-actors`/visibility-style readers
