...
- the minimal session/runtime packet path now freezes that partial-merge burst and persisted account snapshot too: source and target counts are updated, both cells emit `ITEM_UPDATE`, the source item quickslot is preserved because the source stack remains occupied, and target item quickslots are also preserved because the target cell remains occupied by the same item instance with only its count changed
