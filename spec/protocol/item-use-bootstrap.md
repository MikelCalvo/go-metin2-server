...
- the minimal session/runtime packet path now freezes that partial-merge burst and persisted account snapshot too: source and target counts are updated, both cells emit `ITEM_UPDATE`, the source item quickslot is preserved because the source stack remains occupied, and any item quickslot referencing the target cell is deleted because the target stack changed under the dragged source item
- count-only partial refreshes use the existing `ITEM_UPDATE` packet shape for both changed carried cells, do not rewrite source item quickslots, delete target item quickslots, and still preserve non-item quickslots that happen to reference the target byte slot
