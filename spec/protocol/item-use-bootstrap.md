...
- the minimal session/runtime packet path now freezes that full-merge burst and the persisted account snapshot: source stack removed, target stack refreshed with the merged count, all item quickslots for the removed source cell deleted in deterministic quickslot-position order, non-item quickslots with the same byte slot preserved, and target item quickslots left stable even when source and target cells both had item quickslot bindings before the merge
