...
- the minimal session/runtime packet path now freezes that full-merge burst and the persisted account snapshot: source stack removed, target stack refreshed with the merged count through the count-only update family, the target `ITEM_UPDATE` preserves template-authored display sockets/attributes while changing only the count, all item quickslots for the removed source cell deleted in deterministic quickslot-position order, non-item quickslots with the same byte slot preserved, and target item quickslots left stable even when source and target cells both had item quickslot bindings before the merge
...
- if the target has only partial room, the source slot is refreshed with its remainder and the target slot is refreshed at its template `max_count`; both count-only refreshes preserve the resolved template's authored display `sockets` / `attributes`
- the minimal session/runtime packet path now freezes that partial-merge burst and persisted account snapshot too: source and target counts are updated, both cells emit `ITEM_UPDATE` with template-authored display arrays, the source item quickslot is preserved because the source stack remains occupied, and target item quickslots are also preserved because the target cell remains occupied by the same item instance with only its count changed
- count-only partial refreshes use the existing `ITEM_UPDATE` packet shape for both changed carried cells, preserve the resolved template's authored display `sockets` / `attributes`, and do not rewrite source, target, or unrelated non-item quickslots
...
- the resolved template carries authored `confirm_when_use = true`; the minimal session/runtime packet path freezes this as no-mutation behavior with inventory, quickslots, point values, and successful-use placeholder chat unchanged until a later slice owns confirmation; when the template authors non-empty `use_reject_message`, the rejection emits exactly one self-only `CHAT_TYPE_INFO` with that text, otherwise it preserves the older no-frame rejection
...
- the first live `ITEM_USE_TO_ITEM` runtime case now reuses the carried-stack merge path for compatible same-`vnum` inventory stacks, persists the merged inventory, refreshes already-known target stacks with `ITEM_UPDATE`, and deliberately avoids falling back to the normal consumable `use_effect`
