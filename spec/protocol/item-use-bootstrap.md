...
- the minimal session/runtime packet path now freezes that partial-merge burst and persisted account snapshot too: source and target counts are updated, both cells emit `ITEM_UPDATE`, the source item quickslot is preserved because the source stack remains occupied, and all item quickslots referencing the target cell are deleted in deterministic quickslot-position order because the target stack changed under the dragged source item
- count-only partial refreshes use the existing `ITEM_UPDATE` packet shape for both changed carried cells, do not rewrite source item quickslots, delete all target item quickslots, and still preserve non-item quickslots that happen to reference the target byte slot
