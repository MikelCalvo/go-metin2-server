...
- damage, targeting, or death state beyond the currently documented training-dummy/death/reward bootstrap slices; in-place static-actor updates preserve the actor's authored/bootstrap death reward unless a later explicit reward-editing slice changes it
  ...
  - that cross-kind player-registration guard must also consult static-actor map presence, so a partially torn-down static actor that has lost its non-player directory entry but still occupies a map cannot be shadowed by a player using the same client-visible `VID`
