...
- operator/runtime map-occupancy, exact map-local static-actor subset, and static-actor snapshots that can now surface those actors through `internal/worldruntime/scopes.go`, including resolved combat profile rank metadata for combat-profile actors
...
- when a player enters `GAME`, already-visible bootstrap static actors can be appended to that enter-game result with deterministic `CHARACTER_ADD`, `CHAR_ADDITIONAL_INFO`, and `CHARACTER_UPDATE` frames; combat-profile actors copy their resolved profile `level` into `CHAR_ADDITIONAL_INFO.level` and insert as `CHARACTER_ADD.type = 0` (`CHAR_TYPE_MONSTER`), while ordinary interaction/static actors without a resolved combat profile insert as `CHARACTER_ADD.type = 1` (`CHAR_TYPE_NPC`); runtime/operator snapshots expose the resolved profile `rank` as descriptor metadata only
...
- non-empty `name` or stable label
...
- damage, targeting, or death state beyond the currently documented training-dummy/death/reward bootstrap slices; in-place static-actor updates preserve the actor's authored/bootstrap death reward unless a later explicit reward-editing slice changes it
  ...
  - if a bad partial-teardown or repair path leaves duplicate map-bucket presence for the same actor entity ID, later register, lookup, in-place update, remove, map-occupancy snapshot reads, and direct per-map static-actor reads must prune stale same-kind map buckets before inserting or returning the new effective-map presence; player movement/update and direct per-map player reads follow the same same-kind pruning rule so map occupancy must never keep ghost actors for the same entity ID on older maps
  - if duplicate map-only same-kind player/static buckets survive after the primary entity index was lost, lookup/read repair must preserve a surviving bucket that is already canonical for its stored snapshot before pruning stale source buckets, so a current map-only actor is not replaced by an older lower-map ghost during repair
  - if duplicate map-only same-kind player/static buckets survive and the map index still has an effective-map memory for that entity, lookup/read/remove repair must preserve the remembered effective-map bucket first, even when an older stale bucket is also internally canonical for its stored snapshot; effective-map memory represents the last committed occupancy bucket until a surviving primary entity snapshot supersedes it, and tolerant removal must return that remembered occupant while clearing every duplicate bucket plus the effective-map memory
  - map-occupancy snapshot reads must apply the same remembered effective-map repair for map-only static actors before returning `/local/maps`-style occupancy, so stale source buckets do not survive in operator/introspection views after primary-index loss
  - if a surviving same-kind map bucket stores a player or static-actor snapshot whose own effective `MapIndex` points somewhere else, later lookup/read paths must repair that misplaced presence into the snapshot's canonical map bucket instead of continuing to surface the actor on the stale map; cross-kind player/static remnants still fail closed rather than being auto-repaired across kinds
  - if a surviving primary player/static entity entry is paired with an opposite-kind map bucket for the same entity ID, map-occupancy snapshots and direct per-map readers must not rebuild the primary entry into another map bucket; the primary lookup remains available, but its automatic occupancy repair is suppressed until explicit cleanup removes the opposite-kind remnant
  - if a player movement/update path encounters surviving static-actor map presence for the same entity ID after the static actor's primary index has already disappeared, the player update must fail closed and leave both the player and static map buckets intact rather than silently overwriting cross-kind occupancy
  ...
  - if a stale visibility-VID index entry points at a surviving static actor but the key no longer matches that actor's canonical encodable visibility VID, `ByVID` must prune the non-canonical alias and fail closed for that lookup while preserving the actor's current canonical visibility lookup; later register/update attempts by the actor that now canonically owns that VID may prune the stale alias instead of failing on a false conflict
  ...
  - if that orphaned or non-canonical visibility-VID entry would otherwise conflict with later registration or in-place update of the real actor for that VID, the non-player directory must prune it and allow the current actor to claim the VID; only live conflicting actor ownership should fail closed
  - if map-index presence survives after a non-player-directory entry was cleared first, runtime lookup by static-actor entity ID, static-actor visibility `VID`, and all-static/scope snapshot composition must repair the missing directory entry from that map-index presence instead of hiding the actor from visibility, interaction, or target readers until a later edit
  - if that map-index repair would shadow a directory-only player that still owns the same runtime entity ID or client-visible `VID`, the static-actor lookup must fail closed and leave both remnants intact until an explicit cleanup path resolves the conflict
  ...
  - file-backed static actor snapshots must also fail closed when `entity_id` is zero or does not fit the current static-actor visibility `VID` projection (`uint32`), because the bootstrap runtime derives the visible actor `VID` directly from that runtime entity ID
  ...
  - that shared entity-ID namespace remains authoritative during partial teardown in both directions: player registration/update must reject a surviving directory-only static actor for the same entity ID, and static-actor registration/update must reject a surviving directory-only player for the same entity ID, even when map-index presence for the opposite kind has already been lost
  ...
  - map-index registration/update paths must enforce the same shared player/static visible-ID namespace even below the entity registry: player registration/update rejects surviving static-actor primary entries or map buckets whose projected visibility `VID` equals the requested player `VID`, while static-actor registration/update rejects surviving player primary entries or map buckets whose `VID` equals the actor's projected visibility `VID`
  - map-index lookup/read repair must also fail closed in those visible-ID collision shapes; the primary lookup remains available, but automatic map-presence repair is suppressed so map occupancy preserves the surviving opposite-kind bucket until explicit cleanup resolves the conflict
  - all-static and scope visibility snapshots must also fail closed in that collision shape: a directory-only static actor whose projected visible `VID` is already owned by surviving map-index player presence is omitted from snapshot/visibility output while its directory entry remains available for explicit cleanup
  - direct static-actor lookup by entity ID or visibility `VID` must fail closed in that same collision shape, so interaction/target readers cannot resolve a directory-only non-player actor over a surviving map-index player `VID`
...
- runtime/operator map-occupancy snapshots and exact map-local static-actor subset lookups can now surface those static actors on their effective maps
