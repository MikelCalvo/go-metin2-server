...
- damage, targeting, or death state beyond the currently documented training-dummy/death/reward bootstrap slices; in-place static-actor updates preserve the actor's authored/bootstrap death reward unless a later explicit reward-editing slice changes it
  ...
  - if duplicate map-only same-kind player/static buckets survive and the map index still has an effective-map memory for that entity, lookup/read repair must preserve the remembered effective-map bucket first, even when an older stale bucket is also internally canonical for its stored snapshot; effective-map memory represents the last committed occupancy bucket until a surviving primary entity snapshot supersedes it
