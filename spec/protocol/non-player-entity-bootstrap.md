...
- damage, targeting, or death state beyond the currently documented training-dummy/death/reward bootstrap slices; in-place static-actor updates preserve the actor's authored/bootstrap death reward unless a later explicit reward-editing slice changes it
  ...
  - if a bad partial-teardown or repair path leaves duplicate map-bucket presence for the same actor entity ID, later register, lookup, in-place update, remove, or map-occupancy snapshot reads must prune stale same-kind map buckets before inserting or returning the new effective-map presence; player movement/update follows the same same-kind pruning rule so map occupancy must never keep ghost actors for the same entity ID on older maps
