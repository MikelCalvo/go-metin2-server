...
6. `count = 0` moves either target an empty destination slot or a compatible occupied destination stack;
7. the destination stack, when occupied for a merge, is not locked;
8. occupied-destination merges require the same `vnum` and must not overflow `max_count`;
9. zero-count occupied-destination merges move as much of the source stack as the destination can accept, capped by `max_count`, and leave a source remainder when the destination fills first;
10. counted moves do not exceed the source stack count or the relevant template `max_count`;
11. if a counted move equals the source stack count and the destination is compatible, it merges into the destination stack instead of swapping items.
