...
- Loaded item-template metadata is authoritative for stack compatibility, selected-character restrictions, and transfer guards: templates marked `anti_stack`, `anti_drop`, `anti_give`, `anti_sell`, non-stackable, malformed, or whose `max_count` exceeds the currently owned one-byte item refresh count range (`> 255`) are not merged by `CG::ITEM_MOVE`, and templates with authored job/sex restrictions or a `min_level` above the selected character's current level are likewise rejected before stack mutation, even when source and destination share the same `vnum`; if an explicitly authored item-template snapshot is loaded, same-`vnum` occupied-destination merge attempts with missing source-template metadata fail closed instead of falling back to an unbounded merge.
- The carried-stack merge/swap path also requires exactly one live source item and at most one live target item at the addressed cells. Duplicate live occupancy of either cell fails closed with no frames and no inventory/quickslot persistence change before any merge/swap result is built.
...
9. occupied-destination merges require exactly one live source occupant and exactly one live target occupant, the same `vnum`, a valid loaded template that is stackable, not `anti_stack`, not `anti_drop`, not `anti_give`, not `anti_sell`, and must not overflow `max_count`; malformed loaded templates, including `max_count > 255`, fail closed for same-`vnum` occupied-destination merges before any item refresh frame can truncate counts; occupied-destination requests with duplicate source or target occupancy fail closed before merge/swap mutation; when an explicitly authored item-template snapshot is loaded, a missing source template for the same-`vnum` occupied destination is treated like missing stack metadata and the merge fails closed;
...
- counted or zero-count partial merge into a compatible destination: `GC_ITEM_UPDATE(source remainder count)` then `GC_ITEM_UPDATE(destination merged count)`, with source item quickslots preserved when the source stack remains occupied;
