...
20. packet unequip through explicitly authored item-template metadata succeeds only when the equipment source exists, is not locked, has non-zero count no greater than the authored template `max_count`, has a valid template whose `equip_slot` matches the source wear cell, passes selected-character restrictions, is not marked `irremovable`, the destination carried slot is empty, and the runtime can encode both touched cells; missing/invalid template metadata in an authored snapshot, mismatched `vnum`/slot metadata, over-template-max live counts, selected-character restrictions, or `irremovable` metadata fail closed before any equipment, carried-inventory, point, or persistence mutation;
