...
- Loaded item-template metadata is authoritative for stack compatibility and transfer guards: templates marked `anti_stack`, `anti_drop`, `anti_give`, `anti_sell`, or non-stackable are not merged by `CG::ITEM_MOVE`, even when source and destination share the same `vnum`; if an explicitly authored item-template snapshot is loaded, same-`vnum` occupied-destination merge attempts with missing source-template metadata fail closed instead of falling back to an unbounded merge.
