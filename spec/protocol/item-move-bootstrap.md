...
16. packet equip succeeds only when the destination equipment slot is empty, the source stack exists, is not locked, has non-zero count no greater than the authored template `max_count`, a valid loaded item template exists for the source `vnum`, that template's `vnum` matches the live source item, that template authors the same `equip_slot` as the destination equipment cell, the selected character is allowed by that template's job/sex anti flags, transfer guard flags (`anti_stack`, `anti_drop`, `anti_give`, `anti_sell`) are not set, and the runtime can encode the destination equipment cell;
...
20. packet unequip through explicitly authored item-template metadata succeeds only when exactly one equipment source exists in the addressed wear cell, is not locked, has non-zero count no greater than the authored template `max_count`, has a valid template whose `equip_slot` matches the source wear cell, passes selected-character restrictions, is not marked `irremovable`, the destination carried slot is empty, and the runtime can encode both touched cells; missing/invalid template metadata in an authored snapshot, mismatched `vnum`/slot metadata, duplicate equipped-slot occupancy, over-template-max live counts, selected-character restrictions, or `irremovable` metadata fail closed before any equipment, carried-inventory, point, or persistence mutation;
21. packet unequip with the deterministic missing-file fallback preserves the existing bootstrap fixtures only when exactly one equipped item occupies the addressed slot: non-effect, movable fallback equipment may still use the legacy narrow no-template unequip path, while duplicate equipped-slot occupancy fails closed and fallback templates with point effects or `irremovable` metadata are enforced through the template-backed path;
...
- exact counted or zero-count full-stack merge into a compatible destination: `GC_ITEM_DEL(source)` then `GC_ITEM_UPDATE(destination merged count)`, followed by `GC_QUICKSLOT_DEL` for each source item quickslot in deterministic quickslot-position order because the source stack identity is removed; target item quickslots remain unchanged;
- every `GC_ITEM_UPDATE` emitted by compatible carried-stack `ITEM_MOVE` merges carries the resolved source template's authored display sockets and attributes while changing only the count field, matching the already-owned `ITEM_UPDATE` preservation rule for consumable, drag-to-item, merchant, and counted-drop refreshes;
