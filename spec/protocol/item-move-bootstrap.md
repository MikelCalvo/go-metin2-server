...
- Loaded item-template metadata is authoritative for stack compatibility: templates marked `anti_stack` or non-stackable are not merged by `CG::ITEM_MOVE`, even when source and destination share the same `vnum`.
...
9. occupied-destination merges require the same `vnum`, a valid loaded template that is stackable and not `anti_stack`, and must not overflow `max_count`;
...
17. the template-authored `equip_slot` and anti-flag guards are fail-closed for both the equipment mutation and any template-backed point side effect, so a mismatch or restricted selected character emits no frames and leaves live/persisted item state unchanged;
18. packet unequip succeeds only when the equipment source exists and is not locked, the destination carried slot is empty, and the runtime can encode both touched cells;
19. equipment-cell equip or unequip requests require `count = 0`; non-zero counted equipment requests fail closed before mutation.
