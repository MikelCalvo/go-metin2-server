...
- counted or zero-count partial merge into a compatible destination: `GC_ITEM_UPDATE(source remainder count)` then `GC_ITEM_UPDATE(destination merged count)`, followed by `GC_QUICKSLOT_DEL` for each item quickslot bound to the changed target cell; source item quickslots and non-item quickslots sharing the target byte slot are preserved;
