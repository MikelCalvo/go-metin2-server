...
- merchant pricing helpers and client-visible item flags: `shop_buy_price`, `sell_count_per_gold`, `stackable`, `rare`, `unique`, and `confirm_when_use`
...
- `rare`, `unique`, and `confirm_when_use`, when set, project only to the owned `ITEM_SET.flags` bits `ITEM_FLAG_RARE`, `ITEM_FLAG_UNIQUE`, and `ITEM_FLAG_CONFIRM_WHEN_USE`; they are client-visible metadata in this slice and do not add runtime durability, ownership, uniqueness, or confirmation policy yet
...
The store normalizes and persists deterministic JSON: template names and effect messages are trimmed, equipment slot names are normalized, owned boolean flag metadata such as `rare` / `unique` / `confirm_when_use` is written in a stable order, fixed socket/attribute arrays are written in owned packet order, and templates are sorted by `vnum`.
