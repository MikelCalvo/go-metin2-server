...
- merchant pricing helpers and client-visible item flags: `shop_buy_price`, `sell_count_per_gold`, `stackable`, `slow_query`, `rare`, `unique`, `confirm_when_use`, `quest_use`, `quest_use_multiple`, `log`, and `applicable`
...
- `slow_query`, `rare`, `unique`, `confirm_when_use`, `quest_use`, `quest_use_multiple`, `log`, and `applicable`, when set, project only to the owned `ITEM_SET.flags` bits `ITEM_FLAG_SLOW_QUERY`, `ITEM_FLAG_RARE`, `ITEM_FLAG_UNIQUE`, `ITEM_FLAG_CONFIRM_WHEN_USE`, `ITEM_FLAG_QUEST_USE`, `ITEM_FLAG_QUEST_USE_MULTIPLE`, `ITEM_FLAG_LOG`, and `ITEM_FLAG_APPLICABLE`; they are client-visible metadata in this slice and do not add runtime slow-query durability, ownership, uniqueness, quest-use, repeated quest-use, applicable-effect, confirmation, or logging policy yet
...
The store normalizes and persists deterministic JSON: template names and effect messages are trimmed, equipment slot names are normalized, owned boolean flag metadata such as `slow_query` / `rare` / `unique` / `confirm_when_use` / `quest_use` / `quest_use_multiple` / `log` / `applicable` is written in a stable order, fixed socket/attribute arrays are written in owned packet order, and templates are sorted by `vnum`.
...
- `internal/itemstore` freezes deterministic save/load behavior, validation failures, anti-flag metadata round trips, highlight metadata, slow-query/rare/unique/confirm/quest-use/quest-use-multiple/log/applicable item-flag metadata, use/equip effect metadata, and strict load rejection for unknown fields or trailing JSON values.
