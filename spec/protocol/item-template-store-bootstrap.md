...
- `internal/itemstore` freezes deterministic save/load behavior, validation failures, anti-flag metadata round trips including storage/shop anti-flag metadata, highlight metadata, refineable/save/slow-query/rare/unique/make-count/irremovable/confirm/quest-use/quest-use-multiple/log/applicable item-flag metadata, use/equip effect metadata, and strict load rejection for unknown fields or trailing JSON values.
