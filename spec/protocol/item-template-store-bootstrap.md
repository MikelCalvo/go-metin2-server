...
- `use_effect`, when present, must have a non-zero `point_type`, `point_index < 255`, a non-zero reversible signed `point_delta` (`-2147483648` is rejected because the runtime cannot safely reason about its inverse/range edges), an optional `consume_count` that defaults to `1`, and non-empty trimmed `message`; positive deltas increase the authored point and negative deltas decrease it
...
The store normalizes and persists deterministic JSON: template names and effect messages are trimmed, equipment slot names are normalized, owned boolean flag metadata such as `refineable` / `save` / `slow_query` / `rare` / `unique` / `make_count` / `irremovable` / `confirm_when_use` / `quest_use` / `quest_use_multiple` / `log` / `applicable` and owned anti-flag metadata such as `anti_save` / `anti_pk_drop` / `anti_myshop` / `anti_safebox` is written in a stable order, `use_effect.consume_count` is omitted only when it uses the default one-count behavior, fixed socket/attribute arrays are written in owned packet order, and templates are sorted by `vnum`.
