...
- merchant pricing helpers and client-visible item flags: `shop_buy_price`, `sell_count_per_gold`, `stackable`, `rare`, `unique`, `confirm_when_use`, `quest_use`, `quest_use_multiple`, and `applicable`
...
- `rare`, `unique`, `confirm_when_use`, `quest_use`, `quest_use_multiple`, and `applicable`, when set, project only to the owned `ITEM_SET.flags` bits `ITEM_FLAG_RARE`, `ITEM_FLAG_UNIQUE`, `ITEM_FLAG_CONFIRM_WHEN_USE`, `ITEM_FLAG_QUEST_USE`, `ITEM_FLAG_QUEST_USE_MULTIPLE`, and `ITEM_FLAG_APPLICABLE`; they are client-visible metadata in this slice and do not add runtime durability, ownership, uniqueness, quest-use, repeated quest-use, applicable-effect, or confirmation policy yet
...
The store normalizes and persists deterministic JSON: template names and effect messages are trimmed, equipment slot names are normalized, owned boolean flag metadata such as `rare` / `unique` / `confirm_when_use` / `quest_use` / `quest_use_multiple` / `applicable` is written in a stable order, fixed socket/attribute arrays are written in owned packet order, and templates are sorted by `vnum`.
...
- `internal/itemstore` freezes deterministic save/load behavior, validation failures, anti-flag metadata round trips, highlight metadata, rare/unique/confirm/quest-use/quest-use-multiple/applicable item-flag metadata, use/equip effect metadata, and strict load rejection for unknown fields or trailing JSON values.
