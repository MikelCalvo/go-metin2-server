...
- template-backed incompatible occupied-destination carried `ITEM_MOVE` swaps require valid authored source and target templates before refreshing either carried cell, deleting/retargeting item quickslots, or persisting the swap; accepted swaps project each item's authored display metadata into the resulting `ITEM_SET` for that item's new cell
