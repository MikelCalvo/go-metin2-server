...
- authored transfer/acquisition rejection text: `drop_reject_message` and `pickup_reject_message`
...
The store normalizes and persists deterministic JSON: template names, effect messages, optional consumable `info_message` text, optional `drop_reject_message` text, and optional `pickup_reject_message` text are trimmed, equipment slot names are normalized, owned boolean flag metadata such as `refineable` / `save` / `slow_query` / `rare` / `unique` / `make_count` / `irremovable` / `confirm_when_use` / `quest_use` / `quest_use_multiple` / `log` / `applicable` and owned anti-flag metadata such as `anti_save` / `anti_pk_drop` / `anti_myshop` / `anti_safebox` is written in a stable order, `use_effect.consume_count` is omitted only when it uses the default one-count behavior, `use_effect.info_message` is omitted when empty, `drop_reject_message` and `pickup_reject_message` are omitted when empty, fixed socket/attribute arrays are written in owned packet order, and templates are sorted by `vnum`.
