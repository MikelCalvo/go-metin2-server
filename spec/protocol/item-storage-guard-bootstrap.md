...
- keep the shipped runtime fail-closed for transfer/password/money/item-placement semantics until a later storage/safebox slice owns those behaviors, while freezing the local `/open_safebox` / `/close_safebox` presentation seam needed for exchange busy-window policy, the first lab `/open_mall` / `/close_mall` presentation that emits `GC::MALL_OPEN` plus in-range `MALL_SET`, the first accepted `MALL_CHECKOUT` that places a mall cell into carried inventory, the first same-window mall relocate through already-owned `CG::ITEM_MOVE` / `WindowMall`, and durable same-account mall FileStore rematerialize;
...
`internal/game` decodes these four packet layouts only while the session is already in `GAME` and routes each one through an explicit storage-facing handler seam. The default handlers remain fail-closed with no output, so adding the seam does not imply accepted safebox or mall mutation semantics beyond the currently owned check-in/out/item-move, open-mall checkout, and same-window mall `ITEM_MOVE` paths below.

The shipped minimal runtime intentionally rejects ordinary storage requests without output except for the owned `SAFEBOX_CHECKIN` / `SAFEBOX_CHECKOUT` / `SAFEBOX_ITEM_MOVE` / open-mall `MALL_CHECKOUT` / same-window mall `ITEM_MOVE` mutations and `anti_safebox` feedback seams:

- no `GC::SAFEBOX_*` or `GC::MALL_*` storage response frames are emitted for unsupported storage-transfer packets (closed-mall / death-floor `MALL_CHECKOUT` / closed-mall / death-floor mall `ITEM_MOVE`), even though their first codecs are now owned;
...
This contract freezes the presentation seam needed so exchange busy-window policy can observe an open safebox, plus durable same-account rematerialize, the first warehouse password challenge, durable `/safebox_change_password`, and durable warehouse money via open-burst `SAFEBOX_MONEY_CHANGE` plus `/safebox_money_save` / `/safebox_money_withdraw` (successful money mutations also close an active merchant/exchange presentation with `GC::SHOP END` / `GC::EXCHANGE END` before gold/`SAFEBOX_MONEY_CHANGE`, matching check-in/out/move). Lab `/open_mall` now owns mall open presentation plus the first accepted `MALL_CHECKOUT`, the first same-window mall `ITEM_MOVE` relocate, and durable same-account mall FileStore rematerialize; mall password change, cash-shop purchase, mall money, NPC mall open, and mall check-in remain deferred:

- the local slash harness `/open_safebox [size]` remains the no-password lab/debug opener: while the session is already in `GAME` and above the bootstrap zero-HP floor it may mark the selected character's same-socket bootstrap safebox presentation as open without challenging a password and emits `GC::SAFEBOX_SIZE` (+ in-range `SAFEBOX_SET`) plus self-only `GC::SAFEBOX_MONEY_CHANGE`; when the same socket already has an open lab cube presentation, `/open_safebox` and a successful matching `/safebox_password` reject with one self-only `CHAT_TYPE_INFO` `You cannot trade while another trade window is open.` and leave cube open / safebox closed (password challenge stays pending on the password path); lab `/open_safebox` never consults reopen cooldown or open-anchor distance so existing mutation proofs remain usable;
...
- optional authored `size` is a page count in the owned bootstrap range `1..3`; omitted / `0` size defaults to `1`; slash values outside that range (for example `/open_safebox 4`), non-uint8 size tokens, and extra arguments fail closed with no frames, no ordinary talking-chat fallthrough, and no open-state mutation; store/content validation likewise rejects authored `open_safebox` definitions whose `size` is outside `0..3`; the checked-in negative fixture `docs/examples/bootstrap-invalid-open-safebox-oversize-size-bundle.json` is the preferred `/local/content-bundle/validate` dry-run for that oversize reject without improvising JSON; the checked-in negative fixture `docs/examples/bootstrap-invalid-open-safebox-foreign-reward-gold-bundle.json` is the preferred dry-run for illegal turn-in `reward_gold` on warehouse definitions;
...
- `/safebox_money_save <amount>` / `/safebox_money_withdraw <amount>` while the selected character is in `GAME`, above the bootstrap zero-HP floor, and the same-socket safebox presentation is already open deposit/withdraw durable warehouse gold against carried gold: missing / non-positive / non-uint32-parseable / over-`MaxInt32` amounts, insufficient carried/warehouse gold, warehouse or carried gold overflow past `MaxInt32` / exchange gold carrier max (`1<<31-1`), closed presentation, pending-only password challenge, death-floor, and unrecognized extra args stay fail-closed-consume with no frames / no ordinary talking-chat fallthrough and leave any open merchant/exchange presentation untouched; success deducts/credits carried gold, upserts durable money (preserving password + cells), persists account gold together with the safebox write, and emits self-only gold `PLAYER_POINT_CHANGE` plus `GC::SAFEBOX_MONEY_CHANGE` for the new warehouse total; if an active bootstrap merchant window is open and the money mutation would otherwise succeed, the runtime closes that local presentation first with self-only `GC::SHOP END` (before any exchange close frames when both shells are active); if an active bootstrap exchange shell is open and the money mutation would otherwise succeed, the runtime closes that presentation shell first with self/peer `GC::EXCHANGE END`, then emits the gold / warehouse-money refresh frames; write failure rolls back live gold and durable money fail-closed with no frames and without inventing merchant/exchange close frames. This bootstrap slice does not invent a TMP4 CG `SAFEBOX_MONEY` request header (`docs/plans/2026-08-23-safebox-money-busy-shell-auto-close.md`);
...
- the loaded template for that `vnum` must be valid, must match the live item `vnum`, must bound the live stack count with `max_count`, and must **not** author `anti_safebox` or `anti_save`;
...
- on success the runtime removes the whole carried stack from inventory, syncs source item quickslots with the already-owned removal path (`GC::QUICKSLOT_DEL` when the cell is fully removed), stores an independent presence-aware sockets/attributes clone of that item (including explicit zero; omit→omit so later encode may keep template fallback; `docs/plans/2026-09-04-safebox-checkin-preserve-instance-sockets-attributes.md`) in the same-socket presentation table and the durable same-account safebox FileStore (login + character id), persists the inventory/quickslot account snapshot together with that safebox write, and emits self-only `GC::ITEM_DEL` plus `GC::SAFEBOX_SET` for that safebox cell (sockets prefer instance `EffectiveSockets` including explicit zero over template sockets; attributes prefer instance `EffectiveAttributes` including explicit all-zero / type-zero over template attributes — `docs/plans/2026-08-30-attributes-on-instance-contract-freeze.md`); write failure rolls back live inventory and the presentation/durable safebox cells fail-closed;
...
- reconnect / process restart / logout / shared-world leave clear the presentation map and open flag, but durable cells rematerialize on the next successful open for the same login + character id with the same presence-aware instance sockets (`has_sockets` + `socket0/1/2`, including explicit zero) and presence-aware instance attributes (`has_attributes` + `attributes`, including explicit all-zero / type-zero) that check-in persisted (`docs/plans/2026-08-30-safebox-cell-instance-sockets-durable.md`, `docs/plans/2026-08-30-safebox-cell-instance-attributes-durable.md`). Live open-presentation `SAFEBOX_SET` prefers instance `EffectiveAttributes` when the rematerialized cell carries them (`docs/plans/2026-08-30-attributes-on-instance-contract-freeze.md`). Tip-`0015` SQL attribute companions stay deferred.
...
- on success the runtime removes the item from the presentation table and durable safebox FileStore, places/merges it into the destination carried cell while preserving the stored item identity and presence-aware instance sockets/attributes on fresh-cell placement (including explicit zero; omit→omit with template encode fallback; independent clone via `WithInventorySlot` so later writes cannot alias the safebox source; `docs/plans/2026-09-04-safebox-checkout-free-cell-preserve-instance-sockets-attributes.md`), and keeps compatible unlocked same-`vnum` merges count-only with destination presence authoritative (including explicit zero; source presence discarded; omitted destination stays omitted; `docs/plans/2026-09-03-compatible-stack-merge-destination-wins-instance-sockets-attributes.md`), persists the inventory/quickslot account snapshot together with that safebox write, and emits self-only `GC::SAFEBOX_DEL` plus inventory `GC::ITEM_SET` (empty destination) or `GC::ITEM_UPDATE` (compatible merge); write failure rolls back live inventory and the presentation/durable safebox cells fail-closed;
...
- the loaded template for that safebox item `vnum` must be valid, must match the stored item `vnum`, and must bound the stored stack count with `max_count`;
- requested `count` of `0` or exactly the live source count keeps the whole-stack path: empty destination relocates the whole source stack while preserving item identity and an independent presence-aware sockets/attributes clone (including explicit zero; omit→omit so later encode may keep template fallback; `docs/plans/2026-09-04-safebox-item-move-whole-stack-empty-destination-preserve-instance-sockets-attributes.md`) and emits self-only `GC::SAFEBOX_DEL` (source) plus `GC::SAFEBOX_SET` (destination); occupied compatible unlocked unequipped same-`vnum` destination that still fits `max_count` after merging the whole source count clears the source cell and emits self-only `GC::SAFEBOX_DEL` (source) plus `GC::SAFEBOX_SET` (merged destination) as a count-only merge with destination presence authoritative (including explicit zero; source presence discarded; omitted destination stays omitted; `docs/plans/2026-09-03-compatible-stack-merge-destination-wins-instance-sockets-attributes.md`);
- requested `count` in `1..source_count-1` is the partial path: empty destination decrements the source count in place, allocates a new item identity for the destination stack (maximum ID across the selected character's live inventory + equipment + remembered open-presentation safebox cells, then `+ 1`), places the split count there while cloning the source's presence-aware instance sockets/attributes (including explicit zero) onto that new identity so the destination cannot alias the source remainder, and emits self-only `GC::SAFEBOX_SET` (source remainder) plus `GC::SAFEBOX_SET` (destination split); the source remainder keeps source identity/slot plus an independent clone of the source's presence-aware sockets/attributes (including explicit zero; omit→omit with template encode fallback) so later writes cannot alias the pre-split open-presentation pointer (`docs/plans/2026-09-06-safebox-item-move-partial-split-remainder-preserve-instance-sockets-attributes.md`); occupied compatible unlocked unequipped same-`vnum` destination that still fits `max_count` after adding the requested count decrements the source and emits self-only `GC::SAFEBOX_SET` (source remainder) plus `GC::SAFEBOX_SET` (merged destination) as a count-only merge; destination presence-aware instance sockets/attributes remain authoritative on the merged cell (including explicit zero; source presence discarded onto the destination; omitted destination stays omitted so later encode may keep template fallback; `docs/plans/2026-09-03-compatible-stack-merge-destination-wins-instance-sockets-attributes.md`) while the source remainder keeps source identity/slot plus an independent clone of the source's presence-aware sockets/attributes (including explicit zero; omit→omit with template encode fallback) so later writes cannot alias the pre-merge open-presentation pointer (`docs/plans/2026-09-06-safebox-item-move-partial-merge-remainder-preserve-instance-sockets-attributes.md`);
...
Open-presentation `SAFEBOX_CHECKIN` of a carried item whose loaded template authors `anti_save` (without the `anti_safebox` + `safebox_reject_message` chat path) is a silent mutation reject: no frames, no inventory/safebox/quickslot/gold mutation, and no persistence write, besides the already-owned `ITEM_SET.anti_flags` `ANTI_SAVE` projection. Closed presentation and the death-floor stay the same silent fail-closed consume. Mall check-in, mall-only items, cash-shop metadata, and `anti_pk_drop` mutation policy stay deferred.
...
If the template omits `safebox_reject_message`, if `anti_safebox` is absent, if metadata is missing/invalid/mismatched, if the live item is malformed, locked, duplicated, absent, or already over template `max_count`, if the safebox presentation is closed, if the destination `safe_pos` is occupied/out of range, or if the template authors `anti_save`, the request preserves the older no-frame/no-mutation fail-closed behavior (except the accepted open-presentation mutation path above, which also refuses `anti_save`).

That same no-frame/no-mutation rule now also covers the retaliation-owned player-death floor. Once a content practice mob has driven the selected owner's live bootstrap HP to `0`, later `SAFEBOX_CHECKIN`, `SAFEBOX_CHECKOUT`, `SAFEBOX_ITEM_MOVE`, `MALL_CHECKOUT`, and same-window mall `ITEM_MOVE` requests fail closed before any storage response frame, before any template-authored `anti_safebox` info-chat feedback, and before carried inventory/equipment, quickslot, point, gold, ground-handle, or account-persistence side effects can run. After `/restart_here` / `/restart_town` recovery the same anti-safebox `SAFEBOX_CHECKIN` emits the ordinary self-only reject chat again (`TestGameSessionFlowPostFloorSafeboxCheckinFailsClosedBeforeAntiSafeboxFeedback`, `TestGameSessionFlowPostFloorSafeboxCheckinFailsClosedBeforeRestartTown`), and a remembered-cell `SAFEBOX_CHECKOUT` / whole-stack empty-destination `SAFEBOX_ITEM_MOVE` / whole-stack compatible-merge `SAFEBOX_ITEM_MOVE` / partial-count empty-destination `SAFEBOX_ITEM_MOVE` / compatible partial-merge `SAFEBOX_ITEM_MOVE` after fresh `/open_safebox` succeed normally again (`TestGameSessionFlowPostFloorSafeboxCheckoutFailsClosed`, `TestGameSessionFlowPostFloorSafeboxCheckoutFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorSafeboxItemMoveFailsClosed`, `TestGameSessionFlowPostFloorSafeboxItemMoveFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorSafeboxItemMoveWholeMergeFailsClosed`, `TestGameSessionFlowPostFloorSafeboxItemMoveWholeMergeFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorSafeboxItemMovePartialSplitFailsClosed`, `TestGameSessionFlowPostFloorSafeboxItemMovePartialSplitFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorSafeboxItemMovePartialMergeFailsClosed`, `TestGameSessionFlowPostFloorSafeboxItemMovePartialMergeFailsClosedBeforeRestartTown`); an ordinary carried-stack `SAFEBOX_CHECKIN` after fresh `/open_safebox` also succeeds (`TestGameSessionFlowPostFloorSafeboxCheckinSucceedsAfterRestartHere`, `TestGameSessionFlowPostFloorSafeboxCheckinSucceedsAfterRestartTown`). This does not broaden storage itself; it only keeps the existing unsupported storage guard from becoming a post-death escape hatch.

Malformed payload sizes fail at the codec/dispatcher boundary rather than reaching runtime mutation code.

## First bootstrap mall-open presentation seam

This contract now freezes the first same-socket mall open presentation plus one accepted `MALL_CHECKOUT`, one same-window mall `ITEM_MOVE`, and durable same-account mall FileStore rematerialize without inventing mall password change, cash-shop purchase, mall money, NPC mall open, or mall check-in:

- the local slash harness `/open_mall [size]` is the no-password lab opener: while the session is already in `GAME` and above the bootstrap zero-HP floor it marks the selected character's same-socket mall presentation as open and emits self-only `GC::MALL_OPEN` with an in-range size, then rematerializes in-range durable same-account mall cells as self-only `MALL_SET` (`size * 5` capacity; omitted size defaults to `1` on first open and reuses the remembered size on later idempotent refresh);
- out-of-range / invalid `/open_mall` attempts stay fail-closed with no `MALL_OPEN`, no ordinary talking-chat fallthrough, and no open-flag mutation;
- when the same socket already has an open lab cube presentation, `/open_mall` rejects with one self-only `CHAT_TYPE_INFO` `You cannot trade while another trade window is open.` and leaves cube open / mall closed;
- `/close_mall` / `/mall_close` clear the same-socket open flag with no frames and do not delete durable mall cells; Leave/teardown also clear only the presentation flag;
- `CG::MALL_CHECKOUT` is now accepted for one remembered open-presentation mall item while lab `/open_mall` is already open: the selected character must be above the bootstrap zero-HP floor; `mall_pos` must resolve to exactly one in-range mall cell (`size * 5` capacity); the loaded template must be valid and bound the stored stack; the destination must be a carried inventory cell that can accept the whole mall stack via already-owned `SafeboxCheckoutItem` (empty cell or compatible unlocked same-`vnum` under `max_count`); success emits self-only `GC::MALL_DEL` plus inventory `GC::ITEM_SET` (empty destination) or `GC::ITEM_UPDATE` (compatible merge), persists the inventory/quickslot account snapshot together with the durable same-account mall FileStore write, and drops that mall cell so a later `/open_mall` (including after reconnect / process restart for the same login + character id) does not rematerialize it; persist failure restores the previous mall cells fail-closed with no frames; closed presentation, empty/out-of-range source, occupied incompatible / over-max / locked / exchange-displayed destinations, and the death-floor all stay fail-closed with no frames.
- `CG::ITEM_MOVE` is now accepted for one remembered open-presentation mall item while lab `/open_mall` is already open when both packed `TItemPos` windows are `WindowMall`: the selected character must be above the bootstrap zero-HP floor; source and destination cells must be different in-range mall slots (`size * 5` capacity); the source must resolve to exactly one unlocked valid template-bounded stack; `count = 0` (or an exact whole-stack count) relocates that identity into an empty destination through the already-owned safebox whole-stack relocate helper; success emits self-only `GC::MALL_DEL` plus destination `GC::MALL_SET`, updates the durable same-account mall FileStore so a later `/open_mall` (including after reconnect / process restart) rematerializes the destination cell, and leaves carried inventory/equipment/quickslot/gold/account inventory persistence unchanged; persist failure restores the previous mall cells fail-closed with no frames; closed presentation, mixed/unsupported windows, same-cell, out-of-range, occupied destination, oversize/partial count, and the death-floor all stay fail-closed with no frames. `CG::ITEM_USE` against a mall cell stays fail-closed even while `/open_mall` is open. This slice does not invent a mall-checkin header.
...
- warehouse reopen cooldown / distance-from-warehouse gates beyond the currently owned `/safebox_password` 10-second post-`CloseSafebox` cooldown, `ApproxDistance > 1000` open-anchor distance gate, and already-open MOVE / SyncPosition walk-away `CloseSafebox` auto-close (pending-only challenges still stay on the password distance reject path);
- runtime emission policy for mall check-in / mall money / mall merge-or-split frames beyond the currently owned lab `/open_mall` `MALL_OPEN` + in-range `MALL_SET` rematerialize, accepted `MALL_CHECKOUT` `MALL_DEL` + inventory `ITEM_SET` / `ITEM_UPDATE`, and same-window whole-stack mall `ITEM_MOVE` `MALL_DEL` + destination `MALL_SET`;
...
- cash-shop purchase, mall money, NPC mall open, or mall password change;
- SQL import/backfill from quarantined safebox exports beyond the currently owned tip-`0015` password/money/cell projection (additive safebox cell instance-socket SQL stays deferred);
- accepted template-authority policy for mall-only items, cash-shop metadata, or `anti_pk_drop` mutation beyond the currently owned `ITEM_SET.anti_flags` projection, self-only `safebox_reject_message` feedback, and silent open-presentation `anti_save` `SAFEBOX_CHECKIN` reject.
...
- `internal/player` freezes template-backed `SafeboxCheckinRejectText`, whole-stack `SafeboxCheckinItem` live inventory removal for accepted check-in (including fail-closed equipped carried slots and silent `anti_save` reject), and whole-stack `SafeboxCheckoutItem` empty-cell placement / compatible merge for accepted check-out.
- `internal/safeboxstore` freezes dedicated durable same-account safebox FileStore round-trip (including optional durable `password`, defaulting blank/missing to `000000`, optional durable `money` omitted when zero, and presence-aware cell instance sockets via `has_sockets` + `socket0/1/2` including explicit zero), malformed fail-closed validation (non-zero sockets without `has_sockets` reject), deterministic JSON, and backup/restore/crash-temp operator hooks keyed by account login + character id. It also freezes a sibling durable same-account mall FileStore (`mall.json` beside the safebox store directory, distinct `.mall-*.json` crash-temp prefix) with the same presence-aware cell sockets/attributes, deterministic JSON, and login + character id keying; mall snapshots omit warehouse password/money so safebox backup/restore empty-dir checks stay unchanged.
- `internal/minimal` freezes both ordinary no-frame/no-mutation/no-persistence storage guards and the authored `anti_safebox` / `safebox_reject_message` info-chat feedback path through the normal session harness, including active merchant-window and active-exchange-shell teardown before that feedback is delivered. Open-presentation `anti_save` `SAFEBOX_CHECKIN` is a silent mutation reject besides the owned `ITEM_SET.anti_flags` projection; closed presentation and death-floor attempts stay fail-closed with no frames. It also freezes the player-death-floor variant where those same storage-facing requests stay silent and non-mutating after practice-mob retaliation has already driven the selected owner to `0` HP, including the `SAFEBOX_CHECKIN` case that would otherwise be allowed to emit authored `anti_safebox` feedback while alive. The bootstrap `/open_safebox` / `/close_safebox` presentation seam is owned here as well: `/open_safebox [1..3]` remains the no-password lab opener and emits self-only `GC::SAFEBOX_SIZE` (plus rematerialized in-range `SAFEBOX_SET` rows from the durable same-account safebox FileStore), remembers the same-socket open flag (with omitted size defaulting to `1` on first open and reusing the remembered size on later idempotent refresh), warehouse `open_safebox` `INTERACT` emits `ShowMeSafeboxPassword` and `/safebox_password` opens on match or emits `SAFEBOX_WRONG_PASSWORD` / malformed info-chat, `/safebox_change_password` persists a durable password change with success/wrong-password info chat without opening presentation, `/close_safebox` / `/safebox_close` clear that flag with self-only `CHAT_TYPE_COMMAND` `CloseSafebox` while keeping durable cells on disk and clearing only the presentation map on Leave/teardown, and exchange `START` requester/partner busy rejects observe that open flag with the already-owned merchant busy-window chat strings. Accepted open-presentation `SAFEBOX_CHECKIN` freezes inventory removal + quickslot sync + durable `SAFEBOX_SET`, including reconnect/restart reopen rematerialize, exchange-shell close-on-success for non-displayed sources, and fail-closed no-frame rejection when the source carried cell is currently exchange-displayed. Accepted open-presentation `SAFEBOX_CHECKOUT` freezes durable safebox removal + carried empty-cell `ITEM_SET` / compatible `ITEM_UPDATE`, inventory+safebox persistence, reopen without the checked-out row, exchange-shell close-on-success for non-displayed destinations, and fail-closed no-frame rejection when the destination carried cell is currently exchange-displayed. Accepted open-presentation `SAFEBOX_ITEM_MOVE` freezes whole-stack same-session relocate into an empty safebox cell and compatible same-`vnum` merge under template `max_count`, plus partial-count empty-destination split with new item-identity allocation and compatible partial merge, emitting self-only `SAFEBOX_DEL` + `SAFEBOX_SET` for whole-stack success or dual `SAFEBOX_SET` for partial success without inventory/quickslot/gold/account mutation, including the TMP4 inventory-window wire and explicit `WindowSafebox` tooling path for packed positions, closed/out-of-range/unsupported-window/incompatible fail-closed coverage, and exchange-shell close-on-success. Compatible partial-merge remainder presence clone is owned (`TestSafeboxPartialMergeRemainderItemClonesPresenceIndependently`, `TestGameRuntimeSafeboxItemMovePartialMergePreservesInstanceSocketsAndAttributes`); empty-destination split remainder independence is owned (`TestSafeboxPartialSplitRemainderItemClonesPresenceIndependently`, `TestGameRuntimeSafeboxItemMovePartialSplitPreservesInstanceSocketsAndAttributes`). Accepted check-in/out/item-move success also freezes merchant-window auto-close with self-only `GC::SHOP END` before refresh frames (SHOP before exchange when both shells are active). `TestPveVerticalAuthoringBundleClosesGuideUnlockKillCreditAndTurnIn` now also deposits authored drop-table `reward_gold = 60` with `/safebox_money_save` on the first unlocked warehouse open, hits the same-session `/safebox_password` reopen-cooldown reject after that close, then reopens through authored `Warehouse` `INTERACT` + `/safebox_password` (`SAFEBOX_SIZE` size `2` / rematerialized `SAFEBOX_MONEY_CHANGE` `60`) after reconnect / `QuestGuide` re-unlock instead of lab `/open_safebox`, then `/safebox_money_withdraw 60` restores carried gold before check-in, size-2 `SAFEBOX_ITEM_MOVE` occupancy of cell `5` (`0 -> 10` fail-closed), checkout from cell `5`, and sell. Lab `/open_mall [1..3]` now owns the first same-socket mall open presentation: self-only `GC::MALL_OPEN` plus rematerialized in-range `MALL_SET` from durable same-account mall FileStore cells (`size * 5` capacity; omitted size defaults to `1`; out-of-range `/open_mall 4` stays fail-closed with no talking-chat fallthrough), `/close_mall` / `/mall_close` clear only the presentation flag without deleting durable cells, and accepted open-presentation `CG::MALL_CHECKOUT` freezes mall-cell removal plus carried empty-cell `ITEM_SET` / compatible `ITEM_UPDATE` via already-owned `SafeboxCheckoutItem`, dropping that mall cell so reopen (including after reconnect / process restart) does not rematerialize it (`TestGameRuntimeMallCheckoutWhileOpenMovesItemToCarriedInventory`, `TestGameRuntimeMallCheckoutMergesCompatibleDestination`). Accepted open-presentation same-window `CG::ITEM_MOVE` / `WindowMall` freezes whole-stack empty-destination relocate through the already-owned safebox relocate helper, emitting self-only `MALL_DEL` + destination `MALL_SET` and rematerializing the destination cell on later `/open_mall` (`TestGameRuntimeMallItemMoveWhileOpenRelocatesWholeStack`); closed-mall, mixed-window, occupied-destination, oversize-count, and death-floor mall `ITEM_MOVE` stay fail-closed with no frames (`TestGameRuntimeMallItemMoveWithoutOpenOrBadCellsFailsClosedWithoutMutation`, `TestGameRuntimeMallItemMoveFailsClosedAtDeathFloorWithoutMutation`). Closed-mall and death-floor `MALL_CHECKOUT` stay fail-closed with no frames. Durable same-account mall FileStore rematerialize after process restart is owned beside already-owned lab `/open_mall` `MALL_OPEN` + `MALL_SET` and accepted `MALL_CHECKOUT` (`TestGameRuntimeMallCellsSurviveProcessRestartRematerializeOnOpen`, `TestGameRuntimeMallCheckoutSurvivesProcessRestartWithoutRematerialize`, `TestGameRuntimeMallDoesNotLeakForeignCharacterRowsOnSameAccount`). Mall password change, cash-shop purchase, mall money, NPC mall open, and mall check-in remain deferred.
