...
- keep the shipped runtime fail-closed for transfer/password/money/item-placement semantics until a later storage/safebox slice owns those behaviors, while freezing the local `/open_safebox` / `/close_safebox` presentation seam needed for exchange busy-window policy, the first lab `/open_mall` / `/close_mall` presentation that emits `GC::MALL_OPEN` plus in-range `MALL_SET`, the first accepted `MALL_CHECKOUT` that places a seeded mall cell into carried inventory, and the first same-window mall relocate through already-owned `CG::ITEM_MOVE` / `WindowMall`;
...
This contract now freezes the first same-socket mall open presentation plus one accepted `MALL_CHECKOUT` and one same-window mall `ITEM_MOVE` without inventing mall password change, cash-shop purchase, mall money, NPC mall open, mall check-in, or a durable mall FileStore:

- the local slash harness `/open_mall [size]` is the no-password lab opener: while the session is already in `GAME` and above the bootstrap zero-HP floor it marks the selected character's same-socket mall presentation as open and emits self-only `GC::MALL_OPEN` with an in-range size, then rematerializes in-range seeded mall cells as self-only `MALL_SET` (`size * 5` capacity; omitted size defaults to `1` on first open and reuses the remembered size on later idempotent refresh);
...
- `/close_mall` / `/mall_close` clear the same-socket open flag with no frames and do not delete seeded mall cells; Leave/teardown also clear only the presentation flag;
- `CG::MALL_CHECKOUT` is now accepted for one remembered open-presentation seeded mall item while lab `/open_mall` is already open: the selected character must be above the bootstrap zero-HP floor; `mall_pos` must resolve to exactly one in-range seeded mall cell (`size * 5` capacity); the loaded template must be valid and bound the stored stack; the destination must be a carried inventory cell that can accept the whole mall stack via already-owned `SafeboxCheckoutItem` (empty cell or compatible unlocked same-`vnum` under `max_count`); success emits self-only `GC::MALL_DEL` plus inventory `GC::ITEM_SET` (empty destination) or `GC::ITEM_UPDATE` (compatible merge), persists the inventory/quickslot account snapshot, and drops that seeded mall cell from the process-local seed table so a later `/open_mall` does not rematerialize it; closed presentation, empty/out-of-range source, occupied incompatible / over-max / locked / exchange-displayed destinations, and the death-floor all stay fail-closed with no frames.
- `CG::ITEM_MOVE` is now accepted for one remembered open-presentation seeded mall item while lab `/open_mall` is already open when both packed `TItemPos` windows are `WindowMall`: the selected character must be above the bootstrap zero-HP floor; source and destination cells must be different in-range mall slots (`size * 5` capacity); the source must resolve to exactly one unlocked valid template-bounded stack; `count = 0` (or an exact whole-stack count) relocates that identity into an empty destination through the already-owned safebox whole-stack relocate helper; success emits self-only `GC::MALL_DEL` plus destination `GC::MALL_SET`, updates the process-local seed table so a later `/open_mall` rematerializes the destination cell, and leaves carried inventory/equipment/quickslot/gold/account inventory persistence unchanged; closed presentation, mixed/unsupported windows, same-cell, out-of-range, occupied destination, oversize/partial count, and the death-floor all stay fail-closed with no frames. `CG::ITEM_USE` against a mall cell stays fail-closed even while `/open_mall` is open. This slice does not invent a mall-checkin header.
...
- cash-shop purchase, mall money, NPC mall open, or a durable mall FileStore;
