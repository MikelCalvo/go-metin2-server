...
- keep the shipped runtime fail-closed for transfer/password/money/item-placement semantics until a later storage/safebox slice owns those behaviors, while freezing the local `/open_safebox` / `/close_safebox` presentation seam needed for exchange busy-window policy, the first lab `/open_mall` / `/close_mall` presentation that emits `GC::MALL_OPEN` plus in-range `MALL_SET`, and the first accepted `MALL_CHECKOUT` that places a seeded mall cell into carried inventory;
...
`internal/game` decodes these four packet layouts only while the session is already in `GAME` and routes each one through an explicit storage-facing handler seam. The default handlers remain fail-closed with no output, so adding the seam does not imply accepted safebox or mall mutation semantics beyond the currently owned check-in/out/item-move and open-mall checkout paths below.

The shipped minimal runtime intentionally rejects ordinary storage requests without output except for the owned `SAFEBOX_CHECKIN` / `SAFEBOX_CHECKOUT` / `SAFEBOX_ITEM_MOVE` / open-mall `MALL_CHECKOUT` mutations and `anti_safebox` feedback seams:

- no `GC::SAFEBOX_*` or `GC::MALL_*` storage response frames are emitted for unsupported storage-transfer packets (closed-mall / death-floor `MALL_CHECKOUT`), even though their first codecs are now owned;
...
This contract now freezes the first same-socket mall open presentation plus one accepted `MALL_CHECKOUT` without inventing mall password change, cash-shop purchase, mall money, NPC mall open, or a durable mall FileStore:
...
- runtime emission policy for mall item-move / mall check-in / mall money frames beyond the currently owned lab `/open_mall` `MALL_OPEN` + in-range `MALL_SET` rematerialize and accepted `MALL_CHECKOUT` `MALL_DEL` + inventory `ITEM_SET` / `ITEM_UPDATE`;
