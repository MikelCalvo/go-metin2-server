...
- `internal/player` freezes template-backed `SafeboxCheckinRejectText`, whole-stack `SafeboxCheckinItem` live inventory removal for accepted check-in (including fail-closed equipped carried slots), and whole-stack `SafeboxCheckoutItem` empty-cell placement / compatible merge for accepted check-out.
