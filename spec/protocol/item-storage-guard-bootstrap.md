...
- on success the runtime removes the whole carried stack from inventory, syncs source item quickslots with the already-owned removal path (`GC::QUICKSLOT_DEL` when the cell is fully removed), stores the item (including presence-aware instance sockets) in the same-socket presentation table and the durable same-account safebox FileStore (login + character id), persists the inventory/quickslot account snapshot together with that safebox write, and emits self-only `GC::ITEM_DEL` plus `GC::SAFEBOX_SET` for that safebox cell (sockets prefer instance `EffectiveSockets` including explicit zero over template sockets; attributes prefer instance `EffectiveAttributes` including explicit all-zero / type-zero over template attributes when present on the live/carried instance — `docs/plans/2026-08-30-attributes-on-instance-contract-freeze.md`; durable safebox cell attribute rematerialize stays deferred); write failure rolls back live inventory and the presentation/durable safebox cells fail-closed;
...
- reconnect / process restart / logout / shared-world leave clear the presentation map and open flag, but durable cells rematerialize on the next successful open for the same login + character id with the same presence-aware instance sockets (`has_sockets` + `socket0/1/2`, including explicit zero) that check-in persisted (`docs/plans/2026-08-30-safebox-cell-instance-sockets-durable.md`). Live open-presentation `SAFEBOX_SET` prefers instance `EffectiveAttributes` when the live cell carries them (`docs/plans/2026-08-30-attributes-on-instance-contract-freeze.md`). Durable safebox FileStore attribute rematerialize is contract-frozen in `docs/plans/2026-08-30-safebox-cell-instance-attributes-durable.md` (production wiring still deferred until GREEN); tip-`0015` SQL attribute companions stay deferred; until FileStore GREEN lands, restart rematerialize still falls back to template attributes for safebox cells.
