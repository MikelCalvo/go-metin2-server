...
A later storage/safebox slice still owns money and mall. This contract freezes the presentation seam needed so exchange busy-window policy can observe an open safebox, plus durable same-account rematerialize, the first warehouse password challenge, and durable `/safebox_change_password`:
