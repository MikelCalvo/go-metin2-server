...
A later storage/safebox slice still owns password load, money, and mall. This contract freezes the smallest presentation seam needed so exchange busy-window policy can observe an open safebox, plus the first accepted check-in, check-out, and item-move mutations with durable same-account rematerialize:
...
- on success the runtime emits exactly one self-only `GC::SAFEBOX_SIZE` with that page count, remembers an in-memory same-socket `open safebox` presentation flag, hydrates the same-socket presentation table from the durable same-account safebox FileStore keyed by account login + character id (missing/empty file is empty), and then re-emits any hydrated in-range `GC::SAFEBOX_SET` rows for that open session (cells at or beyond the currently opened capacity stay on disk and are not emitted until a larger reopen);
- repeating `/open_safebox` or a successful `open_safebox` `INTERACT` while already open is idempotent presentation refresh: it re-emits `GC::SAFEBOX_SIZE` for the currently remembered size (or the newly requested in-range size), re-hydrates from the durable store, re-emits in-range `SAFEBOX_SET` rows, and leaves inventory/equipment/quickslot/gold/account persistence unchanged;
