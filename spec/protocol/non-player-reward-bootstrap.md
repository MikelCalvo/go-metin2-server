...
- the same combined last-hit also parks that pending handle across same-socket `/phase_select` and abrupt reconnect Leave (floor-leave, not live-owner deletion): still-dead re-entry skips dummy and ground rematerialize while the owner is at `0` HP, then later `/restart_here` rematerializes self-only `ITEM_GROUND_ADD` + `ITEM_OWNERSHIP` after dummy catch-up and ordinary owner `ITEM_PICKUP` persists inventory (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartHereRematerializesKillRewardDrop`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartHereRematerializesKillRewardDrop`)
...
- same-socket `/phase_select` and abrupt reconnect after that combined last-hit park the still-pending handle instead of deleting it, skip ground rematerialize while the owner is still at the floor, then `/restart_here` rematerializes it with self-only `ITEM_GROUND_ADD` + `ITEM_OWNERSHIP` and ordinary owner `ITEM_PICKUP` succeeds (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartHereRematerializesKillRewardDrop`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartHereRematerializesKillRewardDrop`)
