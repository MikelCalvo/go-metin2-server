...
The runtime also exposes the same pending ground occupancy as a flat loopback-only `/local/ground-items` debug snapshot, sorted by visible ground `vid`; this is an operator view only and does not add a gameplay pickup or mutation surface.
