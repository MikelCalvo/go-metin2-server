...
A TCP-level bootstrap regression now freezes that same formula-only profile-default path across the socket boundary: target selection starts from the registered `max_hp`, accepted normal attacks apply the formula-derived damage, and the accepted killing hit emits death/clear before profile-default EXP, gold, ground-add, and ownership frames.
The checked-in formula QA fixture now also owns the combined last-hit composition of that same profile-default descriptor: four formula hits kill `practice.qa_formula_mob`, the killing hit emits EXP `40` / gold `25` / drop `27001` after dummy death/clear/damage-info, and the same accepted hit floors the engaged owner through authored `retaliation_point_delta = -2` (`TestGameSessionFlowAuthoredFormulaProfileKillingHitAlsoFloorsOwnerEmitsProfileDefaultRewardsBeforeOwnerFloor`).
...
Invalid owner login/name metadata is stricter: because every fixed drop in the current descriptor would share the same invalid ownership label, the killing hit skips preparing all item-shaped drop frames and live ground handles before emitting them instead of briefly showing unclaimable or truncated owner metadata to the client.
If the same descriptor also carries valid EXP or gold, those independent scalar rewards still apply and emit their ordinary frames; one colliding or template-restricted drop does not suppress independent scalar rewards or later valid drop entries.
...
Ground pickup also withholds party-style owner delivery if the registered owner entity no longer matches the identity, bootstrap HP point value, and exact runtime location captured when the ground reward was created; this preserves the display ownership label while preventing stale handles from mutating a rebound, retaliated/damaged, or relocated owner snapshot.
Ground reward registration re-checks the registered owner snapshot before trusting the caller-supplied character copy, so a stale owner snapshot cannot create item-shaped or gold-shaped reward ground entries after the registered owner has already changed selected-character identity (`id`, `vid`, or name), changed the current bootstrap HP point value, or moved to a different runtime location.
...
Dedicated coverage now freezes the item-shaped and gold-shaped reward halves of stale owner point-state registration, stale collector-death, stale collector-identity, stale collector-point, stale collector-location, stale owner-identity owner-delivery, stale owner-location owner-delivery, and stale owner-point owner-delivery cases because both reward families share the same ground-entry pickup/removal seam.
For owner point-state registration specifically, a stale owner snapshot fails closed for item-shaped and gold-shaped ground reward creation after the registered owner has already changed its current bootstrap HP point value, even if both owner snapshots are still positive and otherwise at the same identity/location; this mirrors the collector point-state guard and keeps delayed combat retaliation from creating rewards from a stale pre-retaliation owner copy.
...
- item-shaped reward drops are checked against valid owner login/name metadata, including the current fixed-width `ITEM_OWNERSHIP.owner_name` display field, and the currently loaded item-template metadata before frames are appended or ground entries are registered; invalid owner metadata suppresses all item-shaped drop frames for that kill, while restricted or malformed template state suppresses only that drop
...
- that same successful pickup removes the pending reward entry from every local runtime debug snapshot that exposes ground state: map occupancy, the flat ground-item list, and by-VID ground-item lookup
- when a connected **live** owner leaves the shared world, currently owned reward ground entries are removed with deterministic `ITEM_GROUND_DEL` fanout to living peers that can still see the ground entry's original map/AOI location; connected peers already at the bootstrap `0`-HP floor are skipped for both the owner leave visibility delete and owned-ground delete noise
- when that same owner is already at the bootstrap `0`-HP floor, `/phase_select`, abrupt reconnect, and stale reclaim **park** currently `OwnerID`-matched pending handles instead of deleting them: process-local `OwnerID` clears to `0`, durable owner identity plus exclusive/despawn timers stay intact, living peers receive the ordinary owner `CHARACTER_DEL` without `ITEM_GROUND_DEL`, and later matching owner `Join` rebinds `OwnerID` through the same identity-keyed daemon-restart seam (`TestSharedWorldRegistryLeaveAtHPFloorParksOwnedGroundItems`, `TestSharedWorldRegistryJoinReclaimAtHPFloorParksOwnedGroundItems`)
...
- the same combined last-hit also parks that pending handle across same-socket `/phase_select` and abrupt reconnect Leave (floor-leave, not live-owner deletion): still-dead re-entry skips dummy and ground rematerialize while the owner is at `0` HP, then later `/restart_here` or `/restart_town` rematerializes the parked handle for pickup (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartHereRematerializesKillRewardDrop`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartHereRematerializesKillRewardDrop`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartTownRematerializesKillRewardDropOnSourceMapReselect`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartTownRematerializesKillRewardDropOnSourceMapReselect`)
- the same combined last-hit also rematerializes that pending exclusive handle across `gamed` process restart from the dedicated ground-item FileStore while the dummy is still dead: living peers receive still-dead dummy trailing `GC DEAD` plus exclusive add/ownership, still-dead owner EnterGame skips dummy and ground rematerialize, then later `/restart_here` rematerializes self-only add/ownership for pickup (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerDaemonRestartRestartHereRematerializesKillRewardDrop`) or `/restart_town` rematerializes the parked handle after source-map relocate-back (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerDaemonRestartRestartTownRematerializesKillRewardDropOnSourceMapReselect`)
...
- kill-reward EXP is shared in equal integer parts across the same implicit party used for the single-drop ownership roll; each other member's share patches only that member's live EXP point and the matching account point, leaving live position, gold, HP, and the rest of that character in place. Gold stays with the killer, and contribution weights stay deferred
...
- same-socket `/phase_select` and abrupt reconnect after that combined last-hit park the still-pending handle instead of deleting it, skip ground rematerialize while the owner is still at the floor, then `/restart_here` rematerializes it in place or `/restart_town` rematerializes it after source-map relocate-back and ordinary owner `ITEM_PICKUP` succeeds (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartHereRematerializesKillRewardDrop`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartHereRematerializesKillRewardDrop`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartTownRematerializesKillRewardDropOnSourceMapReselect`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartTownRematerializesKillRewardDropOnSourceMapReselect`)
- `gamed` process restart after that combined last-hit rematerializes the still-dead dummy plus the parked exclusive handle from FileStore, skips dummy/ground rematerialize while the owner is still at the floor, then `/restart_here` rematerializes self-only `ITEM_GROUND_ADD` + `ITEM_OWNERSHIP` and ordinary owner `ITEM_PICKUP` succeeds (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerDaemonRestartRestartHereRematerializesKillRewardDrop`); `/restart_town` instead tears the rematerialized handle down from town visibility and rematerializes it after source-map relocate-back (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerDaemonRestartRestartTownRematerializesKillRewardDropOnSourceMapReselect`)
