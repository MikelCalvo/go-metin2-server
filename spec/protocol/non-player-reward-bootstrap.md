...
- pending kill-reward ground handles rematerialize across `gamed` process restart from the dedicated ground-item FileStore with absolute ownership/despawn timers, the same way ordinary player drops do, including a combined last-hit that also floors the owner while the dummy is still dead (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerDaemonRestartRestartHereRematerializesKillRewardDrop`)
