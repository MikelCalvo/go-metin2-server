...
Lookup, pickup-resolution, and removal also re-check the registered collector snapshot before trusting the caller-supplied character copy, so a stale pre-death collector snapshot cannot see, resolve, or remove reward ground entries after that collector has already reached the bootstrap `0`-HP floor.
