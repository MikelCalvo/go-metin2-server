...
- `POST /local/content-bundle/import-preview` returns the same `current` and `candidate` summaries plus top-level count/amount deltas, a deterministic `deltas.interaction_kinds` array for only interaction kinds whose total/referenced/unreferenced counts change, a deterministic `deltas.interaction_definitions` array for authored definitions that are `added`, `removed`, or `changed`, and a deterministic `deltas.maps` array for only map indexes whose tracked authored counts or reward totals change; each interaction-definition delta carries `kind`, `ref`, `change`, and compact current/candidate previews when present, each interaction-kind delta includes `current`, `candidate`, and signed `delta` values for total definitions, referenced definitions, and unreferenced definitions, and each map delta includes the same values for static actors, interactables, `info`/`talk`/`shop_preview`/`warp` service actors, shop catalog entries, spawn groups, reward EXP, reward gold, and reward-drop item counts so operators can see exactly which authored definitions, interaction families, maps, and reward budgets would change before applying a replacement bundle
