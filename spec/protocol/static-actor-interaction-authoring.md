...
- `POST /local/content-bundle/import-preview` returns the same `current` and `candidate` summaries plus top-level count deltas and a deterministic `deltas.maps` array for only map indexes whose tracked authored counts change; each map delta includes `current`, `candidate`, and signed `delta` count values for static actors, interactables, `info`/`talk`/`shop_preview`/`warp` service actors, shop catalog entries, spawn groups, and reward-drop item counts so operators can see which maps would change before applying a replacement bundle
