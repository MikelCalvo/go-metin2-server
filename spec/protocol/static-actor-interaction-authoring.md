...
- the deterministic example bundle at `docs/examples/bootstrap-npc-service-bundle.json` is intentionally self-contained for merchant QA and quest-state inspection: its `item_templates` section carries every item referenced by the `shop_preview` catalog, its `quest_state` section carries one canonical `QuestHero / quest:first_steps / step = 1` seed row, and its quest-flag actors reference definitions that can advance or clear `quest:first_steps.met_guide` for the selected character
...
- `GET /local/content-bundle/maps/{map_index}/quest-flag-routes`, `GET /local/content-bundle/maps/{map_index}/shop-routes`, and `GET /local/content-bundle/maps/{map_index}/warp-routes` are loopback-only map-scoped route readers over the same live exported content-bundle summary; they return deterministic route arrays filtered by source actor `map_index`, return `404` when no authored map row exists for that map, return an empty JSON array for a known authored map with no route of that interaction/service kind, and exist for local QA/introspection rather than gameplay protocol or content mutation
...
- `GET /local/content-bundle/quest-flag-routes/{actor_name}`, `GET /local/content-bundle/shop-routes/{actor_name}`, and `GET /local/content-bundle/warp-routes/{actor_name}` are loopback-only exact-name route readers over the same live exported content-bundle summary; they accept URL-escaped authored static-actor names, reject blank or slash-containing names as path-ambiguous `400` requests, return every deterministic `quest_flag_routes[]`, `shop_routes[]`, or `warp_routes[]` row for that actor name so duplicated authored placements remain inspectable, return `404` when no matching route exists, and exist for local QA/introspection rather than gameplay protocol or content mutation
...
- `POST /local/content-bundle/import-preview/interaction-kinds/{kind}` is the focused no-mutation companion for interaction-family count/scope changes. It runs the same candidate import preview as the broad endpoint, returns only the exact matching `deltas.interaction_kinds[]` row for an owned interaction kind, returns `404` when that kind has no count/referenced/unreferenced delta, rejects unsupported or path-ambiguous kinds with `400`, rejects non-loopback callers with `403`, and accepts only `POST`.
- `POST /local/content-bundle/import-preview/shop-catalogs/{kind}/{ref}` and `POST /local/content-bundle/import-preview/warp-destinations/{kind}/{ref}` are the focused no-mutation companions for service definitions. They run the same candidate import preview as the broad endpoint, accept only `shop_preview` catalog identities and `warp` destination identities respectively, return only the exact matching `deltas.shop_catalogs[]` or `deltas.warp_destinations[]` row, return `404` when that service definition has no added/removed/changed delta, reject malformed or wrong-family identities with `400`, reject non-loopback callers with `403`, and accept only `POST`.
- `POST /local/content-bundle/import-preview/static-actors/{name}` is the focused no-mutation companion for authored static-actor placement changes. It runs the same candidate import preview as the broad endpoint, returns every matching `deltas.static_actors[]` row whose current or candidate actor name equals the URL-decoded path name, returns `404` when that actor name has no added/removed/changed delta, rejects path-ambiguous names with `400`, rejects non-loopback callers with `403`, and accepts only `POST`.
- `POST /local/content-bundle/import-preview/interactable-static-actors/{name}` is the focused no-mutation companion for clickable/static-actor interaction-preview changes. It runs the same candidate import preview as the broad endpoint, returns every matching `deltas.interactable_static_actors[]` row whose current or candidate actor name equals the URL-decoded path name, includes resolved compact preview changes caused by actor metadata, service catalog/destination edits, or definition text edits, returns `404` when that interactable actor name has no added/removed/changed delta, rejects path-ambiguous names with `400`, rejects non-loopback callers with `403`, and accepts only `POST`.
- `POST /local/content-bundle/import-preview/maps/{map_index}` is the focused no-mutation companion for authored map impact. It runs the same candidate import preview as the broad endpoint, returns the exact matching `deltas.maps[]` row with count/amount deltas plus map-local static-actor, spawn-group, quest-flag-route, shop-route, and warp-route delta rows, returns `404` when that map has no added/removed/changed tracked content, rejects malformed or zero map indexes with `400`, rejects non-loopback callers with `403`, and accepts only `POST`.
- `POST /local/content-bundle/import-preview/spawn-groups/{ref}` and `POST /local/content-bundle/import-preview/combat-profiles/{profile}` are focused no-mutation companions for authored non-player content. They run the same candidate import preview as the broad endpoint, return only the exact matching `deltas.spawn_groups[]` or `deltas.combat_profiles[]` row, return `404` when that spawn group or portable combat profile has no added/removed/changed delta, reject malformed identities with `400`, reject non-loopback callers with `403`, and accept only `POST`.
- `POST /local/content-bundle/import-preview/reward-drops/{item_vnum}` is the focused no-mutation companion for authored reward-drop aggregate deltas. It runs the same candidate import preview as the broad endpoint, returns only the exact matching `deltas.reward_drops[]` row keyed by non-zero item vnum, returns `404` when that reward item has no added/removed/changed delta, rejects malformed vnums with `400`, rejects non-loopback callers with `403`, and accepts only `POST`.
- `POST /local/content-bundle/import-preview` uses the same strict decode, invalid UTF-8 rejection, JSON `null` root rejection, and canonicalization rules as import/validate, then returns the same `current` and `candidate` summaries plus top-level count/amount deltas, a deterministic `deltas.static_actors` array for exact portable static-actor rows that are added or removed, a deterministic `deltas.interactable_static_actors` array for clickable actor rows whose resolved interaction preview is added, removed, or changed, a deterministic `deltas.interaction_kinds` array for only interaction kinds whose total/referenced/unreferenced counts change, a deterministic `deltas.interaction_definitions` array for authored definitions that are `added`, `removed`, or `changed`, a deterministic `deltas.combat_profiles` array for portable custom combat-profile snapshots that are `added`, `removed`, or `changed`, deterministic `deltas.quest_flag_routes`, `deltas.shop_routes`, and `deltas.warp_routes` arrays for compact NPC interaction/service routes that are `added`, `removed`, or `changed`, a deterministic `deltas.warp_destinations` array for authored warp destinations that are `added`, `removed`, or `changed`, a deterministic `deltas.reward_drops` array keyed by item `vnum` for grouped reward-drop sources that are `added`, `removed`, or `changed`, and a deterministic `deltas.maps` array for only map indexes whose tracked authored counts or reward totals change; each static-actor delta carries `change` plus the canonical `current` and/or `candidate` portable row (`name`, `map_index`, `x`, `y`, `race_num`, optional `combat_profile`, optional `interaction_kind`, optional `interaction_ref`), each interaction-definition delta carries `kind`, `ref`, `change`, and compact current/candidate previews when present, each combat-profile delta carries `profile`, `change`, and the canonical current/candidate profile snapshot including HP, damage/formula, presentation, respawn delay, and death-reward defaults when present, each quest-flag/shop/warp route delta is keyed by actor name, source `map_index`/`x`/`y`, and interaction `ref` and includes compact current/candidate route records when present, each interaction kind delta carries total, referenced, and unreferenced count deltas, each map delta groups count/amount changes plus per-map static-actor, spawn-group, quest-flag-route, shop-route, and warp-route deltas, and invalid candidate bundles return `400` before the preview callback runs
