...
- `GET /local/content-bundle/summary` is a read-only operator view over the same canonical export path; it returns deterministic counts by content family, including interactable static actors and structured shop catalog entries, per-kind referenced/unreferenced interaction counts, exact referenced/unreferenced interaction definition identities, exact spawn-group identities (`ref`, `name`, `map_index`, `combat_profile`, and reward descriptor), and per-map authored static-actor / interactable static-actor / spawn-group occupancy without returning the full bundle payload
- `POST /local/content-bundle/summary` is a loopback-only dry-run summary for an operator-supplied bundle; it uses the same strict decode and canonicalization rules as import/validate, returns only the compact deterministic summary, includes the same exact referenced/unreferenced interaction definition identities plus exact spawn-group identities, and does not call the live runtime exporter or mutate authored content
