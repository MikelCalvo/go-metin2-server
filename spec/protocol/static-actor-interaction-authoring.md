...
- `POST /local/content-bundle/import-preview` returns the same `current` and `candidate` summaries plus top-level count/amount deltas and a deterministic `deltas.maps` array for only map indexes whose tracked authored counts or reward totals change; each map delta includes `current`, `candidate`, and signed `delta` values for static actors, interactables, `info`/`talk`/`shop_preview`/`warp` service actors, shop catalog entries, spawn groups, reward EXP, reward gold, and reward-drop item counts so operators can see which maps and authored reward budgets would change before applying a replacement bundle
