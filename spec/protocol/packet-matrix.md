...
| `ITEM_UPDATE` | server -> client | `0x0514` | game | documented | count/socket/attribute refresh for an already-known item cell; total frame length `41`; payload is packed `TItemPos` + `count` + three sockets + seven attributes, with no `vnum`, flags, anti-flags, or highlight; the current merchant `SHOP SELL2` partial-stack success path now emits it for the reduced carried cell |
| `QUICKSLOT_ADD` | client -> server | `0x0509` | game | documented | first owned client quickslot add codec and `GAME` dispatch seam; payload is `pos uint8` plus two-byte quickslot tuple `{type uint8, pos uint8}`; default runtime behavior remains fail-closed until accepted quickslot mutation/persistence is owned |
| `QUICKSLOT_DEL` | client -> server | `0x050A` | game | documented | first owned client quickslot clear codec and `GAME` dispatch seam; payload is `pos uint8`; default runtime behavior remains fail-closed until accepted quickslot mutation/persistence is owned |
| `QUICKSLOT_SWAP` | client -> server | `0x050B` | game | documented | first owned client quickslot swap codec and `GAME` dispatch seam; payload is `pos uint8` + `pos_to uint8`; default runtime behavior remains fail-closed until accepted quickslot mutation/persistence is owned |
| `QUICKSLOT_ADD` | server -> client | `0x0519` | game/bootstrap | documented | first owned server quickslot refresh codec; payload is `pos uint8` plus two-byte quickslot tuple `{type uint8, pos uint8}`; runtime quickslot persistence/synchronization remains pending |
| `QUICKSLOT_DEL` | server -> client | `0x051A` | game/bootstrap | documented | first owned server quickslot clear codec; payload is `pos uint8`; future item-mutation slices can reuse it when a tracked item quickslot is removed |
| `QUICKSLOT_SWAP` | server -> client | `0x051B` | game/bootstrap | documented | first owned server quickslot swap codec; payload is `pos uint8` + `pos_to uint8`; accepted runtime quickslot edit handling remains pending |
