...
| `ITEM_UPDATE` | server -> client | `0x0514` | game | documented | codec-owned count/socket/attribute refresh for an already-known item cell; total frame length `41`; payload is packed `TItemPos` + `count` + three sockets + seven attributes, with no `vnum`, flags, anti-flags, or highlight; no runtime path emits it yet |
