...
| `ITEM_MOVE` | client -> server | `0x0504` | game | documented | first owned client-originated carried-slot move/swap request; payload is source packed `TItemPos`, destination packed `TItemPos`, and `count uint8`; current live runtime accepts only carried inventory positions (`window = INVENTORY`, `cell < 90`), now requires `count` to match the authoritative source stack count exactly, fails closed for partial/oversized count requests until stack splitting is owned, and reuses the same selected-character inventory move/swap persistence and self-only `ITEM_DEL` / `ITEM_SET` refresh path already owned by `/inventory_move` |
