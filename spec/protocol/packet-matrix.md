...
| `ITEM_PICKUP` | client -> server | `0x0505` | game | documented | pickup request with `vid uint32`; `internal/game` dispatch is frozen, and the shipped runtime now accepts self-only pickup of same-session ground handles created by accepted carried-slot drops |
...
| `ITEM_GROUND_ADD` | server -> client | `0x0515` | game | documented | first frozen ground-item spawn codec; payload is `vid uint32`, `vnum uint32`, and signed `x/y/z int32` coordinates; current runtime use is self-only after accepted carried-slot drops, with shared-world fanout/pickup still pending |
