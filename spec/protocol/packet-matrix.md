...
| `ITEM_USE_TO_ITEM` | client -> server | `0x0506` | game | documented | first owned drag-to-item item-use path; payload is source packed `TItemPos` followed by target packed `TItemPos`; current live runtime accepts only carried inventory stack-on-stack consolidation for compatible same-`vnum` stackable templates, emits the same self-only inventory refresh family as carried `ITEM_MOVE`, persists the merged inventory, and fails closed without normal `use_effect` fallback for incompatible or unsupported targets |
