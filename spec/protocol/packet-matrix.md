...
| `ITEM_USE_TO_ITEM` | client -> server | `0x0506` | game | documented | first owned drag-to-item item-use codec; payload is source packed `TItemPos` followed by target packed `TItemPos`; this slice freezes the wire identity/layout only, while `GAME` dispatch and runtime effects remain deferred until one concrete legacy-compatible drag-to-item use case is owned |
