...
| `ITEM_DROP` | client -> server | `0x0502` | game | documented | first frozen drop codec only; payload is packed `TItemPos` plus `elk uint32`; live runtime behavior remains unsupported and fail-closed until ground item state is owned |
| `ITEM_DROP2` | client -> server | `0x0503` | game | documented | first frozen counted drop codec only; payload is packed `TItemPos` plus `gold uint32` and `count uint8`; live runtime behavior remains unsupported and fail-closed until ground item state is owned |
| `ITEM_PICKUP` | client -> server | `0x0505` | game | documented | first frozen pickup codec only; payload is `vid uint32`; live runtime behavior remains unsupported and fail-closed until ground item state is owned |
