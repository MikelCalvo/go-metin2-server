...
| `ITEM_GET` | server -> client | `0x0518` | game | documented | first owned pickup-notice companion; payload is `vnum uint32`, `count uint8`, `arg uint8`, and fixed `from_name[25]`; current runtime emits normal/self `arg = 0` after accepted visible-world pickup, after `ITEM_GROUND_DEL` and `ITEM_SET`; party-delivery variants remain deferred |
