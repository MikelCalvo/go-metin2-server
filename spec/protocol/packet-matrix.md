...
| `ITEM_MOVE` | client -> server | `0x0504` | game | documented | first owned client-originated carried-slot move/swap/split/merge request; payload is source packed `TItemPos`, destination packed `TItemPos`, and `count uint8`; current live runtime accepts only carried inventory positions (`window = INVENTORY`, `cell < 90`), treats `count = 0` as full-stack move/swap, accepts empty-destination partial splits and template-bounded compatible occupied-destination partial merges for non-zero counts, fails closed for incompatible occupied destinations, oversized non-zero counts, template-`max_count` overflow, storage-overflowing destination stack counts, and locked source/destination slots until richer stack and item-lock semantics are owned, reuses the selected-character inventory persistence boundary, keeps identity-changing moves/splits on self-only `ITEM_DEL` / `ITEM_SET`, and uses self-only `ITEM_UPDATE` count refreshes for compatible occupied-destination partial merges |
