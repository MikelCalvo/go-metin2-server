...
| `RESTART` | client -> server | `TBD` | game | planned | dedicated client-originated restart / revive ingress that should later select between the already-owned `/restart_here` and `/restart_town` recovery results; exact header and payload layout remain capture-gated while slash commands stay the temporary bootstrap harness |
