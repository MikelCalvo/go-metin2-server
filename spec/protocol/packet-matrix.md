...
| `ITEM_USE_TO_ITEM` | client -> server | `0x0506` | game | documented | first owned drag-to-item item-use codec and `GAME`-phase dispatch seam; payload is source packed `TItemPos` followed by target packed `TItemPos`; the default game-flow handler rejects it silently/fail-closed when no runtime handler is installed, and live runtime effects remain deferred until one concrete legacy-compatible drag-to-item use case is owned |
