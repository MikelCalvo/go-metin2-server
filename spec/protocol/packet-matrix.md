...
| Name | Direction | Header | Phase | Status | Notes |
| --- | --- | --- | --- | --- | --- |
| `SHOP` | client -> server | `0x0801` | game | planned | first owned merchant-window client family now freezes subheaders `BUY` and `END` on top of a merchant session opened from the existing `INTERACT` ingress; `BUY` remains valid only while an active merchant context exists, the second trailing byte after the common `SHOP` envelope is the zero-based authored `catalog_slot`, and the first trailing byte plus `SELL` / `SELL2` remain out of scope |
| `SHOP` | server -> client | `0x0810` | game | planned | first owned merchant-window server family now freezes `START` as the planned open response and `END` as the planned explicit close companion while the socket can still deliver merchant frames; compatibility-oriented success/failure/update subheaders such as `START_EX`, `OK`, `NOT_ENOUGH_MONEY`, `INVENTORY_FULL`, `INVALID_POS`, `UPDATE_ITEM`, and `UPDATE_PRICE` are acknowledged but remain capture-gated before final choreography is claimed |
