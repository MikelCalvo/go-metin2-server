...
| `ITEM_MOVE` | client -> server | `0x0504` | game | documented | first owned client-originated carried-slot move/swap/split request; payload is source packed `TItemPos`, destination packed `TItemPos`, and `count uint8`; current live runtime accepts only carried inventory positions (`window = INVENTORY`, `cell < 90`), accepts full-stack moves/swaps plus the first empty-destination partial split, fails closed for occupied-destination partial splits, oversized/zero counts, and locked source/destination slots until richer stack merge and item-lock semantics are owned, and reuses the same selected-character inventory persistence plus self-only `ITEM_DEL` / `ITEM_SET` refresh path already owned by `/inventory_move` |
