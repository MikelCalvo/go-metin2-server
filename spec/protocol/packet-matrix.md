...
| `ITEM_GET` | server -> client | `0x0518` | game | documented | first owned pickup-notice companion; payload is `vnum uint32`, `count uint8`, `arg uint8`, and fixed `from_name[25]`; current runtime emits normal/self `arg = 0` after accepted owner pickup and the first party-shaped owner-delivery notices for visible live peers, while real party membership remains deferred |
