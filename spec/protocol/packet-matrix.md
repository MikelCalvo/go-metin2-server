...
| `ITEM_DROP` | client -> server | `0x0502` | game | documented | drop request with packed `TItemPos` plus `elk uint32`; `internal/game` routes the shared `ITEM_USE` / `ITEM_DROP` header by payload length, while the shipped runtime still fails closed until ground item state is owned |
| `ITEM_DROP2` | client -> server | `0x0503` | game | documented | counted drop request with packed `TItemPos` plus `gold uint32` and `count uint8`; `internal/game` dispatch is frozen, while the shipped runtime still fails closed until ground item state is owned |
| `ITEM_PICKUP` | client -> server | `0x0505` | game | documented | pickup request with `vid uint32`; `internal/game` dispatch is frozen, while the shipped runtime still fails closed until ground item state is owned |
...
| `ITEM_GROUND_ADD` | server -> client | `0x0515` | game | documented | first frozen ground-item spawn codec only; payload is `vid uint32`, `vnum uint32`, and signed `x/y/z int32` coordinates; runtime fanout remains unowned |
| `ITEM_GROUND_DEL` | server -> client | `0x0516` | game | documented | first frozen ground-item remove codec only; payload is `vid uint32`; runtime fanout remains unowned |
