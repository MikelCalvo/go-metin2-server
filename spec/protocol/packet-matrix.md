...
| `QUICKSLOT_DEL` | server -> client | `0x051A` | game/bootstrap | documented | runtime `CG::QUICKSLOT_DEL` echoes the accepted self-only clear refresh; current item-mutation paths also emit it for removed item quickslots after whole-stack carried drops, last-stack consumable use, and full-source `ITEM_USE_TO_ITEM` stack consolidation |
