...
- foreign fields (`size`, `title`, catalog, warp coords, reward/consume gold/experience, mutating `quest_to`) fail closed at store / content-bundle validation before runtime mutation; the checked-in negative dry-runs are `docs/examples/bootstrap-invalid-open-cube-foreign-size-bundle.json` for illegal warehouse `size` and `docs/examples/bootstrap-invalid-open-cube-foreign-title-bundle.json` for illegal merchant `title`
