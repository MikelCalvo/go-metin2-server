...
- NPC mall open, accepted mall checkout, client `SAFEBOX_CHANGE_PASSWORD` packets, and TMP4 CG `SAFEBOX_MONEY` request header remain deferred; lab `/open_mall` already owns same-socket `GC::MALL_OPEN` plus in-range `MALL_SET` rematerialize without cash-shop purchase; durable same-account cell/money rematerialize, slash password/change-password/money helpers, check-in/out/move, reopen cooldown, open-anchor distance gate, and walk-away auto-close are already owned beside this NPC surface
...
- recipe list / material info / craft binding / make / close continue on the already-owned cube slash helpers once the presentation is open; the composed PvE vertical gameplay proof now also requests `/cube r_info` / `/cube r_info 0` on that authored `CubeMaster` window (`cube r_list 20022 1 27001,1` / `cube m_info 0 1 27002,2/100`) with no gold/inventory mutation before `/close_cube`. After authored sword `SHOP SELL`, the same composed proof now also packet-buys catalog slot `2` (`27002 x2` @ `20g`) from gated `Merchant`, then `/cube add 0 0` / `/cube make` on a second authored `CubeMaster` window, `/close_cube`, and packet-uses the granted `27001` last stack (`docs/plans/2026-09-07-pve-vertical-authored-cube-add-make.md`, `docs/plans/2026-09-07-pve-vertical-authored-cube-grant-use.md`). The dedicated NPC-service proof `TestNpcServiceBundleCubeMasterAddMakeConsumesGrantsAndPersists` already packet-buys that same catalog slot, then binds `/cube add 0 0` / `/cube make`, `/close_cube`s, and packet-uses the granted `27001` last stack including its authored self-only HP-up-red `SPECIAL_EFFECT` (`docs/plans/2026-09-07-npc-service-authored-cube-material-buy.md`, `docs/plans/2026-09-08-npc-service-authored-cube-grant-use.md`)
...
- `open_cube` now opens the current bootstrap cube presentation (`cube open <npcVnum>`) rather than inventing a separate craft packet family; if authored `text` is present, the interacting player first receives one self-only informational chat delivery and then the cube-open command chat. The composed PvE vertical gameplay proof (`TestPveVerticalAuthoringBundleClosesGuideUnlockKillCreditAndTurnIn`) now also inspects that open window with `/cube r_info` / `/cube r_info 0` before `/close_cube`, then later packet-buys catalog slot `2` and `/cube add` / `/cube make` after sword sell. The dedicated NPC-service CubeMaster proof now also `/cube add` / `/cube make` on that window after `QuestGuide` unlock, then `/close_cube` and packet-uses the granted `27001` last stack
...
- `open_cube` now already resolves through `INTERACT` into the bootstrap cube open presentation (`cube open <npcVnum>`) used by the owned recipe-list / craft slash helpers; composed PvE `CubeMaster` now also proves non-mutating `/cube r_info` / `/cube r_info 0` on that window, then packet-buys authored cube materials from gated `Merchant` catalog slot `2` (`27002 x2` @ `20g`) after sword sell and consumes them through `/cube add` / `/cube make`; the dedicated NPC-service CubeMaster proof already packet-buys those same materials, consumes them through `/cube add` / `/cube make`, `/close_cube`s, and packet-uses the granted `27001` last stack
- the project still avoids speculative dialog-window, quest-script, sell-back, NPC mall open, accepted mall checkout, and client change-password packet semantics until those underlying systems exist; lab `/open_mall` already owns the first same-socket mall open presentation
