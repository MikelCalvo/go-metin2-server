...
- foreign fields (`size`, `title`, catalog, warp coords, reward/consume gold/experience, mutating `quest_to`) fail closed at store / content-bundle validation before runtime mutation; the checked-in negative dry-run for illegal warehouse `size` is `docs/examples/bootstrap-invalid-open-cube-foreign-size-bundle.json`
