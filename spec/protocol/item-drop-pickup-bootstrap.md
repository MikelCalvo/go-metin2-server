...
12. If a visible collector picks up a ground handle still owned by another live session, the bootstrap runtime treats that collector as a temporary bootstrap party member. The item is delivered to the owner account/runtime instead of the collector, the collector receives `GC::ITEM_GROUND_DEL` plus `GC::ITEM_GET(arg=2, from_name=<owner>)`, and the owner receives the queued peer delete, deterministic owner inventory refresh frames, and `GC::ITEM_GET(arg=1, from_name=<collector>)`.
13. Owner-delivery pickup uses the same recipient-side placement precedence as self pickup: compatible stackable inventory is filled before a fresh carried slot is used, and the resulting owner inventory mutation is persisted before the temporary ground handle is removed. The collector's carried inventory capacity is not consulted on this owner-delivery path because the collector is not the item recipient.
...
15. If owner-delivery pickup cannot place the item into the owner's current carried inventory, the runtime rejects the pickup with the same inventory-full `CHAT_TYPE_INFO` message to the collector, does not enqueue owner inventory refresh or pickup notice frames, and leaves the temporary ground handle pending for a later retry.
16. Replayed, unknown, or invisible pickups fail closed with no frames; no-free-slot pickups use the inventory-full info chat path above.
