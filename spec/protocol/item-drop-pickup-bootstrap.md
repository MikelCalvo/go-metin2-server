...
- permanent/shared-world ground item entity IDs, DB-backed runtime restoration, trade/shop restrictions, or range/path authorization beyond current visible-world scope;
- `GC::ITEM_DROP`, real party membership checks, or richer per-drop destroy overrides beyond the bootstrap 300-second ground lifetime.
...
The bootstrap runtime emits ownership immediately after every `GC::ITEM_GROUND_ADD` it creates for accepted player drops, visible peer drop fanout, radius-AOI ground re-entry rebuilds, and transfer destination ground re-entry rebuilds. While a pending handle is still inside its exclusive ownership window, that packet carries the owner's character name. When the in-memory exclusive ownership timer elapses, the runtime emits one blank `GC::ITEM_OWNERSHIP` (`owner_name` empty) to currently visible living peers around the handle and the pickup permission transitions to public ordinary collector pickup. Independently of that ownership transition, each pending handle also carries an in-memory destroy deadline of `300` seconds from registration (`bootstrapGroundItemDespawnDuration`). When any live session flushes pending server frames after that deadline, the shared-world registry removes the handle and emits one `GC::ITEM_GROUND_DEL` to currently visible living peers around it, so unclaimed public litter does not remain forever. Pending handles also rematerialize across `gamed` process restart from the dedicated ground-item FileStore with absolute ownership/despawn timers; rematerialized exclusive handles start with process-local `OwnerID = 0` and rebind that `OwnerID` to the fresh shared-world entity id when the matching owner identity rejoins (`docs/plans/2026-08-22-ground-item-exclusive-owner-id-rebind.md`). Real party membership checks remain deferred.
...
9. The shared bootstrap runtime remembers that deterministic ground handle until it is picked up, the owning live session ends, or the in-memory destroy deadline elapses. When the owner session closes while a handle is still pending, the runtime removes the temporary handle and queues `GC::ITEM_GROUND_DEL` to currently visible peers so they do not keep a stale ground item actor. When the destroy deadline elapses first, the same `GC::ITEM_GROUND_DEL` fanout removes the unclaimed handle for currently visible living peers without inventory, gold, quickslot, or persistence mutation.
...
8. Gold markers share the same bootstrap destroy deadline as ordinary item handles (`300` seconds from registration) and the same FileStore rematerialize / exclusive `OwnerID` rebind / Leave-persist deletion path. Richer gold-marker display/count semantics and fallback delivery when owner currency persistence fails remain deferred.
...
2. The dropper receives the same direct `GC::ITEM_GROUND_ADD` + named `GC::ITEM_OWNERSHIP` already owned by the first drop slice, currently visible living peers receive one queued add/ownership pair for the same handle, and later visible sessions that enter `GAME` while the handle remains pending receive that same add/ownership pair after self bootstrap, peer-player bootstrap, and visible static-actor bootstrap frames. Zero-HP/dead peers stay registered for their own restart/teardown flow, but they are skipped as recipients for new ground-handle visibility fanout. Each newly registered pending handle starts an in-memory exclusive ownership window of `30` seconds from registration time (`bootstrapGroundItemOwnershipDuration`) and an independent in-memory destroy deadline of `300` seconds from that same registration time (`bootstrapGroundItemDespawnDuration`). While the exclusive window is active, only the owning session may accept `ITEM_PICKUP` for the handle. When any live session flushes pending server frames after the ownership window elapses, the shared-world registry releases exclusive ownership for due handles, emits one blank `GC::ITEM_OWNERSHIP` (`owner_name` empty) to currently visible living peers around each released handle, and later AOI/transfer rebuilds for a publicly released handle also emit the blank ownership label instead of the original owner name. When any live session flushes pending server frames after the destroy deadline elapses, the registry removes those due handles and emits one `GC::ITEM_GROUND_DEL` to currently visible living peers around each despawned handle; later pickup attempts for a despawned `vid` fail closed with no frames. Despawn never mutates inventory, equipment, quickslots, gold, or persisted account state.
...
Pending bootstrap ground item/gold handles rematerialize across `gamed` process restart from the dedicated ground-item FileStore with absolute `ownership_expires_at` / `despawn_at` timers. Rematerialized exclusive handles keep peer pickup fail-closed via durable owner identity while `OwnerID = 0`, then rebind process-local `OwnerID` to the fresh shared-world entity id when that matching owner rejoins; the durable snapshot still omits `OwnerID`. Graceful shared-world `Leave` and stale reclaim that delete `OwnerID`-matched live handles also persist that deletion through the existing ground-items changed hook so the FileStore no longer rematerializes those rows after restart (`docs/plans/2026-08-22-ground-item-leave-persist-owned-deletion.md`). Durable FileStore records also round-trip presence-aware instance sockets (`has_sockets` + `socket0/1/2`, including explicit zero) so drop → restart → pickup restores authoritative instance sockets instead of silently falling back to template sockets; gold markers stay socket-less and `GC::ITEM_GROUND_ADD` wire fields stay unchanged (`docs/plans/2026-08-29-ground-item-instance-sockets-durable.md`). Broader range/path policy, real party membership, and party-shaped owner-delivery remain deferred; the previous always-on visible-peer owner-delivery approximation is retired now that exclusive ownership plus public release are owned.

Reference-oracle evidence: the TMP4-compatible client exposes `SendItemDropPacket`, `SendItemDropPacketNew`, and `SendItemPickUpPacket` on the game socket, and consumes `GC::ITEM_GROUND_ADD` / `GC::ITEM_GROUND_DEL` to create and remove client-side ground item actors plus `GC::ITEM_OWNERSHIP` to label item ownership. The external legacy behavior oracle uses an exclusive ownership event (default `30` seconds for Europe-shaped drops / mob rewards, longer only for special system-drop cases) that rejects non-owner pickup while active and emits a blank ownership packet on expiry before public pickup, plus a separate ground destroy event that defaults to `300` seconds from the moment the item is added to the ground. This repository owns only the project-written field layouts, the bootstrap in-memory exclusive/public transition, and that same bootstrap `300`-second destroy deadline.
...
- `internal/minimal` accepts carried-item drop requests with self ground-add/ownership echoes, gold drops from the `elk`/`gold` packet field, self/ordinary pickup of pending gold markers, template-authored `pickup_range` reach overrides for authored `vnum = 1` gold markers, exclusive ownership timers with blank public-release ownership frames, fail-closed non-owner pickup during the exclusive window, ordinary public collector pickup after release including template-backed `anti_give` rejection for peer gold pickup after release, the bootstrap `300`-second destroy deadline that emits visible-peer `GC::ITEM_GROUND_DEL` for unclaimed item and gold handles without mutating inventory/gold/persistence, FileStore rematerialize of pending handles with absolute ownership/despawn timers, process-local exclusive `OwnerID` rebind on matching owner `Join` after rematerialize, and FileStore persistence of owned-ground deletion on graceful `Leave` / stale reclaim.
