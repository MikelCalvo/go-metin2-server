...
- `GC::ITEM_DROP`, real party membership checks, graceful Leave→FileStore owned-ground deletion semantics, or richer per-drop destroy overrides beyond the bootstrap 300-second ground lifetime.
...
8. Gold markers share the same bootstrap destroy deadline as ordinary item handles (`300` seconds from registration) and the same FileStore rematerialize / exclusive `OwnerID` rebind path. Richer gold-marker display/count semantics, graceful Leave→FileStore deletion semantics, and fallback delivery when owner currency persistence fails remain deferred.
