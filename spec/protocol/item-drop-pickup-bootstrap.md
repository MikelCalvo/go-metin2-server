...
- `GC::ITEM_DROP`, real party membership checks, durable/restart-restored ownership or despawn timer state, or richer per-drop destroy overrides beyond the bootstrap 300-second ground lifetime.
...
8. Gold markers share the same bootstrap in-memory destroy deadline as ordinary item handles (`300` seconds from registration). Richer gold-marker display/count semantics, durable shared-world ground currency entities, restart-restored ownership/despawn timer state, and fallback delivery when owner currency persistence fails remain deferred.
