...
7. If neither compatible-stack capacity nor a carried inventory slot can accept the whole picked count, pickup fails closed and leaves the temporary ground handle pending.
8. The collector's selected character snapshot is persisted through the same account-store path used by drops before the handle is removed from the temporary ground table.
...
15. Replayed, unknown, invisible, or no-free-slot pickups fail closed with no frames.
