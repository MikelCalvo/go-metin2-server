...
1. Accepted drops are registered as temporary bootstrap ground handles at the dropper's current effective map/position after the selected character mutation is persisted; zero-HP/dead owners and locked item snapshots are rejected at the shared-world ground-handle seam so death-state races or stale lock-state snapshots cannot create new visible handles.
