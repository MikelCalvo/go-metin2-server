...
5. Template-authored `anti_give` fails closed in the template-backed player pickup placement helper before the recipient inventory mutates; visible-party owner-delivery keeps the existing runtime rejection path described below and leaves the temporary handle pending for owner reclaim.
