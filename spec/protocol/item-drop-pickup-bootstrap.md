...
5. Template-authored selected-character restrictions fail closed before the recipient inventory mutates. The template-backed player pickup placement helper now rejects `anti_give` and authored job/sex restrictions (`anti_warrior`, `anti_assassin`, `anti_sura`, `anti_shaman`, `anti_male`, or `anti_female`) for the recipient. The minimal runtime also freezes restricted self pickup through the normal packet path as one inventory-full `CHAT_TYPE_INFO` rejection while leaving the temporary ground handle pending for a later valid retry.
