...
- content-bundle summaries that expose deterministic `shop_catalogs` entries with the resolved template name, stackability, maximum count, optional template buy-price metadata, and optional authored pickup range next to each authored merchant slot / vnum / count / price
