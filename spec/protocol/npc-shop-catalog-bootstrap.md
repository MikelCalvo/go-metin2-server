...
- content-bundle summaries that expose deterministic `shop_catalogs` entries with the resolved template name, stackability, maximum count, optional template buy-price metadata, optional template sell-price metadata, optional template `anti_get` / `anti_sell` acquisition/sell guards, optional template-authored buy/sell rejection messages, and optional authored pickup range next to each authored merchant slot / vnum / count / price
