...
- the checked-in negative fixture `docs/examples/bootstrap-invalid-merchant-catalog-count-above-stack-limit-bundle.json` is the preferred `/local/content-bundle/validate` dry-run for a structured merchant catalog whose entry `count` exceeds the matching bundled template `max_count`
- the checked-in negative fixture `docs/examples/bootstrap-invalid-merchant-catalog-count-above-non-stackable-limit-bundle.json` is the preferred `/local/content-bundle/validate` dry-run for a structured merchant catalog whose entry `count` exceeds `1` for a non-stackable bundled template
...
- content-bundle summaries that expose deterministic `shop_catalogs` entries with the resolved template name, stackability, maximum count, optional template buy-price metadata, optional template sell-price metadata, optional template transfer/acquisition/sell guard flags (`anti_get`, `anti_drop`, `anti_give`, `anti_sell`, `anti_stack`), optional selected-character guard metadata (`anti_male`, `anti_female`, `anti_warrior`, `anti_assassin`, `anti_sura`, `anti_shaman`, `anti_empire_a`, `anti_empire_b`, `anti_empire_c`, `min_level`), optional template-authored buy/sell rejection messages, and optional authored pickup range next to each authored merchant slot / vnum / count / price
...
- content-bundle map import previews now embed map-local `shop_routes` deltas under `deltas.maps[]`, so a focused `POST /local/content-bundle/import-preview/maps/{map_index}` response shows merchant placement/detail changes for that map without separately filtering top-level service-route deltas
- the exact loopback `GET /local/content-bundle/item-templates/{vnum}` reader for a single summarized item-template row, so local QA can inspect one merchant/reward template and its guard/rejection metadata without fetching the full bundle summary or opening an in-game merchant window
