...
This slice owns the packet codec, the internal damage descriptor, visible-peer fanout for standalone bootstrap combat-profile actors, the first self plus visible-peer emission for content-loaded spawn-backed practice-mob hits, the owner self plus visible-peer hit-effect companion for practice-mob retaliation beats including the owner-floor transition, the mob hit-effect companion on an accepted owner-floor immediate hit, the first killing-hit mob `DAMAGE_INFO` companion on the zero-HP death edge, and the combined last-hit burst where that same accepted `ATTACK` both kills the dummy and floors the owner, keeping any owned EXP / gold / drop reward frames between dummy death and the owner-floor suffix.
...
The current client-side rendering surface treats `vid` as the actor receiving the visible damage effect. A non-negative `damage` value is eligible for the normal damage-effect display path. The already-owned `DAMAGE_INFO` hit-effect carrier freezes two presentation bits: `DAMAGE_NORMAL = 1<<0` (`internal/proto/combat.ServerDamageInfoFlagNormal`) and `DAMAGE_CRITICAL = 1<<5` (`internal/proto/combat.ServerDamageInfoFlagCritical`). Public client rendering treats the critical bit as a distinct hit style that still shows the damage number; this slice does not copy that renderer. Runtime `flag = 0` (`ServerDamageInfoFlagNone`) remains the default plain bootstrap style. The first accepted non-lethal standalone built-in `practice_mob` seam now emits both bits together (`flag = DAMAGE_CRITICAL|DAMAGE_NORMAL`). Poison, dodge/miss, block, and penetrate bits stay later companions (`COMBAT-POISON-FLAG` / `COMBAT-MISS-FLAG` / `COMBAT-BLOCK-FLAG`) and are not owned here. No chance formula decides the critical bit.
...
Currently visible live peers receive that same killing-hit mob `DAMAGE_INFO` after `DEAD` (and after their own clear when they still had that actor selected). They still do not receive the attacker's self-only reward point-changes. Connected recipients already at the bootstrap `0`-HP floor remain skipped by the shared-world visibility gate. A killing hit that leaves the owner above the bootstrap `0`-HP floor still does **not** append owner retaliation `PLAYER_POINT_CHANGE` or owner retaliation `DAMAGE_INFO`.

When that same accepted killing hit would also drive the engaged owner to the bootstrap `0`-HP floor through the ordinary immediate retaliation delta, the runtime now keeps both death surfaces in one packet burst instead of dropping the owner-floor half. Dummy death still owns the first prefix (`DEAD(target_vid)` plus selected-session `TARGET(0, 0)` plus mob `DAMAGE_INFO`). Any owned EXP / gold / drop reward frames stay next, while the killer is still live for scalar persist and ground-item registration. Immediate owner-floor then appends `PLAYER_POINT_CHANGE(value=0)` plus `DEAD(owner_vid)` plus another `TARGET(0, 0)` plus owner `DAMAGE_INFO`. Currently visible live peers receive dummy `DEAD` plus dummy `DAMAGE_INFO` from the killing-hit fanout, then the ground-add / ownership pair, then owner `DEAD` plus owner `DAMAGE_INFO` from the floor fanout. They still do not receive the killer's self-only EXP/gold point-changes. Delayed and proximity-armed floors still omit a synthetic mob companion because they never accepted an owner `ATTACK`.

Owner-floor immediate or delayed retaliation beats keep the owned player-death ordering and now append one plain owner `DAMAGE_INFO` companion after that death/clear prefix, using the same `abs(final_clamped_delta)` descriptor as non-floor owner retaliation. An accepted immediate owner-floor hit also appends the ordinary spawn-backed mob companion after death/clear and before that owner companion, using the same authoritative applied-damage descriptor as live hits:
1. `GC PLAYER_POINT_CHANGE(owner_vid, HP, final_clamped_delta, value = 0)`
2. `GC DEAD(owner_vid)`
3. `GC TARGET(0, 0)`
4. `GC DAMAGE_INFO(vid = target_vid, flag = 0, damage = applied_bootstrap_damage)` on an accepted immediate owner-floor hit only
5. `GC DAMAGE_INFO(vid = owner_vid, flag = 0, damage = abs(final_clamped_delta))`

The accepted-hit self burst still starts with the ordinary `GC TARGET(target_vid, hp_percent)` refresh before that death/clear prefix, because the mob remains alive. Delayed and proximity-armed floors never accepted an owner `ATTACK`, so they skip the mob companion and keep owner `DAMAGE_INFO` immediately after `TARGET(0, 0)`.

Currently visible live peers receive that same owner `DAMAGE_INFO` after their queued `DEAD(owner_vid)`. On an accepted immediate owner-floor hit they also receive the matching mob companion after `DEAD(owner_vid)` and before the owner companion. They still do not receive the owner's `PLAYER_POINT_CHANGE` or `TARGET(0, 0)` clear. Connected recipients already at the bootstrap `0`-HP floor remain skipped by the shared-world visibility gate.
...
- standalone bootstrap combat-profile non-lethal hits are authoritative through the selected-target HP refresh and carry one self hit-effect companion; the built-in standalone `practice_mob` companion uses `flag = DAMAGE_CRITICAL|DAMAGE_NORMAL` (`1<<5` together with `1<<0`) while standalone dummy / registered-formula hits stay `flag = 0`,
...
- no miss/block/poison flag policy, critical-chance formulas, PvP damage-info, knockdown, or skill projectiles are owned here.

## Non-goals

This slice does not freeze:
- exact damage formulas beyond the existing bootstrap combat-profile HP mutation rules,
- miss, block, poison, or penetrate flag meanings,
- critical-chance formulas, or any rule that sometimes omits the critical bit,
- player-vs-player damage info,
- skill damage, projectile damage, or multi-target damage,
- whether skills or player-vs-player hits should emit a damage info packet,
...
- accepted standalone bootstrap combat-profile non-lethal normal attacks append one self `DAMAGE_INFO` frame after the `TARGET` HP refresh, using the authoritative attack/defense-derived damage descriptor for registered formula profiles; built-in standalone `practice_mob` hits encode `flag = DAMAGE_CRITICAL|DAMAGE_NORMAL` (`1<<5` together with `1<<0`) while standalone dummy / registered-formula hits stay `flag = 0`,
...
- a killing hit that also floors the owner now appends the dummy death/clear/hit-effect prefix and then the ordinary owner-floor `PLAYER_POINT_CHANGE(value=0)` / `DEAD(owner_vid)` / `TARGET(0, 0)` / owner `DAMAGE_INFO` suffix, while a killing hit that leaves the owner above `0` still omits owner retaliation,
...
- accepted immediate owner-floor hits now also append one self plus visible-peer plain-flag mob `DAMAGE_INFO(target_vid, applied_bootstrap_damage)` after that death/clear prefix and before the owner companion, while delayed and proximity-armed floors still omit a synthetic mob companion,
- later runtime slices can broaden flag meanings or other hit-effect policy without re-discovering the packet layout or recomputing damage outside the authoritative attack seam.
