...
18. later stale merchant `SHOP BUY` / `/shop_buy <slot>` mutations on that socket may still return the self-local inventory/info success burst, but they must not overwrite the persisted account snapshot and must not replace the replacement live owner's exact-name loopback inventory/currency snapshots after reclaim
19. if a stale socket later closes after one of those non-authoritative item mutations, a fresh reconnect/bootstrap must still rebuild from the authoritative persisted snapshot rather than from that stale socket's local divergence
20. later stale `TARGET` / `ATTACK` combat traffic from that socket must fail closed: it must not mutate runtime dummy HP, must not clear or replace the replacement live owner's selected combat target, and must not queue combat-visible frames to the replacement owner after reclaim
