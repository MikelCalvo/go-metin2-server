...
- if the reclaimed stale owner was already at the current bootstrap `0`-HP floor, living visible peers also receive the fresh entry burst followed by the owned `GC DEAD(owner_vid)` replay so the re-entered actor does not visually resurrect
- if a zero-HP owner is only a recipient of stale-reclaim teardown/re-entry fanout, that recipient is skipped instead of receiving peer visibility frames while broader player-death semantics remain out of scope
...
This keeps shared-world delivery and transfer lookup from targeting dead sessions and keeps reconnect/reclaim visibility aligned with the current dead-state replay contract.
...
6. if that reconnected player had already changed a supported `body`, `weapon`, or `head` item through the owned equip/unequip seam before disconnect, that peer re-entry visibility burst reuses the latest projected appearance from the persisted account snapshot
7. if a second fresh session reaches `ENTERGAME` for the same selected character while the original live owner is still registered, the bootstrap runtime now rejects that `ENTERGAME` instead of allowing a ghost `GAME` session with no shared-world ownership
8. that rejected duplicate `ENTERGAME` does not force an immediate socket teardown: the session remains in `LOADING`, and if the original live owner later disappears, the same socket can retry `ENTERGAME` and transition into `GAME` with the normal bootstrap burst
9. that later retry on the same waiting `LOADING` session must refresh from the latest persisted account snapshot before entering `GAME`, rather than reusing stale pre-rejection selected-character state cached before the runtime mutation
10. if still-online visible peers receive that retried owner again, they see the normal peer-entry burst with the latest projected appearance from that refreshed persisted snapshot rather than stale pre-rejection appearance
11. once a session loses live shared-world ownership because another session reclaimed that character, subsequent gameplay fanout from the stale socket must not keep replicating movement/chat to live peers
...
15. later stale `ITEM_MOVE` mutations on that socket may still return self-local item and matching item-quickslot refresh frames, but they must not overwrite the persisted inventory/quickslot snapshot and must not replace the replacement live owner's exact-name loopback inventory snapshot after reclaim
16. later stale quickslot add/delete/swap mutations on that socket may still return self-local quickslot frames, but they must not overwrite the persisted quickslot snapshot after reclaim
17. later stale `/use_item <slot>` mutations on that socket may still return self-local point/item/info frames, but they must not overwrite the persisted account snapshot and must not replace the replacement live owner's exact-name loopback inventory snapshot after reclaim
18. later stale gold-drop packets on that socket may still return self-local gold/ground/ownership frames, but they must not overwrite the persisted account snapshot and must not register peer-visible runtime ground-gold entries after reclaim
19. later stale merchant `SHOP BUY` / `/shop_buy <slot>` mutations on that socket may still return the self-local inventory/info success burst, but they must not overwrite the persisted account snapshot and must not replace the replacement live owner's exact-name loopback inventory/currency snapshots after reclaim
20. if a stale socket later closes after one of those non-authoritative item mutations, a fresh reconnect/bootstrap must still rebuild from the authoritative persisted snapshot rather than from that stale socket's local divergence
21. later stale `TARGET` / `ATTACK` combat traffic from that socket must fail closed: it must not mutate runtime dummy HP, must not clear or replace the replacement live owner's selected combat target, and must not queue combat-visible frames to the replacement owner after reclaim
