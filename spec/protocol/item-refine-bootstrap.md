...
- scroll / hyuniron / musin / black-dragon catalyst consumption beyond the first owned `type = 1` scroll-catalyst consume (`vnum = 39001`) below, money-only / guild fee variants, safe-refine catalyst variants, and peer-facing refine notifications remain deferred; in-place success / `fail_result_vnum` downgrade now also preserve presence-aware instance sockets/attributes through the rewritten cell, result `ITEM_SET`, and account snapshot (`docs/plans/2026-09-03-refine-success-downgrade-preserve-instance-sockets-attributes.md`); template-authored `keep_on_fail` and `fail_result_vnum` for `probability` in `1..99` are owned below.
...
- catalysts remain deferred except the first owned `type = 1` scroll-catalyst consume below; probability values in `1..99` use the owned injected-roll confirm seam below rather than this deterministic `0` path. Template-authored `keep_on_fail` and non-zero `fail_result_vnum` are rejected for `probability = 0` at the item-template store boundary.
...
- catalysts besides the first owned `type = 1` scroll-catalyst consume below, guild / money-only variants remain deferred; omitted/`false` `keep_on_fail` with omitted/`0` `fail_result_vnum` still destroys the whole source on a failed injected roll exactly like `probability = 0`.

## First accepted scroll-catalyst consume (`type = 1`)

This note freezes one extra authored consume on the already-owned confirm-after-preview seams. It does not invent hyuniron / musin / black-dragon types, guild / money-only fees, or catalyst-driven socket/attribute rewrite.

- `internal/proto/item` names `RefineTypeScroll = 1`, `RefineTypeCancel = 255`, and `RefineScrollCatalystVnum = 39001`; preview still echoes the request `type` on `REFINE_INFORMATION_NEW` and does not consume the catalyst
- matching confirm with remembered `type = 1` may mutate only when the same busy / zero-HP / source-identity / template / gold / material / result-template guards already owned by success, destroy-failure, keep-failure, downgrade-failure, and injected-roll paths pass, **and** the loaded item store still authors a valid template for `vnum = 39001`, **and** the selected character still carries at least one unlocked unequipped stack of that `vnum` outside the source refine cell
- on accepted confirm the runtime consumes exactly `1` of `vnum = 39001` from the lowest eligible carried cell (ordinary stack decrement / slot clear already owned by `ConsumeCarriedItemsExcludingSlots`) after gold/materials mutation and before persist; the source refine cell is never used to pay the catalyst
- the owned confirm burst inserts catalyst `ITEM_UPDATE` / `ITEM_DEL` plus item-removal `GC::QUICKSLOT_DEL` for a fully emptied catalyst cell after material refreshes / material-removal quickslot sync and before result `ITEM_SET`, source `ITEM_DEL`, gold `PLAYER_POINT_CHANGE`, and `RefineSuceeded` / `RefineFailed`
- missing catalyst, locked-only / equipped-only stacks, or a missing/invalid catalyst template fail closed with no frames and no inventory/gold/quickslot/persistence mutation, and they leave any still-valid open refine-dialog presentation untouched
- non-`type = 1` confirms (including existing tests that use types `2..7`) still skip catalyst consume even when a `39001` stack is carried
- in-place success / `fail_result_vnum` downgrade still preserve presence-aware instance sockets/attributes through the rewritten cell, result `ITEM_SET`, and account snapshot; this extra consume does not rewrite sockets/attributes

## Deferred behavior

Later slices must write a new contract before broadening this packet beyond the confirm-after-preview success seam, the deterministic `probability = 0` destroy-failure seam, the `probability` in `1..99` injected-roll confirm seam, the template-authored `keep_on_fail` keep-failure companion, the template-authored `fail_result_vnum` downgrade companion, and the first owned `type = 1` scroll-catalyst consume above. In particular, this note still does not freeze:

- refine catalyst semantics beyond the first owned `type = 1` scroll consume (`vnum = 39001`) above; hyuniron / musin / black-dragon types, guild / money-only fees, and safe-refine variants stay deferred
- item socket, metin-stone, attribute, or bonus-changing gameplay beyond the owned in-place success / `fail_result_vnum` downgrade presence preserve (source presence including explicit zero stays on the rewritten cell; omitted stays omitted so later encode may use the result template fallback) and beyond whole-source destroy on the owned destroy-failure / omitted-`keep_on_fail` / omitted-`fail_result_vnum` failed-roll paths; catalysts that would rewrite sockets/attributes stay deferred
...
- `internal/proto/item` freezes `REFINE` encode/decode behavior plus unexpected-header and invalid-payload rejection; it also freezes the `REFINE_INFORMATION` / `REFINE_INFORMATION_NEW` server packet layouts, including the fixed five-row material table and `material_count <= 5` validation, and names `RefineTypeScroll = 1`, `RefineTypeCancel = 255`, and `RefineScrollCatalystVnum = 39001`.
