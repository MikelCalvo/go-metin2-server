...
- random rolls, keep-grade, downgrade, or safe-refine outcomes for probability values in `1..99` beyond the injected-roll contract frozen in `docs/plans/2026-08-22-refine-confirm-probability-1-99-injected-roll.md` (not yet GREEN)
...
- The next honest refine confirm seam is frozen in `docs/plans/2026-08-22-refine-confirm-probability-1-99-injected-roll.md`: remembered `probability` in `1..99` may confirm only under the same guards already owned by success/destroy, using one injected roll in `1..100` (`roll <= probability` → owned success burst / `RefineSuceeded`; `roll > probability` → owned destroy burst / `RefineFailed`). Keep-grade and catalyst variants stay deferred. Runtime GREEN for that injected-roll seam is the next implementation follow-up.
