...
- scroll / hyuniron / musin / black-dragon catalyst consumption, money-only / guild fee variants, socket/attribute copy policy beyond preserving the existing instance id, keep-grade / downgrade / safe-refine failure variants, and peer-facing refine notifications remain deferred.
...
- catalysts and keep-grade / downgrade failure variants remain deferred; probability values in `1..99` use the owned injected-roll confirm seam below rather than this deterministic `0` path.

## First accepted refine injected-roll confirm seam (`probability` in `1..99`)

This note freezes the GREEN injected-roll confirm path for remembered dialogs whose authored `refine_info.probability` is in `1..99`. It reuses the already-owned success and destroy bursts; it does not invent keep-grade, catalyst, or peer-facing refine outcomes.
...
- the player helper `ApplyRefineWithRoll(slot, type, sourceID, remembered, sourceTemplate, resultTemplate, roll)` owns the roll decision:
  ...
  - if `roll > remembered.probability` → apply the owned destroy-failure mutation and emit the owned destroy burst ending in `CHAT_TYPE_COMMAND` `RefineFailed <type>` (including material-removal and destroyed-source `GC::QUICKSLOT_DEL`);
- `probability = 0` and `probability = 100` keep their deterministic paths and do not require a roll;
- keep-grade / downgrade / safe-refine / catalyst / guild / money-only variants remain deferred; failed `1..99` rolls destroy the whole source exactly like `probability = 0`.

## Deferred behavior

Later slices must write a new contract before broadening this packet beyond the confirm-after-preview success seam, the deterministic `probability = 0` destroy-failure seam, and the `probability` in `1..99` injected-roll confirm seam above. In particular, this note still does not freeze:

- refine catalyst semantics beyond the authored dialog-preview material/cost fields above
- keep-grade, downgrade, or safe-refine outcomes for probability values in `1..99` beyond whole-source destroy on a failed injected roll (see `docs/plans/2026-08-22-refine-confirm-probability-1-99-injected-roll.md`)
- item socket, metin-stone, attribute, or bonus-changing behavior beyond preserving the existing carried instance id on success (and beyond whole-source destroy on the owned destroy-failure / failed-roll paths)
