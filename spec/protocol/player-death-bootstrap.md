...
- fail-closed owner-side client/slash item-use attempts at that same floor before inventory consumption, carried-item stacking, point restoration, or private-shop bag setup command chat can run through the local `/use_item` harness path, carried-slot `ITEM_USE`, `ITEM_USE_TO_ITEM`, or the dedicated shop/silk-bag (`50200` / `71049`) USE companion
...
- fail-closed owner-side ground-item / gold pickup attempts at that same floor before ground-handle removal, inventory/gold mutation, pickup feedback, or owner-delivery side effects can run through `ITEM_PICKUP`; after `/restart_here` / `/restart_town` recovery the same owner-side inventory-item `ITEM_PICKUP` and the same gold `ITEM_PICKUP` succeed normally again (`TestGameSessionFlowPostFloorItemPickupFailsClosed`, `TestGameSessionFlowPostFloorItemPickupFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorGoldPickupFailsClosed`, `TestGameSessionFlowPostFloorGoldPickupFailsClosedBeforeRestartTown`, `TestGameSessionFlowPracticeMobItemPickupFailsClosedAfterImmediateRetaliationReachesOwnerHPFloor`)
...
- fail-closed owner-side item-give attempts at that same floor before anti-give feedback, peer delivery, inventory, quickslot, ground-handle, or persistence side effects can run through `ITEM_GIVE`; after `/restart_here` / `/restart_town` recovery the same anti-give `ITEM_GIVE` against a visible living peer emits the ordinary self-only reject chat again (`TestGameSessionFlowPostFloorItemGiveFailsClosedBeforeAntiGiveFeedback`, `TestGameSessionFlowPostFloorItemGiveFailsClosedBeforeRestartTown`)
...
- fail-closed owner-side storage-facing safebox/mall attempts at that same floor before safebox/mall response frames, template-authored `anti_safebox` feedback, carried inventory/equipment, quickslot, point, gold, ground-handle, or persistence side effects can run through `SAFEBOX_CHECKIN`, `SAFEBOX_CHECKOUT`, `SAFEBOX_ITEM_MOVE`, or `MALL_CHECKOUT`
...
- fail-closed owner-side quickslot add/delete/swap attempts at that same retaliation floor before quickslot mutation can run through packet `QUICKSLOT_ADD` / `QUICKSLOT_DEL` / `QUICKSLOT_SWAP`; after `/restart_here` / `/restart_town` recovery the same owner-side `QUICKSLOT_ADD` succeeds normally again (`TestGameSessionFlowPostFloorQuickslotAddFailsClosed`, `TestGameSessionFlowPostFloorQuickslotAddFailsClosedBeforeRestartTown`, `TestGameSessionFlowPracticeMobQuickslotEditsFailClosedAfterImmediateRetaliationReachesOwnerHPFloor`)
...
- one recipient-side non-player rebootstrap visibility gate so direct `VisibleStaticActorFrames(...)` rebuilds also skip static-actor `CHARACTER_ADD` / `CHAR_ADDITIONAL_INFO` / `CHARACTER_UPDATE` bursts for that same still-connected zero-HP owner
...
- one open exchange-window close rule when the death edge occurs while the owner is paired in the current bootstrap `EXCHANGE` shell: the owner receives one self-only `GC::EXCHANGE END`, the paired live peer receives one queued `GC::EXCHANGE END`, and the in-memory exchange pairing/display state is cleared so later `/restart_here` recovery can succeed at a fresh `EXCHANGE START` against the same living peer instead of operating on a stale pairing (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenExchangeShellBeforeRestartExchange`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenExchangeShellBeforeRestartExchange`); the same floor clear also lets `/restart_town` succeed at a fresh destination-map `EXCHANGE START` against a living empire-town peer (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenExchangeShellBeforeRestartTownExchange`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenExchangeShellBeforeRestartTownExchange`)
- one open host-only `MYSHOP` empty-sign close rule when the death edge occurs while the same-socket private-shop open/busy flag is set: the owner receives one empty-sign `GC::SHOP_SIGN` (`host VID` + empty sign) after the death/clear sequence (and after any merchant `GC::SHOP END`, before any lab `cube close` / safebox `CloseSafebox` or exchange `END` frames), currently visible peers receive the same empty-sign around-broadcast, and the busy flag clears so later `/close_myshop` stays silent
- one guest-browse `GC::SHOP END` companion on that same floor: a browsing guest whose host reaches the floor receives one queued self-only `GC::SHOP END` beside the host empty-sign around-broadcast, and a browsing guest who themselves reach the floor appends one self-only `GC::SHOP END` after death/clear before later guest `SHOP END` / `ON_CLICK` stay silent
- one same-socket busy-window teardown for an already-open bootstrap `/open_safebox` presentation and an already-open refine-dialog presentation on that same floor transition: an open safebox clears its busy flag (and shared-world busy publication) and appends one self-only `GC::CHAT` / `CHAT_TYPE_COMMAND` with message `CloseSafebox` after the death/clear sequence (and after any merchant `GC::SHOP END` / MYSHOP empty-sign / lab `cube close`, before any exchange close frames), matching the already-owned `/close_safebox` companion; an open refine-dialog clears its busy flag silently with no extra refine frames, matching `REFINE type = 255` cancel, so later `/restart_here` / `/restart_town` recovery does not keep orphan exchange-busy rejects
- one same-socket lab cube close companion when the death edge occurs while `/open_cube` presentation is open: the owner receives one self-only `CHAT_TYPE_COMMAND` `cube close` after the death/clear sequence (after any merchant `GC::SHOP END` / MYSHOP empty-sign, before any safebox `CloseSafebox` or exchange `END` frames), the peer-visible cube busy bit clears, and later `/close_cube` stays silent
...
- broader full input gating after death beyond the now-owned combat `TARGET` / `ATTACK`, relocation `MOVE` / `SYNC_POSITION`, stance `CHARACTER_POSITION`, static-actor `INTERACT`, merchant-buy / merchant-sell rejection, client/slash item-use / use-to-item / shop-silk-bag USE rejection, item/gold drop rejection, packet storage safebox/mall rejection, slash `/open_safebox` / matching warehouse `/safebox_password` / `/safebox_change_password` / `/safebox_money_save` / `/safebox_money_withdraw` / `/open_cube` open rejection, host-only `CG::MYSHOP` open rejection, guest private-shop browse `CG::ON_CLICK` open rejection, refineable `REFINE` preview open rejection, packet exchange-shell open/rejection/close (including living-peer start against a zero-HP partner), slash inventory-move rejection, slash equipment-mutation rejection, quickslot mutation rejection, peer-facing `CHAT` / `WHISPER` rejection, self-only `CHAT_TYPE_INFO` rejection, and recipient-side server-origin `CHAT_TYPE_NOTICE` skip at `0` HP
...
- accepted `/restart_here` and `/restart_town` now restore a usable live persisted HP from the race create MaxHP as part of recovery, so those seams remain honest once the floor is persisted; restart still fails closed while the owner is alive
- adjacent non-point persistence seams keep their current boundary for partial retaliation loss: position-only saves, carried inventory movement, merchant buy, and merchant sell-back may persist their own owned state while preserving the selected character's pre-retaliation persisted bootstrap HP point until the floor transition itself persists `0`
- if a fresh `ENTERGAME` bootstrap is built from an already-persisted or ticket-provided selected-character snapshot whose bootstrap HP point is `0` or below, the selected-character bootstrap now appends a self-only `GC DEAD(owner_vid)` after the selected-character `PLAYER_POINT_CHANGE` / any sorted quickslot replay and before trailing peer/static/ground visibility frames, so the owner is not silently presented as live at world entry; when that bootstrap HP point is below `0`, the self `PLAYER_POINT_CHANGE` clamps both `amount` and `value` to `0` before the `DEAD` replay instead of echoing a negative visible HP value
- those same restart seams now restore a usable live persisted HP from the race create MaxHP when recovering from a persisted `0`-HP floor; they still fail closed while the owner is alive and when no usable live rebuild snapshot can be produced
- once this floor is reached, the existing delayed retaliation cadence stops and later owner-side combat `TARGET` / `ATTACK` attempts still fail closed as already frozen elsewhere; if that owner had been holding a still-live content practice mob's aggro-lite engagement, the next fresh third-party `TARGET` attempt also treats the zero-HP owner snapshot as stale ownership and releases the gate without waiting for mob death / respawn or disconnect
...
- once this floor is reached, later owner-side client/slash item-use attempts also fail closed with no point-change / item-set success burst and no runtime or persisted inventory / point mutation; after `/restart_here` / `/restart_town` recovery the same owner-side `ITEM_USE` succeeds normally again (`TestGameSessionFlowPostFloorItemUseFailsClosed`, `TestGameSessionFlowPostFloorItemUseFailsClosedBeforeRestartTown`)
- once this floor is reached, later owner-side carried ordinary shop-bag (`50200`) / silk-bag (`71049`) `ITEM_USE` and `/use_item` attempts also fail closed with no `OpenPrivateShop` / `MyShopPriceList` command chat, no busy/armor INFO, and no inventory mutation (`TestGameSessionFlowPostFloorShopBagUseFailsClosed`, `TestGameSessionFlowPostFloorShopBagUseFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorSilkBagUseFailsClosed`, `TestGameSessionFlowPostFloorSilkBagUseFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same bag USE succeeds normally again and still does not consume the bag
- once this floor is reached, later owner-side `ITEM_USE_TO_ITEM` attempts also fail closed with no item-set / item-update / item-del / quickslot success burst and no runtime or persisted carried-item stacking mutation
- once this floor is reached, later owner-side `ITEM_DROP` / `ITEM_DROP2` attempts also fail closed with no point-change, ground-add, ownership, quickslot, or item-set/delete success burst and no runtime or persisted inventory / gold mutation (`TestGameSessionFlowPostFloorItemDropFailsClosed`, `TestGameSessionFlowPostFloorItemDropFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorGoldDropFailsClosed`, `TestGameSessionFlowPostFloorGoldDropFailsClosedBeforeRestartTown`, `TestGameSessionFlowPracticeMobItemDropFailsClosedAfterImmediateRetaliationReachesOwnerHPFloor`, `TestGameSessionFlowPracticeMobGoldDropFailsClosedAfterImmediateRetaliationReachesOwnerHPFloor`); after `/restart_here` / `/restart_town` recovery the same whole-stack `ITEM_DROP` and the same gold `ITEM_DROP` succeed normally again
- once this floor is reached, later owner-side carried-inventory move attempts also fail closed with no item-set success burst and no runtime or persisted carried-slot mutation; this applies to both packet `ITEM_MOVE` and the local `/inventory_move` harness path (`TestGameSessionFlowPostFloorItemMoveFailsClosed`, `TestGameSessionFlowPostFloorItemMoveFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same carried move succeeds normally again
...
- once this floor is reached, later owner-side slash `/open_safebox` and `/open_cube` attempts also fail closed with no `SAFEBOX_SIZE` / `cube open` companions and without setting safebox or cube busy presentation flags (`TestGameSessionFlowPostFloorOpenSafeboxFailsClosed`, `TestGameSessionFlowPostFloorOpenSafeboxFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorOpenCubeFailsClosed`, `TestGameSessionFlowPostFloorOpenCubeFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same lab `/open_safebox` and `/open_cube` succeed normally again
- once this floor is reached, later owner-side matching `/safebox_password` attempts against a still-pending warehouse challenge also fail closed with no `SAFEBOX_SIZE` / money companion and without setting safebox busy (`TestGameSessionFlowPostFloorSafeboxPasswordOpenFailsClosed`, `TestGameSessionFlowPostFloorSafeboxPasswordOpenFailsClosedBeforeRestartTown`); the same floor transition clears that pending challenge so later `/restart_here` / `/restart_town` recovery cannot reopen from the stale pre-death challenge until a fresh warehouse `INTERACT`
- once this floor is reached, later owner-side `/safebox_change_password` attempts also fail closed with no success/wrong-password info chat and without mutating the durable warehouse password (`TestGameSessionFlowPostFloorSafeboxChangePasswordFailsClosed`, `TestGameSessionFlowPostFloorSafeboxChangePasswordFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same command succeeds normally again
- once this floor is reached, later owner-side `/safebox_money_save` / `/safebox_money_withdraw` attempts also fail closed with no gold `PLAYER_POINT_CHANGE` / `SAFEBOX_MONEY_CHANGE` frames and without mutating carried gold or durable warehouse money (`TestGameSessionFlowPostFloorSafeboxMoneyFailsClosed`, `TestGameSessionFlowPostFloorSafeboxMoneyFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery and a fresh `/open_safebox`, the same money commands succeed normally again
- once this floor is reached, later owner-side host-only `CG::MYSHOP` open attempts also fail closed with no `GC::SHOP_SIGN` and without setting the private-shop busy flag (`TestGameSessionFlowPostFloorMyShopOpenFailsClosed`, `TestGameSessionFlowPostFloorMyShopOpenFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same host-only `MYSHOP` open succeeds normally again
- once this floor is reached, later owner-side guest private-shop browse open attempts via `CG::ON_CLICK` against a living peer's already-open `MYSHOP` also fail closed with no `GC::SHOP START` and without setting the guest browse flag (`TestGameSessionFlowPostFloorMyShopGuestOnClickFailsClosed`, `TestGameSessionFlowPostFloorMyShopGuestOnClickFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same living host (same-map or destination-map) can be freshly browsed with ordinary guest `SHOP START`
- once this floor is reached, later owner-side refineable `REFINE` preview attempts also fail closed with no `REFINE_INFORMATION_NEW` and without setting the refine-dialog busy flag (`TestGameSessionFlowPostFloorRefinePreviewOpenFailsClosed`, `TestGameSessionFlowPostFloorRefinePreviewOpenFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same refineable preview succeeds normally again
- once this floor is reached, later owner-side `EXCHANGE START` against a living visible peer also fails closed with no self/peer exchange start frames and without creating exchange pairing/display state (`TestGameSessionFlowPostFloorExchangeStartFailsClosed`, `TestGameSessionFlowPostFloorExchangeStartFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same fresh start succeeds normally again against a living visible peer (same-map or destination-map); a living peer's `EXCHANGE START` aimed at that same still-connected zero-HP owner likewise fails closed, and after the dead owner's `/restart_here` / `/restart_town` recovery that same living peer (same-map or destination-map) can freshly start against the recovered partner with ordinary paired start frames (`TestGameSessionFlowPostFloorExchangeStartAgainstDeadPartnerFailsClosed`, `TestGameSessionFlowPostFloorExchangeStartAgainstDeadPartnerFailsClosedBeforeRestartTown`)
- once this floor is reached, later owner-side slash `/equip_item` and `/unequip_item` attempts also fail closed with no item-delete / item-set / point-change / character-update success burst and no runtime or persisted inventory / equipment / point mutation (`TestGameSessionFlowPostFloorEquipItemFailsClosed`, `TestGameSessionFlowPostFloorEquipItemFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorUnequipItemFailsClosed`, `TestGameSessionFlowPostFloorUnequipItemFailsClosedBeforeRestartTown`, `TestGameSessionFlowPracticeMobEquipItemFailsClosedAfterImmediateRetaliationReachesOwnerHPFloor`, `TestGameSessionFlowPracticeMobUnequipItemFailsClosedAfterDelayedRetaliationReachesOwnerHPFloor`); after `/restart_here` / `/restart_town` recovery the same `/equip_item` and `/unequip_item` succeed normally again
- once this floor is reached, later owner-side packet quickslot add/delete/swap attempts also fail closed with no quickslot refresh frames and no runtime or persisted quickslot or inventory mutation; after `/restart_here` / `/restart_town` recovery the same owner-side `QUICKSLOT_ADD` succeeds normally again (`TestGameSessionFlowPostFloorQuickslotAddFailsClosed`, `TestGameSessionFlowPostFloorQuickslotAddFailsClosedBeforeRestartTown`)
...
- if this floor is reached while the owner is paired in the bootstrap exchange-window shell, the same floor transition now also appends one self-only `GC::EXCHANGE END` after the death/clear sequence, queues one `GC::EXCHANGE END` to the paired live peer, and clears the in-memory exchange pairing/display state so later `EXCHANGE CANCEL` / display requests fail closed instead of operating on stale trade UI state; after `/restart_here` recovery from that same floor, a fresh `EXCHANGE START` against the same living visible peer succeeds with ordinary paired start frames (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenExchangeShellBeforeRestartExchange`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenExchangeShellBeforeRestartExchange`); after `/restart_town` recovery from that same floor, a fresh destination-map `EXCHANGE START` against a living empire-town peer also succeeds (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenExchangeShellBeforeRestartTownExchange`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenExchangeShellBeforeRestartTownExchange`)
- if this floor is reached while the owner already had an accepted host-only `MYSHOP` open, the same floor transition now also clears that same-socket private-shop busy flag and appends one empty-sign `GC::SHOP_SIGN` after the death/clear sequence (and after any merchant `GC::SHOP END`, before any lab `cube close` / safebox `CloseSafebox` or exchange close frames), with the same empty-sign around-broadcast to currently visible peers; later `/close_myshop` stays silent and inventory/gold stay unchanged
- if this floor is reached while a visible peer was browsing that host's open MYSHOP, that browsing guest also receives one queued self-only `GC::SHOP END` beside the empty-sign around-broadcast and clears browse association so later guest `SHOP END` / `ON_CLICK` stay silent
...
- once this floor is reached, direct static-actor visibility rebuilds through `VisibleStaticActorFrames(...)` also return no `CHARACTER_ADD` / `CHAR_ADDITIONAL_INFO` / `CHARACTER_UPDATE` burst for that still-connected zero-HP owner, matching the already-owned ground-entry rebootstrap gate
...
- runtime/operator player snapshots also treat hand-authored or persisted negative bootstrap HP point values as already below that same death floor, so malformed snapshots are not presented as live simply because the point is less than zero rather than exactly zero; the selected-character `ENTERGAME` point refresh mirrors that floor by clamping below-zero HP values to `0` before replaying `DEAD(owner_vid)`
...
- if the player then re-selects the same persisted character and sends a fresh `ENTERGAME` on that same socket, the self bootstrap rebuilds from the persisted account snapshot: once the retaliation floor was reached, that means `PLAYER_POINT_CHANGE` at `0` plus self `GC DEAD(owner_vid)` rather than silently restoring the pre-retaliation live value
...
- after `/restart_here` recovery from that same floor, a fresh `EXCHANGE START` against a living visible peer succeeds instead of returning the open-merchant busy info-chat reject, matching the already-owned MYSHOP / safebox / refine / lab cube post-restart exchange-recovery twins
- after `/restart_town` recovery from that same floor, a fresh `EXCHANGE START` against a living destination-map peer also succeeds instead of returning the open-merchant busy reject, so the town-return busy-clear path matches the same-socket `/restart_here` twin and the already-owned MYSHOP / cube / safebox / refine town recovery twins
- focused coverage freezes both the immediate hit-piggyback floor and the delayed server-origin floor paths for that post-`/restart_here` exchange-recovery twin (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenMerchantBeforeRestartExchange`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenMerchantBeforeRestartExchange`) and the post-`/restart_town` destination-peer twin (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenMerchantBeforeRestartTownExchange`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenMerchantBeforeRestartTownExchange`) beside the already-owned close-only proofs (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenMerchantWindow`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenMerchantWindow`)
...
- proving the post-restart exchange path closes the same orphan-busy hole already owned for MYSHOP / safebox / refine / lab cube without widening merchant buy/sell gameplay
- proving the post-`/restart_town` destination-peer exchange path closes the remaining orphan-busy hole left after the `/restart_here` merchant twin landed

## First owned MYSHOP empty-sign close at the retaliation floor

The current bootstrap player-death contract now also owns one narrow host-only private-shop teardown rule for that same selected live owner session:
- if that session already accepted a host-only `MYSHOP` open before immediate or delayed practice-mob retaliation reached `0` HP, the same owned floor transition clears the same-socket private-shop busy flag and appends one empty-sign `GC::SHOP_SIGN` (`host VID` + empty sign)
- that empty-sign companion arrives only after the already-owned owner-side floor ordering completes first (`GC PLAYER_POINT_CHANGE(value=0)` -> `GC DEAD(owner_vid)` -> `GC TARGET(0, 0)`), after any merchant `GC::SHOP END`, and before any lab `cube close` / safebox `CloseSafebox` or exchange `END` frames
- currently visible peers receive the same empty-sign around-broadcast already owned by the MYSHOP close companion; later `/close_myshop` stays silent and inventory/gold stay unchanged
- after `/restart_here` recovery from that same floor, a fresh `EXCHANGE START` against a living visible peer succeeds instead of returning the open-private-shop busy reject, matching the already-owned safebox / refine / lab cube post-restart exchange-recovery twins
- after `/restart_town` recovery from that same floor, a fresh `EXCHANGE START` against a living destination-map peer also succeeds instead of returning the open-private-shop busy reject, so the town-return busy-clear path matches the same-socket `/restart_here` twin
- focused coverage freezes both the immediate hit-piggyback floor and the delayed server-origin floor paths, including the post-`/restart_here` exchange-recovery twin (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenMyShop`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenMyShop`) and the post-`/restart_town` destination-peer twin (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenMyShopBeforeRestartTownExchange`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenMyShopBeforeRestartTownExchange`)
...
Why this is the current owned boundary:
- merchant and exchange already had explicit close companions on the death edge, but an open private-shop busy/sign could still survive into `/restart_here` without a combat-lane proof
- reusing the already-owned empty-sign `GC::SHOP_SIGN` close companion keeps the teardown honest without inventing a death-specific private-shop packet family
- proving the post-restart exchange path closes the same orphan-busy hole already owned for safebox / refine / lab cube without widening private-shop buy/sell gameplay
- guest browse leave was already owned for explicit `SHOP END` and host `/close_myshop`, but the practice-mob floor still needed combat-lane proof that both host-death and guest-death clear browse with the same `GC::SHOP END` companion
...
- if that session already opened the bootstrap `/open_safebox` presentation before immediate or delayed practice-mob retaliation reached `0` HP, the same owned floor transition clears that same-socket safebox busy flag and its shared-world busy publication, then appends one self-only `GC::CHAT` / `CHAT_TYPE_COMMAND` with message `CloseSafebox` after the death/clear sequence (and after any merchant `GC::SHOP END` / MYSHOP empty-sign / lab `cube close`, before any exchange close frames)
...
- the safebox teardown reuses the already-owned `/close_safebox` companion rather than inventing a death-specific safebox packet; refine teardown stays intentionally silent: no synthetic refine cancel/result frame
- neither teardown mutates inventory, gold, quickslots, or persistence, and neither emits an extra `SAFEBOX_SIZE`
- after `/restart_here` or `/restart_town` recovers from that floor, a later same-socket `EXCHANGE START` against a living visible peer must no longer be rejected by the orphaned safebox/refine/MYSHOP/cube busy-window policy; the recovered owner may open a fresh trade shell normally
- after `/restart_town` recovery from an open-safebox or open-refine floor specifically, a fresh `EXCHANGE START` against a living destination-map peer also succeeds instead of returning the open-safebox / open-refine busy reject, so the town-return busy-clear path matches the same-socket `/restart_here` twin and the already-owned MYSHOP / cube town recovery twins
- accepted `/restart_here` / `/restart_town` also clear any still-open same-socket safebox/refine/cube/MYSHOP busy presentation as part of recovery so a revived owner does not keep a pre-death busy lock
...
- focused coverage freezes the immediate hit-piggyback floor path plus post-`/restart_here` exchange recovery for both busy windows (`TestGameSessionFlowPracticeMobDeathClearsOpenSafeboxBusyBeforeRestartExchange`, `TestGameSessionFlowPracticeMobDeathClearsOpenRefineBusyBeforeRestartExchange`), the delayed server-origin floor paths (`TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenSafebox`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenRefine`), the safebox post-`/restart_town` destination-peer twin (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenSafeboxBeforeRestartTownExchange`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenSafeboxBeforeRestartTownExchange`), and the refine post-`/restart_town` destination-peer twin (`TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenRefineBeforeRestartTownExchange`, `TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenRefineBeforeRestartTownExchange`)
...
- guest MYSHOP browse, lab cube, and safebox already owned both immediate and delayed combat-lane floor proofs; the delayed refine twin closes the same gap for silent refine-dialog busy clear
- the safebox and refine town-return twins close the same orphan-busy hole across the owned empire town-return transfer rebuild already frozen for MYSHOP / cube

## First owned lab cube close at the retaliation floor

The current bootstrap player-death contract now also owns one narrow lab cube teardown rule for that same selected live owner session:
- if that session already opened lab `/open_cube` presentation before immediate or delayed practice-mob retaliation reached `0` HP, the same owned floor transition clears the same-socket cube open flag and its peer-visible shared-world cube busy bit, then appends one self-only `CHAT_TYPE_COMMAND` `cube close`
...
- once that same floor has been reached, later carried ordinary shop-bag (`50200`) / silk-bag (`71049`) `ITEM_USE` / `/use_item` attempts also fail closed before private-shop setup command chat (`OpenPrivateShop` / `MyShopPriceList`) can run; after `/restart_here` / `/restart_town` the same bag USE succeeds normally again without consuming the bag (`TestGameSessionFlowPostFloorShopBagUseFailsClosed`, `TestGameSessionFlowPostFloorShopBagUseFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorSilkBagUseFailsClosed`, `TestGameSessionFlowPostFloorSilkBagUseFailsClosedBeforeRestartTown`)
- once that same floor has been reached, later `ITEM_DROP` / `ITEM_DROP2` attempts also fail closed for both inventory items and gold; after `/restart_here` / `/restart_town` the same whole-stack `ITEM_DROP` and the same gold `ITEM_DROP` succeed normally again (`TestGameSessionFlowPostFloorItemDropFailsClosed`, `TestGameSessionFlowPostFloorItemDropFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorGoldDropFailsClosed`, `TestGameSessionFlowPostFloorGoldDropFailsClosedBeforeRestartTown`)
...
- once that same floor has been reached, later `ITEM_PICKUP` attempts also fail closed before removing the ground item / gold handle, mutating inventory or gold, emitting pickup feedback, or running fallback owner-delivery side effects; after `/restart_here` / `/restart_town` recovery the same owner-side inventory-item `ITEM_PICKUP` and the same gold `ITEM_PICKUP` succeed normally again (`TestGameSessionFlowPostFloorItemPickupFailsClosed`, `TestGameSessionFlowPostFloorItemPickupFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorGoldPickupFailsClosed`, `TestGameSessionFlowPostFloorGoldPickupFailsClosedBeforeRestartTown`)
- once that same floor has been reached, later packet `ITEM_MOVE` and slash `/inventory_move` attempts also fail closed for carried-inventory moves before runtime slot mutation, persistence, item-set frames, or quickslot sync can run; after `/restart_here` / `/restart_town` the same carried move succeeds normally again (`TestGameSessionFlowPostFloorItemMoveFailsClosed`, `TestGameSessionFlowPostFloorItemMoveFailsClosedBeforeRestartTown`)
- once that same floor has been reached, later packet `ITEM_GIVE` attempts also fail closed before the template-authored `anti_give` feedback path, peer delivery, inventory, quickslot, ground-handle, or persistence side effects can run; after `/restart_here` / `/restart_town` recovery the same anti-give `ITEM_GIVE` against a visible living peer emits the ordinary self-only reject chat again (`TestGameSessionFlowPostFloorItemGiveFailsClosedBeforeAntiGiveFeedback`, `TestGameSessionFlowPostFloorItemGiveFailsClosedBeforeRestartTown`)
...
- once that same floor has been reached, later storage-facing `SAFEBOX_CHECKIN`, `SAFEBOX_CHECKOUT`, `SAFEBOX_ITEM_MOVE`, and `MALL_CHECKOUT` attempts also fail closed before safebox/mall response frames, template-authored `anti_safebox` feedback, carried inventory/equipment, quickslot, point, gold, ground-handle, or persistence side effects can run
- the denial stays intentionally quiet in this slice: no synthetic failure info chat, no fallback revive packet, and no broader consumable/drop/move/give/refine/storage UI contract change are claimed yet

This keeps the next post-floor expansion small and honest too:
- after merchant-buy denial, client/slash item use, item drop, item pickup, packet item move, packet item give, packet refine, and storage-facing safebox/mall packets were the next dangerous already-open gameplay contexts because they could still mutate runtime and persisted inventory/gold, restore points, stack or move carried items, remove visible ground handles, register visible ground items, emit item-template rejection feedback, or later grow into two-party/economic/storage mutations even if the owner had already died in the current bootstrap retaliation loop
- the repo still does **not** yet claim revive policy or a broader general post-death action-lock contract at `0` HP

## First owned post-floor busy-shell open denial

The current bootstrap player-death contract now also owns one narrow busy-shell **open** denial for that same selected live owner session while still at the `0`-HP floor before `/restart_here` / `/restart_town` recovery:

- later slash `/open_safebox` attempts fail closed with no `SAFEBOX_SIZE` / money companion and without setting the same-socket safebox busy presentation flag (`TestGameSessionFlowPostFloorOpenSafeboxFailsClosed`, `TestGameSessionFlowPostFloorOpenSafeboxFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same lab open succeeds normally again
- later matching `/safebox_password` attempts against a pending warehouse `ShowMeSafeboxPassword` challenge also fail closed with no `SAFEBOX_SIZE` / money companion and without setting safebox busy (`TestGameSessionFlowPostFloorSafeboxPasswordOpenFailsClosed`, `TestGameSessionFlowPostFloorSafeboxPasswordOpenFailsClosedBeforeRestartTown`); the floor transition clears that pending challenge so `/restart_here` / `/restart_town` cannot reopen the warehouse from a stale pre-death password attempt until a fresh warehouse `INTERACT` re-arms the challenge
- later `/safebox_change_password <old> <new>` attempts fail closed with no success/wrong-password info chat and without mutating the durable warehouse password (`TestGameSessionFlowPostFloorSafeboxChangePasswordFailsClosed`, `TestGameSessionFlowPostFloorSafeboxChangePasswordFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same command succeeds normally again
- later `/safebox_money_save <amount>` / `/safebox_money_withdraw <amount>` attempts fail closed with no gold `PLAYER_POINT_CHANGE` / `SAFEBOX_MONEY_CHANGE` frames and without mutating carried gold or durable warehouse money (`TestGameSessionFlowPostFloorSafeboxMoneyFailsClosed`, `TestGameSessionFlowPostFloorSafeboxMoneyFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery and a fresh `/open_safebox`, the same money commands succeed normally again
- later slash `/open_cube` attempts fail closed with no `cube open` command chat and without setting the peer-visible cube busy bit (`TestGameSessionFlowPostFloorOpenCubeFailsClosed`, `TestGameSessionFlowPostFloorOpenCubeFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same lab open succeeds normally again
- later host-only `CG::MYSHOP` open attempts fail closed with no `GC::SHOP_SIGN` and without setting the private-shop busy flag (`TestGameSessionFlowPostFloorMyShopOpenFailsClosed`, `TestGameSessionFlowPostFloorMyShopOpenFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same host-only `MYSHOP` open succeeds normally again
- later guest private-shop browse open attempts via `CG::ON_CLICK` against a living peer's already-open `MYSHOP` also fail closed with no `GC::SHOP START` and without setting the guest browse flag (`TestGameSessionFlowPostFloorMyShopGuestOnClickFailsClosed`, `TestGameSessionFlowPostFloorMyShopGuestOnClickFailsClosedBeforeRestartTown`); after `/restart_here` recovery the same living host can be freshly browsed with ordinary guest `SHOP START`, and after `/restart_town` recovery a living destination-map host's open `MYSHOP` can likewise be freshly browsed; this is the busy-shell **open** twin beside host-only `CG::MYSHOP` / bag USE, distinct from the already-owned floor **close** companions that clear an already-open guest browse
- later carried ordinary shop-bag (`50200`) / silk-bag (`71049`) `ITEM_USE` / `/use_item` attempts fail closed with no `OpenPrivateShop` / `MyShopPriceList` command chat and without inventory mutation (`TestGameSessionFlowPostFloorShopBagUseFailsClosed`, `TestGameSessionFlowPostFloorShopBagUseFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorSilkBagUseFailsClosed`, `TestGameSessionFlowPostFloorSilkBagUseFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same bag USE succeeds normally again and still does not consume the bag; this is the private-shop setup companion beside host-only `CG::MYSHOP`, not a consumable point-use path
- later refineable `REFINE` preview attempts fail closed with no `REFINE_INFORMATION_NEW` and without setting the same-socket refine-dialog busy flag (`TestGameSessionFlowPostFloorRefinePreviewOpenFailsClosed`, `TestGameSessionFlowPostFloorRefinePreviewOpenFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same refineable preview succeeds normally again; this is distinct from the already-owned non-refineable reject-feedback denial (`TestGameSessionFlowPostFloorItemRefineFailsClosedBeforeRejectFeedback`)
- later requester `EXCHANGE START` against a living visible peer fails closed with no self/peer exchange start frames and without creating exchange pairing/display state (`TestGameSessionFlowPostFloorExchangeStartFailsClosed`, `TestGameSessionFlowPostFloorExchangeStartFailsClosedBeforeRestartTown`); after `/restart_here` recovery the same living visible peer can be freshly started with ordinary paired start frames, and after `/restart_town` recovery a living destination-map peer can likewise be freshly started
- a living peer's `EXCHANGE START` aimed at that same still-connected zero-HP owner also fails closed with no requester/partner exchange frames; after that dead owner's `/restart_here` recovery, the same living peer can freshly start against the recovered partner with ordinary paired start frames and no inventory/gold mutation (`TestGameSessionFlowPostFloorExchangeStartAgainstDeadPartnerFailsClosed`); after `/restart_town` recovery, a living destination-map peer can likewise freshly start against the recovered partner (`TestGameSessionFlowPostFloorExchangeStartAgainstDeadPartnerFailsClosedBeforeRestartTown`)
...
- already-owned floor **close** companions for shells that were open before the death edge, plus post-`/restart_*` exchange-recovery twins for those pre-open shells, remain separate seams; this section freezes fresh open attempts while still dead and proves the same post-restart recovery path already owned by MYSHOP / refine / safebox / cube / bag-USE opens
...
- if that same floor is reached while the owner already had a merchant preview open, the same floor transition now also appends one self-only `GC::SHOP END` after self `GC DEAD(owner_vid)` plus self `GC TARGET(0, 0)` and clears the active merchant context so later client `SHOP END` fails closed too; after `/restart_here` recovery from that same floor, a fresh `EXCHANGE START` against a living visible peer succeeds instead of returning the open-merchant busy reject; after `/restart_town` recovery from that same floor, a fresh `EXCHANGE START` against a living destination-map peer also succeeds instead of returning the open-merchant busy reject
- if that same floor is reached while the owner already had an accepted host-only `MYSHOP` open, the same floor transition now also appends one empty-sign `GC::SHOP_SIGN` after self `GC DEAD(owner_vid)` plus self `GC TARGET(0, 0)` (after any merchant `GC::SHOP END`, before any lab `cube close` / safebox `CloseSafebox` or exchange close frames), fans that same empty sign to currently visible peers, and clears the private-shop busy flag so later `/close_myshop` stays silent; after `/restart_here` recovery from that same floor, a fresh `EXCHANGE START` against a living visible peer succeeds instead of returning the open-private-shop busy reject
- if that same floor is reached while a visible peer was browsing that open MYSHOP, the browsing guest also receives one queued self-only `GC::SHOP END` beside the empty-sign around-broadcast; if the dying session itself was browsing another peer's open MYSHOP, that session appends one self-only `GC::SHOP END` after death/clear and clears browse without inventory/gold mutation
- if that same floor is reached while the owner already had a lab `/open_cube` presentation open, the same floor transition now also appends one self-only `CHAT_TYPE_COMMAND` `cube close` after self `GC DEAD(owner_vid)` plus self `GC TARGET(0, 0)` (after any merchant `GC::SHOP END` / MYSHOP empty-sign, before any safebox `CloseSafebox` or exchange close frames), clears the peer-visible cube busy bit, and leaves later `/close_cube` silent
- if that same floor is reached while the owner already had a bootstrap exchange shell open, the same floor transition now also appends one self-only `GC::EXCHANGE END` after self `GC DEAD(owner_vid)` plus self `GC TARGET(0, 0)`, queues one `GC::EXCHANGE END` to the paired live peer, and clears the exchange state so later exchange-display or cancel requests fail closed too; after `/restart_here` recovery from that same floor, a fresh `EXCHANGE START` against the same living visible peer succeeds with ordinary paired start frames instead of operating on a stale exchange pairing; after `/restart_town` recovery from that same floor, a fresh `EXCHANGE START` against a living destination-map peer also succeeds instead of operating on a stale exchange pairing
- if that same floor is reached while the owner already had an open `/open_safebox` presentation, the same floor transition now also clears that same-socket safebox busy flag and appends one self-only `CHAT_TYPE_COMMAND` `CloseSafebox` after the death/clear sequence (and after any merchant `GC::SHOP END` / MYSHOP empty-sign / lab `cube close`, before any exchange close frames); an already-open refine-dialog presentation still clears silently with no extra refine frames
- once that same floor is reached, later owner-side slash `/use_item` and carried-slot `ITEM_USE` attempts also fail closed before runtime/persisted inventory consumption or point restoration can run; after `/restart_here` / `/restart_town` recovery the same owner-side `ITEM_USE` succeeds normally again (`TestGameSessionFlowPostFloorItemUseFailsClosed`, `TestGameSessionFlowPostFloorItemUseFailsClosedBeforeRestartTown`)
- once that same floor is reached, later owner-side packet `ITEM_MOVE` and slash `/inventory_move` attempts also fail closed before runtime/persisted carried-inventory slot mutation or item-set success frames can run; after `/restart_here` / `/restart_town` recovery the same carried move succeeds normally again (`TestGameSessionFlowPostFloorItemMoveFailsClosed`, `TestGameSessionFlowPostFloorItemMoveFailsClosedBeforeRestartTown`)
- once that same floor is reached, later owner-side slash `/equip_item` attempts also fail closed before carried/equipped item movement, equip-effect point mutation, or appearance refresh can run; after `/restart_here` / `/restart_town` recovery the same `/equip_item` succeeds normally again (`TestGameSessionFlowPostFloorEquipItemFailsClosed`, `TestGameSessionFlowPostFloorEquipItemFailsClosedBeforeRestartTown`)
- once that same floor is reached, later owner-side slash `/unequip_item` attempts also fail closed before equipped/carried item movement, inverse equip-effect point mutation, or appearance refresh can run; after `/restart_here` / `/restart_town` recovery the same `/unequip_item` succeeds normally again (`TestGameSessionFlowPostFloorUnequipItemFailsClosed`, `TestGameSessionFlowPostFloorUnequipItemFailsClosedBeforeRestartTown`)
- once that same floor is reached, later owner-side packet `ITEM_DROP` / `ITEM_DROP2` attempts also fail closed before inventory mutation, gold debit, ground-item registration, ownership delivery, quickslot sync, or persistence mutation can run; after `/restart_here` / `/restart_town` recovery the same whole-stack `ITEM_DROP` and the same gold `ITEM_DROP` succeed normally again (`TestGameSessionFlowPostFloorItemDropFailsClosed`, `TestGameSessionFlowPostFloorItemDropFailsClosedBeforeRestartTown`, `TestGameSessionFlowPostFloorGoldDropFailsClosed`, `TestGameSessionFlowPostFloorGoldDropFailsClosedBeforeRestartTown`)
- once that same floor is reached, later owner-side packet quickslot add/delete/swap attempts also fail closed before quickslot refresh frames or runtime/persisted quickslot / inventory mutation can run; after `/restart_here` / `/restart_town` recovery the same owner-side `QUICKSLOT_ADD` succeeds normally again (`TestGameSessionFlowPostFloorQuickslotAddFailsClosed`, `TestGameSessionFlowPostFloorQuickslotAddFailsClosedBeforeRestartTown`)
- once that same floor is reached, later owner-side packet `ITEM_GIVE` attempts also fail closed before anti-give feedback, peer delivery, inventory, quickslot, ground-handle, or persistence side effects can run; after `/restart_here` / `/restart_town` recovery the same anti-give `ITEM_GIVE` against a visible living peer emits the ordinary self-only reject chat again (`TestGameSessionFlowPostFloorItemGiveFailsClosedBeforeAntiGiveFeedback`, `TestGameSessionFlowPostFloorItemGiveFailsClosedBeforeRestartTown`)
...
- once retaliation has already driven the owning character to `0` HP and released that owner's practice-mob engagement, later living peers that reacquire and land a non-lethal accepted hit (or fire a non-floor delayed retaliation beat) against the still-live mob still queue the owned mob/owner `DAMAGE_INFO` companions to living observers, but those same hit-effect frames skip the still-connected zero-HP former owner entirely
