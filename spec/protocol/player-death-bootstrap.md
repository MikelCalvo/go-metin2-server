...
- one recipient-side destination ground-item visibility gate for later transfer/rebootstrap paths whose queued `ITEM_GROUND_ADD` / `ITEM_OWNERSHIP` burst would otherwise be delivered to that same still-connected zero-HP owner after relocating into a map/AOI with existing ground items
...
- once this floor is reached, later transfer/rebootstrap paths that move that same zero-HP owner into visibility of existing ground items also skip the destination `ITEM_GROUND_ADD` / `ITEM_OWNERSHIP` burst for that owner, while the ground item remains live and available to living visible sessions
...
- the earlier slash-command seams stay separate here: `/quit`, `/logout`, and `/phase_select` keep their current independent behavior, the owned same-socket `/restart_here` and `/restart_town` recovery seams are frozen separately in `player-restart-here-bootstrap.md` and `player-restart-town-bootstrap.md`, and the already-owned `/shop_buy`, `/use_item`, `ITEM_USE`, `/inventory_move`, `/equip_item`, and `/unequip_item` denial paths keep their existing post-floor rules
