...
- later matching `/safebox_password` attempts against a pending warehouse `ShowMeSafeboxPassword` challenge also fail closed with no `SAFEBOX_SIZE` / money companion and without setting safebox busy (`TestGameSessionFlowPostFloorSafeboxPasswordOpenFailsClosed`); the floor transition clears that pending challenge so `/restart_here` / `/restart_town` cannot reopen the warehouse from a stale pre-death password attempt until a fresh warehouse `INTERACT` re-arms the challenge
- later `/safebox_change_password <old> <new>` attempts fail closed with no success/wrong-password info chat and without mutating the durable warehouse password (`TestGameSessionFlowPostFloorSafeboxChangePasswordFailsClosed`); after `/restart_here` / `/restart_town` recovery the same command succeeds normally again
