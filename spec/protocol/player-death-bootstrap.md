...
- once this floor is reached, later owner-side carried ordinary shop-bag (`50200`) / silk-bag (`71049`) `ITEM_USE` and `/use_item` attempts also fail closed with no `OpenPrivateShop` / `MyShopPriceList` command chat, no busy/armor INFO, and no inventory mutation (`TestGameSessionFlowPostFloorShopBagUseFailsClosed`, `TestGameSessionFlowPostFloorSilkBagUseFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same bag USE succeeds normally again and still does not consume the bag
...
- later carried ordinary shop-bag (`50200`) / silk-bag (`71049`) `ITEM_USE` / `/use_item` attempts fail closed with no `OpenPrivateShop` / `MyShopPriceList` command chat and without inventory mutation (`TestGameSessionFlowPostFloorShopBagUseFailsClosed`, `TestGameSessionFlowPostFloorSilkBagUseFailsClosedBeforeRestartTown`); after `/restart_here` / `/restart_town` recovery the same bag USE succeeds normally again and still does not consume the bag; this is the private-shop setup companion beside host-only `CG::MYSHOP`, not a consumable point-use path
