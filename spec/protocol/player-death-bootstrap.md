...
- one recipient-side peer-visibility teardown gate for later visible peer `CHARACTER_DEL` frames from peer leave, relocate-away transfer, or AOI `MOVE` / `SYNC_POSITION` out-of-range teardown aimed at that same still-connected zero-HP owner through the current shared-world leave / relocate / AOI visibility-transition paths
