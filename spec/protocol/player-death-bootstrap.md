...
- after `/restart_town` recovery from an open-safebox floor specifically, a fresh `EXCHANGE START` against a living destination-map peer also succeeds instead of returning the open-safebox busy reject, so the town-return busy-clear path matches the same-socket `/restart_here` twin and the already-owned MYSHOP / cube town recovery twins
...
- the safebox town-return twin closes the same orphan-busy hole across the owned empire town-return transfer rebuild already frozen for MYSHOP / cube; refine town-return remains a follow-on twin
