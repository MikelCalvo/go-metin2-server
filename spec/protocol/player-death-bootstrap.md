...
- if this floor is reached while the owner already had an accepted host-only `MYSHOP` open, the same floor transition now also clears that same-socket private-shop busy flag and appends one empty-sign `GC::SHOP_SIGN` after the death/clear sequence (and after any merchant `GC::SHOP END`, before any safebox `CloseSafebox` or exchange close frames), with the same empty-sign around-broadcast to currently visible peers; later `/close_myshop` stays silent and inventory/gold stay unchanged
...
- that empty-sign companion arrives only after the already-owned owner-side floor ordering completes first (`GC PLAYER_POINT_CHANGE(value=0)` -> `GC DEAD(owner_vid)` -> `GC TARGET(0, 0)`), after any merchant `GC::SHOP END`, and before any safebox `CloseSafebox` or exchange `END` frames
...
- if that session already opened the bootstrap `/open_safebox` presentation before immediate or delayed practice-mob retaliation reached `0` HP, the same owned floor transition clears that same-socket safebox busy flag and its shared-world busy publication, then appends one self-only `GC::CHAT` / `CHAT_TYPE_COMMAND` with message `CloseSafebox` after the death/clear sequence (and after any merchant `GC::SHOP END` / MYSHOP empty-sign, before any exchange close frames)
