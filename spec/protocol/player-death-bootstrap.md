...
- runtime/operator player snapshots also treat hand-authored or persisted negative bootstrap HP point values as already below that same death floor, so malformed snapshots are not presented as live simply because the point is less than zero rather than exactly zero
