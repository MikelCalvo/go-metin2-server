...
- one selected-character `ENTERGAME` replay rule for account/ticket snapshots that are already at the bootstrap `0`-HP floor before world entry: the self bootstrap remains ordered normally, then appends one self-only `GC DEAD(owner_vid)` before any trailing world visibility frames
...
- if a fresh `ENTERGAME` bootstrap is built from an already-persisted or ticket-provided `0`-HP selected-character snapshot, the selected-character bootstrap now appends a self-only `GC DEAD(owner_vid)` after the selected-character `PLAYER_POINT_CHANGE` / any sorted quickslot replay and before trailing peer/static/ground visibility frames, so the owner is not silently presented as live at world entry
