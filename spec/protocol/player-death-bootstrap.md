...
- those immediate and delayed retaliation point-loss beats stay runtime-only for the selected live session today: they do **not** yet write the persisted account snapshot, so fresh `/phase_select` re-entry, reconnect, and the owned same-socket `/restart_here` and `/restart_town` recovery seams all still rebuild from the pre-retaliation point value
- the next owned player-death persistence slice freezes the opposite reconnect rule without inventing corpse gameplay: when either immediate or delayed practice-mob retaliation reaches the bootstrap `0`-HP floor, the runtime must persist that selected-character bootstrap HP point as `0` before or with the owned death/clear frames, so a later reconnect or fresh `ENTERGAME` rebuilds from the dead snapshot (self `PLAYER_POINT_CHANGE` at `0` plus self `GC DEAD(owner_vid)`) instead of silently restoring the pre-retaliation live value
- that same persistence slice must keep `/restart_here` and `/restart_town` honest: once the floor is persisted, accepted restart recovery has to write a usable live persisted HP as part of the recovery rebuild rather than depending on the old runtime-only loss remaining non-persisted; restart must still fail closed while the owner is alive and when no usable live rebuild snapshot can be produced
- adjacent non-point persistence seams keep their current boundary until that death-persistence slice lands: position-only saves, carried inventory movement, merchant buy, and merchant sell-back may persist their own owned state while preserving the selected character's pre-retaliation persisted bootstrap HP point, so a later reconnect shows those accepted non-combat mutations without leaking runtime-only retaliation damage into account storage
...
- those same restart seams now explicitly require that the persisted selected-character snapshot they would replay is above the bootstrap HP floor; if an account snapshot is already persisted at `0` HP, the slash restart fails closed with no self recovery frames, no peer-visible rebuild/transfer deltas, and no persisted town-coordinate mutation
