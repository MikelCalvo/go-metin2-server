...
- one recipient-side peer-visibility teardown gate for later visible peer `CHARACTER_DEL` frames from peer leave or relocate-away transfer aimed at that same still-connected zero-HP owner through the current shared-world leave / transfer paths
...
- once this floor is reached, later peer leave or relocate-away transfer teardown that would ordinarily queue a visible peer `CHARACTER_DEL` to that same still-connected dead owner also skips that zero-HP owner recipient entirely while the leaving or transferred live peer still receives its ordinary origin-side visibility cleanup
...
- the transferred live peer still receives the ordinary origin-side peer-entry burst for the already-dead owner under the current bootstrap visibility model
- this slice stays recipient-only: it does not yet claim that fresh joiners, movers, syncers, or transferred peers stop seeing the dead owner, and it does not yet widen beyond the current shared-world join / move / `SYNC_POSITION` / transfer seams
...
- the leaving or transferred live peer still receives its ordinary origin-side visibility cleanup; this slice only narrows delivery to the already-dead recipient
- this slice does **not** yet claim the same recipient skip for every later AOI `MOVE` / `SYNC_POSITION` out-of-range teardown path; those remain pending until a later slice opens that exact runtime seam honestly

Why this is the current owned boundary:
- after chat/notice recipient skips plus peer-entry recipient skips were already owned, later queued peer `CHARACTER_DEL` teardown on disconnect/transfer became the next smallest remaining player-visibility surface that could still deliver ordinary world teardown to a still-connected zero-HP owner
- reusing the existing leave / transfer teardown loops keeps the widening honest without pretending a broader corpse-state observer model or every AOI teardown seam is already frozen
...
- once retaliation has already driven the owning character to `0` HP, later peer leave or relocate-away transfer teardown also keeps the ordinary origin-side visibility cleanup for the live leaving/transferred peer, but the same queued peer `CHARACTER_DEL` teardown skips the still-connected zero-HP owner recipient entirely
