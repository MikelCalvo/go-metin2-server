...
- optional `reward_items` is the structured carried-item grant table for successful turn-ins (`0..8` entries). Each entry requires non-zero `item_vnum` and `count` in `1..255`. Content bundles must include a matching item template for every entry, and each count must fit that template (`<= max_count`; non-stackable templates require `count == 1`). The checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-reward-item-missing-from-item-templates-bundle.json` is a `quest_flag` turn-in whose `reward_items` references both `27001` and `11200` while `item_templates` only covers `27001`, so `/local/content-bundle/validate` can prove that incomplete present-templates reject without improvising JSON. The checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-reward-item-without-item-templates-bundle.json` is a `quest_flag` turn-in whose `reward_items` references `27001` with no top-level `item_templates` collection, so `/local/content-bundle/validate` can prove the missing item-template backing reject without improvising JSON. The checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-reward-item-count-above-stack-limit-bundle.json` is a `quest_flag` turn-in whose `reward_items` references `27001` with `count = 11` while the matching bundled template has `max_count = 10`, so `/local/content-bundle/validate` can prove the over-stack reward count reject without improvising JSON. The checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-reward-item-count-above-non-stackable-limit-bundle.json` is a `quest_flag` turn-in whose `reward_items` references `11200` with `count = 2` while the matching bundled template is non-stackable with `max_count = 1`, so `/local/content-bundle/validate` can prove the non-stackable reward count reject without improvising JSON.
...
- optional `consume_items` is the structured carried-item debit table for successful turn-ins (`0..8` entries). Each entry requires non-zero `item_vnum` and `count` in `1..255`. Duplicate vnums are allowed and are summed at runtime. Content bundles must include a matching item template for every entry, and each count must fit that template (`<= max_count`; non-stackable templates require `count == 1`). There is no scalar shorthand for consume items. The checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-consume-item-missing-from-item-templates-bundle.json` is a `quest_flag` turn-in whose `consume_items` references both `27001` and `11200` while `item_templates` only covers `27001`, so `/local/content-bundle/validate` can prove that incomplete present-templates consume reject without improvising JSON. The checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-consume-item-without-item-templates-bundle.json` is a `quest_flag` turn-in whose `consume_items` references `27001` with no top-level `item_templates` collection, so `/local/content-bundle/validate` can prove the missing item-template backing reject without improvising JSON. The checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-consume-item-count-above-stack-limit-bundle.json` is a `quest_flag` turn-in whose `consume_items` references `27001` with `count = 11` while the matching bundled template has `max_count = 10`, so `/local/content-bundle/validate` can prove the over-stack consume count reject without improvising JSON. The checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-consume-item-count-above-non-stackable-limit-bundle.json` is a `quest_flag` turn-in whose `consume_items` references `11200` with `count = 2` while the matching bundled template is non-stackable with `max_count = 1`, so `/local/content-bundle/validate` can prove the non-stackable consume count reject without improvising JSON.
...
`info`, `talk`, `warp`, `shop_preview`, `open_safebox`, and `open_cube` definitions may optionally carry a selected-character quest-flag prerequisite without becoming `quest_flag` mutators:
...
- when the live selected character's current flag value matches `quest_from`, the ordinary `info` / `talk` / `warp` / `shop_preview` / `open_safebox` / `open_cube` outcome continues unchanged
- when the current value mismatches, the client receives the same self-only `CHAT_TYPE_INFO` text already owned by `quest_flag` mismatch (`Quest requirements are not met.`) and no authored info/talk text, transfer, merchant window, warehouse password prompt, safebox presentation, or cube open is delivered
...
- content-bundle warp destination/route summaries, shop-route summaries, open-safebox route summaries, and open-cube route summaries now surface the authored gate fields so operators can audit teleporter/merchant/warehouse/craftsman prerequisites without opening the live interaction path

The checked-in QA example `docs/examples/bootstrap-npc-service-bundle.json` now gates `npc:qa_guide`, `lore:qa_square`, `npc:qa_teleporter`, `npc:qa_merchant`, `npc:qa_warehouse`, and `npc:qa_cube` on `quest:first_steps.met_guide = 1`, so the owned service unlock loop is: interact with `QuestGuide` once, then use the guide/signpost/teleporter/merchant/warehouse/craftsman. The composed authoring fixture `docs/examples/bootstrap-pve-vertical-authoring-bundle.json` carries the same gated talk/info/warp actors, and `TestPveVerticalAuthoringBundleClosesGuideUnlockKillCreditAndTurnIn` now INTERACTs them on the authoring-form import path: mismatch before `QuestGuide`, then `VillageGuide` talk, `VillageSignpost` info, and same-map `Teleporter` warp to `470200,964200` (text then transfer/rebootstrap, then walk back into the 300-unit QA-square radius) without mutating quest-state. After the first unlocked warehouse close and same-session password cooldown reject, the same proof INTERACTs gated `CubeMaster` (`cube open 20022`) then `/cube r_info` / `/cube r_info 0` (`cube r_list 20022 1 27001,1` / `cube m_info 0 1 27002,2/100`) with no gold/inventory/quest mutation before `/close_cube` and reconnect. The same fixture also closes the first combat-adjacent quest loop: kill `QAPveVerticalMob` to advance `quest:first_steps.killed_qa_mob`, then pick up the authored practice-mob drop (`ITEM_GROUND_ADD` / `ITEM_OWNERSHIP` then `ITEM_PICKUP` for `Small Red Potion` `27001`) and interact with `QuestHunter` (`quest:first_steps_kill_turnin`) to clear that flag through an ordinary `quest_flag` compare-and-set turn-in that consumes authored `consume_items` (`Small Red Potion` `27001` x1), `consume_gold = 25`, and `consume_experience = 10`, and grants authored `reward_gold = 100`, `reward_experience = 50`, and `reward_items` (`Wooden Sword` `11200` x1). `TestPveVerticalAuthoringBundleClosesGuideUnlockKillCreditAndTurnIn` now proves that pickup on the authoring-form import path instead of seeding the potion in account inventory. After that turn-in the same proof reopens the QA merchant, proves catalog slot `1` (`Wooden Sword` `11200` @ `500g`) returns one self-only `GC::SHOP NOT_ENOUGH_MONEY` with no gold/inventory mutation, then packet-buys catalog slot `0` (`Small Red Potion` `27001` @ `50g`) into the first free carried slot (`ITEM_SET` at slot `1`, no extra `GC::SHOP OK`) so live/persisted gold debit by `50` while the turn-in sword stays in slot `0`. It then packet-sells that bought potion with whole-stack `SHOP SELL` of carried slot `1` (`ITEM_DEL` then `PLAYER_POINT_CHANGE(POINT_GOLD)` for authored `shop_sell_price = 2`, no extra `GC::SHOP OK`) so live/persisted gold credit by `2` and inventory keeps only the turn-in sword, then packet-buys catalog slot `0` again into slot `1` so the later last-stack `ITEM_USE` still has a potion. It then INTERACTs `QuestResetGuide` (`quest:first_steps_reset`) while that same window is still open: the reset INTERACT prepends one self-only `GC::SHOP END`, returns `Quest cleared: first_steps.met_guide = 0.`, drops the selected character's `met_guide` row, and leaves gold/inventory at the post-rebuy snapshot; a later packet `SHOP BUY` against that already-closed window is silent/no-frame, and gated merchant/talk INTERACT mismatch until `QuestGuide` writes `met_guide = 1` again. After that closed window the same proof packet-uses the bought last-stack `27001` (`ITEM_USE` echo, `PLAYER_POINT_CHANGE` `+50` from authored `use_effect`, `ITEM_DEL`, info chat `consume:27001:+50`) then packet-equips the turn-in `11200` onto the authored `equip_slot = weapon` wear cell (`ITEM_DEL` / equipment `ITEM_SET` / self `CHARACTER_UPDATE`). After `QuestGuide` re-unlock reopens the QA merchant, the same proof warehouse-INTERACTs (prepends `GC::SHOP END`, then `ShowMeSafeboxPassword`), opens with `/safebox_password 000000` (`SAFEBOX_SIZE` size `2` / rematerialized `SAFEBOX_MONEY_CHANGE` `60`), withdraws that deposited kill-reward gold with `/safebox_money_withdraw 60`, fail-closed `SAFEBOX_CHECKIN` while the sword is still worn, packet-unequips that sword onto carried slot `0`, stores it through `SAFEBOX_CHECKIN`, relocates it with whole-stack `SAFEBOX_ITEM_MOVE` `0 -> 5` (cell `10` fail-closed on size `2`), retrieves it from cell `5` through `SAFEBOX_CHECKOUT`, then packet-sells it for authored `shop_sell_price = 100`. A later operator `POST /local/quest-state/transition` that clears `met_guide` while a previously opened QA merchant window is still open must still auto-close that stale window on the next buy/sell attempt instead of letting the transaction continue under a revoked prerequisite.
...
- the checked-in negative fixture `docs/examples/bootstrap-invalid-orphan-quest-gate-bundle.json` is the gated kill-quest-only drop-table authoring shape with the `met_guide` writer deliberately omitted, so `/local/content-bundle/validate` can prove that orphan-require-gate reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-orphan-require-quest-from-bundle.json` is a spawn group with complete kill-quest credit plus orphan `require_quest_from` (both require identities omitted), so `/local/content-bundle/validate` can prove that orphan-`require_quest_from` reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-partial-require-quest-gate-bundle.json` is a spawn group with complete kill-quest credit plus partial require gate (`require_quest_ref` without `require_quest_flag`), so `/local/content-bundle/validate` can prove that partial-require-gate reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-reverse-partial-require-quest-gate-bundle.json` is a spawn group with complete kill-quest credit plus reverse partial require gate (`require_quest_flag` without `require_quest_ref`), so `/local/content-bundle/validate` can prove that reverse-partial-require-gate reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-partial-kill-quest-credit-bundle.json` is a spawn group with incomplete kill-quest credit (`reward_quest_ref` / `reward_quest_flag` / `reward_quest_to` without `reward_quest_text`), so `/local/content-bundle/validate` can prove that partial-kill-quest-credit reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-kill-quest-from-equals-to-bundle.json` is a spawn group with complete kill-quest credit where `reward_quest_from == reward_quest_to`, so `/local/content-bundle/validate` can prove that no-op kill-quest transition reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-partial-drop-table-kill-quest-credit-bundle.json` is a drop-table authoring shape with incomplete kill-quest credit (`reward_quest_ref` / `reward_quest_flag` / `reward_quest_to` without `reward_quest_text`) expanded by a referencing spawn group, so `/local/content-bundle/validate` can prove that partial drop-table kill-quest-credit reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-orphan-service-quest-gate-bundle.json` is the gated `shop_preview` service shape with the `met_guide` writer deliberately omitted, so `/local/content-bundle/validate` can prove that orphan service-gate reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-quest-state-seed-alone-gate-writer-bundle.json` is a gated `talk` service backed only by a portable `quest_state` seed row for `quest:first_steps.met_guide` (no `quest_flag` interaction and no kill-quest credit writer), so `/local/content-bundle/validate` can prove that seed-alone-is-not-a-writer reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-partial-service-quest-gate-bundle.json` is a `talk` service with partial quest gate (`quest_ref` without `quest_flag`), so `/local/content-bundle/validate` can prove that partial service-gate reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-reverse-partial-service-quest-gate-bundle.json` is a `talk` service with reverse partial quest gate (`quest_flag` without `quest_ref`), so `/local/content-bundle/validate` can prove that reverse-partial service-gate reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-orphan-service-quest-from-bundle.json` is an ungated `talk` service with orphan `quest_from` (no `quest_ref` / `quest_flag`), so `/local/content-bundle/validate` can prove that orphan service `quest_from` reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-orphan-service-quest-to-bundle.json` is an ungated `talk` service with orphan `quest_to` (no `quest_ref` / `quest_flag`), so `/local/content-bundle/validate` can prove that orphan service `quest_to` reject without improvising JSON
- the checked-in negative fixture `docs/examples/bootstrap-invalid-gated-service-quest-to-bundle.json` is a gated `talk` service with complete `quest_ref` / `quest_flag` / `quest_from` plus mutating `quest_to` (and an in-bundle `quest_flag` writer so the only fail-closed reason is the mutate rule), so `/local/content-bundle/validate` can prove that gated service `quest_to` must remain `0` without improvising JSON
- a completely empty `drop_tables` row (no EXP/gold/drop channels and no kill-quest credit) remains invalid; the checked-in negative fixture `docs/examples/bootstrap-invalid-empty-drop-table-bundle.json` is the preferred `/local/content-bundle/validate` dry-run for that reject without improvising JSON
...
- when an authored EXP and/or gold death-reward scalar was tentatively applied but its selected-character account save fails, the accepted non-player death and any independently valid drop handling remain authoritative, but the associated kill-quest credit is not attempted: no `ApplyTransition` and no quest chat. The live scalar rollback therefore cannot leave durable quest progress claiming a reward payout that did not persist.
