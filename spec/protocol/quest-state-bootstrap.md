...
- non-`quest_flag` interaction kinds must keep `reward_experience`, `reward_gold`, reward-item, and consume-item fields absent/`0`.
...
3. when `consume_items` is authored, `gamed` first proves the selected character's carried inventory can supply every required count (ascending slot order; skip equipped/locked stacks). Insufficient materials leave quest state / gold / inventory unchanged and return the self-only mismatch chat `Quest requirements are not met.`,
4. `gamed` then applies the transition to the selected character name through the same quest-state store primitive used by `/local/quest-state/transition`,
5. if and only if the transition applies, the client receives one self-only `GC_CHAT` with `type = INFO`, `vid = 0`, `empire = 0`, and `message = definition.text`,
6. when that successful transition also authors `reward_gold > 0`, the same response appends one self-only `PLAYER_POINT_CHANGE` gold frame for the granted amount and persists the updated selected-character gold into the account snapshot; overflow / unavailable selected character / account-save failure fail closed (no frames; live gold/experience/inventory and quest transition are rolled back when needed),
7. when that successful transition also authors `reward_experience > 0`, the same response appends one self-only `PLAYER_POINT_CHANGE` experience frame for the granted amount (after any gold frame) and persists the updated selected-character experience point into the account snapshot; overflow / account-save failure fail closed with the same rollback posture as gold,
8. when that successful transition also authors one or more `consume_items` entries, the same response appends ordinary carried-inventory `ITEM_UPDATE` / `ITEM_DEL` frames for each consumed stack (after any gold/experience frames), plus item-quickslot deletes for fully cleared slots, and persists the updated selected-character inventory/quickslots into the account snapshot,
9. when that successful transition also authors one or more `reward_items` entries, the same response appends the ordinary carried-inventory SET/UPDATE frames for each granted placement in authored table order (after any consume frames) and persists the updated selected-character inventory into the account snapshot; sequential placement capacity for every entry is preflighted before the quest transition applies. Inventory-full and owned restriction rejects (`anti_get` / selected-character `CanUseTemplate` job/sex/empire/`min_level`) leave quest-state / gold / experience / inventory unchanged and now return one self-only `CHAT_TYPE_INFO` frame instead of disappearing silently: inventory-full uses `You have too many items.`; restricted grants use the template-authored `buy_reject_message` when present and otherwise `You cannot receive this quest reward.`. Missing templates, gold/experience overflow, store errors, and account-save / post-apply rollback failures still fail closed with no frames,
10. when the compare-and-set result is `current_value_mismatch`, the quest-state snapshot remains unchanged, no gold/experience/item is granted or consumed, and the client now receives one self-only `GC_CHAT` with `type = INFO`, `vid = 0`, `empire = 0`, and `message = "Quest requirements are not met."`,
11. invalid transition definitions, store errors, unsupported content, and other non-CAS failures still fail closed with no frames and no peer fanout.

Loopback interaction visibility now mirrors that player-facing branch without mutation: `GET /local/interaction-visibility` and `GET /local/interaction-visibility/{character}` preview a `quest_flag` actor by dry-running the selected character's compare-and-set transition and, when authored, live inventory sufficiency for `consume_items` plus reward-item placement / restriction checks. A transition that would apply with sufficient materials and placeable unrestricted rewards previews `definition.text`, optionally annotated with `[reward_gold N]`, `[reward_experience N]`, one `[reward_item <name|vnum> x<count>]` marker per authored reward entry, and one `[consume_item <name|vnum> x<count>]` marker per authored consume entry; a `current_value_mismatch` or insufficient-materials preview uses `Quest requirements are not met.`; an inventory-full reward preview uses `You have too many items.`; a restricted reward preview uses the template-authored `buy_reject_message` when present and otherwise `You cannot receive this quest reward.`. Other dry-run failures surface as a fail-closed `resolution_failure` marker rather than mutating the quest-state store.

This is still a bootstrap quest-state trigger, not a client quest UI, branching dialog tree, or script runtime. The optional experience, gold, carried-item grants, and carried-item consumes reuse already-owned economy / inventory packet paths so the kill -> pickup -> turn-in loop can deliver client-visible payoffs without inventing quest mail, ground-drop turn-in rewards, or a second reward runtime. The mismatch acknowledgement and its loopback preview exist only so authored-state failures are not silent; they do not expose a quest window, objective tracker, or alternate branch.
