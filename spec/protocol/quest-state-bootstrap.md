...
- when the current value mismatches, the client receives the same self-only `CHAT_TYPE_INFO` text already owned by `quest_flag` mismatch (`Quest requirements are not met.`) and no authored info/talk text, transfer, merchant window, warehouse password prompt, or safebox presentation is delivered
