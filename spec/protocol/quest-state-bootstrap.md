...
9. when that successful transition also authors one or more `reward_items` entries, the same response appends the ordinary carried-inventory SET/UPDATE frames for each granted placement in authored table order (after any consume frames) and persists the updated selected-character inventory into the account snapshot; missing template / AntiGet / inventory-full / account-save failure fail closed with the same rollback posture as gold overflow, and sequential placement capacity for every entry is preflighted before the quest transition applies,
