...
- that same accepted stance transition also emits one `GC CHANGE_SPEED(selected_vid, moving_speed=150)` to the owner and one matching companion to currently visible live peers, using the already-owned bootstrap `CHARACTER_ADD` / `CHARACTER_UPDATE` default,
- accepted client `CHARACTER_POSITION(position=3)` emits the same ground-sit presentation as `position=4`, including the same self and peer `CHANGE_SPEED` companion,
