...
- the checked-in composed PvE authoring/canonical bundle resolves its `Wooden Sword` (`vnum = 11200`) `equip_effect` on the authored `weapon` slot to `type = 1`, `amount = +10` on equip and `-10` on unequip, with `value = updated Points[1]`; ordinary authored equipment moves emit `ITEM_DEL`, equipment/carried `ITEM_SET`, this self-only point frame, then `CHARACTER_UPDATE` (plus the existing quickslot removal companion when applicable). The dedicated NPC-service example now authors that same `11200.equip_effect` and proves merchant-bought equip/unequip through `TestNpcServiceBundleMerchantSwordBuyEquipsAppearanceAndEquipEffect`
...
- the same bootstrap HP point (index `1`) is projected through the already-owned tip-`0011` `character-point-state` export / quarantine / import as signed `character_points.value` without inventing a later HP-only catalog identity; above-floor (partial) loss, death-floor `0`, and restart MaxHP stay unclamped at that SQL boundary
