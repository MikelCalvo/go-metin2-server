...
- template-backed equip point effects fail closed when the item template's authored `equip_slot` does not match the runtime equipment slot being equipped or unequipped, or when the live equipment snapshot does not contain a valid equipped item in that slot with the same template `vnum`; mismatches emit no `PLAYER_POINT_CHANGE` and must not mutate live points
