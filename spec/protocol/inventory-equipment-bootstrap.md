...
- occupied-wear swap/replace onto an already-occupied matching wear cell emits `ITEM_SET(inventory_from with former worn item)` then `ITEM_SET(equipment_cell with former source item)`, then any template-backed invert then apply `PLAYER_POINT_CHANGE` frames for the previous/new `equip_effect` transition, then one self-only `CHARACTER_UPDATE`; source-cell item quickslots remain bound because the carried cell stays occupied; if the same socket has an active bootstrap exchange shell, the requester first receives `GC::EXCHANGE END` and the paired peer receives one queued `GC::EXCHANGE END` before those refresh frames
...
- counted `ITEM_MOVE` also accepts a compatible occupied-destination merge: the source stack is decremented or removed, the destination stack's existing item instance grows by the moved count, and the response uses self-only `ITEM_UPDATE` count refreshes for live source/destination stacks or `ITEM_DEL(source)` plus `ITEM_UPDATE(destination)` when the counted move consumes the full source stack; those compatible carried-stack `ITEM_MOVE` `ITEM_UPDATE` refreshes project presence-aware instance sockets/attributes via ordinary `EffectiveSockets` / `EffectiveAttributes` (destination presence including explicit zero wins on the merged cell; omitted destination keeps template fallback; partial-merge source remainder keeps an independent clone of source presence)
...
Although the first logical item snapshot contract still does not assign gameplay meaning to most legacy flag bits yet, the first codec carries sockets and attributes as opaque compatibility fields so later slices do not need to redraw the frame boundary. Presence-aware **instance sockets** are already owned end-to-end (`HasSockets` / `EffectiveSockets`, including explicit zero). Presence-aware **instance attributes** are now owned beside sockets (`HasAttributes` / `EffectiveAttributes`, including explicit all-zero / type-zero): bootstrap `ITEM_SET` / `ITEM_UPDATE` prefer instance attributes when present and otherwise keep the template-authored fallback (`docs/plans/2026-08-30-attributes-on-instance-contract-freeze.md`). Durable pending ground FileStore rematerialize round-trips those instance attributes (`has_attributes` + `attributes`) across drop → `gamed` restart → pickup (`docs/plans/2026-08-30-ground-item-instance-attributes-durable.md`); durable safebox FileStore cells now also round-trip them across check-in → restart → reopen / checkout (`docs/plans/2026-08-30-safebox-cell-instance-attributes-durable.md`). Tip SQL attribute companions stay deferred. Template-backed `ITEM_SET.flags` currently project `stackable`, `sell_count_per_gold`, `rare`, `unique`, `confirm_when_use`, `quest_use`, `quest_use_multiple`, and `applicable`; unowned flag bits remain zero until a later slice owns their template metadata and runtime behavior. Owned anti-flag projection now includes transfer/job/sex/empire guards plus the storage/shop metadata bits `anti_save`, `anti_pk_drop`, `anti_myshop`, and `anti_safebox` (see `item-template-store-bootstrap.md`); remaining unowned anti-flag bits stay zero until their template metadata and runtime behavior are owned.
