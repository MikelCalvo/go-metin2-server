...
- template-backed `irremovable` equipment unequip attempts from both the slash `/unequip_item` seam and packet `ITEM_MOVE` equipment-cell source path now fail closed with one self-only `CHAT_TYPE_INFO` rejection: the message uses template-authored `unequip_reject_message` when present and otherwise falls back to `You cannot remove this item.`; no equipment/carry inventory state, points, appearance, quickslots, or persistence are mutated
