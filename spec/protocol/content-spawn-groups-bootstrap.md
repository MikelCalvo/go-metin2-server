...
`POST /local/spawn-groups/{entity_id}/return-step?max_step=<positive-int>` is the first loopback-only one-step return trigger. It accepts no request body, plans one capped step with the default leash radius, persists the materialized actor at `step.next` before mutating runtime state, preserves HP/death/reward/combat metadata and selected-target ownership, reuses the ordinary static-actor visibility transition path, returns `{actor,step}`, and no-ops without persistence/frames for actors that are already `at_home` or `within_radius`. It is a QA/recovery bridge toward later autonomous return AI, not final mob movement.
