...
- when either the immediate retaliation tick or a delayed follow-up beat reaches that owner-side `0`-HP floor, the current slice now emits one self-only `GC POINT_CHANGE(HP, -1, 0)`, then one self-only `GC DEAD(owner_vid)`, then clears the stale engaged target with one self-only `GC TARGET(0, 0)`, then one self `GC DAMAGE_INFO(owner_vid, flag = 0, damage = abs(final_clamped_delta))`; currently visible peers also receive one queued `GC DEAD(owner_vid)` followed by that same owner `DAMAGE_INFO` companion while broader player-death semantics remain out of scope
