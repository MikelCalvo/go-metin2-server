...
- registered profile defaults are used by the same shared-world target/attack/death loop as built-in profiles: target selection starts from the registered `max_hp`, accepted normal attacks subtract the registered `damage_per_normal_attack`, HP percent is derived from that registered max, and spawn-backed deaths can resolve the registered profile's reward descriptor
