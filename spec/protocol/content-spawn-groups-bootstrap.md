...
- `regen_spawns[]`
  ...
  - the composed QA fixture `docs/examples/bootstrap-pve-vertical-authoring-bundle.json` authors one-count kill-quest `regen_spawns` plus one denser multi-count practice pack (`count = 2`, `pack_spacing = 100` → `practice.qa_pve_vertical_pack.m01` / `.m02`) with gated kill-quest `drop_tables` and the NPC quest loop so operators can validate/import the authoring form end-to-end; `docs/examples/bootstrap-npc-service-bundle.json` remains the byte-canonical runtime form of that same owned quest loop without the denser pack
