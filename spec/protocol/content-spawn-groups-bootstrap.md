...
- `DEAD(vid)` plus selected-session `TARGET(0, 0)` still owns the zero-HP edge; accepted killing hits now also append one self plus visible-peer plain `GC DAMAGE_INFO(target_vid, flag = 0, damage = applied_bootstrap_damage)` companion after that death/clear prefix and before any owned reward frames, while the killing hit still omits owner retaliation `POINT_CHANGE` / owner `DAMAGE_INFO`
