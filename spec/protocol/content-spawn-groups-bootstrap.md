...
1. **Still-dead EnterGame / reconnect bootstrap**
   ...
   - same-socket `/restart_here` after a combined last-hit that also floors the owner now uses that same still-dead trailing-`DEAD` catch-up for the recovered owner (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartHereCatchesUpStillDeadDummy`); if that last-hit also registered an owned kill-reward drop, the same recovery rematerializes the pending ground handle with self-only `ITEM_GROUND_ADD` + `ITEM_OWNERSHIP` after that dummy catch-up (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartHereRematerializesKillRewardDrop`); same-socket `/restart_town` instead tears the still-dead dummy and pending drop down from town visibility and rematerializes both only after source-map relocate-back (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartTownRematerializesKillRewardDropOnSourceMapReselect`)
