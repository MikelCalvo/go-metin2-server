...
- once that retaliation floor has already reached `0`, later peer-originated local `CHAT` requests with `type = TALKING` from still-visible sessions still return the live sender's ordinary self echo, but queued peer delivery skips that zero-HP owner recipient entirely
