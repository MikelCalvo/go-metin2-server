...
- once that retaliation floor has already reached `0`, later normal `ATTACK` attempts from that same engaged owner against the same practice mob also fail closed instead of continuing to mutate runtime dummy HP before broader player-death semantics exist
