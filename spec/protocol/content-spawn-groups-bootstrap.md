...
- `reward_experience`, `reward_gold`, `reward_drop_vnums`
  ...
  - `POST /local/content-bundle/import-preview/maps/{map_index}` returns one exact per-map delta row for a candidate replacement bundle, so operators can audit one map's static-actor, spawn-group, and reward-budget impact without fetching and filtering the full preview response
