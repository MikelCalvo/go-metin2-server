...
1. **Still-dead EnterGame / reconnect bootstrap**
   ...
   - same-socket `/restart_here` after a combined last-hit that also floors the owner now uses that same still-dead trailing-`DEAD` catch-up for the recovered owner (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartHereCatchesUpStillDeadDummy`)
