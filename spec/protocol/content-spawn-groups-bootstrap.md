...
- `regen_spawns[]`
  ...
  - each entry has the same placement, visual, combat-profile, and reward descriptor fields as one `spawn_groups[]` row plus `count`, optional `pack_spacing`, optional multi-count `sync_respawn`, optional multi-count `shared_hp`, optional one-count rectangle extents `sx` / `sy`, optional `time` / `regen_time_ms` / `respawn_delay_ms` overlays, and optional `direction` / `facing` / `angle` overlays
  - **current owned behavior:** `count` may be `1` or an integer in `2..8`
    - `count == 1` keeps the authored `ref` / `name` and requires `pack_spacing` omitted or `0` and `sync_respawn` / `shared_hp` omitted/false
    ...
    - `count` in `2..8` requires `pack_spacing > 0`, forbids `sx` / `sy`, and expands into exactly `count` ordinary independent `spawn_groups[]` rows; optional `sync_respawn` and `shared_hp` are process-local overlays on that authored prefix (not public `spawn_groups` fields)
  ...
  - fail closed before runtime mutation when `count` is omitted/`0`/`> 8`, when `count >= 2` lacks positive `pack_spacing`, when `count == 1` carries `pack_spacing > 0` or `sync_respawn` / `shared_hp`, when a rectangle row is partial / negative / over-max / combined with multi-count or `pack_spacing`, when a regen timer overlay is negative / overflow / disagreeing across aliases, when a regen facing overlay is non-finite / disagreeing across aliases, or when any synthesized member ref is non-canonical or collides
