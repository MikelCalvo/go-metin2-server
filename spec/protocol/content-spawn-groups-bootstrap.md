...
5. **Leave / transfer ownership cleanup**
   - owner Leave / logout / close, phase-select leave, EnterGame reclaim that drops stale engagement ownership, owner death floor, client-originated `TARGET(0)` clear-target, and owner transfer/warp away clear engagement / selected-target / pending chase ownership without resurrecting dead combat state or inventing a second spawn instance; focused warp INTERACT coverage now also arms within_radius homeward after a chase displace (`TestGameRuntimeWarpInteractClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace`) beside the owned operator transfer twin
