...
- still-dead trailing `GC DEAD` replay is owned for both `training_dummy` and content-loaded `spawn_groups` EnterGame / reconnect add-style visibility, including fail-closed target/attack while the dead interval remains open and one-ref/one-actor lookup after that still-dead bootstrap; same-socket `/restart_here` after a combined last-hit now reuses that trailing-`DEAD` catch-up for the recovered owner, and rematerializes still-pending kill-reward ground handles with self-only `ITEM_GROUND_ADD` + `ITEM_OWNERSHIP` after that dummy catch-up; same-socket `/restart_town` after that combined last-hit keeps the source-map handle registered and rematerializes the still-dead dummy plus drop only after relocate-back; combined last-hit `gamed` process restart rematerializes the still-dead dummy plus parked exclusive handle from FileStore, skips dummy/ground rematerialize while the owner is still at the floor, then `/restart_here` rematerializes self-only add/ownership
