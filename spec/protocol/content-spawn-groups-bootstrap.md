...
- `regen_spawns[]`
  ...
  - each entry has the same placement, visual, combat-profile, and reward descriptor fields as one `spawn_groups[]` row plus `count`, optional `pack_spacing`, optional multi-count `sync_respawn`, optional multi-count `shared_hp`, and optional one-count rectangle extents `sx` / `sy`
  ...
  - fail closed before runtime mutation when `count` is omitted/`0`/`> 8`, when `count >= 2` lacks positive `pack_spacing`, when `count == 1` carries `pack_spacing > 0` or `sync_respawn` / `shared_hp`, when a rectangle row is partial / negative / over-max / combined with multi-count or `pack_spacing`, or when any synthesized member ref is non-canonical or collides
  ...
  - runtime import/export and live respawn still see only independent one-actor `spawn_groups[]` descriptors; one-count rectangle rows bake a deterministic sampled home at canonicalize time. This shape does not add a pack object, live RNG, per-respawn rectangle re-rolls, direction, or legacy regen timers
