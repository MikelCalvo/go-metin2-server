...
- when either the immediate retaliation tick or a delayed follow-up beat reaches that owner-side `0`-HP floor, the current slice also clears the stale engaged target with one self-only `GC TARGET(0, 0)` before broader player-death semantics exist
