...
- atomic bootstrap visibility for content-bundle replacement: live sessions receive newly imported static-actor visibility only after the full static-actor replacement succeeds, and a failed replacement/rollback does not leak partial spawn visibility frames
...
- `combat_profile`
  ...
  - if a bundle carries duplicate `combat_profiles` snapshots for the same profile name, the import fails closed before registering that profile or materializing spawn actors so hand-authored portable bundles cannot race two conflicting/default definitions through the runtime seam
...
- content-bundle import now suppresses live static-actor bootstrap fanout while the replacement is in progress, then flushes the imported actors' visibility only after the full replacement succeeds; if a later spawn in the same bundle fails and the runtime rolls back, connected sessions do not receive partial `CHARACTER_ADD` / `CHAR_ADDITIONAL_INFO` / `CHARACTER_UPDATE` bursts for actors that were never committed
