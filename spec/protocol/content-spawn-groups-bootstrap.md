...
- `regen_spawns[]`
  ...
  - **current owned behavior:** `count` must be exactly `1`; omitted, zero, or multi-count regen entries fail bundle canonicalization before runtime mutation
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-regen-count-bundle.json` authors one `regen_spawns` row with `count = 2` so operators can prove that multi-count reject through `/local/content-bundle/validate` without inventing pack runtime
  ...
  - runtime import/export and live respawn still see only the canonical one-actor `spawn_groups[]` descriptor; this shape does not yet add pack size, radius, direction, random placement, timers, or autonomous AI
  ...
  - **next frozen authoring widen (not implemented yet):** multi-count pack placement is frozen in `docs/plans/2026-08-23-multi-count-regen-pack-placement-contract-freeze.md` before any RED that accepts `count > 1`. The intended expansion keeps live runtime on independent one-actor `spawn_groups` only:
    - `count` in `2..8` plus required positive `pack_spacing` (world units)
    - member `ref` = `{authored_ref}.m{NN}` (`m01`..`m08`), member `name` = `{authored name} {i}`
    ...
    - shared combat profile / reward / kill-quest fields copy onto every member; synthesized refs must stay unique and canonical
    - pack AI, synchronized respawn, random rectangle placement, direction, and legacy regen timers remain out of scope for that first widen
...
- candidate bundles may include a narrow authoring-only `regen_spawns` collection for one-entry regen-style authoring; canonicalization expands valid entries into ordinary `spawn_groups`, rejects `count != 1` under the currently owned path, strips `regen_spawns` before runtime import/export, and leaves live behavior on the existing spawn-group lifecycle instead of adding pack/multi-count runtime semantics. The docs-first multi-count pack-placement freeze (`docs/plans/2026-08-23-multi-count-regen-pack-placement-contract-freeze.md`) is the next intended authoring widen and must land before any RED that accepts `count > 1`; even after that GREEN, live import/export remains independent one-actor `spawn_groups` with no pack AI object
