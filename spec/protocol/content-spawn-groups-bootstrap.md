...
- the current profile-default seam is deliberately compact: `max_hp`, `damage_per_normal_attack`, `attack_value`, `defense_value`, `respawn_delay`, and the reward descriptor documented in `non-player-reward-bootstrap.md`
- `attack_value` / `defense_value` are now profile-owned authored stat defaults for the next damage-formula slices; they are validated and round-trip through the in-process profile registration seam, but the current accepted-hit damage path still consumes the existing fixed `damage_per_normal_attack` value until a later formula slice explicitly replaces it
...
- registered profile defaults are used by the same shared-world target/attack/death/respawn loop as built-in profiles: target selection starts from the registered `max_hp`, accepted normal attacks subtract the registered `damage_per_normal_attack`, HP percent is derived from that registered max, spawn-backed deaths can resolve the registered profile's reward descriptor, the dead timer uses the registered `respawn_delay`, and the rebuild restores the actor to the registered full HP
