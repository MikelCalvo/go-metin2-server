...
- dynamic difficulty, live RNG drift, or weighted spawn tables
...
- `drop_tables[].reward_experience`, `drop_tables[].reward_gold`, `drop_tables[].drop_vnums`, optional weighted `drop_tables[].entries` (`item_vnum` + `weight`), optional kill-quest credit fields (`drop_tables[].reward_quest_ref`, `drop_tables[].reward_quest_flag`, `drop_tables[].reward_quest_from`, `drop_tables[].reward_quest_to`, `drop_tables[].reward_quest_text`, plus optional require-gate fields `require_quest_ref`, `require_quest_flag`, `require_quest_from`), and `spawn_groups[].reward_drop_table_ref`
  ...
  - optional `entries` is the first owned weighted loot pool: two to eight `{item_vnum, weight}` rows, each `item_vnum` non-zero and unique, each `weight` a positive integer; every `item_vnum` must resolve to a bundled `item_templates` row
  - canonicalization still expands EXP/gold/sorted `reward_drop_vnums` (the union of `drop_vnums` and every `entries[].item_vnum`) plus optional kill-quest credit into the referencing spawn group, then strips both `drop_tables` and `reward_drop_table_ref` from the portable/canonical JSON so validate/export stay drop-all shaped
  - weighted `entries` are a process-local overlay kept from the authored candidate at import; they do not round-trip through exported canonical JSON, FileStore static-actor snapshots, or import-preview reward totals
  - on the accepted killing hit, a spawn group whose imported overlay has weighted `entries` emits exactly one ground drop: the deterministic FNV-1a 64 pick over authored order and positive weights, seeded from `spawn_group_ref`, authored map/x/y, and the materialized actor entity id. The same kill repeats the same vnum. Tables without `entries` keep the existing drop-all `reward_drop_vnums` behavior
  - this is not live RNG, party split, corpse loot, or pack AI; pickup still uses the already-owned exclusive-ownership ground path
  ...
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-info-embedded-nul-text-bundle.json` is an `info` definition whose `text` contains JSON `\u0000` (`visible\u0000hidden`), so `/local/content-bundle/validate` can prove the embedded-NUL chat-text reject without improvising JSON
  ...
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-reward-drop-item-missing-from-item-templates-bundle.json` is a spawn group whose `reward_drop_vnums` references `27002` while `item_templates` only covers `27001`, so `/local/content-bundle/validate` can prove the incomplete present-templates reward-drop reject without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-combat-profile-death-reward-item-missing-from-item-templates-bundle.json` is a portable `combat_profiles` row whose `death_reward.drop_vnums` references `27002` while `item_templates` only covers `27001`, so `/local/content-bundle/validate` can prove the incomplete present-templates combat-profile death-reward reject without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-combat-profile-death-reward-without-item-templates-bundle.json` is a portable `combat_profiles` row whose `death_reward.drop_vnums` is non-empty and no top-level `item_templates` is present, so `/local/content-bundle/validate` can prove the missing item-template backing reject for profile-default death-reward drops without improvising JSON
  ...
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-merchant-catalog-item-missing-from-item-templates-bundle.json` is a `shop_preview` interaction definition whose catalog references both `27001` and `11200` while `item_templates` only covers `27001`, so `/local/content-bundle/validate` can prove the incomplete present-templates merchant-catalog reject without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-merchant-catalog-count-above-stack-limit-bundle.json` is a `shop_preview` interaction definition whose catalog references `27001` with `count = 11` while the matching bundled template has `max_count = 10`, so `/local/content-bundle/validate` can prove the over-stack merchant catalog count reject without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-reward-item-missing-from-item-templates-bundle.json` is a `quest_flag` turn-in whose structured `reward_items` references both `27001` and `11200` while `item_templates` only covers `27001`, so `/local/content-bundle/validate` can prove the incomplete present-templates quest-flag reward reject without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-reward-item-without-item-templates-bundle.json` is a `quest_flag` turn-in whose structured `reward_items` references `27001` with no top-level `item_templates`, so `/local/content-bundle/validate` can prove the missing item-template backing reject for quest-flag rewards without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-consume-item-missing-from-item-templates-bundle.json` is a `quest_flag` turn-in whose structured `consume_items` references both `27001` and `11200` while `item_templates` only covers `27001`, so `/local/content-bundle/validate` can prove the incomplete present-templates quest-flag consume reject without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-consume-item-without-item-templates-bundle.json` is a `quest_flag` turn-in whose structured `consume_items` references `27001` with no top-level `item_templates`, so `/local/content-bundle/validate` can prove the missing item-template backing reject for quest-flag consumes without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-reward-item-count-above-stack-limit-bundle.json` is a `quest_flag` turn-in whose structured `reward_items` references `27001` with `count = 11` while the matching bundled template has `max_count = 10`, so `/local/content-bundle/validate` can prove the over-stack quest-flag reward count reject without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-reward-item-count-above-non-stackable-limit-bundle.json` is a `quest_flag` turn-in whose structured `reward_items` references `11200` with `count = 2` while the matching bundled template is non-stackable with `max_count = 1`, so `/local/content-bundle/validate` can prove the non-stackable quest-flag reward count reject without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-consume-item-count-above-stack-limit-bundle.json` is a `quest_flag` turn-in whose structured `consume_items` references `27001` with `count = 11` while the matching bundled template has `max_count = 10`, so `/local/content-bundle/validate` can prove the over-stack quest-flag consume count reject without improvising JSON
  - the checked-in negative fixture `docs/examples/bootstrap-invalid-quest-flag-consume-item-count-above-non-stackable-limit-bundle.json` is a `quest_flag` turn-in whose structured `consume_items` references `11200` with `count = 2` while the matching bundled template is non-stackable with `max_count = 1`, so `/local/content-bundle/validate` can prove the non-stackable quest-flag consume count reject without improvising JSON
...
- `regen_spawns[]`
  ...
  - each entry has the same placement, visual, combat-profile, and reward descriptor fields as one `spawn_groups[]` row plus `count`, optional `pack_spacing`, optional multi-count `formation`, optional multi-count `sync_respawn`, optional multi-count `shared_hp`, optional one-count rectangle extents `sx` / `sy`, optional `time` / `regen_time_ms` / `respawn_delay_ms` overlays, and optional `direction` / `facing` / `angle` overlays
  - **current owned behavior:** `count` may be `1` or an integer in `2..8`
    - `count == 1` keeps the authored `ref` / `name` and requires `pack_spacing` omitted or `0`, `formation` omitted, and `sync_respawn` / `shared_hp` omitted/false
    ...
    - `count` in `2..8` requires `pack_spacing > 0`, forbids `sx` / `sy`, and expands into exactly `count` ordinary independent `spawn_groups[]` rows; optional `formation` is omitted or `"grid"` (the owned grid) or `"line"` (one file along Y); optional `sync_respawn` and `shared_hp` are process-local overlays on that authored prefix (not public `spawn_groups` fields)
    - member `ref` = `{authored_ref}.m{NN}` (`m01`..`m08`), member `name` = `{trimmed authored name} {i}`
    - deterministic grid offsets from the authored origin: `cols = ceil(sqrt(count))`, member 1 stays at `(x,y)`, later members step by `pack_spacing` along columns then rows. Omitted `formation` and `formation = "grid"` keep that grid. `formation = "line"` places the same members in one file along Y: member `i` stays at `x` and `y + (i-1)*pack_spacing`. Any other formation name fails closed. A member coordinate that would overflow `int32` fails closed before runtime mutation
    ...
    - optional `time` / `regen_time_ms` / `respawn_delay_ms` are aliases for one process-local ReadyAt overlay: omitted/zero keeps the combat-profile delay; a positive delay that fits `ValidStaticActorCombatProfileRespawnDelayMs` overrides ReadyAt for that expanded spawn (one-count keeps the authored ref; multi-count copies onto every `{ref}.mNN` member); disagreeing aliases fail closed
    - optional `direction` / `facing` / `angle` are aliases for one process-local CHARACTER_ADD angle overlay: omitted/zero keeps `angle=0`; a finite non-zero angle copies onto that expanded spawn (one-count keeps the authored ref; multi-count copies onto every `{ref}.mNN` member); disagreeing or non-finite aliases fail closed
  - fail closed before runtime mutation when `count` is omitted/`0`/`> 8`, when `count >= 2` lacks positive `pack_spacing`, when `count == 1` carries `pack_spacing > 0`, any `formation` name, or `sync_respawn` / `shared_hp`, when `count >= 2` names a `formation` other than omitted / `"grid"` / `"line"`, when a synthesized member coordinate would overflow `int32`, when a rectangle row is partial / negative / over-max / combined with multi-count or `pack_spacing`, when a regen timer overlay is negative / overflow / disagreeing across aliases, when a regen facing overlay is non-finite / disagreeing across aliases, or when any synthesized member ref is non-canonical or collides
  ...
  - one-count rectangle authoring (`sx` / `sy`) is covered by focused canonicalize proofs (`TestCanonicalizeExpandsOneCountRegenRectangleIntoSpawnGroup`, `TestCanonicalizeRegenRectanglePlacementIsStableAcrossCalls`) that pin the FNV cell and reject partial / multi-count / pack-spacing / over-max extents
  ...
  - runtime import/export and live respawn still see only independent one-actor `spawn_groups[]` descriptors; one-count rectangle rows bake a deterministic sampled home at canonicalize time. This shape does not add a pack object, live RNG, or per-respawn rectangle re-rolls; CHARACTER_ADD angle stays `0` unless a regen row opts in with `direction` / `facing` / `angle`
  ...
  - the first opt-in pack-wide synchronized respawn companion now lives on multi-count `regen_spawns[].sync_respawn`; one-count refs and live siblings stay independent, and when two or more already-dead same-prefix members share that overlay they take one later respawn instant. Canonical JSON still strips `regen_spawns` / `sync_respawn` (process-local overlay, like weighted `entries`). Default packs and the composed PvE independent-member proof stay unchanged (`TestGameRuntimeOptInPackSyncRespawnSharesReadyAtForAlreadyDeadMembers`, `TestGameRuntimeOptInPackSyncRespawnKeepsLiveSiblingIndependent`, `TestGameRuntimeDefaultPackRespawnStaysIndependentPerMember`). Pack AI assist, MOVE, and a pack object stay out of this companion
  - the first opt-in shared-HP companion now lives on multi-count `regen_spawns[].shared_hp`; one-count refs fail closed, default packs and live one-count refs stay independent, and an accepted live hit copies remaining HP onto other live same-prefix siblings. Canonical JSON still strips `regen_spawns` / `shared_hp` (process-local overlay, like `sync_respawn`). The composed PvE independent-member proof stays unchanged (`TestGameRuntimeOptInPackSharedHPCopiesRemainingHPOntoLiveSibling`, `TestGameRuntimeDefaultPackHPStaysIndependentPerMember`, `TestContentBundleRejectsOneCountRegenSharedHP`). Pack AI assist, MOVE, and a pack object stay out of this companion
  - the first opt-in legacy regen-timer companion now lives on `regen_spawns[].time` / `regen_time_ms` / `respawn_delay_ms`; omitted/zero keeps the combat-profile delay, and a positive delay that fits `ValidStaticActorCombatProfileRespawnDelayMs` overrides ReadyAt for that expanded spawn (one-count keeps the authored ref; multi-count copies onto every `{ref}.mNN` member). Canonical JSON still strips `regen_spawns` (process-local overlay, like `sync_respawn` / `shared_hp` / weighted `entries`). Default regen rows plus `spawn_groups` without the overlay stay on the profile clock (`TestGameRuntimeOptInRegenTimerOverridesReadyAtForOneCount`, `TestGameRuntimeOptInRegenTimerCopiesOntoEveryPackMember`, `TestGameRuntimeDefaultRegenAndSpawnGroupsStayOnProfileClock`). Random rectangle re-rolls, live RNG, and a pack object stay out of this companion
  - the first opt-in regen facing companion now lives on `regen_spawns[].direction` / `facing` / `angle`; omitted/zero keeps CHARACTER_ADD `angle=0`, and a finite non-zero angle copies onto that expanded spawn (one-count keeps the authored ref; multi-count copies onto every `{ref}.mNN` member). Canonical JSON still strips `regen_spawns` (process-local overlay, like `time` / `regen_time_ms` / `respawn_delay_ms`, `sync_respawn`, `shared_hp`, weighted `entries`). Default regen rows plus `spawn_groups` without the overlay stay at `angle=0` (`TestGameRuntimeOptInRegenFacingCopiesCharacterAddAngleForOneCount`, `TestGameRuntimeOptInRegenFacingCopiesOntoEveryPackMember`, `TestGameRuntimeDefaultRegenAndSpawnGroupsStayAtZeroAngle`). Pack AI, random rectangle re-rolls, live RNG, MOVE rotation, and a pack object stay out of this companion
  - the first opt-in pack formation besides the owned grid now lives on multi-count `regen_spawns[].formation`: omitted or `"grid"` keeps the deterministic `pack_spacing` grid, and `"line"` places the same `{ref}.mNN` members in one file along Y. One-count rows reject any formation name, unknown names fail closed, and a member coordinate that would overflow `int32` fails closed before runtime mutation. Canonical JSON still strips `regen_spawns` / `formation`. Pack AI assist, shared HP, synchronized respawn, and a pack object stay out of this companion (`TestCanonicalizeExpandsLineRegenSpawnIntoPackMembers`, `TestCanonicalizeExplicitGridFormationKeepsOwnedPackSpacingGrid`, `TestGameRuntimeOptInLineFormationPlacesPackMembersAlongY`)
...
- the same successful respawn retires any pre-death return-step, chase-step, and homeward-step deadlines before the rebuilt actor can consume them; the fresh actor remains unengaged at authored home until a new owned lifecycle boundary arms a later step

What is **not** yet frozen here:
- per-group custom respawn delays unless a `regen_spawns[]` row opts in with `time` / `regen_time_ms` / `respawn_delay_ms` (default regen rows and `spawn_groups` without that overlay stay on the combat-profile clock)
- per-spawn CHARACTER_ADD facing unless a `regen_spawns[]` row opts in with `direction` / `facing` / `angle` (default regen rows and `spawn_groups` without that overlay stay at `angle=0`)
...
- authoring-only `drop_tables` are malformed: `ref` must use the same canonical dotted lowercase identity rule as `spawn_groups.ref`, a table must carry either at least one non-zero/non-empty combat descriptor channel (`reward_experience`, `reward_gold`, `drop_vnums`, or weighted `entries`) or a complete kill-quest credit descriptor (empty combat rewards alone remain invalid), scalar values must fit the current bootstrap point-change carrier, `drop_vnums` must be non-zero, duplicate-free, and backed by bundled `item_templates`, optional `entries` must have two to eight unique non-zero `item_vnum` rows with positive `weight` also backed by bundled `item_templates`, optional kill-quest fields must be complete when any are present, every table must be referenced by a `spawn_groups[].reward_drop_table_ref`, and each referencing spawn group must omit explicit `reward_experience`, `reward_gold`, and `reward_drop_vnums` so table expansion cannot silently conflict with a direct descriptor; a spawn group that already authors kill-quest credit also cannot expand a table that carries kill-quest credit
- authoring-only `regen_spawns` are malformed: `ref` follows the same canonical dotted lowercase identity rule and uniqueness constraints after expansion as `spawn_groups.ref`, `name` / placement / `race_num` / optional `combat_profile` use the same validation rules as `spawn_groups`, `count` must be `1` or `2..8` with the owned `pack_spacing`, optional `formation` (`"grid"` / `"line"`), and one-count `sx` / `sy` rectangle rules from `docs/plans/2026-08-23-multi-count-regen-pack-placement-contract-freeze.md`, any reward descriptor or `reward_drop_table_ref` must satisfy the same fixed reward rules as a direct spawn group, and optional `direction` / `facing` / `angle` overlays must be omitted/zero, finite, and agreeing when more than one alias is present
...
- candidate bundles may include a narrow authoring-only `regen_spawns` collection for one-count, one-count rectangle, or multi-count regen-style authoring; canonicalization expands valid entries into ordinary independent `spawn_groups` (one-count keeps the authored ref and either the authored point or one FNV-sampled rectangle cell; `count` in `2..8` synthesizes `{ref}.m{NN}` members on a deterministic `pack_spacing` grid, or in one file along Y when `formation` is `"line"`), rejects omitted/`0`/`>8` counts, invalid `pack_spacing`, one-count `formation` / `sync_respawn` / `shared_hp`, unknown formation names, member coordinates that would overflow `int32`, partial/over-max/multi-count rectangle extents, invalid or disagreeing `time` / `regen_time_ms` / `respawn_delay_ms` overlays, and invalid or disagreeing `direction` / `facing` / `angle` overlays, strips `regen_spawns` before runtime import/export, and leaves live behavior on the existing spawn-group lifecycle plus the opt-in same-prefix dead-member ReadyAt overlay, the opt-in same-prefix remaining-HP overlay, the opt-in per-expanded-spawn ReadyAt overlay, and the opt-in per-expanded-spawn CHARACTER_ADD angle overlay instead of adding pack AI / live RNG / per-respawn re-rolls / MOVE rotation
...
- when a successful import materializes a spawn-backed actor while players are already online, the runtime enqueues the normal static-actor visibility bootstrap burst (`CHARACTER_ADD`, `CHAR_ADDITIONAL_INFO`, `CHARACTER_UPDATE`) only to sessions that currently share the actor's visible world/AOI; spawn-backed combat-profile actors insert as `CHARACTER_ADD.type = 0` (`CHAR_TYPE_MONSTER`) so the client can treat them as attackable practice mobs instead of ordinary NPCs
...
1. **Still-dead EnterGame / reconnect bootstrap**
   ...
   - if that combatant died after a within_radius chase displace, the still-dead bootstrap uses the death coords rather than authored home; due respawn still rebuilds at authored home (`TestGameRuntimeKillingHitAfterChaseDisplaceKeepsStillDeadAtDeathCoordsAndRespawnsAtAuthoredHome`)
   ...
   - same-socket `/restart_here` after a combined last-hit that also floors the owner now uses that same still-dead trailing-`DEAD` catch-up for the recovered owner (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartHereCatchesUpStillDeadDummy`); combined last-hit `/phase_select` re-entry and abrupt reconnect keep that dummy skipped while the owner is still at the zero-HP floor, then reuse the same `/restart_here` still-dead catch-up (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartHereCatchesUpStillDeadDummy`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartHereCatchesUpStillDeadDummy`); if that last-hit also registered an owned kill-reward drop, the same recovery rematerializes the pending ground handle with self-only `ITEM_GROUND_ADD` + `ITEM_OWNERSHIP` after that dummy catch-up (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartHereRematerializesKillRewardDrop`); combined last-hit `/phase_select` and abrupt reconnect park that exclusive handle on floor-leave instead of deleting it, skip ground rematerialize while the owner is still at the floor, then reuse the same `/restart_here` add/ownership catch-up (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartHereRematerializesKillRewardDrop`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartHereRematerializesKillRewardDrop`); same-socket `/restart_town` instead tears the still-dead dummy and pending drop down from town visibility and rematerializes both only after source-map relocate-back (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartTownRematerializesKillRewardDropOnSourceMapReselect`); combined last-hit `gamed` process restart rematerializes the still-dead dummy plus that parked exclusive handle from FileStore, skips dummy/ground rematerialize while the owner is still at the floor, then `/restart_here` rematerializes self-only add/ownership (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerDaemonRestartRestartHereRematerializesKillRewardDrop`) or `/restart_town` reuses the same transfer teardown plus source-map relocate-back rematerialize (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerDaemonRestartRestartTownRematerializesKillRewardDropOnSourceMapReselect`)
...
5. **Leave / transfer ownership cleanup**
   - owner Leave / logout / close, phase-select leave, EnterGame reclaim that drops stale engagement ownership, owner death floor, client-originated `TARGET(0)` clear-target, and owner transfer/warp away clear engagement / selected-target / pending chase ownership without resurrecting dead combat state or inventing a second spawn instance; focused warp INTERACT plus exact-position `MOVE` / `SYNC_POSITION` transfer-trigger coverage now also arms within_radius homeward after a chase displace (`TestGameRuntimeWarpInteractClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace`, `TestGameRuntimeMoveTransferTriggerClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace`, and `TestGameRuntimeSyncPositionTransferTriggerClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace`) beside the owned operator transfer twin
...
- still-dead trailing `GC DEAD` replay is owned for both `training_dummy` and content-loaded `spawn_groups` EnterGame / reconnect add-style visibility, including fail-closed target/attack while the dead interval remains open and one-ref/one-actor lookup after that still-dead bootstrap; same-socket `/restart_here` after a combined last-hit now reuses that trailing-`DEAD` catch-up for the recovered owner, and rematerializes still-pending kill-reward ground handles with self-only `ITEM_GROUND_ADD` + `ITEM_OWNERSHIP` after that dummy catch-up; same-socket `/restart_town` after that combined last-hit keeps the source-map handle registered and rematerializes the still-dead dummy plus drop only after relocate-back; combined last-hit `gamed` process restart rematerializes the still-dead dummy plus parked exclusive handle from FileStore, skips dummy/ground rematerialize while the owner is still at the floor, then `/restart_here` rematerializes self-only add/ownership or `/restart_town` rematerializes the parked handle after source-map relocate-back
- a live killing hit after a within_radius chase displace now has focused still-dead / respawn coverage at the death coords rather than authored home: chase/homeward stay cleared (no corpse `MOVE`), late EnterGame add/info/update + trailing `GC DEAD` uses the displaced current position, and due respawn rebuilds at authored home requiring fresh reselect (`TestGameRuntimeKillingHitAfterChaseDisplaceKeepsStillDeadAtDeathCoordsAndRespawnsAtAuthoredHome`); daemon restart now rematerializes that same still-dead corpse at the persisted death coords with the remaining absolute deadline and no homeward/return/chase arm (`TestGameRuntimeKillingHitAfterChaseDisplaceStillDeadPersistsAcrossDaemonRestart`); still-dead `return_required` restore now also omits the internal return-step deadline so a due flush cannot MOVE the corpse (`TestGameRuntimeRestoreStillDeadReturnRequiredSpawnGroupDoesNotArmReturnStep`)
...
- daemon-restart still-dead spawn-group timer persistence is now owned: the static-actor snapshot carries optional spawn-backed `combat_current_hp=0` plus absolute `respawn_ready_at`, death persists those fields, process restart rematerializes the same authored `spawn_group_ref` as still-dead / non-targetable through the remaining deadline with trailing `GC DEAD` on add-style visibility, due timers still preflight into ordinary live rebuild, and successful respawn clears the still-dead persistence fields; engagement / proximity-suppress / selected-target / chase / return ownership stay fail-closed across restart; a chase-displaced still-dead corpse now rematerializes at the persisted death coords rather than authored home (`TestGameRuntimeKillingHitAfterChaseDisplaceStillDeadPersistsAcrossDaemonRestart`); a still-dead `return_required` restore re-reads after the overlay so it cannot arm return-step from the live register snapshot (`TestGameRuntimeRestoreStillDeadReturnRequiredSpawnGroupDoesNotArmReturnStep`)
...
- a chase-displaced still-dead corpse now rematerializes at the persisted current X/Y (death coords) rather than snapping to authored home, stays non-targetable, does not arm homeward/return/chase, and due respawn still rebuilds at authored home (`TestGameRuntimeKillingHitAfterChaseDisplaceStillDeadPersistsAcrossDaemonRestart`)
- a still-dead `return_required` restore now re-reads after the overlay instead of syncing the pre-still-dead register snapshot, so it does not arm an automatic return-step deadline and a due flush cannot MOVE the corpse (`TestGameRuntimeRestoreStillDeadReturnRequiredSpawnGroupDoesNotArmReturnStep`)
...
- `DEAD(vid)` plus selected-session `TARGET(0, 0)` still owns the zero-HP edge; accepted killing hits now also append one self plus visible-peer plain `GC DAMAGE_INFO(target_vid, flag = 0, damage = applied_bootstrap_damage)` companion after that death/clear prefix and before any owned reward frames. A killing hit that leaves the owner above `0` still omits owner retaliation `POINT_CHANGE` / owner `DAMAGE_INFO`. When that same accepted killing hit would also drive the engaged owner to the bootstrap `0`-HP floor through the ordinary immediate retaliation delta, dummy death still owns the first prefix, any owned EXP / gold / drop reward frames stay next, and the owner-floor suffix (`PLAYER_POINT_CHANGE(value=0)` -> `DEAD(owner_vid)` -> `TARGET(0, 0)` -> owner `DAMAGE_INFO`) now follows in the same packet burst
...
- when either the immediate retaliation tick or a delayed follow-up beat reaches that owner-side `0`-HP floor, the current slice now emits one self-only `GC POINT_CHANGE(HP, -1, 0)`, then one self-only `GC DEAD(owner_vid)`, then clears the stale engaged target with one self-only `GC TARGET(0, 0)`, then one self `GC DAMAGE_INFO(owner_vid, flag = 0, damage = abs(final_clamped_delta))`; an accepted immediate owner-floor hit also inserts one self plus visible-peer mob `GC DAMAGE_INFO(target_vid, applied_bootstrap_damage)` after that clear and before the owner companion, while delayed floors skip a synthetic mob companion; currently visible peers also receive one queued `GC DEAD(owner_vid)` followed by that same owner `DAMAGE_INFO` companion while broader player-death semantics remain out of scope
...
- those same fixtures also author `retaliation_point_delta = -2` so immediate and delayed owner-side retaliation smoke exercises a non-default hostility amount beside bootstrap `-1`
...
- those same fixtures also author `retaliation_point_delta = -2` so hostility smoke is not limited to bootstrap `-1`
...
## First owned profile-authored return-delay seam

Question frozen here:

**Once profile-authored `chase_delay_ms` already owns live chase arming / re-arm, and the live return-step executor still hard-codes bootstrap `1s` (`bootstrapSpawnGroupReturnStepDelay`), what is the smallest honest authored combat-profile extension that can widen or narrow that return-step arming delay per registered profile without inventing a second scheduler, return packets, or absolute deadline rematerialize?**

This Track A follow-on after owned profile-authored `chase_delay_ms` is now GREEN. Cross-map return MOVE / warp choreography remains deferred behind the packet freeze in `spawn-leash-bootstrap.md` and must not be opened as speculative RED. Absolute chase / return / homeward due-at rematerialize across daemon restart stays cancelled as re-arm-from-now. Homeward-step delay stays on its own bootstrap `1s` seam and is not opened by this freeze.

Contract for optional authored `return_delay_ms` on portable `combat_profiles` / `StaticActorCombatProfileDefaults`:

- field name: `return_delay_ms` (JSON) / `ReturnDelay` (`time.Duration` on Go defaults) / `ReturnDelayMs` (int64 on snapshots)
- omitempty / zero means "use bootstrap default": effective delay = `1s` (`bootstrapSpawnGroupReturnStepDelay`)
- when present and positive, the effective return-step arming / re-arm delay for that registered profile is exactly the authored duration
- validation fails closed when `return_delay_ms < 0`
- validation fails closed when a positive authored delay is `< 250` ms (bootstrap lower bound so return cadence stays independently observable beside the owned flush order)
- validation fails closed when a positive authored delay exceeds `60000` ms (bootstrap upper bound for this seam)
- built-in `practice_mob` / `training_dummy` profiles keep effective delay `1s` and do not require an authored field
- the pure resolver is `EffectiveStaticActorSpawnReturnDelay(profile)` (plus `EffectiveStaticActorSpawnReturnDelayForActor(actor)` / `...FromDefaults`) in `internal/worldruntime`: it returns the effective duration without mutating actor state, engagement, timers, or packets
- live return-step arming and post-step re-arm for a spawn-backed actor must reuse that same effective delay for the actor's current combat profile; they must not keep hard-coding `bootstrapSpawnGroupReturnStepDelay` once a profile authors a different value
- content-bundle import/export and file-backed static-actor combat-profile snapshots must round-trip a non-default authored `return_delay_ms` the same way `aggro_radius` / `leash_radius` / `chase_delay_ms` / `respawn_delay_ms` already round-trip
- pending return inspection endpoints continue to report absolute `ready_at` / `remaining_ms` derived from the armed deadline; they do not invent a separate authored-delay field on the schedule row

Explicit non-goals for this profile-authored return-delay freeze alone:

- changing `max_step`, chase-step delay, or homeward-step delay in the same slice
- inventing return packets, a second scheduler/goroutine, or operator return POST beyond the already-owned surfaces
- absolute chase / return / homeward due-at rematerialize across daemon restart (cancelled as re-arm-from-now)
- pack AI, pathfinding, target switching, or cross-map MOVE / `GC WARP`
- aggro hysteresis / a drop radius distinct from the acquire radius
...
- omitempty / zero means "use bootstrap default": effective delay = `1s` (`bootstrapPracticeMobServerOriginRetaliationDelay` / `DefaultSpawnReactionDelay`)
...
- live delayed server-origin retaliation arming for a spawn-backed actor reuses that same effective delay for the actor's current combat profile on both accepted-hit and proximity-armed engagement paths; post-beat re-arm of the next non-floor delayed tick also consumes it
...
- content-bundle import/export and file-backed static-actor combat-profile snapshots round-trip a non-default authored `reaction_delay_ms` the same way `aggro_radius` / `leash_radius` / `chase_delay_ms` / `return_delay_ms` / `homeward_delay_ms` / `max_step` / `respawn_delay_ms` already round-trip
...
- optional authored `reaction_delay_ms` is owned on portable `combat_profiles` / `StaticActorCombatProfileDefaults` and round-trips through content-bundle canonicalize/import/export and file-backed static-actor snapshots
- `EffectiveStaticActorSpawnReactionDelay(profile)` / `EffectiveStaticActorSpawnReactionDelayForActor(actor)` / `...FromDefaults` resolve omitted/zero to `DefaultSpawnReactionDelay` (`1s`) and honor positive authored values in `250ms..60s`
- live delayed server-origin retaliation arming / re-arm reuse that effective delay instead of hard-coding `bootstrapPracticeMobServerOriginRetaliationDelay`
- negative reaction delays, positive delays below `250` ms, and positive delays above `60000` ms fail closed at registration / bundle / static-snapshot validation
- migration `0020_static_actor_combat_profile_reaction_delay` adds the SQL column; static-actor content-state SQL import requires tip `0020` beside additive chase/return/homeward delay and max-step columns
- focused coverage: `TestGameRuntimeAuthoredReactionDelayArmsAndRearmsAtTwoSeconds`

Explicit non-goals for this profile-authored reaction-delay seam:
