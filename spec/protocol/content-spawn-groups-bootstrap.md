...
- `level` / `rank` are now profile-owned metadata for later mob presentation, reward, or formula slices, but the first freeze keeps them descriptor-only: built-in `training_dummy` and `practice_mob` default to `level = 1` and `rank = 0`, registered profiles preserve explicit values, and current HP mutation, reward payout, target carriers, and respawn timing do not yet read those fields
