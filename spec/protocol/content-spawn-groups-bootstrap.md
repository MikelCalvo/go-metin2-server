...
- those immediate and delayed owner-side retaliation point-loss beats are currently live-runtime only for that engaged selected session: they do **not** write the persisted account snapshot, and later position-only persistence helpers (`MOVE`, `SYNC_POSITION`, or transfer rebootstrap saves), successful slash `/use_item` saves, plus non-point-bearing slash `/inventory_move` and merchant-buy saves now keep their coordinate, authored use-effect point delta + item consumption, carried-slot, or purchase state without overwriting that pre-retaliation point value, so a fresh `/phase_select` re-entry or reconnect rebuilds from the pre-retaliation point value plus any later owned use-item delta until broader player-death persistence or respawn semantics are owned
...
- a same-socket `/quit` or `/logout` now counts as that live-session disappearance boundary immediately in the current bootstrap slice: either command removes the owner from shared-world visibility, cancels any pending delayed follow-up beat, and releases the current aggro-lite target gate before the client disconnect or outer socket close finishes; `/quit` still stays in `GAME` long enough to return its self `CHAT_TYPE_COMMAND quit` delivery while `/logout` continues to transition toward close
