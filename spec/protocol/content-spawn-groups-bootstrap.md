...
- `attack_value` / `defense_value` are now profile-owned authored stat defaults used by the first deterministic registered-profile damage formula (`max(1, attack_value - defense_value)`, capped by max HP); `damage_per_normal_attack` remains the legacy fallback used to canonicalize omitted attack values and preserve older bootstrap profile behavior
- `level` / `rank` are now profile-owned metadata for later mob presentation, reward, or formula slices, but the first freeze keeps them descriptor-only: built-in `training_dummy` and `practice_mob` default to `level = 1` and `rank = 0`, registered profiles preserve explicit values, omitted registered-profile `level` canonicalizes to the same bootstrap level `1`, omitted registered-profile `rank` remains `0`, and current HP mutation, reward payout, target carriers, and respawn timing do not yet read those fields
