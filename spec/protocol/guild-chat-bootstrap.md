...
- queue the same `GC_CHAT` guild delivery to the other connected bootstrap sessions
...
This slice intentionally treats all currently connected `GAME` sessions in the bootstrap runtime as one implicit temporary guild.

That is a temporary bootstrap policy only.
It is not a claim that real guild membership/invite/state semantics already exist.
