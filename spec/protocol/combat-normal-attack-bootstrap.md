...
- Non-floor delayed server-origin retaliation beats now append the same owner self `DAMAGE_INFO(owner_vid, flag=0, damage=abs(retaliation_delta))` after their ordinary point-change and queue that same owner companion to currently visible live peers; owner-floor retaliation beats still omit that companion and keep `PLAYER_POINT_CHANGE(value=0)` -> `DEAD(owner_vid)` -> `TARGET(0, 0)`.
