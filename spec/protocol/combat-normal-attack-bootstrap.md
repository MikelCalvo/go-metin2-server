...
- same-socket `/quit`, `/logout`, and `/phase_select` now all count as immediate live-ownership loss boundaries for that cadence: each path removes the owner from shared-world visibility, cancels any pending delayed beat, and releases the current practice-mob engagement right away; `/quit` still remains in `GAME` just long enough to return its self `CHAT_TYPE_COMMAND quit` delivery, `/logout` continues toward close, and `/phase_select` now returns to character select while any later fresh bootstrap still requires a new `TARGET`
...
- same-socket `/quit`, `/logout`, and `/phase_select` now all count as immediate owner-disappearance boundaries for that queued delayed cadence: each path removes the owner from shared-world visibility, cancels any pending delayed beat, and releases the same still-live practice mob right away; `/quit` still remains in `GAME` just long enough to return its self `CHAT_TYPE_COMMAND quit` delivery, `/logout` continues toward close, and `/phase_select` returns to character select while any later fresh bootstrap still requires a new `TARGET`
