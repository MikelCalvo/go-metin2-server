...
- when the engaged actor reaches the accepted non-player zero-HP death edge, the killing hit keeps the owned mob death choreography (`GC DEAD(target_vid)` + `GC TARGET(0, 0)` + mob `DAMAGE_INFO`); if that same accepted hit would also drive the engaged owner to the bootstrap `0`-HP floor through the ordinary immediate retaliation delta, any owned EXP / gold / drop reward frames stay next and the owner-floor suffix (`PLAYER_POINT_CHANGE(value=0)` -> `DEAD(owner_vid)` -> `TARGET(0, 0)` -> owner `DAMAGE_INFO`) now follows in the same packet burst, while a killing hit that leaves the owner above `0` still omits owner retaliation. The killing hit still cancels any pending delayed retaliation beat before the respawn delay elapses, and keeps the cadence stopped after the respawn rebuild until a fresh post-respawn `TARGET` / accepted hit starts a new engagement
