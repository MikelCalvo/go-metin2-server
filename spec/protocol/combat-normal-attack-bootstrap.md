...
- Owner-floor immediate, delayed, and proximity-armed retaliation beats now keep `PLAYER_POINT_CHANGE(value=0)` -> `DEAD(owner_vid)` -> `TARGET(0, 0)` and then append one owner self plus visible-peer plain `DAMAGE_INFO(owner_vid, flag=0, damage=abs(final_clamped_delta))` after that death/clear prefix. The immediate owner-floor hit still omits mob `DAMAGE_INFO` on that same accepted attack.
