...
- when an immediate or delayed retaliation beat reaches that owner-side `0`-HP floor, the current slice also clears the live combat target with one self-only `GC TARGET(0, 0)` instead of leaving the stale engagement selected while broader player-death choreography stays out of scope
...
- when either the immediate retaliation tick or a delayed follow-up beat reaches that owner-side `0`-HP floor, the current slice also emits one self-only `GC TARGET(0, 0)` clear so the stale engaged mob is no longer kept as the active combat target
