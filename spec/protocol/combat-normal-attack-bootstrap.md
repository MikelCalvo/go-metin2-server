...
- once that first authoritative hit establishes or preserves the current practice-mob engagement, any other session's already-selected shared-world combat-target ownership for that same mob is cleared immediately, so a preselected third party cannot keep a stale selected-target bypass alive and later land `ATTACK` against a mob another session already engaged first; those stale follow-up attempts still fail closed with no proactive clear-target companion yet
