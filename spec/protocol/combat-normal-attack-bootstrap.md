...
- those immediate and delayed owner-side retaliation point-loss beats currently mutate only the engaged selected-session live runtime; they do **not** write the persisted account snapshot, and later successful slash `/use_item` saves now keep their authored use-effect point delta plus item consumption without overwriting that pre-retaliation point value, so a fresh `/phase_select` re-entry or reconnect still rebuilds from the pre-retaliation point value plus any later owned use-item delta until broader player-death persistence or respawn semantics are owned
