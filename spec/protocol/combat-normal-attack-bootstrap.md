...
- operator/runtime in-place update of the currently selected dummy now counts as the same kind of combat-reset boundary for snapshot-bound target ownership: after the ordinary actor refresh or visibility-transition frames from that update, any still-selected live session also receives one queued self-only `GC TARGET(0, 0)` so later stale `ATTACK` attempts fail closed until the dummy is reselected
