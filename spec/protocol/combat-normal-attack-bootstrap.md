...
- Spawn-backed practice-mob non-lethal normal hits now append one owner self `DAMAGE_INFO(target_vid, flag=0, damage=applied_damage)` after the existing immediate retaliation `PLAYER_POINT_CHANGE` when the owner remains alive, then one owner self `DAMAGE_INFO(owner_vid, flag=0, damage=abs(retaliation_delta))`, and queue only the mob hit-effect frame to currently visible live peers; peers do not receive the owner's self `TARGET` refresh, retaliation point-change, or owner retaliation damage-info.
- Non-floor delayed server-origin retaliation beats now append the same owner self `DAMAGE_INFO(owner_vid, flag=0, damage=abs(retaliation_delta))` after their ordinary point-change; owner-floor retaliation beats still omit that companion and keep `PLAYER_POINT_CHANGE(value=0)` -> `DEAD(owner_vid)` -> `TARGET(0, 0)`.
...
- Richer flag meanings, killing-hit damage-info, owner-retaliation peer fanout, and broader hit-result policy remain later runtime emission policy.
