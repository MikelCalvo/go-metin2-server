...
- the cadence fails closed and stops if the engaged owner loses live shared-world ownership, clears or replaces the selected target, successfully crosses a transfer / rebootstrap reset boundary, or the engaged actor dies / rebuilds before the next delay expires
- same-socket `/quit`, `/logout`, and `/phase_select` now all count as immediate live-ownership loss boundaries for that cadence, and abrupt session close does too: each path removes the owner from shared-world visibility, cancels any pending delayed beat, and releases the current practice-mob engagement right away; `/quit` still remains in `GAME` just long enough to return its self `CHAT_TYPE_COMMAND quit` delivery, `/logout` continues toward close, `/phase_select` now returns to character select while any later fresh bootstrap still requires a new `TARGET`, and close tears the session down without a compensating gameplay packet
