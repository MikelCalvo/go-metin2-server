...
Combat-profile defaults now also carry descriptor-only `level` and `rank` metadata. The built-in bootstrap `training_dummy` and `practice_mob` profiles both currently freeze `level = 1` and `rank = 0`, and registered profile lookups preserve explicit level/rank values for later mob presentation, reward, or formula slices. These fields do not yet change the current normal-attack formula, target HP carrier, reward payout, or respawn timing.
