...
- when the engaged actor dies, a killing hit that leaves the owner above `0` still does not append an extra owner-side retaliation point-change; a killing hit that would also floor the owner now appends the dummy death/clear/hit-effect prefix and then the ordinary owner-floor suffix in the same packet burst. Any pending delayed follow-up beat is canceled before respawn, and the respawn rebuild does not resurrect stale retaliation work without a fresh target / accepted hit
