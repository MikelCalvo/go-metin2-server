...
The server -> client `CHARACTER_POSITION` update family and the self-only `CHANGE_SPEED` companion are documented separately in `character-position-change-speed-bootstrap.md`. This ingress handling prevents known client sit/stand traffic from becoming an unexpected-packet disconnect/error while leaving final chair-object placement, battle-mode, persistent stance, skill, and projectile presentation policy to later dedicated slices. The first `STUN` companion is a later sitting dummy-hit presentation, not a stance-packet side effect; sit/stand itself still does not emit `STUN`.
...
- Non-floor delayed server-origin retaliation beats now append the same owner self `DAMAGE_INFO(owner_vid, flag=0, damage=abs(retaliation_delta))` after their ordinary point-change and queue that same owner companion to currently visible live peers for both hit-armed and proximity-armed delayed cadences.
- Owner-floor immediate, delayed, and proximity-armed retaliation beats now keep `PLAYER_POINT_CHANGE(value=0)` -> `DEAD(owner_vid)` -> `TARGET(0, 0)` and then append one owner self plus visible-peer plain `DAMAGE_INFO(owner_vid, flag=0, damage=abs(final_clamped_delta))` after that death/clear prefix. An accepted immediate owner-floor hit also appends one self plus visible-peer plain mob `DAMAGE_INFO(target_vid, flag=0, damage=applied_bootstrap_damage)` after that death/clear prefix and before the owner companion; delayed and proximity-armed floors still omit a synthetic mob companion because they never accepted an owner `ATTACK`.
- `DEAD(vid)` plus `TARGET(0, 0)` still owns the zero-HP edge; accepted killing hits now also append one self plus visible-peer plain `DAMAGE_INFO(target_vid, flag=0, damage=applied_damage)` companion after that death/clear prefix and before any owned reward frames.
- When that same accepted killing hit would also drive the engaged owner to the bootstrap `0`-HP floor through the ordinary immediate retaliation delta, dummy death still owns the first prefix and any owned EXP / gold / drop reward frames stay next, then the owner-floor suffix (`PLAYER_POINT_CHANGE(value=0)` -> `DEAD(owner_vid)` -> `TARGET(0, 0)` -> owner `DAMAGE_INFO`) follows in the same packet burst. A killing hit that leaves the owner above `0` still omits owner retaliation.
- Richer flag meanings and broader hit-result policy remain later runtime emission policy.
- The first owned `STUN` companion is documented in `player-stun-bootstrap.md`. It is skill-less presentation only: an accepted non-lethal standalone dummy hit while the owner already holds the owned ground-sit presentation queues one self-only `GC STUN(target_vid)` after the ordinary `TARGET` + `DAMAGE_INFO` burst. Standing hits, spawn-backed hits, killing hits, cadence-denied repeats, sit/stand itself, death, respawn, restart, skill, and PvP still do not emit `STUN`.
...
## First skill-intent presentation

The current bootstrap runtime owns `USE_SKILL` as a `GAME`-phase ingress and as one presentation-only success path.
...
- one accepted request whose `skill_vnum` is the bootstrap presentation value `1` and whose `target_vid` matches the currently selected visible combat target emits one self-only `GC CREATE_FLY(type = 0, start_vid = owner_vid, end_vid = target_vid)` using the already-owned fly codec frozen in `combat-fly-effect-bootstrap.md`
- that companion is presentation only: it does not mutate selected-target HP, normal-attack cadence, retaliation timers, peer queues, points, inventory, or account persistence, and it does not spend skill points or start a cooldown
...
- profile-authored per-mob delayed reaction arming delay beyond the bootstrap `1s` default is owned as optional `combat_profiles.reaction_delay_ms` in `content-spawn-groups-bootstrap.md`; live delayed server-origin retaliation arming / re-arm consume `EffectiveStaticActorSpawnReactionDelay` (omit/zero keeps bootstrap `1s`)
...
- those immediate and delayed owner-side retaliation point-loss beats now persist the selected-character bootstrap HP point, including partial (above-floor) loss, so a later reconnect / `/phase_select` / `ENTERGAME` rebuilds the reduced HP from the account snapshot. Later successful slash `/use_item`, carried-slot `ITEM_USE`, `/equip_item`, and `/unequip_item` saves keep their authored use/equip-effect point delta plus consumed or carried/equipped item state beside that already-reduced HP instead of restoring the pre-retaliation full value. Once either retaliation beat reaches the `0`-HP floor, the same snapshot is persisted as `0` with the owned death/clear frames so those rebuilds replay the dead snapshot; accepted `/restart_here` / `/restart_town` restore race create MaxHP into that persisted snapshot
...
- when the engaged actor reaches the accepted non-player zero-HP death edge, the killing hit keeps the owned mob death choreography (`GC DEAD(target_vid)` + `GC TARGET(0, 0)` + mob `DAMAGE_INFO`); if that same accepted hit would also drive the engaged owner to the bootstrap `0`-HP floor through the ordinary immediate retaliation delta, any owned EXP / gold / drop reward frames stay next and the owner-floor suffix (`PLAYER_POINT_CHANGE(value=0)` -> `DEAD(owner_vid)` -> `TARGET(0, 0)` -> owner `DAMAGE_INFO`) now follows in the same packet burst, while a killing hit that leaves the owner above `0` still omits owner retaliation. The killing hit still cancels any pending delayed retaliation beat before the respawn delay elapses, and keeps the cadence stopped after the respawn rebuild until a fresh post-respawn `TARGET` / accepted hit starts a new engagement. The checked-in formula QA fixture now also owns that combined last-hit on its portable profile-default descriptor: four formula hits kill `practice.qa_formula_mob`, the killing hit emits EXP `40` / gold `25` / drop `27001` before the authored `-2` owner-floor suffix, and stale same-target `ATTACK` then fails closed (`TestGameSessionFlowAuthoredFormulaProfileKillingHitAlsoFloorsOwnerEmitsProfileDefaultRewardsBeforeOwnerFloor`)
...
- how and when broader player-death / revive state beyond the current persisted above-floor HP snapshot and bootstrap `0`-HP floor should eventually own corpse, loot-on-death, or richer revive policy; above-floor retaliation loss and the floor itself already persist with the selected-character snapshot
...
- those immediate and delayed owner-side retaliation point-loss beats now persist the selected-character bootstrap HP point, including partial (above-floor) loss, so a later reconnect rebuilds the reduced HP from the account snapshot. Later position-only persistence helpers (`MOVE`, `SYNC_POSITION`, or transfer rebootstrap saves), successful slash `/use_item`, carried-slot `ITEM_USE`, `/equip_item`, and `/unequip_item` saves, plus non-point-bearing slash `/inventory_move` and merchant-buy saves keep their coordinate, authored use/equip-effect point delta + consumed or carried/equipped item state, carried-slot, or purchase state beside that already-reduced HP instead of restoring the pre-retaliation full value. Once either retaliation beat reaches the `0`-HP floor, the same snapshot is persisted as `0` with the owned death/clear frames so a fresh `/phase_select` re-entry, reconnect, or `ENTERGAME` rebuilds from the dead snapshot; accepted `/restart_here` / `/restart_town` restore race create MaxHP into that persisted snapshot
...
- when the engaged actor dies, a killing hit that leaves the owner above `0` still does not append an extra owner-side retaliation point-change; a killing hit that would also floor the owner now appends the dummy death/clear/hit-effect prefix, then any owned EXP / gold / drop reward frames, then the ordinary owner-floor suffix in the same packet burst. Any pending delayed follow-up beat is canceled before respawn, and the respawn rebuild does not resurrect stale retaliation work without a fresh target / accepted hit
