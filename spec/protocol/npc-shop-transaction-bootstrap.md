...
- the resolved template does not carry a selected-character job/sex/empire/level restriction (`anti_warrior`, `anti_assassin`, `anti_sura`, `anti_shaman`, `anti_male`, `anti_female`, `anti_empire_a`, `anti_empire_b`, `anti_empire_c`, or `min_level` above the selected character's level)
...
- the same summary rows now also surface direct-use guard metadata (`confirm_when_use`, `quest_use`, `quest_use_multiple`, `applicable`, and `use_reject_message`) so confirmation-flagged / quest-/applicable-gated consumables are visible in dry-run content audits even when they appear only as merchant catalog entries or reward drops
...
NPC merchant `SHOP BUY` / `BuyMerchantItem` / `GrantCarriedItem` compatible-stack merges stay count-only with destination presence-aware instance sockets/attributes authoritative (including explicit zero; omitted destination stays omitted so later encode may keep template fallback), and free-cell grant remainders stay omit→template encode rather than inventing template-copy onto instance presence (`docs/plans/2026-09-04-merchant-buy-compatible-merge-destination-wins-instance-sockets-attributes.md`, `docs/plans/2026-09-03-compatible-stack-merge-destination-wins-instance-sockets-attributes.md`).
Guest private-shop buy now emits this `UPDATE_ITEM` companion with `vnum = 0` for sold display slots, and whole-stack free-cell guest placement preserves the host live presence-aware instance sockets/attributes (including explicit zero) onto the guest `ITEM_SET` / persisted inventory while compatible-stack merges stay count-only with destination presence-aware instance sockets/attributes authoritative (including explicit zero; host/source presence discarded; omitted destination stays omitted; `docs/plans/2026-08-24-myshop-guest-buy-mutation-contract-freeze.md`, `docs/plans/2026-09-03-myshop-guest-buy-preserve-instance-sockets-attributes.md`, `docs/plans/2026-09-03-compatible-stack-merge-destination-wins-instance-sockets-attributes.md`).
...
- partner-side open player-shop exchange `START` / `ACCEPT` busy rejects are now owned beside merchant/safebox/refine (see Owned partner-side open player-shop exchange busy-window reject seam); partner-side open cube `START` / `ACCEPT` busy-window rejection text is already owned beside that same gate (see Owned lab cube open/close presentation seam)
...
### Owned `GC::SHOP_SIGN` codec seam

The legacy server publishes private-shop signs as `GC::SHOP_SIGN` (`0x0811`) with a host VID plus a fixed NUL-padded sign field.
The repository now owns that packet shape at the codec level in `internal/proto/shop` (`HeaderServerShopSign`, `EncodeServerShopSign`, `DecodeServerShopSign`):
...
- partner-side open cube `START` / `ACCEPT` busy-window rejection text is already owned beside that same gate (see Owned lab cube open/close presentation seam)
...
- empty sign / zero count / duplicate or invalid stock / `anti_give|anti_myshop` / open merchant|safebox|refine|cube stay fail-closed with no second live `SHOP_SIGN`
- busy shells reject with the same self-only requester busy info-chat already owned by exchange START for merchant/safebox/refine/cube; an active same-socket bootstrap exchange is **not** a busy reject for host `CG::MYSHOP` open
...
- listed-price gold overflow past `math.MaxInt32` rejects with self-only `CHAT_TYPE_INFO` `You cannot open a private shop because it would exceed 2 Billion Yang.` after armor and before banword/stock walk (sum every request-table listed `price` plus live gold; no stock-row resolution required yet — `docs/plans/2026-08-27-myshop-open-gold-overflow-gate-reorder.md`)
- already-open host second accepted-decode `CG::MYSHOP` (non-empty sign + non-zero count) clears/closes with the owned empty-sign companion after other busy shells and body-armor, before gold/banword/stock: one empty-sign `GC::SHOP_SIGN` to self + peer around-broadcast, guest browse cleared with one guest-only `GC::SHOP END` as already owned, no busy INFO, no bag refund / inventory/quickslot/gold/persistence mutation (`docs/plans/2026-08-27-myshop-open-already-open-second-open-empty-sign-close.md`); empty-sign / zero-count second packets stay silent and leave the shop open; armor still wins over already-open close
- structural stock fails (empty sign / zero count / missing cell / count-vnum mismatch / duplicate display) stay silent/no-frame
- after those gates, the bag branch prefers an unlisted carried unlocked unequipped silk bag (`vnum = 71049`) when present: accepted open skips ordinary `50200` consume, leaves inventory/quickslots/gold unchanged by the bag branch, and emits bag-less live `GC::SHOP_SIGN` (+ peer around-broadcast as already owned) without inventing GD/DB `MYSHOP_PRICELIST_*` packets (`docs/plans/2026-08-27-myshop-silk-bag-71049-consume-skip.md`); listed-only / locked-only silk bags do not unlock that path; silk-path success also replaces the same-socket process-local remembered unit-price map with `unitPrice = listed_price / listed_count` per distinct listed stock `vnum` and persists that map on the selected character account FileStore as optional `myshop_unit_prices` so a later silk-bag `ITEM_USE` / `/use_item` can rematerialize `MyShopPriceList` command chat across reconnect / process restart without GD/DB packets (`docs/plans/2026-08-28-myshop-bag-use-openprivateshop-pricelist.md`, `docs/plans/2026-08-29-myshop-unit-prices-durable-filestore.md`)
- otherwise accepted open requires one carried unlocked ordinary shop bag (`vnum = 50200`) whose cell is not also listed in this open's stock table; success consumes exactly one bag (lowest eligible cell), persists inventory/quickslots, emits self-only bag `ITEM_UPDATE`/`ITEM_DEL` (+ emptied-cell `QUICKSLOT_DEL` when needed) before the owned live `GC::SHOP_SIGN`, and peer around-broadcast stays owned; missing / listed-only / locked-only bag stays silent/no-frame with no open (`docs/plans/2026-08-27-myshop-shop-bag-50200-require-consume.md`)
- on those accepted success paths only, an active same-socket bootstrap exchange is cancelled before the live `SHOP_SIGN`: ordinary bag → bag refresh → self `GC::EXCHANGE END` → live `SHOP_SIGN` (peer queued `END` then around-broadcast live sign); silk → self `END` → live `SHOP_SIGN` (peer queued `END` then live sign); cancel clears pairing/display/accept with no inventory/equipment/quickslot/gold/ground mutation from the cancel itself (`docs/plans/2026-08-28-myshop-open-cancel-active-exchange.md`)
- reject paths (armor / gold / banword / cash / equipped / locked / structural silent / missing-bag / merchant|safebox|refine|cube busy) leave any open exchange shell cancellable and do not emit `SHOP_SIGN`
- open does not yet remove listed carried stock, invent GD/DB silk-bag pricelist packets, polymorph, Canada-locale banword bypass, DB banword reload, or quest-running open blocks
- carried ordinary shop-bag (`50200`) / silk-bag (`71049`) `ITEM_USE` / `/use_item` is the owned client-visible private-shop setup companion (`docs/plans/2026-08-28-myshop-bag-use-openprivateshop-pricelist.md`): success never consumes the bag; busy merchant/safebox/refine/myshop/cube/exchange rejects with self-only `CHAT_TYPE_INFO` `You cannot use a shop bag or silk bag while another trade window is open.`; worn body armor reuses `You must unequip your armor to open a private shop.`; ordinary bag success emits one self-only `CHAT_TYPE_COMMAND` `OpenPrivateShop`; first silk USE in the same-socket session rematerializes remembered unit prices (hydrated from durable `myshop_unit_prices` on select/EnterGame when present) as `MyShopPriceList <vnum> <unitPrice>` (dummy `MyShopPriceList 1 0` when empty) then `OpenPrivateShop`, and later same-session silk USE emits only `OpenPrivateShop`
- accepted open success deactivates listed auto-potion `socket0` (`vnum` in `72723..72730` with live/effective `socket0 == 1` → `0`) via the minimal per-instance inventory sockets substrate: seed instance sockets from the resolved template when absent, clear `socket0` only, persist the host inventory with the open Save boundary, and emit self-only `ITEM_UPDATE` after any bag refresh and before exchange `END` / live `SHOP_SIGN` (ordinary bag → bag refresh → auto-potion `ITEM_UPDATE`(s) → `END` (if any) → `SHOP_SIGN`; silk → auto-potion `ITEM_UPDATE`(s) → `END` (if any) → `SHOP_SIGN`); listed cells with `socket0 != 1`, non-auto / non-listed cells, and reject/busy/missing-bag/already-open-close paths stay non-mutating (`docs/plans/2026-08-28-myshop-open-auto-potion-socket0-deactivate.md`); auto-potion USE/affects, shopkeeper polymorph / horse / mount teardown, `MYSHOP_PRICELIST`, quest-running, and bag-missing INFO stay deferred
...
- while the same-socket private-shop open/busy flag is set, host `CG::MOVE` / self-targeted `CG::SYNC_POSITION` fail closed with no frames and no live/persisted position mutation (shop stays open — no walk-away auto-close / no reject chat; oracle `CanMove` style — `docs/plans/2026-08-29-myshop-open-host-move-fail-closed-contract-freeze.md`); empty-sign close clears the deny
- accepted open also `EnqueueToVisibleSessions` the same live `GC::SHOP_SIGN` bytes to currently visible peer sessions (host still returns bag refresh frames when the ordinary `50200` path ran, otherwise bag-less silk success is `END` (when cancelling) then one self `SHOP_SIGN`; no second host self sign through the peer queue)
...
See `docs/plans/2026-08-23-myshop-accepted-open-presentation-contract-freeze.md`, `docs/plans/2026-08-26-myshop-open-reject-chat-hardening.md`, `docs/plans/2026-08-27-myshop-shop-bag-50200-require-consume.md`, `docs/plans/2026-08-27-myshop-silk-bag-71049-consume-skip.md`, `docs/plans/2026-08-27-myshop-open-banword-sign-reject.md`, and `docs/plans/2026-08-27-myshop-open-myshop-reject-message.md`, and `docs/plans/2026-08-27-myshop-open-gold-overflow-gate-reorder.md`, and `docs/plans/2026-08-27-myshop-open-already-open-second-open-empty-sign-close.md`, and `docs/plans/2026-08-28-myshop-open-cancel-active-exchange.md`, and `docs/plans/2026-08-28-myshop-open-auto-potion-socket0-deactivate.md`.
...
- the same empty-sign bytes are also `EnqueueToVisibleSessions` to currently visible peer sessions when the flag was open; already-closed close stays silent for both host and peers
...
- guest browse open/leave/buy are owned separately below; partner player-shop exchange busy rejects are owned separately; exchange open-cube busy rejects are owned separately; MYSHOP/safebox/refine open/confirm cube rejects are owned; cube `r_info`/`r_list`/`m_info` are owned (`docs/plans/2026-08-25-cube-r-info-result-list-implementation.md`, `docs/plans/2026-08-25-cube-m-info-material-info-implementation.md`); craft-slot add/del → `cube info` is owned (`docs/plans/2026-08-25-cube-add-del-slot-binding-implementation.md`); list / cancel / make stay deferred; view-entry rematerialization of a remembered live sign is owned separately below

See `docs/plans/2026-08-23-myshop-close-sign-clear-contract-freeze.md`.

### Owned MYSHOP peer SHOP_SIGN around-broadcast seam

Accepted host-only open/close now also own peer around-broadcast of the already-owned live/empty `GC::SHOP_SIGN` frames:
...
- success emits exactly one guest-only `GC::SHOP START` (`OwnerVID` = host VID + fixed `[ShopHostItemMax]` table filled by remembered `display_pos`); sockets/attributes project from the host's current live matching carried cell via presence-aware `EffectiveSockets` / `EffectiveAttributes` (instance presence including explicit zero wins over template; omitted keeps template fallback encode), otherwise that display slot stays empty (`TestGameRuntimeMyShopGuestBrowsePrefersInstancePresenceOverTemplate`)
...
- guest `SHOP SELL` / `SELL2` while browsing a private shop are owned fail-closed with no frames and no inventory/gold/host mutation (`docs/plans/2026-08-25-myshop-guest-sell-while-browsing-fail-closed.md`); ordinary shop-bag `50200` consume on host open is owned (`docs/plans/2026-08-27-myshop-shop-bag-50200-require-consume.md`); tax/empire multipliers and silk-bag `MYSHOP_PRICELIST` DB rematerialize stay deferred; silk-bag `71049` consume-skip on open is owned (`docs/plans/2026-08-27-myshop-silk-bag-71049-consume-skip.md`); craft-slot add/del → `cube info` is owned (`docs/plans/2026-08-25-cube-add-del-slot-binding-implementation.md`); list / cancel / make stay deferred (cube `r_info`/`r_list`/`m_info` owned separately)
...
- accepted sells remove the whole stack or decrement the stack count and credit the selected character's live gold total with the owned template-authored or template-derived sell credit only when both the credited amount and resulting gold value fit the current signed `PLAYER_POINT_CHANGE` carrier
- templates with non-zero `shop_sell_price` use that authored value as the explicit per-unit sell credit: `shop_sell_price * sold_count`; this branch bypasses the older derived `/5` and 3% tax floors because the authored sell price is already the content-owned runtime credit
- when `shop_sell_price` is omitted or zero, ordinary sell credit derives from loaded item-template `shop_buy_price` as `floor((shop_buy_price * sold_count) / 5)` minus `floor(3% tax)`
- when `shop_sell_price` is omitted or zero, templates flagged `sell_count_per_gold` follow the legacy count-per-gold branch first: use `floor(sold_count / shop_buy_price)` when `shop_buy_price > 0`, or `sold_count` when it is zero, then apply the same `/5` and 3% tax floor; if the resulting credit is zero, the bootstrap runtime fails closed
- templates flagged `anti_sell` or `anti_stack` fail closed before credit calculation and leave live plus persisted inventory/currency unchanged; `anti_stack` still returns bare self-only `GC::SHOP INVALID_POS`, while `anti_sell` returns that same merchant invalid-position companion and then appends one self-only `CHAT_TYPE_INFO` rejection text sourced from template `sell_reject_message` when present or the deterministic fallback `"The merchant refuses to buy this item."` when omitted
- the player mutation boundary also exposes a template-backed sell helper that rejects `anti_sell` / `anti_stack` templates, template/item-`vnum` mismatches, invalid template metadata, invalid counts, malformed carried item snapshots (`id = 0`, `vnum = 0`, or stale equipment-slot metadata on an unequipped carried item), live carried stacks above the template-authored `max_count`, locked carried items, equipped carried items, selected-character job/sex/empire/level restrictions, and zero-credit pricing before mutating live inventory or gold
...
- selected-character `min_level` and empire anti-flag restrictions are checked through the same template-use guard as job/sex anti-flags; packet `SHOP BUY`, packet `SHOP SELL` / `SELL2`, and the template-backed player-runtime sell helper all fail closed before inventory, quickslot, gold, or persistence mutation when the selected character fails an authored `anti_empire_a` / `anti_empire_b` / `anti_empire_c` guard or is below the authored minimum level (`docs/plans/2026-08-22-merchant-buy-sell-empire-restriction-matrix.md`)
- the updated selected-character snapshot is persisted before the live shared-world registration is refreshed
- if a still-live owner has already taken runtime-only practice-mob retaliation HP loss before an accepted sell, the sell persistence writes only the merchant-owned inventory/quickslot/gold changes and keeps the persisted bootstrap HP point at its pre-retaliation value; a later reconnect rebuilds from that pre-retaliation HP while preserving the accepted sell result
...
- `anti_sell` is the only current merchant sell rejection that appends a template-authored self-only info chat after `GC::SHOP INVALID_POS`; adjacent invalid-price/stack/count/equipment/lock/restriction failures keep the narrower merchant-family error frame only until their own UI text contracts are frozen
...
The packet/runtime path now loads the item shop-sell price, shop-buy price, count-per-gold flag, first anti-sell policy, and anti-sell rejection text through `itemstore.Template.shop_sell_price`, `itemstore.Template.shop_buy_price`, `itemstore.Template.sell_count_per_gold`, `itemstore.Template.anti_sell`, and `itemstore.Template.sell_reject_message`. Non-zero `shop_sell_price` is preferred as explicit per-unit sell credit; otherwise the runtime applies the legacy-compatible count/price branch before the shared `/5` and 3% tax floors. Template-authored sell credit is still clamped to the current signed point-change carrier used by the bootstrap gold refresh; over-carrier credit or resulting gold fails closed with `INVALID_POS` while preserving live and persisted inventory/currency. The first runtime-locked and malformed carried item-instance guards are also owned for packet sell-back: locked or invalid carried items fail closed with `INVALID_POS` and leave live plus persisted inventory/currency unchanged. This still is not a full 1:1 pricing claim: broader bound-item policy, locale-specific tax variants, and final UI/result choreography remain later slices.
...
- the composed PvE vertical authoring fixture now packet-unequips turn-in `11200` and packet-sells that last stack for authored `shop_sell_price = 100` after `QuestGuide` re-unlock (`TestPveVerticalAuthoringBundleClosesGuideUnlockKillCreditAndTurnIn`); `SHOP SELL` while the sword is still worn stays fail-closed `INVALID_POS`. The dedicated NPC-service merchant proof now packet-buys catalog slot `1` (`Wooden Sword` `11200` @ `500g`) after `QuestGuide` unlock, then packet-equips/unequips that bought last stack through the ordinary template-backed appearance/`equip_effect` burst (`TestNpcServiceBundleMerchantSwordBuyEquipsAppearanceAndEquipEffect`, `docs/plans/2026-09-08-npc-service-authored-sword-appearance-equip.md`). `MerchantSellCredit` for that same template returns `100` and fails closed when `shop_sell_price` / `shop_buy_price` are both omitted (`TestMerchantSellCreditPrefersAuthoredWoodenSwordShopSellPrice`)
