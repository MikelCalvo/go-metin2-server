...
- after those gates, the bag branch prefers an unlisted carried unlocked unequipped silk bag (`vnum = 71049`) when present: accepted open skips ordinary `50200` consume, leaves inventory/quickslots/gold unchanged by the bag branch, and emits bag-less live `GC::SHOP_SIGN` (+ peer around-broadcast as already owned) without inventing `MYSHOP_PRICELIST` DB packets (`docs/plans/2026-08-27-myshop-silk-bag-71049-consume-skip.md`); listed-only / locked-only silk bags do not unlock that path
...
- open does not yet remove listed carried stock, invent silk-bag pricelist DB packets, polymorph, Canada-locale banword bypass, DB banword reload, or quest-running open blocks
