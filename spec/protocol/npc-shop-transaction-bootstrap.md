...
- next frozen (docs-only; not yet GREEN): accepted open success must deactivate listed auto-potion `socket0` (`vnum` in `72723..72730` with live `socket0 == 1` → `0`) via a minimal per-instance inventory sockets substrate, emitting self-only `ITEM_UPDATE` after any bag refresh and before exchange `END` / live `SHOP_SIGN`, then persist (`docs/plans/2026-08-28-myshop-open-auto-potion-socket0-deactivate.md`); auto-potion USE/affects, shopkeeper polymorph / horse / mount teardown, `MYSHOP_PRICELIST`, quest-running, and bag-missing INFO stay deferred
