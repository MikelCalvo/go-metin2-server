...
- the same summary rows now surface template-authored `anti_get` and `anti_sell` guard flags so operators can see merchant buy/sell deny reasons next to the existing reject-message text before applying a bundle
