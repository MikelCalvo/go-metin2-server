...
Guest private-shop buy now emits this `UPDATE_ITEM` companion with `vnum = 0` for sold display slots, and whole-stack free-cell guest placement preserves the host live presence-aware instance sockets/attributes (including explicit zero) onto the guest `ITEM_SET` / persisted inventory while compatible-stack merges stay count-only (`docs/plans/2026-08-24-myshop-guest-buy-mutation-contract-freeze.md`, `docs/plans/2026-09-03-myshop-guest-buy-preserve-instance-sockets-attributes.md`).
