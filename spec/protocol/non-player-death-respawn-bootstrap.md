...
- if a live session is shown that same still-dead combatant again before the timer expires through any later add-style visibility presentation — fresh bootstrap, visibility re-entry, a retained delete-plus-rebootstrap refresh, same-socket `/restart_here` after a combined last-hit that also floored the owner, or source-map relocate-back after same-socket `/restart_town` on that same combined last-hit — it first receives the ordinary `CHARACTER_ADD` + `CHAR_ADDITIONAL_INFO` + `CHARACTER_UPDATE` burst and then one `GC DEAD(vid)` replay so the actor does not silently look alive again; the combined last-hit `/restart_here` path is covered by `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartHereCatchesUpStillDeadDummy`, combined last-hit `/phase_select` re-entry and abrupt reconnect keep that dummy skipped while the owner is still at the zero-HP floor then reuse the same `/restart_here` still-dead catch-up (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartHereCatchesUpStillDeadDummy`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartHereCatchesUpStillDeadDummy`); if that last-hit also registered an owned kill-reward drop, those same Leave paths park the pending handle instead of deleting it, still-dead re-entry skips ground rematerialize, and later `/restart_here` rematerializes self-only `ITEM_GROUND_ADD` + `ITEM_OWNERSHIP` (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerPhaseSelectRestartHereRematerializesKillRewardDrop`, `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerReconnectRestartHereRematerializesKillRewardDrop`); and the combined last-hit `/restart_town` relocate-back path is covered by `TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartTownRematerializesKillRewardDropOnSourceMapReselect`. If that same last-hit also registered an owned kill-reward drop, `/restart_here` rematerializes the pending handle with self-only `ITEM_GROUND_ADD` + `ITEM_OWNERSHIP` after that still-dead catch-up (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerRestartHereRematerializesKillRewardDrop`); `/restart_town` rematerializes the same handle only after source-map relocate-back, including after combined last-hit `gamed` process restart (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerDaemonRestartRestartTownRematerializesKillRewardDropOnSourceMapReselect`)
- content-loaded `spawn_groups` combatants inherit that same still-dead add-style trailing-`DEAD` rule for EnterGame / reconnect / visibility re-entry; focused shared-world coverage now freezes that content-loaded EnterGame path beside the existing `training_dummy` bootstrap proof, including fail-closed target/attack while the owned dead interval is still open; still-dead content-bundle replacement anti-resurrect and the remaining Track A #6 anti-leak matrix rows stay owned by `content-spawn-groups-bootstrap.md`
...
- a live killing hit after a within_radius chase displace keeps that still-dead corpse at the death coords (no homeward `MOVE`, late EnterGame add/info/update + trailing `GC DEAD` at those coords) until the due respawn rebuild snaps to authored home; focused coverage: `TestGameRuntimeKillingHitAfterChaseDisplaceKeepsStillDeadAtDeathCoordsAndRespawnsAtAuthoredHome`. Daemon restart rematerializes that same still-dead corpse at the persisted death coords with the remaining absolute deadline and no homeward/return/chase arm (`TestGameRuntimeKillingHitAfterChaseDisplaceStillDeadPersistsAcrossDaemonRestart`). A still-dead `return_required` restore likewise must not arm an automatic return-step deadline or MOVE the corpse toward home (`TestGameRuntimeRestoreStillDeadReturnRequiredSpawnGroupDoesNotArmReturnStep`). A non-identical content reimport that keeps the same authored ref rematerializes a live displaced actor at that FileStore current position while a still-dead corpse stays on these death coords (`TestGameRuntimeContentReimportRematerializesLiveSpawnPositionDistinctFromAuthoredHome`). The older at-home session flow, at-home daemon-restart, and synthetic timer-cleanup twins stay owned separately
- if a same-profile runtime/operator update moves a dead spawn-backed combatant away from authored home before that pending respawn fires, the actor can still inspect as `dead` and `return_required`, but return-step scheduling remains suppressed while the respawn timer is authoritative; daemon restart of that same still-dead `return_required` snapshot follows the same suppression instead of re-arming from the pre-still-dead register snapshot
