...
- content-loaded `spawn_groups` combatants inherit that same still-dead add-style trailing-`DEAD` rule for EnterGame / reconnect / visibility re-entry; the multi-map / reconnect anti-leak matrix in `content-spawn-groups-bootstrap.md` freezes that content-loaded EnterGame coverage as the next honest focused RED after the existing `training_dummy` bootstrap proof
