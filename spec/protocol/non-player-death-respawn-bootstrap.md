...
- the same pending respawn row can be inspected directly through loopback-only `GET /local/static-actor-respawns/{entity_id}`; missing, already-flushed, or malformed entity IDs fail closed without leaking stale actor data
