...
- keep richer trade-target eligibility beyond the owned exchange-distance gate, requester-side open-merchant / open-safebox PREVENT_TRADE_WINDOW start reject, and partner-side open-merchant / open-safebox busy-window rejection text, rollback/audit policy, and any finalize/result choreography beyond the first mutual-accept mutation + shell close deferred until a later exchange/trade slice owns those semantics
...
- clear any source item quickslots that pointed at transferred whole stacks using the already-owned quickslot deletion ordering
...
- richer trade target eligibility beyond the current visible-player check plus owned exchange-distance gate (`ApproxDistance < 1000`), the requester-side open-merchant / open-safebox PREVENT_TRADE_WINDOW start reject, and the partner-side open-merchant / open-safebox busy-window rejection text
...
- `internal/minimal` freezes the visible-peer `START` / `CANCEL` shell with paired `GC::EXCHANGE START` / `END` frames and no persisted inventory, quickslot, equipment, or gold mutation for the display-only path; display-only `ITEM_ADD` / `ITEM_DEL` / `GOLD_ADD` / first-side `ACCEPT`; duplicate display-slot/source-item suppression; accept-marker reset before accepted display changes; accept-time live item/gold revalidation; template-backed accept-time displayed-item revalidation against the currently loaded template metadata; second-accept receiver item-id collision, over-template-max compatible-stack, inventory-capacity, and gold-overflow precondition guards; the first mutual-accept finalize (displayed item/gold transfer, source quickslot clear, dual account persistence, and shell `END`); fail-closed origin-save and partner-save persistence failures that leave both accounts and live snapshots unchanged while the shell remains cancellable; `ALREADY` / `LESS_GOLD`; unsupported final-result no-frame guards; active-shell `anti_give` item-add feedback; the owned exchange-distance `START` reject and walk-away shell close; the requester-side open-merchant / open-safebox PREVENT_TRADE_WINDOW `START` reject that returns self-only info chat and leaves the merchant or safebox presentation open; the partner-side open-merchant / open-safebox busy-window `START` reject that returns self-only info chat and leaves the partner merchant or safebox presentation open; and teardown-before-feedback/mutation coverage for owned lifecycle, item-use, item-move/equipment, drag-to-item, drop, merchant, refine, storage, item-give, and accepted quickslot-edit paths.
