...
- no final result server frames are emitted; stale requester displayed items or displayed-item template metadata at accept time emit no frames, stale already-accepted partner displayed items, displayed-item template metadata, or gold also emit no frames on the second accept, and the owned stale requester displayed-gold accept exception emits only the same self-only `LESS_GOLD` status frame already used by over-budget `GOLD_ADD`
...
- `internal/minimal` freezes the visible-peer `START` / `CANCEL` shell with paired `GC::EXCHANGE START` / `END` frames and no persisted inventory, quickslot, equipment, or gold mutation; display-only `ITEM_ADD` / `ITEM_DEL` / `GOLD_ADD` / `ACCEPT`; duplicate display-slot/source-item suppression; accept-marker reset before accepted display changes; accept-time live item/gold revalidation; template-backed accept-time displayed-item revalidation against the currently loaded template metadata; `ALREADY` / `LESS_GOLD`; unsupported final-result no-frame guards; active-shell `anti_give` item-add feedback; and teardown-before-feedback/mutation coverage for owned lifecycle, item-use, item-move/equipment, drag-to-item, drop, merchant, refine, storage, and item-give paths.
