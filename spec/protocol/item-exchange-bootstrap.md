...
- own the first two-party exchange-window shell (`START` / display-only `ITEM_ADD` / display-only `ITEM_DEL` / display-only `GOLD_ADD` / display-only `ACCEPT` / accept reset / accept-time displayed-gold revalidation / `END`) without mutating inventory, equipment, quickslots, gold, ground items, or persisted account state, and close that display shell before successful carried-item use, carried/equipment item moves, drag-to-item stack consolidation, carried-item drop refresh frames, or template-backed carried-drop rejection feedback is exposed for an item already shown in it
...
The shell is deliberately not an item/gold transfer system. It performs no inventory, equipment, quickslot, gold, ground-item, or persisted-account mutation. It now emits display-only exchange `ITEM_ADD` responses for valid carried items in the active shell, display-only `ITEM_DEL` responses for occupied display slots in that shell, display-only `GOLD_ADD` responses for non-zero active-shell gold placement requests that do not exceed the requester's live gold, display-only `ACCEPT` responses for active-shell accept requests, and one self-only `LESS_GOLD` response when an accept attempt finds that a previously displayed gold amount no longer fits the requester's current live gold.
...
The display-only `ACCEPT` path is accepted only when the requester is currently paired in the bootstrap exchange shell and, if that requester previously displayed gold, the displayed amount is still less than or equal to the requester's current live gold. When the revalidation passes, the requester receives one direct `GC::EXCHANGE ACCEPT` with `is_me = 1`, and the paired peer receives one queued `GC::EXCHANGE ACCEPT` with `is_me = 0`; both packets carry `arg1 = 1` and leave the remaining fields zero. When the requester's remembered displayed gold is now greater than current live gold, the requester instead receives one direct `GC::EXCHANGE LESS_GOLD`, no peer accept marker is queued, and the accepted marker is not changed. Neither branch closes the exchange, transfers items or gold, emits an exchange result, or persists trade state. The shell remains cancellable after both sides have displayed `ACCEPT` or after a stale-gold accept rejection.
...
- no final result server frames are emitted; the owned stale displayed-gold accept exception emits only the same self-only `LESS_GOLD` status frame already used by over-budget `GOLD_ADD`
...
- `internal/minimal` freezes the visible-peer `START` / `CANCEL` shell with paired `GC::EXCHANGE START` / `END` frames and no persisted inventory, quickslot, equipment, or gold mutation; it also freezes the self-only `GC::EXCHANGE ALREADY` response when a third visible requester targets an already-paired peer, the active-shell `GC::EXCHANGE ITEM_ADD` display echo/peer queue with template-authored sockets/attributes, duplicate display-slot and duplicate source-item suppression, the active-shell `GC::EXCHANGE ITEM_DEL` display clear/peer queue plus display-slot/source-item reuse, the active-shell `GC::EXCHANGE GOLD_ADD` display echo/peer queue, the active-shell `GC::EXCHANGE ACCEPT` display echo/peer queue without finalization, accept-marker reset frames before accepted item/gold display changes, the self-only `GC::EXCHANGE LESS_GOLD` response when displayed gold exceeds live gold, the no-frame fail-closed behavior for unsupported final result requests, the active-shell self-only `CHAT_TYPE_INFO` rejection frame when the carried item's template authors `anti_give` and `give_reject_message` while no-shell guarded item-add attempts stay silent, the post-floor death-edge `GC::EXCHANGE END` close for open exchange shells, same-socket `/quit` / `/logout` / `/phase_select` exchange teardown that emits self `END` before the lifecycle frame and queues peer `END` without mutating carried or persisted state, successful carried `ITEM_USE` teardown that emits self/peer `END` before the requester receives the item-use mutation frames, successful carried-to-carried `ITEM_MOVE` and slash `/inventory_move` teardown that emits self/peer `END` before item/quickslot refresh frames, successful carried/equipment `ITEM_MOVE` plus slash `/equip_item` / `/unequip_item` teardown that emits self `END` before equipment refresh frames and peer `END` before queued appearance updates, successful `ITEM_USE_TO_ITEM` stack-consolidation teardown that emits self/peer `END` before item/quickslot merge refresh frames, successful carried-item `ITEM_DROP` / `ITEM_DROP2` teardown that emits self `END` before item/quickslot/ground/ownership frames and peer `END` before visible ground frames, template-backed carried-drop rejection teardown that emits self/peer `END` before the self-only rejection chat, successful merchant `SHOP BUY` / `SELL` / `SELL2` teardown that emits self/peer `END` before merchant item/gold refresh frames, template-authored `REFINE` rejection/preview exchange teardown that emits self/peer `END` before the self-only refine feedback frame, and template-authored `ITEM_GIVE` anti-give feedback teardown that emits self/peer `END` before the self-only item-give rejection chat.
