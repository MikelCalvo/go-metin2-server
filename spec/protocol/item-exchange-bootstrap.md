...
- If the requester already has an open same-socket merchant window, `START` fails closed with no exchange frames, no pairing/display mutation, and the merchant window left open.
...
No richer finalize/result packet family is introduced in this slice. If either side's live state drifts after the second accept marker would otherwise pass, or if either persistence write fails, the mutual-accept finalize fails closed with no trade mutation and leaves the shell cancellable until a later slice owns stronger rollback/audit policy.
