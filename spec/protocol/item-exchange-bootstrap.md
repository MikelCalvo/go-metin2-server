...
- keep richer trade-target eligibility beyond the owned exchange-distance gate, rollback/audit policy, and any finalize/result choreography beyond the first mutual-accept mutation + shell close deferred until a later exchange/trade slice owns those semantics
...
- richer trade target eligibility beyond the current visible-player check plus owned exchange-distance gate (`ApproxDistance < 1000`)
