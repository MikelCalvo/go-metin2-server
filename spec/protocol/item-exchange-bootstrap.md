...
- own the first two-party exchange-window shell (`START` / display-only `ITEM_ADD` / display-only `ITEM_DEL` / display-only `GOLD_ADD` / display-only `ACCEPT` / accept reset / `END`) without mutating inventory, equipment, quickslots, gold, ground items, or persisted account state, and close that display shell before successful carried-item use, carried/equipment item moves, drag-to-item stack consolidation, or carried-item drop refresh frames are exposed for an item already shown in it
