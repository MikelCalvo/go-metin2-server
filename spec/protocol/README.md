...
- `spawn-leash-bootstrap.md` — first pure runtime classification seam for authored spawn home vs current position, capped return-step planning/execution, read-only pending return-step inspection, the first pure chase-step planner helper, the first pending-frame chase-step executor, read-only pending chase-step inspection, and the frozen next chase `MOVE` packet choreography seam that still awaits its runtime consumer
