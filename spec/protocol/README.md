...
- `content-spawn-groups-bootstrap.md` — first authored `spawn_groups` content contract for loading stationary attackable non-player spawns through `combat_profile` + map placement, including the implemented proximity aggro-radius acquisition seam that can establish hit-independent engagement ownership
