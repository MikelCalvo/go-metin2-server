...
- `item-exchange-bootstrap.md` — first owned `EXCHANGE` client codec, shared server response codec, `GAME` dispatch seam, visible-peer `START` / `CANCEL` exchange-window shell, and template-authored `anti_give` / `give_reject_message` self-only `ITEM_ADD` guard while item/gold/accept finalization remains deferred
