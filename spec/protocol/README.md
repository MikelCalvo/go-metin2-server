...
- `spawn-leash-bootstrap.md` — first pure runtime classification seam for authored spawn home vs current position, capped return-step planning/execution, read-only pending return-step inspection, and the first pure chase-step planner helper before broader live mob chase/leash/return movement is implemented
