...
- `combat-damage-info-bootstrap.md` — first owned server `DAMAGE_INFO` codec contract plus standalone hit-effect fanout, owner self-only spawn-backed practice-mob hit-effect emission, owner-floor retaliation hit-effect after `DEAD` plus `TARGET(0, 0)`, the mob companion on an accepted owner-floor immediate hit, and the first killing-hit mob companion after `DEAD` plus selected-session `TARGET(0, 0)`
...
- `content-spawn-groups-bootstrap.md` — first authored `spawn_groups` content contract for loading stationary attackable non-player spawns through `combat_profile` + map placement, including the implemented proximity aggro-radius acquisition seam, the frozen multi-map / reconnect anti-leak matrix (still-dead EnterGame trailing `DEAD`, one-ref/one-actor, map-scoped import visibility, cross-map return-home membership, still-dead content-bundle replacement anti-resurrect), the owned daemon-restart still-dead timer persistence seam, the owned daemon-restart live damaged HP persistence seam, and the owned live damaged HP remapping seam across non-identical content-bundle replacement
- `spawn-leash-bootstrap.md` — first pure runtime classification seam for authored spawn home vs current position, capped return-step planning/execution, read-only pending return-step inspection, the first pure chase-step planner helper, the first pending-frame chase-step executor, read-only pending chase-step inspection, retained-viewer chase `MOVE` replication for successful same-map chase steps, retained-viewer return-step / return-home `MOVE` replication for successful same-map return recovery, retained-viewer same-map live spawn-backed operator/runtime position `MOVE` (presentation refreshes / respawn rebuild stay on delete/readd), and the frozen cross-map return client choreography that keeps delete/readd / direct-home rebuild (no invented cross-map `MOVE` / `GC WARP`)
...
- `item-exchange-bootstrap.md` — first owned `EXCHANGE` client codec, shared server response codec, `GAME` dispatch seam, visible-peer `START` / `CANCEL` exchange-window shell, display-only `GC::EXCHANGE ITEM_ADD` frames with template-authored sockets/attributes, display-only `GC::EXCHANGE ITEM_DEL` slot clears, display-only gold/accept markers with accept reset on later display changes, stale requester/accepted-partner display revalidation, second-accept receiver inventory-capacity/gold-overflow precondition guards, same-socket `/quit` / `/logout` / `/phase_select` plus successful carried-item-use / carried item-move / slash inventory-move / drag-to-item stack-consolidation / drop exchange teardown, and template-authored `anti_give` / `give_reject_message` self-only `ITEM_ADD` guard while finalization remains deferred
