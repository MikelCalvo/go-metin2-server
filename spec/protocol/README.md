...
- `combat-normal-attack-bootstrap.md` — first owned `ATTACK` wire contract, selected-dummy normal-attack gate, deterministic bootstrap HP refresh loop, safe `USE_SKILL`/`SHOOT` ingress guards, and clear-target companion on top of the visible `training_dummy` target slice
