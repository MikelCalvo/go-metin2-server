...
- `combat-pvp-duel-bootstrap.md` — first owned server `PVP` and `DUEL_START` packet shapes, currently codec/documentation only and deliberately not emitted by the bootstrap runtime yet
...
- `item-exchange-bootstrap.md` — first owned `EXCHANGE` client codec, shared server response codec, and `GAME` dispatch seam, intentionally fail-closed in the shipped runtime until two-party trade semantics are owned, with the current template-authored `anti_give` / `give_reject_message` self-only `ITEM_ADD` guard documented
