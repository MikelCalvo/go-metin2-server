...
- `item-exchange-bootstrap.md` — first owned `EXCHANGE` client codec and `GAME` dispatch seam, intentionally fail-closed in the shipped runtime until two-party trade semantics are owned, with the current template-authored `anti_give` / `give_reject_message` self-only `ITEM_ADD` guard documented
