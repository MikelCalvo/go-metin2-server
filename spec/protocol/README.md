...
- `player-death-bootstrap.md` — first owned retaliation-driven player-death edge for engaged owners, including self `DEAD` + clear, one visibility-gated peer `DEAD(owner_vid)` fanout, and the first post-floor `MOVE` / `SYNC_POSITION`, peer-facing `CHAT` / `WHISPER`, and self-only `CHAT_TYPE_INFO` denial seams
...
- `item-use-bootstrap.md` — first owned self-only consumable item-use bootstrap contract before the final legacy ingress exists
