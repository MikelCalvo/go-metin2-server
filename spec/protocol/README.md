...
- `combat-damage-info-bootstrap.md` — first owned server `DAMAGE_INFO` codec contract plus standalone hit-effect fanout, owner self-only spawn-backed practice-mob hit-effect emission, and the first killing-hit mob companion after `DEAD` plus selected-session `TARGET(0, 0)`
