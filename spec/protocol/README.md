...
- `spawn-leash-bootstrap.md` — first pure runtime classification seam for authored spawn home vs current position, capped return-step planning/execution, read-only pending return-step inspection, and the first planner-only chase-step contract before broader mob chase/leash/return movement is implemented
