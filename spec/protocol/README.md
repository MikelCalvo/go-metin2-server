...
- `character-position-change-speed-bootstrap.md` — first owned server `CHARACTER_POSITION` and `CHANGE_SPEED` packet shapes, currently codec/documentation only and deliberately not emitted by bootstrap movement/combat/death runtime yet
...
- `spawn-leash-bootstrap.md` — first pure runtime classification seam for authored spawn home vs current position before mob chase/leash/return movement is implemented
...
- `item-give-bootstrap.md` — first owned `ITEM_GIVE` client codec and `GAME` dispatch seam, intentionally fail-closed in the shipped runtime until player/NPC transfer semantics are owned
- `item-exchange-bootstrap.md` — first owned `EXCHANGE` client codec, shared server response codec, `GAME` dispatch seam, visible-peer `START` / `CANCEL` exchange-window shell, display-only `GC::EXCHANGE ITEM_ADD` frames with template-authored sockets/attributes, display-only `GC::EXCHANGE ITEM_DEL` slot clears, display-only gold/accept markers with accept reset on later display changes, same-socket `/quit` / `/logout` / `/phase_select` plus successful carried-item-use / drag-to-item stack-consolidation / drop exchange teardown, and template-authored `anti_give` / `give_reject_message` self-only `ITEM_ADD` guard while finalization remains deferred
...
- `item-storage-guard-bootstrap.md` — first owned safebox/mall client request codecs (`SAFEBOX_CHECKIN`, `SAFEBOX_CHECKOUT`, `SAFEBOX_ITEM_MOVE`, `MALL_CHECKOUT`) and `GAME` dispatch seam, intentionally fail-closed in the shipped runtime until storage semantics are owned
