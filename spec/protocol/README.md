...
- `item-exchange-bootstrap.md` — first owned `EXCHANGE` client codec, shared server response codec, `GAME` dispatch seam, visible-peer `START` / `CANCEL` exchange-window shell, display-only `GC::EXCHANGE ITEM_ADD` frames with template-authored sockets/attributes, display-only `GC::EXCHANGE ITEM_DEL` slot clears, display-only gold/accept markers with accept reset on later display changes, and template-authored `anti_give` / `give_reject_message` self-only `ITEM_ADD` guard while finalization remains deferred
