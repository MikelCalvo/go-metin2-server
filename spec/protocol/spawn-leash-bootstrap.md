...
- hit-armed / still-selected engagement is intentionally asymmetric: walking outside aggro while the owner still holds a selected combat target for that actor and remains inside combat-target range / leash / visibility must **not** release engagement or clear pending chase solely because aggro radius was left; chase / retaliation continue under the owned hit-engagement rules until an explicit release boundary such as `TARGET(0)`, death floor, disconnect/transfer, return recovery, operator update, combat-range loss clear, or AOI / visibility-loss clear
...
- focused shared-world coverage now also freezes the complementary combat-range-loss release after a within_radius chase displace: `TestGameRuntimeCombatRangeLossClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace` owns accepted-hit chase arm → due chase displace → walk outside combat-target range (`300`) while remaining inside visibility → one self-only `TARGET(0, 0)` clear → chase clear + within_radius homeward arm + retained-watcher due homeward `MOVE`
- focused shared-world coverage now also freezes the complementary AOI / visibility-loss release after a within_radius chase displace: `TestGameRuntimeVisibilityLossClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace` owns accepted-hit chase arm → due chase displace → walk outside visibility → self `CHARACTER_DEL` plus one self-only `TARGET(0, 0)` clear → chase clear + within_radius homeward arm + retained-watcher due homeward `MOVE`
- focused shared-world coverage now also freezes authored warp INTERACT after a within_radius chase displace: `TestGameRuntimeWarpInteractClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace` owns accepted-hit chase arm → due chase displace → warp INTERACT to another map → self `TARGET(0, 0)` in the rebootstrap response → chase clear + within_radius homeward arm + retained-watcher due homeward `MOVE`; the older at-home warp proof still owns TARGET clear plus aggro release without requiring a displace
- focused shared-world coverage now also freezes a live killing hit after a within_radius chase displace: `TestGameRuntimeKillingHitAfterChaseDisplaceKeepsStillDeadAtDeathCoordsAndRespawnsAtAuthoredHome` owns accepted-hit chase arm → due chase displace → remaining killing hits → `DEAD` + `TARGET(0)` + no chase/homeward arm or MOVE during the still-dead interval, late EnterGame add/info/update + trailing `GC DEAD` at the death coords, and due respawn rebuild at authored home requiring fresh reselect. This is the live-path twin of the synthetic timer cleanup in `TestGameRuntimeRespawnClearsStaleSpawnGroupChaseAndHomewardStepSchedules`; actor death does **not** arm homeward the way live engagement-release does
- focused daemon-restart coverage now also freezes that same chase-displaced still-dead corpse across `gamed` process restart: persist/restore keeps current death coords with `combat_current_hp=0` plus absolute `respawn_ready_at`, rematerialize stays still-dead / non-targetable at those coords with no homeward/return/chase arm, late EnterGame trailing `GC DEAD` uses the death coords, and due respawn rebuilds at authored home (`TestGameRuntimeKillingHitAfterChaseDisplaceStillDeadPersistsAcrossDaemonRestart`)
- focused still-dead `return_required` restore coverage now also freezes the complementary register-snapshot hole: `loadPersistedStaticActors` must re-read after the still-dead overlay so a rematerialized `return_required` corpse does not arm an internal return-step deadline, pending inspection stays omitted, and a due return-step flush does not MOVE the corpse from death coords (`TestGameRuntimeRestoreStillDeadReturnRequiredSpawnGroupDoesNotArmReturnStep`)
- dead actors waiting on respawn do not arm chase; a successful respawn rebuild starts unengaged at authored home and clears any pre-death return, chase, and homeward deadlines, so none can move the fresh actor after its old lifecycle boundary
...
- profile-authored per-mob chase / return / homeward step cap beyond the bootstrap `max_step = 100` default is owned as optional `combat_profiles.max_step` in `content-spawn-groups-bootstrap.md`; live chase / return / homeward planners / due-step execution / pending inspection consume `EffectiveStaticActorSpawnMaxStep` for the actor's combat profile (omit/zero keeps bootstrap `100`)
...
- profile-authored per-mob chase / return / homeward step cap beyond the bootstrap `max_step = 100` default is owned as optional `combat_profiles.max_step` in `content-spawn-groups-bootstrap.md`; live homeward planning / due-step execution consume `EffectiveStaticActorSpawnMaxStep` for the actor's combat profile (omit/zero keeps bootstrap `100`)
- arm from engagement-release paths that also clear chase (client `TARGET(0)`, proximity leave-radius walk-away, leave/logout/close, phase-select, transfer, authored warp INTERACT, EnterGame reclaim, owner death floor, and other owned engagement releases) after the actor is already displaced `within_radius`
...
- actor death after a within_radius chase displace is **not** an engagement-release that arms homeward: the killing hit clears chase and homeward, the corpse stays at the death coords through the still-dead interval, and only the due respawn rebuild snaps to authored home (`TestGameRuntimeKillingHitAfterChaseDisplaceKeepsStillDeadAtDeathCoordsAndRespawnsAtAuthoredHome`); daemon restart of that same corpse rematerializes at the death coords and still does not arm homeward/return (`TestGameRuntimeKillingHitAfterChaseDisplaceStillDeadPersistsAcrossDaemonRestart`); a still-dead `return_required` restore likewise must not arm return-step from the pre-still-dead register snapshot (`TestGameRuntimeRestoreStillDeadReturnRequiredSpawnGroupDoesNotArmReturnStep`)
...
- authored warp INTERACT now reuses that same transfer helper (`rebootstrap = true`) after a within_radius chase displace: the rebootstrap response carries one self-only `TARGET(0, 0)`, pending chase clears, and within_radius homeward re-arms (`TestGameRuntimeWarpInteractClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace`); the older at-home warp proof still owns TARGET clear plus aggro release without requiring a displace
- exact-position `MOVE` and `SYNC_POSITION` transfer triggers now have the matching client-ingress twins through that same helper: after a within_radius chase displace, each transfer rebootstrap carries one self-only `TARGET(0, 0)`, clears pending chase, arms within_radius homeward, and leaves a retained watcher to receive the due homeward `MOVE` (`TestGameRuntimeMoveTransferTriggerClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace` / `TestGameRuntimeSyncPositionTransferTriggerClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace`)
- EnterGame reclaim / Join now snapshots engagements owned by reclaimable stale subjects before `removeStaleOwnership` clears `engaged_by`, then re-syncs chase prune + within_radius homeward before encoding visibility (matching slash leave / transfer / warp); focused coverage: `TestGameRuntimeEnterGameReclaimClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace`
...
- hit-armed chase that already displaced a practice mob `within_radius` now has the matching focused twin for combat-range loss: walking outside combat-target range (`300`) while remaining inside visibility clears with one self-only `TARGET(0, 0)`, releases engagement, clears pending chase, and arms within_radius homeward (`TestGameRuntimeCombatRangeLossClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace`)
- hit-armed chase that already displaced a practice mob `within_radius` now has the matching focused twin for AOI / visibility loss: walking outside visibility queues `CHARACTER_DEL` plus one self-only `TARGET(0, 0)`, releases engagement, clears pending chase, and arms within_radius homeward (`TestGameRuntimeVisibilityLossClearsPendingSpawnGroupChaseAndArmsHomewardAfterChaseDisplace`)
- a live killing hit after that same within_radius chase displace now has the matching focused twin that does **not** arm homeward: the corpse stays at the death coords, chase/homeward stay cleared through the still-dead interval (including late EnterGame trailing `GC DEAD` at those coords), and due respawn rebuilds at authored home (`TestGameRuntimeKillingHitAfterChaseDisplaceKeepsStillDeadAtDeathCoordsAndRespawnsAtAuthoredHome`); daemon restart now rematerializes that same still-dead corpse at the persisted death coords without arming homeward/return/chase (`TestGameRuntimeKillingHitAfterChaseDisplaceStillDeadPersistsAcrossDaemonRestart`); still-dead `return_required` restore now also omits the internal return-step deadline so a due flush cannot MOVE the corpse (`TestGameRuntimeRestoreStillDeadReturnRequiredSpawnGroupDoesNotArmReturnStep`)
...
- re-engage / chase eligibility, return-step, successful respawn, remove, return-home, operator/runtime `UpdateStaticActor`, and content-bundle prune/restore paths clear or restore homeward deadlines alongside the chase/return schedules; focused respawn coverage now proves that synthetic stale chase and homeward deadlines are both retired with the older return-step deadline (`TestGameRuntimeRespawnClearsStaleSpawnGroupChaseAndHomewardStepSchedules`)
...
## Opt-in idle roam / wander step

`combat_profiles.roam_delay_ms` is optional. Omitted or zero keeps a live unengaged `at_home` spawn-backed actor stationary, so idle-flush proofs that expect no MOVE stay green. A positive authored delay (`250ms` through `60s`) arms one deterministic same-map wander step around authored home. The step reuses the already-owned `max_step` (bootstrap `100` when the profile omits it) and the leash: it moves `+x` by `max_step`, refuses a destination outside the leash, and emits the retained-viewer MOVE already used by chase without a chase `CHANGE_SPEED`. Flush order is due respawns, then return, then homeward, then this one roam step, then chase. Chase, homeward, and return still win over roam. After the step the actor is `within_radius` and homeward owns the walk back; roam does not chain. Pack AI assist, pathfinding around obstacles, and authored patrol routes stay deferred (WORLD-PATROL).
