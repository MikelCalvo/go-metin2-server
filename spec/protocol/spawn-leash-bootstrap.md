...
- profile-authored per-mob return-step arming delay beyond the bootstrap `1s` default is frozen separately as optional `combat_profiles.return_delay_ms` in `content-spawn-groups-bootstrap.md`; until that seam is GREEN, live return arming / re-arm keep using `bootstrapSpawnGroupReturnStepDelay`
