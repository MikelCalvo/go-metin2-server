...
- combined last-hit `gamed` process restart is not a town-return path: it rematerializes the still-dead dummy plus parked exclusive handle from FileStore, skips dummy/ground rematerialize while the owner is still at the floor, then `/restart_here` rematerializes self-only add/ownership for pickup (`TestGameSessionFlowPracticeMobKillingHitAlsoFloorsOwnerDaemonRestartRestartHereRematerializesKillRewardDrop`; `player-restart-here-bootstrap.md`)
