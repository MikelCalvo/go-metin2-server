...
const (
	BackupManifestFilename = "quest-state-backup-manifest.json"
	BackupManifestFormat   = "go-metin2-quest-state-backup-v1"
)

var (
	ErrBackupDirRequired      = errors.New("quest state backup dir is required")
	ErrBackupDirNotEmpty      = errors.New("quest state backup dir is not empty")
	ErrBackupDirInsideStore   = errors.New("quest state backup dir is inside quest state store")
	ErrRestoreSourceRequired  = errors.New("quest state restore source dir is required")
	ErrRestoreSourceNotFound  = errors.New("quest state restore source dir not found")
	ErrRestoreDirNotEmpty     = errors.New("quest state restore dir is not empty")
	ErrRestoreDirInsideSource = errors.New("quest state restore dir is inside quest state backup source")
	ErrBackupManifestRequired = errors.New("quest state backup manifest is required")
	ErrInvalidBackupManifest  = errors.New("invalid quest state backup manifest")
)
...
func (s *FileStore) BackupTo(dstDir string) error {
	...
	if err := ensureEmptyDir(dstDir, ErrBackupDirNotEmpty, "read quest state backup dir"); err != nil {
		return err
	}
	...
	if err := os.MkdirAll(dstDir, 0o755); err != nil {
		return fmt.Errorf("create quest state backup dir: %w", err)
	}
	...
	if hasSnapshot {
		...
		if err := backup.Save(snapshot); err != nil {
			return s.rollbackBackupFailure(dstDir, committedSnapshot, fmt.Errorf("backup quest state snapshot: %w", err))
		}
	}
	...
	if err := s.syncStoreDir(dstDir); err != nil {
		return s.rollbackBackupFailure(dstDir, committedSnapshot, fmt.Errorf("sync quest state backup dir: %w", err))
	}
	return nil
}
...
func (s *FileStore) rollbackBackupFailure(dstDir string, snapshotCommitted bool, backupErr error) error {
	...
	if snapshotCommitted {
		if err := os.Remove(filepath.Join(dstDir, filepath.Base(s.path))); err != nil && !errors.Is(err, os.ErrNotExist) {
			rollbackErrs = append(rollbackErrs, fmt.Errorf("remove backup quest state snapshot: %w", err))
		}
	}
	if err := os.Remove(filepath.Join(dstDir, BackupManifestFilename)); err != nil && !errors.Is(err, os.ErrNotExist) {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("remove quest state backup manifest: %w", err))
	}
	if err := s.syncStoreDir(dstDir); err != nil {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("sync quest state backup rollback dir: %w", err))
	}
...
func writeBackupManifest(dir string, snapshotFilename string, summary SnapshotSummary, hasSnapshot bool) error {
	...
	if hasSnapshot {
		...
		if err != nil {
			return fmt.Errorf("read quest state backup snapshot for manifest: %w", err)
		}
	...
	return writeJSONFileAtomically(dir, BackupManifestFilename, manifest, "quest state backup manifest")
}
...
func (s *FileStore) RestoreFrom(srcDir string) error {
	...
	if err := ensureEmptyDir(storeDir, ErrRestoreDirNotEmpty, "read quest state restore dir"); err != nil {
		return err
	}
	...
	if err := os.MkdirAll(storeDir, 0o755); err != nil {
		return fmt.Errorf("create quest state restore dir: %w", err)
	}

	committedSnapshot := false
	if hasSnapshot {
		if err := s.Save(snapshot); err != nil {
			return s.rollbackRestoreFailure(true, fmt.Errorf("restore quest state snapshot: %w", err))
		}
		committedSnapshot = true
	}
	...
	if err := s.syncStoreDir(storeDir); err != nil {
		return s.rollbackRestoreFailure(committedSnapshot, fmt.Errorf("sync quest state restore dir: %w", err))
	}
	return nil
}
...
func (s *FileStore) rollbackRestoreFailure(snapshotCommitted bool, restoreErr error) error {
	...
	if snapshotCommitted {
		if err := os.Remove(s.path); err != nil && !errors.Is(err, os.ErrNotExist) {
			rollbackErrs = append(rollbackErrs, fmt.Errorf("remove restored quest state snapshot: %w", err))
		}
	}
	if err := os.Remove(filepath.Join(storeDir, BackupManifestFilename)); err != nil && !errors.Is(err, os.ErrNotExist) {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("remove restored quest state backup manifest: %w", err))
	}
	if err := s.syncStoreDir(storeDir); err != nil {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("sync quest state restore rollback dir: %w", err))
	}
	if len(rollbackErrs) == 0 {
		return restoreErr
	}
...
func (s *FileStore) loadBackupSnapshotForRestore(srcDir string) (SnapshotSummary, Snapshot, bool, error) {
	...
	if _, err := os.Stat(srcDir); err != nil {
		...
		return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("stat quest state restore source dir: %w", err)
	}
...
func (s *FileStore) validateActiveBackupManifest() error {
	...
	if _, err := os.Lstat(manifestPath); err != nil {
		...
		return fmt.Errorf("stat active quest state backup manifest: %w", err)
	}
	...
	if _, err := os.Stat(s.path); err == nil && !hasSnapshot {
		return fmt.Errorf("%w: active manifest omits committed quest state snapshot", ErrInvalidBackupManifest)
	} else if err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("stat active quest state snapshot for backup manifest: %w", err)
	}
	return nil
}
...
func (s *FileStore) validateBackupManifestWithCoverage(srcDir string, requireClosedDirectory bool) (SnapshotSummary, Snapshot, bool, error) {
	...
	if err := rejectBackupEntrySymlink(manifestPath, "quest state backup manifest"); err != nil {
		return SnapshotSummary{}, Snapshot{}, false, err
	}
	...
	if err != nil {
		...
		return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("read quest state backup manifest: %w", err)
	}
	...
	if len(manifest.Files) == 0 {
		...
		if file.Filename != filepath.Base(s.path) {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: manifest filename %q does not match quest state snapshot filename", ErrInvalidBackupManifest, file.Filename)
		}
		committedFiles[file.Filename] = struct{}{}
		snapshotPath := filepath.Join(srcDir, file.Filename)
		if err := rejectBackupEntrySymlink(snapshotPath, fmt.Sprintf("quest state backup snapshot %q", file.Filename)); err != nil {
			return SnapshotSummary{}, Snapshot{}, false, err
		}
		rawSnapshot, err := os.ReadFile(snapshotPath)
		if err != nil {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: read manifest quest state snapshot: %v", ErrInvalidBackupManifest, err)
		}
		if int64(len(rawSnapshot)) != file.SizeBytes {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: quest state snapshot size mismatch", ErrInvalidBackupManifest)
		}
		checksum := sha256.Sum256(rawSnapshot)
		if got := hex.EncodeToString(checksum[:]); got != file.SHA256 {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: quest state snapshot checksum mismatch", ErrInvalidBackupManifest)
		}
...
func validateBackupDirectoryEntries(srcDir string, manifestFiles map[string]struct{}) error {
	...
	if err != nil {
		return fmt.Errorf("read quest state backup dir for manifest coverage: %w", err)
	}
...
func summaryWithoutCrashTemps(summary SnapshotSummary) SnapshotSummary {
	...
	if summary.Characters == nil {
		summary.Characters = []string{}
	}
	if summary.QuestRefs == nil {
		summary.QuestRefs = []string{}
	}
	if summary.FlagKeys == nil {
		summary.FlagKeys = []string{}
	}
	return summary
}
...
func snapshotSummariesEqual(a, b SnapshotSummary) bool {
	...
	if a.FlagCount != b.FlagCount || len(a.Characters) != len(b.Characters) || len(a.QuestRefs) != len(b.QuestRefs) || len(a.FlagKeys) != len(b.FlagKeys) {
		return false
	}
	...
	for i := range a.QuestRefs {
		if a.QuestRefs[i] != b.QuestRefs[i] {
			return false
		}
	}
	for i := range a.FlagKeys {
		if a.FlagKeys[i] != b.FlagKeys[i] {
			return false
		}
	}
	return true
...
func writeJSONFileAtomically(dir, filename string, value any, context string) error {
	temp, err := os.CreateTemp(dir, ".quest-state-*.json")
	if err != nil {
...
func rejectBackupDestinationInsideStore(storeDir string, dstDir string) error {
	return rejectPathInsideOrEqual(storeDir, dstDir, ErrBackupDirInsideStore, "quest state store", "quest state backup")
}

func rejectRestoreDestinationInsideSource(srcDir string, storeDir string) error {
	return rejectPathInsideOrEqual(srcDir, storeDir, ErrRestoreDirInsideSource, "quest state restore source", "quest state restore")
}
