...
// homeward-delay (0018) / max-step (0019) / reaction-delay (0020) columns that
...
// (static_actor_combat_profile_reaction_delay) because combat-profile inserts bind
// chase_delay_ms, return_delay_ms, homeward_delay_ms, max_step, and
...
func requireStaticActorContentStateSchema(ctx context.Context, querier dbmigrations.SQLLedgerQuerier) error {
	...
	for _, entry := range ledger {
		...
		if entry.Version == StaticActorCombatProfileMaxStepMigrationVersion && entry.Name == StaticActorCombatProfileMaxStepMigrationName {
			hasMaxStep = true
		}
		if entry.Version == StaticActorCombatProfileReactionDelayMigrationVersion && entry.Name == StaticActorCombatProfileReactionDelayMigrationName {
			hasReactionDelay = true
		}
	}
	if hasContentState && hasChaseDelay && hasReturnDelay && hasHomewardDelay && hasMaxStep && hasReactionDelay {
		return nil
	}
	if !hasContentState {
	...
	if !hasMaxStep {
		return fmt.Errorf("%w: ledger tip %d missing version %d %q", ErrStaticActorContentStateImportSchemaRequired, latest, StaticActorCombatProfileMaxStepMigrationVersion, StaticActorCombatProfileMaxStepMigrationName)
	}
	return fmt.Errorf("%w: ledger tip %d missing version %d %q", ErrStaticActorContentStateImportSchemaRequired, latest, StaticActorCombatProfileReactionDelayMigrationVersion, StaticActorCombatProfileReactionDelayMigrationName)
}
