...
const (
	BackupManifestFilename = "interaction-backup-manifest.json"
	BackupManifestFormat   = "go-metin2-interaction-backup-v1"
)

var (
	ErrBackupDirRequired      = errors.New("interaction backup dir is required")
	ErrBackupDirNotEmpty      = errors.New("interaction backup dir is not empty")
	ErrBackupDirInsideStore   = errors.New("interaction backup dir is inside interaction store")
	ErrRestoreSourceRequired  = errors.New("interaction restore source dir is required")
	ErrRestoreSourceNotFound  = errors.New("interaction restore source dir not found")
	ErrRestoreDirNotEmpty     = errors.New("interaction restore dir is not empty")
	ErrRestoreDirInsideSource = errors.New("interaction restore dir is inside interaction backup source")
	ErrBackupManifestRequired = errors.New("interaction backup manifest is required")
	ErrInvalidBackupManifest  = errors.New("invalid interaction backup manifest")
)
...
func (s *FileStore) BackupTo(dstDir string) error {
	...
	if err := ensureEmptyDir(dstDir, ErrBackupDirNotEmpty, "read interaction backup dir"); err != nil {
		return err
	}
	...
	if err := os.MkdirAll(dstDir, 0o755); err != nil {
		return fmt.Errorf("create interaction backup dir: %w", err)
	}
	...
	if hasSnapshot {
		...
		if err := backup.Save(snapshot); err != nil {
			return s.rollbackBackupFailure(dstDir, committedSnapshot, fmt.Errorf("backup interaction snapshot: %w", err))
		}
	}
	...
	if err := s.syncStoreDir(dstDir); err != nil {
		return s.rollbackBackupFailure(dstDir, committedSnapshot, fmt.Errorf("sync interaction backup dir: %w", err))
	}
	return nil
}
...
func (s *FileStore) rollbackBackupFailure(dstDir string, snapshotCommitted bool, backupErr error) error {
	...
	if snapshotCommitted {
		if err := os.Remove(filepath.Join(dstDir, filepath.Base(s.path))); err != nil && !errors.Is(err, os.ErrNotExist) {
			rollbackErrs = append(rollbackErrs, fmt.Errorf("remove backup interaction snapshot: %w", err))
		}
	}
	if err := os.Remove(filepath.Join(dstDir, BackupManifestFilename)); err != nil && !errors.Is(err, os.ErrNotExist) {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("remove interaction backup manifest: %w", err))
	}
	if err := s.syncStoreDir(dstDir); err != nil {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("sync interaction backup rollback dir: %w", err))
	}
...
func writeBackupManifest(dir string, snapshotFilename string, summary SnapshotSummary, hasSnapshot bool) error {
	...
	if hasSnapshot {
		...
		if err != nil {
			return fmt.Errorf("read interaction backup snapshot for manifest: %w", err)
		}
	...
	return writeJSONFileAtomically(dir, BackupManifestFilename, manifest, "interaction backup manifest")
}
...
func (s *FileStore) RestoreFrom(srcDir string) error {
	...
	if err := ensureEmptyDir(storeDir, ErrRestoreDirNotEmpty, "read interaction restore dir"); err != nil {
		return err
	}
	...
	if err := os.MkdirAll(storeDir, 0o755); err != nil {
		return fmt.Errorf("create interaction restore dir: %w", err)
	}

	committedSnapshot := false
	if hasSnapshot {
		if err := s.Save(snapshot); err != nil {
			return s.rollbackRestoreFailure(true, fmt.Errorf("restore interaction snapshot: %w", err))
		}
		committedSnapshot = true
	}
	...
	if err := s.syncStoreDir(storeDir); err != nil {
		return s.rollbackRestoreFailure(committedSnapshot, fmt.Errorf("sync interaction restore dir: %w", err))
	}
	return nil
}
...
func (s *FileStore) rollbackRestoreFailure(snapshotCommitted bool, restoreErr error) error {
	...
	if snapshotCommitted {
		if err := os.Remove(s.path); err != nil && !errors.Is(err, os.ErrNotExist) {
			rollbackErrs = append(rollbackErrs, fmt.Errorf("remove restored interaction snapshot: %w", err))
		}
	}
	if err := os.Remove(filepath.Join(storeDir, BackupManifestFilename)); err != nil && !errors.Is(err, os.ErrNotExist) {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("remove restored interaction backup manifest: %w", err))
	}
	if err := s.syncStoreDir(storeDir); err != nil {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("sync interaction restore rollback dir: %w", err))
	}
	if len(rollbackErrs) == 0 {
		return restoreErr
	}
...
func (s *FileStore) loadBackupSnapshotForRestore(srcDir string) (SnapshotSummary, Snapshot, bool, error) {
	...
	if _, err := os.Stat(srcDir); err != nil {
		...
		return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("stat interaction restore source dir: %w", err)
	}
...
func (s *FileStore) validateActiveBackupManifest() error {
	...
	if _, err := os.Lstat(manifestPath); err != nil {
		...
		return fmt.Errorf("stat active interaction backup manifest: %w", err)
	}
	...
	if _, err := os.Stat(s.path); err == nil && !hasSnapshot {
		return fmt.Errorf("%w: active manifest omits committed interaction snapshot", ErrInvalidBackupManifest)
	} else if err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("stat active interaction snapshot for backup manifest: %w", err)
	}
	return nil
}
...
func (s *FileStore) validateBackupManifestWithCoverage(srcDir string, requireClosedDirectory bool) (SnapshotSummary, Snapshot, bool, error) {
	...
	if err := rejectBackupEntrySymlink(manifestPath, "interaction backup manifest"); err != nil {
		return SnapshotSummary{}, Snapshot{}, false, err
	}
	...
	if err != nil {
		...
		return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("read interaction backup manifest: %w", err)
	}
	...
	if len(manifest.Files) == 0 {
		...
		if file.Filename != filepath.Base(s.path) {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: manifest filename %q does not match interaction snapshot filename", ErrInvalidBackupManifest, file.Filename)
		}
		committedFiles[file.Filename] = struct{}{}
		snapshotPath := filepath.Join(srcDir, file.Filename)
		if err := rejectBackupEntrySymlink(snapshotPath, fmt.Sprintf("interaction backup snapshot %q", file.Filename)); err != nil {
			return SnapshotSummary{}, Snapshot{}, false, err
		}
		rawSnapshot, err := os.ReadFile(snapshotPath)
		if err != nil {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: read manifest interaction snapshot: %v", ErrInvalidBackupManifest, err)
		}
		if int64(len(rawSnapshot)) != file.SizeBytes {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: interaction snapshot size mismatch", ErrInvalidBackupManifest)
		}
		checksum := sha256.Sum256(rawSnapshot)
		if got := hex.EncodeToString(checksum[:]); got != file.SHA256 {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: interaction snapshot checksum mismatch", ErrInvalidBackupManifest)
		}
...
func validateBackupDirectoryEntries(srcDir string, manifestFiles map[string]struct{}) error {
	...
	if err != nil {
		return fmt.Errorf("read interaction backup dir for manifest coverage: %w", err)
	}
...
func summaryWithoutCrashTemps(summary SnapshotSummary) SnapshotSummary {
	...
	if summary.DefinitionKeys == nil {
		summary.DefinitionKeys = []string{}
	}
	return summary
}
...
func snapshotSummariesEqual(a, b SnapshotSummary) bool {
	...
	if a.DefinitionCount != b.DefinitionCount || len(a.DefinitionKeys) != len(b.DefinitionKeys) {
		return false
	}
	for i := range a.DefinitionKeys {
		if a.DefinitionKeys[i] != b.DefinitionKeys[i] {
			return false
		}
	}
	return true
...
func writeJSONFileAtomically(dir, filename string, value any, context string) error {
	temp, err := os.CreateTemp(dir, ".interaction-definitions-*.json")
	if err != nil {
...
func rejectBackupDestinationInsideStore(storeDir string, dstDir string) error {
	return rejectPathInsideOrEqual(storeDir, dstDir, ErrBackupDirInsideStore, "interaction store", "interaction backup")
}

func rejectRestoreDestinationInsideSource(srcDir string, storeDir string) error {
	return rejectPathInsideOrEqual(srcDir, storeDir, ErrRestoreDirInsideSource, "interaction restore source", "interaction restore")
}
