...
func TestFileStoreIssueRejectsDuplicateInventorySlots(t *testing.T) {
	...
	if err := store.Issue(ticket); !errors.Is(err, ErrInvalidTicket) {
		t.Fatalf("expected ErrInvalidTicket for duplicate inventory slot, got %v", err)
	}
}

func TestFileStoreLoadRejectsDuplicateInventorySlots(t *testing.T) {
	store := NewFileStore(t.TempDir())
	raw := []byte(`{"login":"mkmk","login_key":16909060,"issued_at":"2026-04-17T10:21:00Z","characters":[{"id":1,"name":"MkmkWar","inventory":[{"id":1001,"vnum":27001,"count":3,"slot":8},{"id":1002,"vnum":27002,"count":1,"slot":8}],"equipment":[],"quickslots":[]}]}`)
	if err := os.WriteFile(store.ticketPath(0x01020304), raw, 0o644); err != nil {
		t.Fatalf("write duplicate-inventory-slot ticket snapshot: %v", err)
	}

	_, err := store.Load("mkmk", 0x01020304)
	if !errors.Is(err, ErrInvalidTicket) {
		t.Fatalf("expected ErrInvalidTicket for loaded duplicate inventory slot, got %v", err)
	}
}
...
func TestFileStoreIssueRejectsDuplicateNonItemQuickslotTuples(t *testing.T) {
	cases := []struct {
		name       string
		quickslots []Quickslot
	}{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ticket := Ticket{
				...
				Characters: []Character{{
					ID:         1,
					Name:       "MkmkWar",
					Quickslots: tc.quickslots,
				}},
			}

			if err := store.Issue(ticket); !errors.Is(err, ErrInvalidTicket) {
				t.Fatalf("expected ErrInvalidTicket for duplicate quickslot tuple, got %v", err)
			}
		})
	}
}

func TestFileStoreIssueAllowsDuplicateItemQuickslotTuples(t *testing.T) {
	store := NewFileStore(t.TempDir())
	ticket := Ticket{
		Login:    "mkmk",
		LoginKey: 0x01020304,
	...
	if _, err := store.Load("mkmk", 0x01020304); err != nil {
		t.Fatalf("expected issued duplicate item quickslot tuples to stay loadable, got %v", err)
	}
}

func TestFileStoreIssueAllowsSameQuickslotSlotForDifferentTypes(t *testing.T) {
	store := NewFileStore(t.TempDir())
	ticket := Ticket{
		Login:    "mkmk",
		LoginKey: 0x01020304,
...
func TestFileStoreLoadRejectsDuplicateNonItemQuickslotTuples(t *testing.T) {
	store := NewFileStore(t.TempDir())
	raw := []byte(`{"login":"mkmk","login_key":16909060,"issued_at":"2026-04-17T10:21:00Z","characters":[{"id":1,"name":"MkmkWar","inventory":[],"equipment":[],"quickslots":[{"position":2,"type":2,"slot":8},{"position":3,"type":2,"slot":8}]}]}`)
	if err := os.WriteFile(store.ticketPath(0x01020304), raw, 0o644); err != nil {
		t.Fatalf("write duplicate-quickslot-tuple ticket snapshot: %v", err)
	}
...
func TestFileStoreAllowsMultipleEmptyCharacterSlots(t *testing.T) {
	...
	got, err := store.Load("mkmk", 0x12345678)
	if err != nil {
		t.Fatalf("load ticket with empty character slots: %v", err)
	}
...
func TestFileStoreRejectsZeroIDCharacterSlotsWithNonEmptyState(t *testing.T) {
	cases := []struct {
		...
		{name: "name", character: Character{Name: "Ghost"}},
		{name: "vid", character: Character{VID: 0x01020304}},
		{name: "map", character: Character{MapIndex: 1}},
		{name: "inventory", character: Character{Inventory: []inventory.ItemInstance{{ID: 1001, Vnum: 27001, Count: 1, Slot: 8}}}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			store := NewFileStore(t.TempDir())
			ticket := Ticket{Login: "mkmk", LoginKey: 0x12345678, Characters: []Character{tc.character}}
			if err := store.Issue(ticket); !errors.Is(err, ErrInvalidTicket) {
				t.Fatalf("expected ErrInvalidTicket for zero-id slot with %s state, got %v", tc.name, err)
			}
		})
	}
}
