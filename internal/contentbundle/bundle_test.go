...
func TestCanonicalizeRejectsQuestFlagRewardItemsWithoutBundledItemTemplates(t *testing.T) {
	_, err := Canonicalize(Bundle{
		InteractionDefinitions: []interactionstore.Definition{{
			Kind:      interactionstore.KindQuestFlag,
			Ref:       "quest:first_steps_kill_turnin_without_templates",
			Text:      "Quest updated: first_steps.killed_qa_mob = 0.",
			QuestRef:  "quest:first_steps",
			QuestFlag: "killed_qa_mob",
			QuestFrom: 1,
	...
	if !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for quest-flag reward items without bundled item templates, got %v", err)
	}
}
...
func TestCanonicalizeRejectsQuestFlagConsumeItemsWithoutBundledItemTemplates(t *testing.T) {
	_, err := Canonicalize(Bundle{
		InteractionDefinitions: []interactionstore.Definition{{
			Kind:      interactionstore.KindQuestFlag,
			Ref:       "quest:first_steps_kill_turnin_consume_without_templates",
			Text:      "Quest updated: first_steps.killed_qa_mob = 0.",
			QuestRef:  "quest:first_steps",
			QuestFlag: "killed_qa_mob",
			QuestFrom: 1,
	...
	if !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for quest-flag consume items without bundled item templates, got %v", err)
	}
}
...
func TestCanonicalizeRejectsQuestFlagRewardItemCountAboveNonStackableLimit(t *testing.T) {
	_, err := Canonicalize(Bundle{
		ItemTemplates: []itemcatalog.Template{{Vnum: 11200, Name: "Wooden Sword", Stackable: false, MaxCount: 1}},
		InteractionDefinitions: []interactionstore.Definition{{
			Kind:      interactionstore.KindQuestFlag,
	...
	if !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for quest-flag reward item count above non-stackable limit, got %v", err)
	}
}
...
func TestCanonicalizeRejectsCheckedInUnsupportedInteractionKindExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid unsupported interaction kind example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in unsupported interaction kind example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInDuplicateStaticActorExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid duplicate static actor example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in duplicate static actor example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInRewardDropWithoutItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid reward drop without item templates example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in reward drop without item templates example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInMerchantCatalogWithoutItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid merchant catalog without item templates example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in merchant catalog without item templates example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInMerchantCatalogItemMissingFromItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid merchant catalog item missing from item templates example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in merchant catalog item missing from item templates example, got %v", err)
	}
}
...
func TestCanonicalizeRejectsCheckedInMerchantCatalogCountAboveNonStackableLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid merchant catalog count above non-stackable limit example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in merchant catalog count above non-stackable limit example, got %v", err)
	}
}
...
func TestCanonicalizeRejectsCheckedInRewardDropItemMissingFromItemTemplatesExample(t *testing.T) {
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid reward drop item missing from item templates example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
...
func TestCanonicalizeRejectsCheckedInQuestFlagRewardItemWithoutItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid quest-flag reward item without item templates example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in quest-flag reward item without item templates example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInQuestFlagRewardItemMissingFromItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid quest-flag reward item missing from item templates example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in quest-flag reward item missing from item templates example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInQuestFlagConsumeItemWithoutItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid quest-flag consume item without item templates example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in quest-flag consume item without item templates example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInQuestFlagConsumeItemMissingFromItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid quest-flag consume item missing from item templates example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in quest-flag consume item missing from item templates example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInQuestFlagRewardItemCountAboveStackLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid quest-flag reward item count above stack limit example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in quest-flag reward item count above stack limit example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInQuestFlagRewardItemCountAboveNonStackableLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid quest-flag reward item count above non-stackable limit example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in quest-flag reward item count above non-stackable limit example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInQuestFlagConsumeItemCountAboveStackLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid quest-flag consume item count above stack limit example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
...
func TestCanonicalizeRejectsCheckedInOpenCubeForeignSizeExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign size example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign size example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignTitleExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign title example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign title example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignTitleExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign title example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign title example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignCatalogExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign catalog example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign catalog example, got %v", err)
	}
}
...
func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignWarpCoordsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign warp coords example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign warp coords example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignWarpCoordsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign warp coords example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign warp coords example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxOversizeSizeExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox oversize size example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox oversize size example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignRewardGoldExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign reward gold example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign reward gold example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignRewardGoldExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign reward gold example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign reward gold example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignRewardExperienceExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign reward experience example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign reward experience example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignRewardExperienceExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign reward experience example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign reward experience example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignConsumeGoldExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign consume gold example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign consume gold example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignConsumeGoldExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign consume gold example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign consume gold example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignConsumeExperienceExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign consume experience example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign consume experience example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignConsumeExperienceExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign consume experience example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign consume experience example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignRewardItemsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign reward items example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign reward items example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignRewardItemsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign reward items example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign reward items example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignConsumeItemsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign consume items example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign consume items example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignConsumeItemsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign consume items example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign consume items example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignQuestToExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign quest_to example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign quest_to example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignQuestToExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign quest_to example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign quest_to example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenSafeboxForeignRewardItemVnumExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_safebox foreign reward_item_vnum example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_safebox foreign reward_item_vnum example, got %v", err)
	}
}

func TestCanonicalizeRejectsCheckedInOpenCubeForeignRewardItemVnumExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate contentbundle test file")
	}
	...
	var bundle Bundle
	if err := json.Unmarshal(raw, &bundle); err != nil {
		t.Fatalf("decode invalid open_cube foreign reward_item_vnum example bundle: %v", err)
	}
	if _, err := Canonicalize(bundle); !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for checked-in open_cube foreign reward_item_vnum example, got %v", err)
	}
}
...
func TestCanonicalizePveVerticalAuthoringExampleExpandsQuestLoop(t *testing.T) {
	...
	wantTemplates := []itemcatalog.Template{
		{Vnum: 11200, Name: "Wooden Sword", Stackable: false, MaxCount: 1, ShopSellPrice: 100, AppearanceVnum: 11201, EquipSlot: "weapon", EquipEffect: &itemcatalog.PointEffect{PointType: 1, PointIndex: 1, PointDelta: 10}},
		{Vnum: 27001, Name: "Small Red Potion", Stackable: true, MaxCount: 200, ShopBuyPrice: 5, ShopSellPrice: 2, UseEffect: &itemcatalog.UseEffect{PointType: 1, PointIndex: 1, PointDelta: 50, Message: "consume:27001:+50", SpecialEffectType: 1}},
	...
	wantShopCatalog := ShopCatalogSummary{
		...
		Entries: []ShopCatalogEntrySummary{
			...
			{Slot: 1, ItemVnum: 11200, ItemName: "Wooden Sword", Count: 1, Price: 500, Stackable: false, MaxCount: 1, ShopSellPrice: 100, AppearanceVnum: 11201, EquipSlot: "weapon", EquipEffect: &itemcatalog.PointEffect{PointType: 1, PointIndex: 1, PointDelta: 10}},
...
func TestCanonicalizeRoundTripsAuthoredCombatProfileReturnDelay(t *testing.T) {
	const profile = "practice_authored_return_delay_wolf"

	canonical, err := Canonicalize(Bundle{
		SpawnGroups: []SpawnGroup{{
			Ref:           "practice.authored_return_delay_wolf",
			Name:          "Authored Return Delay Wolf",
			MapIndex:      42,
			X:             1775,
			Y:             2875,
			RaceNum:       101,
	...
	if err != nil {
		t.Fatalf("canonicalize authored return-delay combat profile: %v", err)
	}
	if len(canonical.CombatProfiles) != 1 || canonical.CombatProfiles[0].ReturnDelayMs != 2000 {
		t.Fatalf("expected canonical combat profile to preserve return_delay_ms 2000, got %#v", canonical.CombatProfiles)
	}
}

func TestCanonicalizeRejectsCombatProfileReturnDelayBelowBootstrapFloor(t *testing.T) {
	_, err := Canonicalize(Bundle{
		SpawnGroups: []SpawnGroup{{
			Ref:           "practice.too_fast_return_delay_wolf",
			Name:          "Too Fast Return Delay Wolf",
			MapIndex:      42,
			X:             1800,
			Y:             2900,
			RaceNum:       101,
			CombatProfile: "practice_too_fast_return_delay_wolf",
		}},
		CombatProfiles: []worldruntime.StaticActorCombatProfileSnapshot{{
			Profile:        "practice_too_fast_return_delay_wolf",
			MaxHP:          24,
			AttackValue:    8,
			DefenseValue:   2,
			RespawnDelayMs: 1500,
	...
	if !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for combat-profile return delay < 250ms, got %v", err)
	}
}

func TestCanonicalizeRejectsCombatProfileReturnDelayAboveBootstrapCap(t *testing.T) {
	_, err := Canonicalize(Bundle{
		SpawnGroups: []SpawnGroup{{
			Ref:           "practice.too_slow_return_delay_wolf",
			Name:          "Too Slow Return Delay Wolf",
			MapIndex:      42,
			X:             1800,
			Y:             2900,
			RaceNum:       101,
			CombatProfile: "practice_too_slow_return_delay_wolf",
		}},
		CombatProfiles: []worldruntime.StaticActorCombatProfileSnapshot{{
			Profile:        "practice_too_slow_return_delay_wolf",
			MaxHP:          24,
			AttackValue:    8,
			DefenseValue:   2,
			RespawnDelayMs: 1500,
	...
	if !errors.Is(err, ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for combat-profile return delay above 60s, got %v", err)
	}
}
