...
func TestRuntimeUseItemRejectsMissingUseEffectWithoutMutation(t *testing.T) {
	persisted := loginticket.Character{
		ID:        0x01030102,
		VID:       0x02040102,
		Name:      "PeerTwo",
	...
	template := itemcatalog.Template{Vnum: 27001, Name: "Small Red Potion", Stackable: true, MaxCount: 200, ShopBuyPrice: 5, ShopSellPrice: 2}

	if result, ok := runtime.UseItem(5, template); ok {
		t.Fatalf("expected missing use_effect item use to fail closed, got %+v", result)
	}
	if got := runtime.LiveCharacter(); !reflect.DeepEqual(got, before) {
		t.Fatalf("missing use_effect item use mutated live character: got %#v want %#v", got, before)
	}
	if got := runtime.PersistedSnapshot(); !reflect.DeepEqual(got.Inventory, persisted.Inventory) || got.Points[1] != persisted.Points[1] {
		t.Fatalf("missing use_effect item use mutated persisted state: inventory=%#v points[1]=%d", got.Inventory, got.Points[1])
	}
}
...
func TestRuntimeUseItemOnItemRejectsMalformedTargetWithoutMutation(t *testing.T) {
	cases := []struct {
		name       string
		targetItem inventory.ItemInstance
	}{
		{name: "zero count", targetItem: inventory.ItemInstance{ID: 12, Vnum: 27001, Count: 0, Slot: 8}},
		{name: "zero id", targetItem: inventory.ItemInstance{ID: 0, Vnum: 27001, Count: 3, Slot: 8}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			persisted := loginticket.Character{
			...
			runtime := NewRuntime(persisted, SessionLink{Login: "malformed-target-peer", CharacterIndex: 1})
			before := runtime.LiveCharacter()
			template := itemcatalog.Template{Vnum: 27001, Name: "Small Red Potion", Stackable: true, MaxCount: 200}

			if result, ok := runtime.UseItemOnItem(5, 8, template); ok {
				t.Fatalf("expected %s target use-to-item to fail closed, got %+v", tc.name, result)
			}
			if got := runtime.LiveCharacter(); !reflect.DeepEqual(got, before) {
				t.Fatalf("%s target use-to-item mutated live character: got %#v want %#v", tc.name, got, before)
			}
			if got := runtime.PersistedSnapshot(); !reflect.DeepEqual(got.Inventory, persisted.Inventory) || !reflect.DeepEqual(got.Quickslots, persisted.Quickslots) {
				t.Fatalf("%s target use-to-item mutated persisted state: inventory=%#v quickslots=%#v", tc.name, got.Inventory, got.Quickslots)
			}
		})
	}
}
...
func TestRuntimeUseItemOnItemRejectsTemplateTransferGuardsWithoutMutation(t *testing.T) {
	cases := []struct {
		name     string
		template itemcatalog.Template
	}{
		...
		},
		{
			...
			template: itemcatalog.Template{Vnum: 27001, Name: "Anti Give", Stackable: true, MaxCount: 200, AntiGive: true},
		},
		{
			name:     "anti-sell template",
			template: itemcatalog.Template{Vnum: 27001, Name: "Anti Sell", Stackable: true, MaxCount: 200, AntiSell: true},
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, ok := runtime.UseItemOnItem(5, 6, tc.template); ok {
				t.Fatalf("expected use-to-item to reject %s even when stacks otherwise match", tc.name)
			}
			if got := runtime.LiveCharacter(); !reflect.DeepEqual(got, before) {
				t.Fatalf("%s use-to-item mutated live character: got %#v want %#v", tc.name, got, before)
			}
			if !reflect.DeepEqual(runtime.PersistedSnapshot().Inventory, persisted.Inventory) {
				t.Fatalf("%s use-to-item mutated persisted inventory: got %#v", tc.name, runtime.PersistedSnapshot().Inventory)
			}
		})
	}
}
...
func TestRuntimeUseItemOnItemRejectsEquippedLockedSourceOrTargetWithoutMutation(t *testing.T) {
	cases := []struct {
		...
		{
			name: "equipped source snapshot",
			inventory: []inventory.ItemInstance{
				{ID: 11, Vnum: 27001, Count: 3, Slot: 5, Equipped: true, EquipSlot: inventory.EquipmentSlotBody},
				{ID: 12, Vnum: 27001, Count: 4, Slot: 6},
			},
		},
		{
			name: "equipped target snapshot",
			inventory: []inventory.ItemInstance{
				{ID: 11, Vnum: 27001, Count: 3, Slot: 5},
...
func TestRuntimeUseItemOnItemRejectsMalformedSourceOrTargetWithoutMutation(t *testing.T) {
	cases := []struct {
		name      string
		inventory []inventory.ItemInstance
	}{
		{
			name: "malformed source item",
			inventory: []inventory.ItemInstance{
				{ID: 0, Vnum: 27001, Count: 3, Slot: 5},
				{ID: 12, Vnum: 27001, Count: 4, Slot: 6},
			},
		},
		{
			name: "malformed target item",
			inventory: []inventory.ItemInstance{
				{ID: 11, Vnum: 27001, Count: 3, Slot: 5},
				{ID: 0, Vnum: 27001, Count: 4, Slot: 6},
			},
		},
	}
	for _, tc := range cases {
...
func TestRuntimeUseItemRejectsTemplateVnumMismatchWithoutMutation(t *testing.T) {
	persisted := loginticket.Character{
		ID:        0x01030102,
		VID:       0x02040102,
		Name:      "MismatchPeer",
	...
	runtime := NewRuntime(persisted, SessionLink{Login: "mismatch-peer", CharacterIndex: 1})
	before := runtime.LiveCharacter()
	template := bootstrapConsumableTemplate(27002, 1, 1, 50, "consume:27002:+50")

	if result, ok := runtime.UseItem(5, template); ok {
		t.Fatalf("expected template/source-vnum mismatch item use to fail closed, got %+v", result)
	}
	if got := runtime.LiveCharacter(); !reflect.DeepEqual(got, before) {
		t.Fatalf("template/source-vnum mismatch item use mutated live character: got %#v want %#v", got, before)
	}
	if got := runtime.PersistedSnapshot(); !reflect.DeepEqual(got.Inventory, persisted.Inventory) || !reflect.DeepEqual(got.Quickslots, persisted.Quickslots) || got.Points[1] != persisted.Points[1] {
		t.Fatalf("template/source-vnum mismatch item use mutated persisted state: inventory=%#v quickslots=%#v points[1]=%d", got.Inventory, got.Quickslots, got.Points[1])
	}
}
...
func TestRuntimeUseItemOnItemRejectsTemplateVnumMismatchWithoutMutation(t *testing.T) {
	...
	runtime := NewRuntime(persisted, SessionLink{Login: "use-to-item-mismatch-peer", CharacterIndex: 1})
	before := runtime.LiveCharacter()
	template := bootstrapConsumableTemplate(27002, 1, 1, 50, "consume:27002:+50")

	if result, ok := runtime.UseItemOnItem(5, 6, template); ok {
		t.Fatalf("expected template/source-vnum mismatch use-to-item to fail closed, got %+v", result)
	}
	if got := runtime.LiveCharacter(); !reflect.DeepEqual(got, before) {
...
func TestRuntimeUseItemOnItemRejectsDuplicateSourceTargetItemIDWithoutMutation(t *testing.T) {
	persisted := loginticket.Character{
		ID:        0x01030102,
		VID:       0x02040102,
		Name:      "UseToItemDuplicateIDPeer",
		Points:    [255]int32{1: 700},
	...
	runtime := NewRuntime(persisted, SessionLink{Login: "use-to-item-duplicate-id-peer", CharacterIndex: 1})
	before := runtime.LiveCharacter()
	template := bootstrapConsumableTemplate(27001, 1, 1, 50, "consume:27001:+50")

	if result, ok := runtime.UseItemOnItem(5, 6, template); ok {
		t.Fatalf("expected duplicate source/target item ID use-to-item to fail closed, got %+v", result)
	}
	if got := runtime.LiveCharacter(); !reflect.DeepEqual(got, before) {
		t.Fatalf("duplicate source/target item ID use-to-item mutated live character: got %#v want %#v", got, before)
	}
	if got := runtime.PersistedSnapshot(); !reflect.DeepEqual(got.Inventory, persisted.Inventory) || !reflect.DeepEqual(got.Quickslots, persisted.Quickslots) || got.Points[1] != persisted.Points[1] {
		t.Fatalf("duplicate source/target item ID use-to-item mutated persisted state: inventory=%#v quickslots=%#v points[1]=%d", got.Inventory, got.Quickslots, got.Points[1])
	}
}

func TestRuntimeUseItemOnItemLateValidationFailureDoesNotMutateLiveState(t *testing.T) {
	cases := []struct {
		name        string
		rewriteSlot inventory.SlotIndex
	}{
		{name: "source", rewriteSlot: 5},
		{name: "target", rewriteSlot: 6},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			persisted := loginticket.Character{
				ID:        0x01030102,
				VID:       0x02040102,
				Name:      "UseToItemLateValidatePeer",
				Points:    [255]int32{1: 700},
				Inventory: []inventory.ItemInstance{{ID: 11, Vnum: 27001, Count: 3, Slot: 5}, {ID: 12, Vnum: 27001, Count: 198, Slot: 6}},
				Quickslots: []loginticket.Quickslot{
					{Position: 2, Type: quickslotproto.TypeItem, Slot: 5},
					{Position: 3, Type: quickslotproto.TypeItem, Slot: 6},
				},
			}
			runtime := NewRuntime(persisted, SessionLink{Login: "use-to-item-late-validate-peer", CharacterIndex: 1})
			before := runtime.LiveCharacter()
			template := bootstrapConsumableTemplate(27001, 1, 1, 50, "consume:27001:+50")

			if result, ok := runtime.useItemOnItem(5, 6, template, func(item inventory.ItemInstance) inventory.ItemInstance {
				if item.Slot == tc.rewriteSlot {
					item.Count = 0
				}
				return item
			}); ok {
				t.Fatalf("expected late %s validation failure to fail closed, got %+v", tc.name, result)
			}
			if got := runtime.LiveCharacter(); !reflect.DeepEqual(got, before) {
				t.Fatalf("late %s validation failure mutated live character: got %#v want %#v", tc.name, got, before)
			}
			if got := runtime.PersistedSnapshot(); !reflect.DeepEqual(got.Inventory, persisted.Inventory) || !reflect.DeepEqual(got.Quickslots, persisted.Quickslots) || got.Points[1] != persisted.Points[1] {
				t.Fatalf("late %s validation failure mutated persisted state: inventory=%#v quickslots=%#v points[1]=%d", tc.name, got.Inventory, got.Quickslots, got.Points[1])
			}
		})
	}
}
...
func TestRuntimeUseItemOnItemRejectsMalformedRuntimeTemplateWithoutMutation(t *testing.T) {
	cases := []struct {
		...
		{
			name: "missing use-effect message",
			mutate: func(template *itemcatalog.Template) {
				template.UseEffect.Message = ""
			},
		},
	}
	for _, tc := range cases {
...
func TestRuntimeUseItemOnItemFullMergeDeletesAllSourceItemQuickslotsInOrder(t *testing.T) {
	persisted := loginticket.Character{
		ID:        0x01030102,
		VID:       0x02040102,
		Name:      "PeerTwo",
	...
	wantDeleted := []loginticket.Quickslot{{Position: 2, Type: quickslotproto.TypeItem, Slot: 5}, {Position: 7, Type: quickslotproto.TypeItem, Slot: 5}}
	if !reflect.DeepEqual(deletedQuickslots, wantDeleted) {
		t.Fatalf("unexpected deleted source quickslots after full use-to-item merge: got %#v want %#v", deletedQuickslots, wantDeleted)
	}
	wantLiveQuickslots := []loginticket.Quickslot{{Position: 3, Type: quickslotproto.TypeSkill, Slot: 5}, {Position: 4, Type: quickslotproto.TypeItem, Slot: 6}}
	if !reflect.DeepEqual(runtime.LiveQuickslots(), wantLiveQuickslots) {
		t.Fatalf("unexpected live quickslots after full use-to-item merge: got %#v want %#v", runtime.LiveQuickslots(), wantLiveQuickslots)
	}
}
