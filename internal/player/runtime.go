...
func (r *Runtime) UseItem(slot inventory.SlotIndex, template itemcatalog.Template) (ItemUseResult, bool) {
	if r == nil || slot >= inventory.CarriedInventorySlotCount || !r.CanUseTemplate(template) || template.EquipSlot != "" || template.UseEffect == nil || template.ConfirmWhenUse || template.QuestUse || template.QuestUseMultiple || template.Applicable || template.AntiStack || template.AntiGet || template.AntiDrop || template.AntiGive || template.AntiSell {
