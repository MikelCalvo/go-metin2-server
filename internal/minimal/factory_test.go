...
func TestGameRuntimeAuthLoginTicketHandoffExportProjectsCommittedTickets(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("runtime auth login-ticket handoff export: %v", err)
	}
...
func TestGameRuntimeMigrationStatusPlansBuiltInCatalogWithoutExecutingSQL(t *testing.T) {
	...
	ninth := plan.Pending[8]
	if ninth.Version != 9 || ninth.Name != "item_template_refine_info" || ninth.Direction != dbmigrations.DirectionUp || ninth.Path != "0009_item_template_refine_info.up.sql" {
		t.Fatalf("unexpected ninth pending migration step: %#v", ninth)
	}
	tenth := plan.Pending[9]
	...
	fourteenth := plan.Pending[13]
	if fourteenth.Version != 14 || fourteenth.Name != "character_safebox_state" || fourteenth.Direction != dbmigrations.DirectionUp || fourteenth.Path != "0014_character_safebox_state.up.sql" {
		t.Fatalf("unexpected fourteenth pending migration step: %#v", fourteenth)
	}
	fifteenth := plan.Pending[14]
	if fifteenth.Version != 15 || fifteenth.Name != "character_safebox_money" || fifteenth.Direction != dbmigrations.DirectionUp || fifteenth.Path != "0015_character_safebox_money.up.sql" {
		t.Fatalf("unexpected fifteenth pending migration step: %#v", fifteenth)
	}
	sixteenth := plan.Pending[15]
	if sixteenth.Version != 16 || sixteenth.Name != "static_actor_combat_profile_chase_delay" || sixteenth.Direction != dbmigrations.DirectionUp || sixteenth.Path != "0016_static_actor_combat_profile_chase_delay.up.sql" {
		t.Fatalf("unexpected sixteenth pending migration step: %#v", sixteenth)
	}
	seventeenth := plan.Pending[16]
	if seventeenth.Version != 17 || seventeenth.Name != "static_actor_combat_profile_return_delay" || seventeenth.Direction != dbmigrations.DirectionUp || seventeenth.Path != "0017_static_actor_combat_profile_return_delay.up.sql" {
		t.Fatalf("unexpected seventeenth pending migration step: %#v", seventeenth)
	}
	eighteenth := plan.Pending[17]
	if eighteenth.Version != 18 || eighteenth.Name != "static_actor_combat_profile_homeward_delay" || eighteenth.Direction != dbmigrations.DirectionUp || eighteenth.Path != "0018_static_actor_combat_profile_homeward_delay.up.sql" {
		t.Fatalf("unexpected eighteenth pending migration step: %#v", eighteenth)
	}
	nineteenth := plan.Pending[18]
	...
	twentyFirst := plan.Pending[20]
	if twentyFirst.Version != 21 || twentyFirst.Name != "item_template_refine_keep_on_fail" || twentyFirst.Direction != dbmigrations.DirectionUp || twentyFirst.Path != "0021_item_template_refine_keep_on_fail.up.sql" {
		t.Fatalf("unexpected twenty-first pending migration step: %#v", twentyFirst)
	}
	twentySecond := plan.Pending[21]
	...
	if len(plan.Pending) < 25 {
		t.Fatalf("expected character safebox item instance-sockets pending step, got %#v", plan.Pending)
	}
	twentyFifth := plan.Pending[24]
	...
	if len(plan.Pending) < 27 {
		t.Fatalf("expected character item instance-attributes pending step, got %#v", plan.Pending)
	}
	twentySeventh := plan.Pending[26]
	if twentySeventh.Version != 27 || twentySeventh.Name != "character_item_instance_attributes" || twentySeventh.Direction != dbmigrations.DirectionUp || twentySeventh.Path != "0027_character_item_instance_attributes.up.sql" {
		t.Fatalf("unexpected twenty-seventh pending step: %#v", twentySeventh)
	}
	if len(plan.Pending) < 28 {
		t.Fatalf("expected character safebox item instance-attributes pending step, got %#v", plan.Pending)
	}
	twentyEighth := plan.Pending[27]
	if twentyEighth.Version != 28 || twentyEighth.Name != "character_safebox_item_instance_attributes" || twentyEighth.Direction != dbmigrations.DirectionUp || twentyEighth.Path != "0028_character_safebox_item_instance_attributes.up.sql" {
		t.Fatalf("unexpected twenty-eighth pending step: %#v", twentyEighth)
	}
	if len(plan.Pending) < 29 {
		t.Fatalf("expected bootstrap ground item instance-attributes pending step, got %#v", plan.Pending)
	}
	twentyNinth := plan.Pending[28]
	if twentyNinth.Version != 29 || twentyNinth.Name != "bootstrap_ground_item_instance_attributes" || twentyNinth.Direction != dbmigrations.DirectionUp || twentyNinth.Path != "0029_bootstrap_ground_item_instance_attributes.up.sql" {
		t.Fatalf("unexpected twenty-ninth pending step: %#v", twentyNinth)
	}
...
func TestGameRuntimeMigrationCatalogSummaryReturnsMetadataOnlyCatalog(t *testing.T) {
	...
	if summary.Format != dbmigrations.CatalogSummaryFormat || summary.LatestVersion < 29 {
	...
	if latest.Version != summary.LatestVersion || latest.Name != "bootstrap_ground_item_instance_attributes" || latest.DownSHA256 == "" {
...
func TestGameRuntimePreviewQuestStateTransitionDoesNotMutateStore(t *testing.T) {
	questStatePath := filepath.Join(t.TempDir(), "state", "quest-state.json")
	if err := queststate.NewFileStore(questStatePath).Save(queststate.Snapshot{Flags: []queststate.Flag{{Character: "QuestHero", QuestRef: "quest:first_steps", Name: "step", Value: 1}}}); err != nil {
		t.Fatalf("save initial quest state snapshot: %v", err)
	}
	rawBefore, err := os.ReadFile(questStatePath)
	if err != nil {
	...
	result, err := runtime.PreviewQuestStateTransition(queststate.Transition{
	...
	if !result.Result.Applied || result.Result.Reason != "" || result.Result.CurrentValue != 0 {
		t.Fatalf("unexpected runtime preview result: %+v", result.Result)
	}
	wantSummary := queststate.SnapshotSummary{FlagCount: 2, Characters: []string{"AnotherHero", "QuestHero"}, QuestRefs: []string{"quest:first_steps"}, FlagKeys: []string{"AnotherHero:quest:first_steps:met_guard", "QuestHero:quest:first_steps:step"}}
	if !reflect.DeepEqual(result.Summary, wantSummary) {
		t.Fatalf("unexpected runtime preview summary:\n got: %#v\nwant: %#v", result.Summary, wantSummary)
	}
	rawAfter, err := os.ReadFile(questStatePath)
	if err != nil {
		t.Fatalf("read post-preview quest state snapshot: %v", err)
	}
	if string(rawAfter) != string(rawBefore) {
		t.Fatalf("expected runtime preview not to rewrite snapshot:\n before: %s\n after: %s", string(rawBefore), string(rawAfter))
	}
}
...
func TestGameRuntimePersistenceStatusReportsAccountEmptyCharacterSlots(t *testing.T) {
	accountStore := accountstore.NewFileStore(t.TempDir())
	if err := accountStore.Save(accountstore.Account{Login: "mkmk", Empire: 2, Characters: []loginticket.Character{{ID: 1, Name: "MkmkWar"}, {}, {ID: 2, Name: "MkmkSura"}}}); err != nil {
		t.Fatalf("save account snapshot with empty slot: %v", err)
	}
	runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, loginticket.NewFileStore(t.TempDir()), accountStore, nil, nil, itemcatalog.NewFileStore(filepath.Join(t.TempDir(), "item-templates.json")), nil)
	if err != nil {
		t.Fatalf("new game runtime: %v", err)
	}

	status := runtime.PersistenceStatus()
	if !status.OK || !status.AccountStore.Valid {
		t.Fatalf("expected aggregate persistence status to be ok: %#v", status)
	}
	want := accountstore.SnapshotSummary{AccountCount: 1, CharacterCount: 3, EmptyCharacterSlotCount: 1, Logins: []string{"mkmk"}}
	if !reflect.DeepEqual(status.AccountStore.Summary, want) {
		t.Fatalf("unexpected empty-slot account summary: got %#v want %#v", status.AccountStore.Summary, want)
	}
}
...
func TestNewGameSessionFactoryEquipAppliesNegativeTemplateBackedPointEffect(t *testing.T) {
	store := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	characters := stubCharacters()
	characters[1].Points[bootstrapPlayerPointValueIndex] = 700
	characters[1].Inventory = []inventory.ItemInstance{{ID: 1001, Vnum: 12201, Count: 1, Slot: 8}}
	characters[1].Equipment = []inventory.ItemInstance{}
	wantVID := characters[1].VID
	if err := store.Issue(loginticket.Ticket{Login: StubLogin, LoginKey: 0x01020304, Empire: 2, Characters: characters}); err != nil {
		t.Fatalf("issue login ticket: %v", err)
	...
	equipOut, err := flow.HandleClientFrame(decodeSingleFrame(t, chatproto.EncodeClientChat(chatproto.ClientChatPacket{Type: chatproto.ChatTypeTalking, Message: "/equip_item 8 weapon"})))
	if err != nil {
		t.Fatalf("unexpected negative-effect equip error: %v", err)
	}
	if len(equipOut) != 4 {
		t.Fatalf("expected delete+set+negative-point-change+update frames for equip, got %d", len(equipOut))
	}
	pointPacket, err := worldproto.DecodePlayerPointChange(decodeSingleFrame(t, equipOut[2]))
	if err != nil {
		t.Fatalf("decode negative-effect equip point-change frame: %v", err)
	}
	if pointPacket.VID != wantVID || pointPacket.Type != bootstrapPlayerPointType || pointPacket.Amount != -10 || pointPacket.Value != 690 {
		t.Fatalf("unexpected negative-effect equip point-change packet: %+v", pointPacket)
	}
	account, err := accounts.Load(StubLogin)
	if err != nil {
		t.Fatalf("load persisted account: %v", err)
	}
	if got := account.Characters[1].Points[bootstrapPlayerPointValueIndex]; got != 690 {
		t.Fatalf("expected persisted points[1] to be 690 after negative-effect equip, got %d", got)
	}
}
...
func TestNewGameSessionFactoryUnequipReversesNegativeTemplateBackedPointEffect(t *testing.T) {
	store := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	characters := stubCharacters()
	characters[1].Points[bootstrapPlayerPointValueIndex] = 690
	characters[1].Inventory = []inventory.ItemInstance{}
	characters[1].Equipment = []inventory.ItemInstance{{ID: 2002, Vnum: 12201, Count: 1, Slot: 0, Equipped: true, EquipSlot: inventory.EquipmentSlotWeapon}}
	wantVID := characters[1].VID
	if err := store.Issue(loginticket.Ticket{Login: StubLogin, LoginKey: 0x01020304, Empire: 2, Characters: characters}); err != nil {
		t.Fatalf("issue login ticket: %v", err)
	}
	...
	unequipOut, err := flow.HandleClientFrame(decodeSingleFrame(t, chatproto.EncodeClientChat(chatproto.ClientChatPacket{Type: chatproto.ChatTypeTalking, Message: "/unequip_item weapon 4"})))
	if err != nil {
		t.Fatalf("unexpected negative-effect unequip error: %v", err)
	}
	if len(unequipOut) != 4 {
		t.Fatalf("expected delete+set+positive-point-change+update frames for unequip, got %d", len(unequipOut))
	}
	pointPacket, err := worldproto.DecodePlayerPointChange(decodeSingleFrame(t, unequipOut[2]))
	if err != nil {
		t.Fatalf("decode negative-effect unequip point-change frame: %v", err)
	}
	if pointPacket.VID != wantVID || pointPacket.Type != bootstrapPlayerPointType || pointPacket.Amount != 10 || pointPacket.Value != 700 {
		t.Fatalf("unexpected negative-effect unequip point-change packet: %+v", pointPacket)
	}
	account, err := accounts.Load(StubLogin)
	if err != nil {
		t.Fatalf("load persisted account: %v", err)
	...
	if got := account.Characters[1].Points[bootstrapPlayerPointValueIndex]; got != 700 {
		t.Fatalf("expected persisted points[1] to be 700 after negative-effect unequip, got %d", got)
	}
}
...
func TestNewGameSessionFactoryItemUseToItemFullMergeDeletesSourceQuickslotAndPersists(t *testing.T) {
	...
	if len(mergeOut) != 3 {
		t.Fatalf("expected source delete, target update, and source quickslot delete, got %d frames", len(mergeOut))
	}
	...
	targetUpdate, err := itemproto.DecodeUpdate(decodeSingleFrame(t, mergeOut[1]))
	if err != nil {
		t.Fatalf("decode target update: %v", err)
	}
	if targetUpdate.Position != itemproto.InventoryPosition(8) || targetUpdate.Count != 5 {
		t.Fatalf("expected target slot 8 count 5 update, got %+v", targetUpdate)
	}
