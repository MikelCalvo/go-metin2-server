...
func TestGameSessionFlowItemUseRejectsAntiGetTemplateWithoutMutation(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	owner := peerVisibilityCharacter("UseAntiGet", 0x0103053e, 0x0204053e, 1100, 2100, 0, 101, 201)
	owner.Points[bootstrapPlayerPointValueIndex] = 25
	owner.Inventory = []inventory.ItemInstance{{ID: 206, Vnum: 27004, Count: 2, Slot: 5}}
	owner.Quickslots = []loginticket.Quickslot{{Position: 2, Type: quickslotproto.TypeItem, Slot: 5}}
	issuePeerTicket(t, ticketStore, "item-use-anti-get", 0x5050503e, owner)
	if err := accounts.Save(accountstore.Account{Login: "item-use-anti-get", Empire: owner.Empire, Characters: cloneCharacters([]loginticket.Character{owner})}); err != nil {
		t.Fatalf("seed anti-get item-use account: %v", err)
	}
	itemStore := newItemTemplateStore(t, []itemcatalog.Template{{
		Vnum:      27004,
		Name:      "Anti Get Potion",
		Stackable: true,
		MaxCount:  200,
		AntiGet:   true,
		UseEffect: &itemcatalog.UseEffect{PointType: bootstrapPlayerPointType, PointIndex: bootstrapPlayerPointValueIndex, PointDelta: 50, Message: "must not consume"},
	...
	runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, ticketStore, accounts, nil, nil, itemStore, nil)
	if err != nil {
		t.Fatalf("unexpected anti-get item-use runtime error: %v", err)
	}
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "item-use-anti-get", 0x5050503e)
	defer closeSessionFlow(t, flow)

	out, err := flow.HandleClientFrame(decodeSingleFrame(t, itemproto.EncodeClientUse(itemproto.ClientUsePacket{Position: itemproto.InventoryPosition(5)})))
	if err != nil {
		t.Fatalf("unexpected anti-get item-use packet error: %v", err)
	}
	if len(out) != 0 {
		t.Fatalf("expected anti-get ITEM_USE to emit no frames, got %d", len(out))
	}
	if queued := flushServerFrames(t, flow); len(queued) != 0 {
		t.Fatalf("expected no queued frames after anti-get ITEM_USE rejection, got %d", len(queued))
	}
	persisted, err := accounts.Load("item-use-anti-get")
	if err != nil {
		t.Fatalf("load persisted anti-get item-use account: %v", err)
	}
	if !reflect.DeepEqual(persisted.Characters[0].Inventory, owner.Inventory) {
		t.Fatalf("anti-get ITEM_USE mutated inventory: got %+v want %+v", persisted.Characters[0].Inventory, owner.Inventory)
	}
	if !reflect.DeepEqual(persisted.Characters[0].Quickslots, owner.Quickslots) {
		t.Fatalf("anti-get ITEM_USE mutated quickslots: got %+v want %+v", persisted.Characters[0].Quickslots, owner.Quickslots)
	}
	if persisted.Characters[0].Points[bootstrapPlayerPointValueIndex] != owner.Points[bootstrapPlayerPointValueIndex] {
		t.Fatalf("anti-get ITEM_USE mutated point value: got %d want %d", persisted.Characters[0].Points[bootstrapPlayerPointValueIndex], owner.Points[bootstrapPlayerPointValueIndex])
	}
}
...
func TestGameSessionFlowItemUseRejectsConfirmWhenUseTemplateWithoutMutation(t *testing.T) {
	...
	owner.Inventory = []inventory.ItemInstance{{ID: 206, Vnum: 27004, Count: 2, Slot: 5}}
	owner.Quickslots = []loginticket.Quickslot{{Position: 2, Type: quickslotproto.TypeItem, Slot: 5}}
	issuePeerTicket(t, ticketStore, "item-use-confirm", 0x5050503f, owner)
	if err := accounts.Save(accountstore.Account{Login: "item-use-confirm", Empire: owner.Empire, Characters: cloneCharacters([]loginticket.Character{owner})}); err != nil {
		t.Fatalf("seed confirm-when-use item-use account: %v", err)
	}
	itemStore := newItemTemplateStore(t, []itemcatalog.Template{{
		Vnum:           27004,
		Name:           "Confirm Potion",
	...
	runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, ticketStore, accounts, nil, nil, itemStore, nil)
	if err != nil {
		t.Fatalf("unexpected confirm-when-use item-use runtime error: %v", err)
	}
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "item-use-confirm", 0x5050503f)
	defer closeSessionFlow(t, flow)
...
func TestGameSessionFlowItemUseRejectTextSurvivesHypotheticalPointOverflow(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	owner := peerVisibilityCharacter("UseConfirmOverflowText", 0x01030542, 0x02040542, 1100, 2100, 0, 101, 201)
	owner.Points[bootstrapPlayerPointValueIndex] = 1<<31 - 1
	owner.Inventory = []inventory.ItemInstance{{ID: 211, Vnum: 27014, Count: 2, Slot: 5}}
	owner.Quickslots = []loginticket.Quickslot{{Position: 2, Type: quickslotproto.TypeItem, Slot: 5}}
...
func TestGameSessionFlowItemUseToItemFullMergeDeletesOnlySourceItemQuickslots(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	owner := peerVisibilityCharacter("UseToItemFullQS", 0x0103054d, 0x0204054d, 1100, 2100, 0, 101, 201)
	owner.Inventory = []inventory.ItemInstance{
		{ID: 231, Vnum: 27001, Count: 4, Slot: 5},
		{ID: 232, Vnum: 27001, Count: 6, Slot: 6},
	}
	owner.Quickslots = []loginticket.Quickslot{
		{Position: 2, Type: quickslotproto.TypeItem, Slot: 5},
		{Position: 3, Type: quickslotproto.TypeSkill, Slot: 5},
	...
	issuePeerTicket(t, ticketStore, "item-use-to-item-full-qs", 0x5050508e, owner)
	if err := accounts.Save(accountstore.Account{Login: "item-use-to-item-full-qs", Empire: owner.Empire, Characters: cloneCharacters([]loginticket.Character{owner})}); err != nil {
		t.Fatalf("seed item-use-to-item full quickslot account: %v", err)
	}
	itemStore := newItemTemplateStore(t, []itemcatalog.Template{{
		Vnum:      27001,
		Name:      "Template Potion",
	...
	runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, ticketStore, accounts, nil, nil, itemStore, nil)
	if err != nil {
		t.Fatalf("unexpected item-use-to-item full quickslot runtime error: %v", err)
	}
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "item-use-to-item-full-qs", 0x5050508e)
	defer closeSessionFlow(t, flow)
	...
	sourceDel, err := itemproto.DecodeDel(decodeSingleFrame(t, out[0]))
	if err != nil {
		t.Fatalf("decode full merge source delete: %v", err)
	}
	if sourceDel.Position != itemproto.InventoryPosition(5) {
	...
	quickslotDel, err := quickslotproto.DecodeDel(decodeSingleFrame(t, out[2]))
	if err != nil {
		t.Fatalf("decode first full merge quickslot delete: %v", err)
	}
	if quickslotDel.Position != 2 {
		t.Fatalf("expected source item quickslot position 2 deleted first, got %+v", quickslotDel)
	}
	quickslotDel, err = quickslotproto.DecodeDel(decodeSingleFrame(t, out[3]))
	if err != nil {
		t.Fatalf("decode second full merge quickslot delete: %v", err)
	}
	if quickslotDel.Position != 5 {
		t.Fatalf("expected source item quickslot position 5 deleted second, got %+v", quickslotDel)
	}

	persisted, err := accounts.Load("item-use-to-item-full-qs")
	if err != nil {
		t.Fatalf("load persisted item-use-to-item full quickslot account: %v", err)
	}
	wantInventory := []inventory.ItemInstance{{ID: 232, Vnum: 27001, Count: 10, Slot: 6}}
	if !reflect.DeepEqual(persisted.Characters[0].Inventory, wantInventory) {
		t.Fatalf("expected full merge inventory persisted, got %+v want %+v", persisted.Characters[0].Inventory, wantInventory)
	}
	wantQuickslots := []loginticket.Quickslot{
		{Position: 3, Type: quickslotproto.TypeSkill, Slot: 5},
		{Position: 4, Type: quickslotproto.TypeItem, Slot: 6},
	...
	if !reflect.DeepEqual(persisted.Characters[0].Quickslots, wantQuickslots) {
		t.Fatalf("expected full merge to delete only source item quickslots, got %+v want %+v", persisted.Characters[0].Quickslots, wantQuickslots)
	}
}

func TestGameSessionFlowItemUseToItemStalePartialMergeAfterReclaimIsSelfLocalOnly(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	owner := peerVisibilityCharacter("UseToItemStalePartial", 0x0103054b, 0x0204054b, 1100, 2100, 0, 101, 201)
	owner.Inventory = []inventory.ItemInstance{
		{ID: 211, Vnum: 27001, Count: 7, Slot: 5},
		{ID: 212, Vnum: 27001, Count: 8, Slot: 6},
	}
	...
	issuePeerTicket(t, ticketStore, "item-use-to-item-stale-partial", 0x5050508b, owner)
	if err := accounts.Save(accountstore.Account{Login: "item-use-to-item-stale-partial", Empire: owner.Empire, Characters: cloneCharacters([]loginticket.Character{owner})}); err != nil {
		t.Fatalf("seed stale partial item-use-to-item account: %v", err)
	}
	itemStore := newItemTemplateStore(t, []itemcatalog.Template{{
		Vnum:      27001,
		Name:      "Template Potion",
	...
	runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, ticketStore, accounts, nil, nil, itemStore, nil)
	if err != nil {
		t.Fatalf("unexpected stale partial item-use-to-item runtime error: %v", err)
	}
	staleFlow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "item-use-to-item-stale-partial", 0x5050508b)
	closeSessionFlow(t, staleFlow)

	issuePeerTicket(t, ticketStore, "item-use-to-item-stale-partial", 0x5050508a, owner)
	replacementFlow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "item-use-to-item-stale-partial", 0x5050508a)
	defer closeSessionFlow(t, replacementFlow)

	staleOut, err := staleFlow.HandleClientFrame(decodeSingleFrame(t, itemproto.EncodeClientUseToItem(itemproto.ClientUseToItemPacket{Source: itemproto.InventoryPosition(5), Target: itemproto.InventoryPosition(6)})))
	if err != nil {
		t.Fatalf("unexpected stale partial item-use-to-item packet error: %v", err)
	}
	if len(staleOut) != 2 {
		t.Fatalf("expected stale partial item-use-to-item to emit self-local source and target updates only, got %d", len(staleOut))
	}
	staleSourceUpdate, err := itemproto.DecodeUpdate(decodeSingleFrame(t, staleOut[0]))
	if err != nil {
		t.Fatalf("decode stale partial item-use-to-item source update: %v", err)
	}
	if staleSourceUpdate.Position != itemproto.InventoryPosition(5) || staleSourceUpdate.Count != 5 {
		t.Fatalf("unexpected stale partial item-use-to-item source update: %+v", staleSourceUpdate)
	}
	staleTargetUpdate, err := itemproto.DecodeUpdate(decodeSingleFrame(t, staleOut[1]))
	if err != nil {
		t.Fatalf("decode stale partial item-use-to-item target update: %v", err)
	}
	if staleTargetUpdate.Position != itemproto.InventoryPosition(6) || staleTargetUpdate.Count != 10 {
		t.Fatalf("unexpected stale partial item-use-to-item target update: %+v", staleTargetUpdate)
	}
	persisted, err := accounts.Load("item-use-to-item-stale-partial")
	if err != nil {
		t.Fatalf("load stale partial item-use-to-item account: %v", err)
	}
	if !reflect.DeepEqual(persisted.Characters[0].Inventory, owner.Inventory) {
		t.Fatalf("expected stale partial item-use-to-item to leave authoritative inventory unchanged, got %+v", persisted.Characters[0].Inventory)
	}
	if !reflect.DeepEqual(persisted.Characters[0].Quickslots, owner.Quickslots) {
		t.Fatalf("expected stale partial item-use-to-item to leave authoritative quickslots unchanged, got %+v", persisted.Characters[0].Quickslots)
	}

	replacementOut, err := replacementFlow.HandleClientFrame(decodeSingleFrame(t, itemproto.EncodeClientUseToItem(itemproto.ClientUseToItemPacket{Source: itemproto.InventoryPosition(5), Target: itemproto.InventoryPosition(6)})))
	if err != nil {
		t.Fatalf("unexpected replacement partial item-use-to-item packet error: %v", err)
	}
	if len(replacementOut) != 2 {
		t.Fatalf("expected replacement owner to still see the original partial merge after stale local use, got %d frames", len(replacementOut))
	}
	replacementSourceUpdate, err := itemproto.DecodeUpdate(decodeSingleFrame(t, replacementOut[0]))
	if err != nil {
		t.Fatalf("decode replacement partial item-use-to-item source update: %v", err)
	}
	if replacementSourceUpdate.Position != itemproto.InventoryPosition(5) || replacementSourceUpdate.Count != 5 {
		t.Fatalf("expected replacement owner source to merge from unchanged authoritative state, got %+v", replacementSourceUpdate)
	}
	replacementTargetUpdate, err := itemproto.DecodeUpdate(decodeSingleFrame(t, replacementOut[1]))
	if err != nil {
		t.Fatalf("decode replacement partial item-use-to-item target update: %v", err)
	}
	if replacementTargetUpdate.Position != itemproto.InventoryPosition(6) || replacementTargetUpdate.Count != 10 {
		t.Fatalf("expected replacement owner target to merge from unchanged authoritative state, got %+v", replacementTargetUpdate)
	}
}
