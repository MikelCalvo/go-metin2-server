...
func TestGameRuntimeEnterGameReclaimKeepsStaleMerchantBuyMutationNonAuthoritative(t *testing.T) {
	...
	infoPacket, err := chatproto.DecodeChatDelivery(decodeSingleFrame(t, buyOut[1]))
	if err != nil {
		t.Fatalf("decode stale merchant buy info frame: %v", err)
	}
	if infoPacket.Type != chatproto.ChatTypeInfo || infoPacket.VID != 0 || infoPacket.Message != "Merchant purchase complete." {
		t.Fatalf("unexpected stale merchant buy info frame: %+v", infoPacket)
	}
	if queued := flushServerFrames(t, flowOwnerNew); len(queued) != 0 {
...
func TestGameSessionFlowShopBuyPacketDebitsCurrencyAndAddsItem(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("decode packet shop-buy delivery frame: %v", err)
	}
	if delivery.Type != chatproto.ChatTypeInfo || delivery.Message != "Merchant purchase complete." {
		t.Fatalf("unexpected packet shop-buy delivery: %+v", delivery)
	}
	if queued := flushServerFrames(t, flow); len(queued) != 0 {
...
func TestGameSessionFlowShopBuyPacketMergesIntoExistingCompatibleCarriedStack(t *testing.T) {
	...
	delivery, err := chatproto.DecodeChatDelivery(decodeSingleFrame(t, buyOut[1]))
	if err != nil {
		t.Fatalf("decode merged merchant packet buy delivery: %v", err)
	}
	if delivery.Type != chatproto.ChatTypeInfo || delivery.Message != "Merchant purchase complete." {
		t.Fatalf("unexpected merged merchant packet buy delivery: %+v", delivery)
	}
	currencySnapshot, ok := runtime.CurrencySnapshot(buyer.Name)
	if !ok {
...
func TestGameSessionFlowShopBuyPacketFansOutAcrossSeveralExistingCompatibleStacksThenUsesFreshSlot(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("decode distributed-merge-plus-slot merchant packet buy delivery: %v", err)
	}
	if delivery.Type != chatproto.ChatTypeInfo || delivery.Message != "Merchant purchase complete." {
		t.Fatalf("unexpected distributed-merge-plus-slot merchant packet buy delivery: %+v", delivery)
	}
	currencySnapshot, ok := runtime.CurrencySnapshot(buyer.Name)
	if !ok {
...
func TestGameSessionFlowShopBuyPacketReturnsInfoDeliveryOnInsufficientCurrency(t *testing.T) {
	...
	if len(buyOut) != 1 {
		t.Fatalf("expected insufficient-currency packet shop buy to emit 1 info frame, got %d", len(buyOut))
	}
	delivery, err := chatproto.DecodeChatDelivery(decodeSingleFrame(t, buyOut[0]))
	if err != nil {
		t.Fatalf("decode insufficient-currency packet merchant buy delivery: %v", err)
	}
	if delivery.Type != chatproto.ChatTypeInfo || delivery.Message != "Not enough gold." {
		t.Fatalf("unexpected insufficient-currency packet merchant buy delivery: %+v", delivery)
	}
	currencySnapshot, ok := runtime.CurrencySnapshot(buyer.Name)
	if !ok {
...
func TestGameSessionFlowShopBuyPacketReturnsInfoDeliveryOnNoValidPlacement(t *testing.T) {
	...
	if len(buyOut) != 1 {
		t.Fatalf("expected no-valid-placement packet shop buy to emit 1 info frame, got %d", len(buyOut))
	}
	delivery, err := chatproto.DecodeChatDelivery(decodeSingleFrame(t, buyOut[0]))
	if err != nil {
		t.Fatalf("decode no-valid-placement packet merchant buy delivery: %v", err)
	}
	if delivery.Type != chatproto.ChatTypeInfo || delivery.Message != "Inventory full." {
		t.Fatalf("unexpected no-valid-placement packet merchant buy delivery: %+v", delivery)
	}
	currencySnapshot, ok := runtime.CurrencySnapshot(buyer.Name)
	if !ok {
...
func TestGameSessionFlowPracticeMobRetaliationStopsAtOwnerHPFloorAfterImmediateBeat(t *testing.T) {
	...
	if len(attackOut) != 2 {
		t.Fatalf("expected immediate target-refresh plus self-only point-loss retaliation on owner-HP-floor hit, got %d frames", len(attackOut))
	}
	pointChange, err := worldproto.DecodePlayerPointChange(decodeSingleFrame(t, attackOut[1]))
	if err != nil {
...
func TestGameSessionFlowPracticeMobDelayedRetaliationStopsAtOwnerHPFloor(t *testing.T) {
	...
	queued := flushServerFrames(t, flow)
	if len(queued) != 1 {
		t.Fatalf("expected exactly 1 queued delayed retaliation beat before the owner HP floor is reached, got %d frames", len(queued))
	}
...
func TestGameSessionFlowPracticeMobAttackFailsClosedAfterImmediateRetaliationReachesOwnerHPFloor(t *testing.T) {
	...
	if len(attackOut) != 2 {
		t.Fatalf("expected immediate target-refresh plus self-only point-loss retaliation before zero-HP owner attack denial after immediate retaliation, got %d frames", len(attackOut))
	}
...
func TestGameSessionFlowPracticeMobAttackFailsClosedAfterDelayedRetaliationReachesOwnerHPFloor(t *testing.T) {
	...
	queued := flushServerFrames(t, flow)
	if len(queued) != 1 {
		t.Fatalf("expected exactly 1 queued delayed retaliation beat before zero-HP owner attack denial after delayed retaliation, got %d frames", len(queued))
	}
	pointChange, err := worldproto.DecodePlayerPointChange(decodeSingleFrame(t, queued[0]))
	if err != nil {
...
func TestGameSessionFlowPracticeMobTargetFailsClosedAfterImmediateRetaliationReachesOwnerHPFloor(t *testing.T) {
	...
	if len(attackOut) != 2 {
		t.Fatalf("expected immediate target-refresh plus self-only point-loss retaliation before zero-HP owner target denial after immediate retaliation, got %d frames", len(attackOut))
	}
...
func TestGameSessionFlowPracticeMobTargetFailsClosedAfterDelayedRetaliationReachesOwnerHPFloor(t *testing.T) {
	...
	queued := flushServerFrames(t, flow)
	if len(queued) != 1 {
		t.Fatalf("expected exactly 1 queued delayed retaliation beat before zero-HP owner target denial after delayed retaliation, got %d frames", len(queued))
	}
	pointChange, err := worldproto.DecodePlayerPointChange(decodeSingleFrame(t, queued[0]))
	if err != nil {
