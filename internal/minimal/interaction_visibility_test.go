...
func TestGameRuntimeInteractionVisibilityReturnsQuestFlagConsumeItemPreview(t *testing.T) {
	...
	entry := snapshots[0].VisibleInteractableStaticActors[0]
	if entry.Name != "QuestHunter" || entry.Preview != "Quest updated: first_steps.killed_qa_mob = 0. [reward_gold 100] [reward_experience 50] [reward_item Wooden Sword x1] [consume_gold 25] [consume_experience 10..." || entry.ResolutionFailure != "" {
...
func TestGameRuntimeInteractionVisibilityReturnsQuestFlagConsumeExperiencePreview(t *testing.T) {
	store := loginticket.NewFileStore(t.TempDir())
	peer := peerVisibilityCharacter("PeerOne", 0x01030125, 0x02040125, 1100, 2100, 0, 101, 201)
	peer.Inventory = []inventory.ItemInstance{{ID: 54, Vnum: 27001, Count: 1, Slot: 0}}
	peer.Gold = 40
	peer.Points[bootstrapExperiencePointType] = 40
	...
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "peer-one-consume-exp", 0x1919191a)
	defer closeSessionFlow(t, flow)

	snapshots := runtime.InteractionVisibility()
	if len(snapshots) != 1 || len(snapshots[0].VisibleInteractableStaticActors) != 1 {
		t.Fatalf("expected one visible quest-flag consume-experience interactable, got %+v", snapshots)
	}
	entry := snapshots[0].VisibleInteractableStaticActors[0]
	if entry.Name != "QuestHunter" || entry.Preview != "Quest updated: first_steps.killed_qa_mob = 0. [reward_gold 100] [reward_experience 50] [reward_item Wooden Sword x1] [consume_gold 25] [consume_experience 10..." || entry.ResolutionFailure != "" {
		t.Fatalf("unexpected quest-flag consume-experience interaction visibility entry: %+v", entry)
	}
}
...
func TestGameRuntimeInteractionVisibilityReturnsQuestFlagInsufficientConsumeGoldPreview(t *testing.T) {
	...
	entry := snapshots[0].VisibleInteractableStaticActors[0]
	if entry.Name != "QuestHunter" || entry.Preview != questFlagInsufficientGoldInfoMessage || entry.ResolutionFailure != "" {
		t.Fatalf("unexpected quest-flag consume-gold mismatch interaction visibility entry: %+v", entry)
	}
}

func TestGameRuntimeInteractionVisibilityReturnsQuestFlagInsufficientConsumeExperiencePreview(t *testing.T) {
	store := loginticket.NewFileStore(t.TempDir())
	peer := peerVisibilityCharacter("PeerOne", 0x01030126, 0x02040126, 1100, 2100, 0, 101, 201)
	peer.Inventory = []inventory.ItemInstance{{ID: 55, Vnum: 27001, Count: 1, Slot: 0}}
	peer.Gold = 40
	...
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "peer-one-consume-exp-miss", 0x1a1a1a1a)
	defer closeSessionFlow(t, flow)

	snapshots := runtime.InteractionVisibility()
	if len(snapshots) != 1 || len(snapshots[0].VisibleInteractableStaticActors) != 1 {
		t.Fatalf("expected one visible quest-flag consume-experience mismatch interactable, got %+v", snapshots)
	}
	entry := snapshots[0].VisibleInteractableStaticActors[0]
	if entry.Name != "QuestHunter" || entry.Preview != questFlagInsufficientExperienceInfoMessage || entry.ResolutionFailure != "" {
		t.Fatalf("unexpected quest-flag consume-experience mismatch interaction visibility entry: %+v", entry)
	}
}
...
func TestGameRuntimeInteractionVisibilityReturnsQuestFlagRewardGoldOverflowPreview(t *testing.T) {
	...
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "peer-one-reward-gold-overflow", 0x1c1c1c1c)
	defer closeSessionFlow(t, flow)

	snapshots := runtime.InteractionVisibility()
	if len(snapshots) != 1 || len(snapshots[0].VisibleInteractableStaticActors) != 1 {
		t.Fatalf("expected one visible quest-flag reward-gold overflow interactable, got %+v", snapshots)
	}
	entry := snapshots[0].VisibleInteractableStaticActors[0]
	...
	if !reflect.DeepEqual(loaded, before) {
		t.Fatalf("quest-flag reward-gold overflow preview mutated quest-state:\n got: %#v\nwant: %#v", loaded, before)
	}
}
...
func TestGameRuntimeInteractionVisibilityReturnsQuestFlagRewardExperienceOverflowPreview(t *testing.T) {
	...
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "peer-one-reward-exp-overflow", 0x1d1d1d1d)
	defer closeSessionFlow(t, flow)

	snapshots := runtime.InteractionVisibility()
	if len(snapshots) != 1 || len(snapshots[0].VisibleInteractableStaticActors) != 1 {
		t.Fatalf("expected one visible quest-flag reward-experience overflow interactable, got %+v", snapshots)
	}
	entry := snapshots[0].VisibleInteractableStaticActors[0]
	...
	if !reflect.DeepEqual(loaded, before) {
		t.Fatalf("quest-flag reward-experience overflow preview mutated quest-state:\n got: %#v\nwant: %#v", loaded, before)
	}
}

func TestGameRuntimeInteractionVisibilityReturnsQuestFlagInsufficientConsumeItemsPreview(t *testing.T) {
	store := loginticket.NewFileStore(t.TempDir())
	peer := peerVisibilityCharacter("PeerOne", 0x01030127, 0x02040127, 1100, 2100, 0, 101, 201)
	peer.Gold = 40
	peer.Points[bootstrapExperiencePointType] = 40
	...
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "peer-one-consume-items-miss", 0x1b1b1b1b)
	defer closeSessionFlow(t, flow)

	snapshots := runtime.InteractionVisibility()
	if len(snapshots) != 1 || len(snapshots[0].VisibleInteractableStaticActors) != 1 {
		t.Fatalf("expected one visible quest-flag consume-items mismatch interactable, got %+v", snapshots)
	}
	entry := snapshots[0].VisibleInteractableStaticActors[0]
	if entry.Name != "QuestHunter" || entry.Preview != questFlagInsufficientMaterialsInfoMessage || entry.ResolutionFailure != "" {
		t.Fatalf("unexpected quest-flag consume-items mismatch interaction visibility entry: %+v", entry)
	}
}
