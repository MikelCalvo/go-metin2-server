...
func TestGameRuntimePlayerDeathFloorRematerializesAcrossDaemonRestart(t *testing.T) {
	...
	if len(attackOut) != 6 {
		t.Fatalf("expected immediate target-refresh, point-loss, self dead, clear-target, mob damage-info, and owner damage-info frames before death-floor daemon restart, got %d", len(attackOut))
	}
