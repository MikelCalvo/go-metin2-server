...
func TestGameRuntimePreviewContentBundleImportReturnsDeltasWithoutMutatingRuntime(t *testing.T) {
	...
	wantDeltas := contentbundle.SummaryDeltas{
		...
		InteractableStaticActors: []contentbundle.InteractableStaticActorDelta{
			{Change: "added", Candidate: &contentbundle.InteractableStaticActorSummary{Name: "Merchant", MapIndex: 42, X: 1800, Y: 2900, RaceNum: 20302, InteractionKind: interactionstore.KindShopPreview, InteractionRef: "npc:merchant", Preview: defaultMerchantPreview}},
			{Change: "removed", Current: &contentbundle.InteractableStaticActorSummary{Name: "VillageGuide", MapIndex: 42, X: 1700, Y: 2800, RaceNum: 20300, InteractionKind: interactionstore.KindTalk, InteractionRef: "npc:guide", Preview: "VillageGuide:\nWelcome."}},
		},
...
func TestGameRuntimeImportContentBundleRejectsNonCanonicalCombatProfileSnapshotIdentity(t *testing.T) {
	...
	_, err = runtime.ImportContentBundle(contentbundle.Bundle{
		SpawnGroups: []contentbundle.SpawnGroup{{
			Ref:           "practice.runtime_noncanonical_wolf",
			Name:          "Runtime Noncanonical Wolf",
			MapIndex:      42,
			X:             1800,
			Y:             2900,
			RaceNum:       101,
	...
	if !errors.Is(err, contentbundle.ErrInvalidBundle) {
		t.Fatalf("expected ErrInvalidBundle for non-canonical combat profile snapshot identity, got %v", err)
	}
	if worldruntime.ValidStaticActorCombatProfile(profile) {
		t.Fatalf("expected rejected non-canonical combat profile snapshot %q not to stay registered", profile)
	}
	if spawnGroups := runtime.SpawnGroups(); len(spawnGroups) != 0 {
		t.Fatalf("expected rejected non-canonical combat profile import not to materialize spawn groups, got %+v", spawnGroups)
	}
}
