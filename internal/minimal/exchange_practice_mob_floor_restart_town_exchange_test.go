...
func TestGameSessionFlowPracticeMobImmediateRetaliationFloorClosesOpenExchangeShellBeforeRestartTownExchange(t *testing.T) {
	...
	owner := peerVisibilityCharacter("ExchangeTownImmOwner", 0x01030f90, 0x02040f90, 1100, 2100, 0, 101, 201)
	owner.Points[bootstrapPlayerPointValueIndex] = 1
	owner.Gold = 125
	sourcePeer := peerVisibilityCharacter("ExchangeTownImmSource", 0x01030f91, 0x02040f91, 1120, 2120, 0, 101, 201)
	sourcePeer.Gold = 22222
	townPeer := peerVisibilityCharacter("ExchangeTownImmTown", 0x01030f92, 0x02040f92, 52070, 166600, 4, 103, 203)
	...
	if len(actors) != 1 {
		t.Fatalf("expected one exchange town immediate practice mob, got %#v", actors)
	}
	...
	startOut, err := ownerFlow.HandleClientFrame(decodeSingleFrame(t, itemproto.EncodeClientExchange(itemproto.ClientExchangePacket{Subheader: itemproto.ExchangeSubheaderStart, Arg1: sourcePeer.VID})))
	if err != nil {
	...
	assertExchangeStartFrame(t, startOut[0], sourcePeer.VID, "owner exchange start before death")
	sourceStart := flushServerFrames(t, sourceFlow)
	if len(sourceStart) != 1 {
		t.Fatalf("expected source peer exchange start before owner death, got %d", len(sourceStart))
	}
	assertExchangeStartFrame(t, sourceStart[0], owner.VID, "source peer exchange start before owner death")
	...
	if err != nil {
		t.Fatalf("unexpected target selection before exchange town immediate floor-close: %v", err)
	}
	if len(selectOut) != 1 {
		t.Fatalf("expected one target selection frame before exchange town immediate floor-close, got %d", len(selectOut))
	}
	...
	if err != nil {
		t.Fatalf("unexpected exchange town immediate floor-close attack: %v", err)
	}
	if len(attackOut) != 5 {
	...
	if pointChange.VID != owner.VID || pointChange.Type != bootstrapPlayerPointType || pointChange.Amount != -1 || pointChange.Value != 0 {
		t.Fatalf("unexpected exchange town immediate floor-close point-change: %+v", pointChange)
	}
	...
	if dead.VID != owner.VID {
		t.Fatalf("expected exchange town immediate floor-close self dead for owner %#08x, got %#08x", owner.VID, dead.VID)
	}
	clearTarget, err := combatproto.DecodeServerTarget(decodeSingleFrame(t, attackOut[3]))
	if err != nil {
		t.Fatalf("decode exchange town immediate floor-close target clear: %v", err)
	}
	if clearTarget.TargetVID != 0 || clearTarget.HPPercent != 0 {
		t.Fatalf("expected exchange town immediate floor-close to clear active combat target, got %+v", clearTarget)
	}
	assertExchangeEndFrame(t, attackOut[4], "owner exchange END after immediate death")

	sourceQueued := flushServerFrames(t, sourceFlow)
	if len(sourceQueued) != 2 {
		t.Fatalf("expected source peer visible DEAD plus exchange END after owner death, got %d frames", len(sourceQueued))
	}
	peerDead, err := worldproto.DecodeDead(decodeSingleFrame(t, sourceQueued[0]))
	if err != nil {
		t.Fatalf("decode source peer queued owner dead before exchange END: %v", err)
	}
	if peerDead.VID != owner.VID {
		t.Fatalf("expected source peer queued owner dead for %#08x, got %#08x", owner.VID, peerDead.VID)
	}
	assertExchangeEndFrame(t, sourceQueued[1], "source peer exchange END after owner death")
	...
	if queued := flushServerFrames(t, sourceFlow); len(queued) != 0 {
		t.Fatalf("expected no stale source peer exchange frames after post-floor cancel, got %d", len(queued))
	}
...
func TestGameSessionFlowPracticeMobDelayedRetaliationFloorClosesOpenExchangeShellBeforeRestartTownExchange(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	owner := peerVisibilityCharacter("ExchangeTownDelOwner", 0x01030f93, 0x02040f93, 1100, 2100, 0, 101, 201)
	owner.Points[bootstrapPlayerPointValueIndex] = 2
	owner.Gold = 125
	sourcePeer := peerVisibilityCharacter("ExchangeTownDelSource", 0x01030f94, 0x02040f94, 1120, 2120, 0, 101, 201)
	sourcePeer.Gold = 22222
	townPeer := peerVisibilityCharacter("ExchangeTownDelTown", 0x01030f95, 0x02040f95, 52070, 166600, 4, 103, 203)
	townPeer.MapIndex = 21
	login := "exch-town-del"
	...
	actors := runtime.StaticActors()
	if len(actors) != 1 {
		t.Fatalf("expected one exchange town delayed practice mob, got %#v", actors)
	}
	targetVID := uint32(actors[0].EntityID)
	...
	sourceFlow, sourceEnter := enterGameWithLoginTicket(t, runtime.SessionFactory(), sourceLogin, sourceLoginKey)
	if len(sourceEnter) != 11 {
		t.Fatalf("expected 11 delayed source peer bootstrap frames with visible owner and mob, got %d", len(sourceEnter))
	}
	defer closeSessionFlow(t, sourceFlow)
	if queued := flushServerFrames(t, ownerFlow); len(queued) != 3 {
	...
	assertExchangeStartFrame(t, startOut[0], sourcePeer.VID, "delayed owner exchange start before death")
	sourceStart := flushServerFrames(t, sourceFlow)
	if len(sourceStart) != 1 {
		t.Fatalf("expected delayed source peer exchange start before owner death, got %d", len(sourceStart))
	}
	assertExchangeStartFrame(t, sourceStart[0], owner.VID, "delayed source peer exchange start before owner death")

	selectOut, err := ownerFlow.HandleClientFrame(decodeSingleFrame(t, combatproto.EncodeClientTarget(combatproto.ClientTargetPacket{TargetVID: targetVID})))
	if err != nil {
		t.Fatalf("unexpected target selection before delayed exchange town floor-close: %v", err)
	}
	if len(selectOut) != 1 {
		t.Fatalf("expected one target selection frame before delayed exchange town floor-close, got %d", len(selectOut))
	}
	...
	firstPeerQueued := flushServerFrames(t, sourceFlow)
	if len(firstPeerQueued) != 2 {
		t.Fatalf("expected delayed exchange source peer mob + owner retaliation damage-info after first hit, got %d", len(firstPeerQueued))
	}
	assertDamageInfoFrame(t, firstPeerQueued[0], targetVID, int32(worldruntime.TrainingDummyBootstrapDamagePerNormalAttack), "delayed exchange source peer first hit mob")
	assertDamageInfoFrame(t, firstPeerQueued[1], owner.VID, -bootstrapPracticeMobRetaliationPointDelta, "delayed exchange source peer first hit owner retaliation")
	...
	sourceQueued := flushServerFrames(t, sourceFlow)
	if len(sourceQueued) != 2 {
		t.Fatalf("expected source peer visible DEAD plus exchange END after delayed owner death, got %d frames", len(sourceQueued))
	}
	peerDead, err := worldproto.DecodeDead(decodeSingleFrame(t, sourceQueued[0]))
	if err != nil {
		t.Fatalf("decode source peer queued owner dead before delayed exchange END: %v", err)
	}
	if peerDead.VID != owner.VID {
		t.Fatalf("expected source peer queued delayed owner dead for %#08x, got %#08x", owner.VID, peerDead.VID)
	}
	assertExchangeEndFrame(t, sourceQueued[1], "source peer exchange END after delayed owner death")
	...
	if queued := flushServerFrames(t, sourceFlow); len(queued) != 0 {
		t.Fatalf("expected no stale delayed source peer exchange frames after post-floor cancel, got %d", len(queued))
	}
	...
	selfAdd, err := worldproto.DecodeCharacterAdd(decodeSingleFrame(t, restartOut[0]))
	if err != nil {
		t.Fatalf("decode self character add after exchange town delayed /restart_town: %v", err)
	}
	if selfAdd.VID != owner.VID || selfAdd.X != 52070 || selfAdd.Y != 166600 {
