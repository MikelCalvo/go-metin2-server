...
func TestGameSessionFlowStaticActorQuestFlagConsumeItemsRemovesInventoryAndGrantsReward(t *testing.T) {
	...
	if len(out) != 4 {
		t.Fatalf("expected chat + gold + consume + reward frames for quest-flag consume-items interaction, got %d", len(out))
	}
	...
	consumeDel, err := itemproto.DecodeDel(decodeSingleFrame(t, out[2]))
	...
	itemSet, err := itemproto.DecodeSet(decodeSingleFrame(t, out[3]))
	...
	if account.Characters[0].Gold != 125 {
		t.Fatalf("expected persisted gold 125 after quest-flag consume-items interaction, got %d", account.Characters[0].Gold)
	}
