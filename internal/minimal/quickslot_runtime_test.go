...
func TestGameSessionFlowQuickslotAddRetargetsDuplicateTupleQuickslot(t *testing.T) {
	cases := []struct {
		...
		oldPosition    uint8
		newPosition    uint8
		retargetedSlot quickslotproto.Slot
		otherQuickslot loginticket.Quickslot
	}{
		{
			name:           "item",
			login:          "quickslot-retarget-item",
			characterName:  "RetargetItem",
			oldPosition:    2,
			newPosition:    4,
			retargetedSlot: quickslotproto.Slot{Type: quickslotproto.TypeItem, Position: 5},
			otherQuickslot: loginticket.Quickslot{Position: 3, Type: quickslotproto.TypeSkill, Slot: 5},
		},
		{
			name:           "skill",
			login:          "quickslot-retarget-skill",
			characterName:  "RetargetSkill",
			oldPosition:    2,
			newPosition:    4,
			retargetedSlot: quickslotproto.Slot{Type: quickslotproto.TypeSkill, Position: 5},
			otherQuickslot: loginticket.Quickslot{Position: 3, Type: quickslotproto.TypeItem, Slot: 5},
		},
		{
			name:           "command",
			login:          "quickslot-retarget-command",
			characterName:  "RetargetCommand",
			oldPosition:    2,
			newPosition:    4,
			retargetedSlot: quickslotproto.Slot{Type: quickslotproto.TypeCommand, Position: 5},
			otherQuickslot: loginticket.Quickslot{Position: 3, Type: quickslotproto.TypeItem, Slot: 5},
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			ticketStore := loginticket.NewFileStore(t.TempDir())
			accounts := accountstore.NewFileStore(t.TempDir())
			owner := peerVisibilityCharacter(tc.characterName, 0x0103058c, 0x0204058c, 1100, 2100, 0, 101, 201)
			owner.Inventory = []inventory.ItemInstance{{ID: 401, Vnum: 27001, Count: 2, Slot: 5}}
			owner.Quickslots = []loginticket.Quickslot{
				{Position: tc.oldPosition, Type: tc.retargetedSlot.Type, Slot: tc.retargetedSlot.Position},
				tc.otherQuickslot,
			}
			issuePeerTicket(t, ticketStore, tc.login, 0x5050508c, owner)
			if err := accounts.Save(accountstore.Account{Login: tc.login, Empire: owner.Empire, Characters: cloneCharacters([]loginticket.Character{owner})}); err != nil {
				t.Fatalf("seed %s quickslot retarget account: %v", tc.name, err)
			}
			runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, ticketStore, accounts, nil, nil, nil, nil)
			if err != nil {
				t.Fatalf("unexpected %s quickslot retarget runtime error: %v", tc.name, err)
			}
			flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), tc.login, 0x5050508c)
			defer closeSessionFlow(t, flow)

			out, err := flow.HandleClientFrame(decodeSingleFrame(t, quickslotproto.EncodeClientAdd(quickslotproto.ClientAddPacket{Position: tc.newPosition, Slot: tc.retargetedSlot})))
			if err != nil {
				t.Fatalf("unexpected %s quickslot retarget packet error: %v", tc.name, err)
			}
			wantFrames := [][]byte{
				quickslotproto.EncodeDel(quickslotproto.DelPacket{Position: tc.oldPosition}),
				quickslotproto.EncodeAdd(quickslotproto.AddPacket{Position: tc.newPosition, Slot: tc.retargetedSlot}),
			}
			if !reflect.DeepEqual(out, wantFrames) {
				t.Fatalf("unexpected %s quickslot retarget frames:\n got %#v\nwant %#v", tc.name, out, wantFrames)
			}
			persisted, err := accounts.Load(tc.login)
			if err != nil {
				t.Fatalf("load %s quickslot retarget account: %v", tc.name, err)
			}
			wantQuickslots := []loginticket.Quickslot{
				tc.otherQuickslot,
				{Position: tc.newPosition, Type: tc.retargetedSlot.Type, Slot: tc.retargetedSlot.Position},
			}
			if !reflect.DeepEqual(persisted.Characters[0].Quickslots, wantQuickslots) {
				t.Fatalf("unexpected persisted quickslots after %s retarget: got %+v want %+v", tc.name, persisted.Characters[0].Quickslots, wantQuickslots)
			}
		})
	}
}

func TestGameSessionFlowQuickslotAddRejectsLockedItemWithoutMutation(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	owner := peerVisibilityCharacter("QuickslotLockedAdd", 0x0103058d, 0x0204058d, 1100, 2100, 0, 101, 201)
	owner.Inventory = []inventory.ItemInstance{{ID: 411, Vnum: 27001, Count: 2, Slot: 5, Locked: true}}
	owner.Quickslots = []loginticket.Quickslot{{Position: 3, Type: quickslotproto.TypeSkill, Slot: 5}}
	issuePeerTicket(t, ticketStore, "quickslot-locked-add", 0x5050508d, owner)
	if err := accounts.Save(accountstore.Account{Login: "quickslot-locked-add", Empire: owner.Empire, Characters: cloneCharacters([]loginticket.Character{owner})}); err != nil {
		t.Fatalf("seed locked-item quickslot add account: %v", err)
	}
	runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, ticketStore, accounts, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("unexpected locked-item quickslot add runtime error: %v", err)
	}
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "quickslot-locked-add", 0x5050508d)
	defer closeSessionFlow(t, flow)
	...
	if err != nil {
		t.Fatalf("unexpected locked-item quickslot add packet error: %v", err)
	}
	if len(out) != 0 {
		t.Fatalf("expected locked-item quickslot add to emit no frames, got %d", len(out))
	}
	if queued := flushServerFrames(t, flow); len(queued) != 0 {
		t.Fatalf("expected no queued frames after locked-item quickslot add rejection, got %d", len(queued))
	}
	persisted, err := accounts.Load("quickslot-locked-add")
	if err != nil {
		t.Fatalf("load locked-item quickslot add account: %v", err)
	}
	if !reflect.DeepEqual(persisted.Characters[0].Quickslots, owner.Quickslots) {
		t.Fatalf("locked-item quickslot add mutated quickslots: got %+v want %+v", persisted.Characters[0].Quickslots, owner.Quickslots)
	}
	if !reflect.DeepEqual(persisted.Characters[0].Inventory, owner.Inventory) {
		t.Fatalf("locked-item quickslot add mutated inventory: got %+v want %+v", persisted.Characters[0].Inventory, owner.Inventory)
	}
}
...
func TestGameSessionFlowPracticeMobQuickslotEditsFailClosedAfterImmediateRetaliationReachesOwnerHPFloor(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(attackOut) != 6 {
				t.Fatalf("expected immediate retaliation floor attack to emit 6 frames before post-floor quickslot denial, got %d", len(attackOut))
			}
