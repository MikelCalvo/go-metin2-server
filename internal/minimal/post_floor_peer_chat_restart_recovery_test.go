...
func TestGameSessionFlowPostFloorPeerFacingChatFailsClosed(t *testing.T) {
	...
	if queued := flushServerFrames(t, peerFlow); len(queued) != 3 {
		t.Fatalf("expected peer DEAD plus mob damage-info plus owner damage-info fanout after owner floor before peer chat, got %d", len(queued))
	}
...
func TestGameSessionFlowPostFloorPeerFacingChatFailsClosedBeforeRestartTown(t *testing.T) {
	...
	if queued := flushServerFrames(t, sourceFlow); len(queued) != 3 {
		t.Fatalf("expected source peer DEAD plus mob damage-info plus owner damage-info fanout after owner floor before town peer chat, got %d", len(queued))
	}
