...
func TestGameRuntimeItemBootstrapFramesCarryTemplateAntiFlags(t *testing.T) {
	...
	itemStore := newItemTemplateStore(t, []itemcatalog.Template{
		{Vnum: 27091, Name: "Bound Practice Potion", Stackable: true, MaxCount: 200, Refineable: true, Save: true, SellCountPerGold: true, SlowQuery: true, Rare: true, Unique: true, MakeCount: true, Irremovable: true, ConfirmWhenUse: true, Log: true, AntiDrop: true, AntiGive: true, AntiSell: true, AntiStack: true, AntiGet: true, AntiSave: true, AntiPKDrop: true, AntiMyShop: true, AntiSafebox: true},
	...
	wantCarriedAntiFlags := itemproto.AntiFlagDrop | itemproto.AntiFlagGive | itemproto.AntiFlagSell | itemproto.AntiFlagStack | itemproto.AntiFlagGet | itemproto.AntiFlagSave | itemproto.AntiFlagPKDrop | itemproto.AntiFlagMyShop | itemproto.AntiFlagSafebox
...
func TestGameRuntimeItemMoveCountOnlyRefreshesCarryTemplateDisplayMetadata(t *testing.T) {
	testCases := []struct {
		name        string
		login       string
		key         uint32
		owner       loginticket.Character
		request     itemproto.ClientMovePacket
		wantUpdates []itemproto.UpdatePacket
		wantSet     *itemproto.SetPacket
	}{
		{
			name:  "zero-count partial merge",
			login: "move-display-attrs-partial",
			key:   0x606060AE,
			owner: func() loginticket.Character {
				owner := peerVisibilityCharacter("MoveAttrsPartial", 0x010306AE, 0x020406AE, 1300, 2300, 0, 101, 201)
				owner.Inventory = []inventory.ItemInstance{
					{ID: 6261, Vnum: 27005, Count: 7, Slot: 5},
					{ID: 6262, Vnum: 27005, Count: 8, Slot: 6},
				}
				return owner
			}(),
			request: itemproto.ClientMovePacket{Source: itemproto.InventoryPosition(5), Destination: itemproto.InventoryPosition(6), Count: 0},
			wantUpdates: []itemproto.UpdatePacket{
				{Position: itemproto.InventoryPosition(5), Count: 5},
				{Position: itemproto.InventoryPosition(6), Count: 10},
			},
		},
		{
			name:  "zero-count full merge",
			login: "move-display-attrs-full",
			key:   0x606060AF,
			owner: func() loginticket.Character {
				owner := peerVisibilityCharacter("MoveAttrsFull", 0x010306AF, 0x020406AF, 1300, 2300, 0, 101, 201)
				owner.Inventory = []inventory.ItemInstance{
					{ID: 6263, Vnum: 27005, Count: 2, Slot: 5},
					{ID: 6264, Vnum: 27005, Count: 8, Slot: 6},
				}
				return owner
			}(),
			request: itemproto.ClientMovePacket{Source: itemproto.InventoryPosition(5), Destination: itemproto.InventoryPosition(6), Count: 0},
		...
		{
			name:  "counted partial merge",
			login: "move-attrs-counted",
			key:   0x606060B1,
			owner: func() loginticket.Character {
				owner := peerVisibilityCharacter("MoveAttrsCounted", 0x010306B1, 0x020406B1, 1300, 2300, 0, 101, 201)
				owner.Inventory = []inventory.ItemInstance{
					{ID: 6266, Vnum: 27005, Count: 7, Slot: 5},
					{ID: 6267, Vnum: 27005, Count: 8, Slot: 6},
				}
				return owner
			}(),
			request: itemproto.ClientMovePacket{Source: itemproto.InventoryPosition(5), Destination: itemproto.InventoryPosition(6), Count: 2},
			wantUpdates: []itemproto.UpdatePacket{
				{Position: itemproto.InventoryPosition(5), Count: 5},
				{Position: itemproto.InventoryPosition(6), Count: 10},
			},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			ticketStore := loginticket.NewFileStore(t.TempDir())
			accounts := accountstore.NewFileStore(t.TempDir())
			issuePeerTicket(t, ticketStore, tc.login, tc.key, tc.owner)
			if err := accounts.Save(accountstore.Account{Login: tc.login, Empire: tc.owner.Empire, Characters: cloneCharacters([]loginticket.Character{tc.owner})}); err != nil {
				t.Fatalf("seed display-metadata item-move account: %v", err)
			}
			itemStore := newItemTemplateStore(t, []itemcatalog.Template{{
				Vnum:      27005,
				Name:      "Socketed Move Potion",
				Stackable: true,
				MaxCount:  10,
			...
			runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, ticketStore, accounts, nil, nil, itemStore, nil)
			if err != nil {
				t.Fatalf("unexpected display-metadata item-move runtime error: %v", err)
			}
			flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), tc.login, tc.key)
			defer closeSessionFlow(t, flow)

			out, err := flow.HandleClientFrame(decodeSingleFrame(t, itemproto.EncodeClientMove(tc.request)))
			if err != nil {
				t.Fatalf("unexpected display-metadata item-move packet error: %v", err)
			}
			wantFrames := len(tc.wantUpdates)
			if tc.wantSet != nil {
				wantFrames++
			}
			if tc.name == "zero-count full merge" {
				wantFrames++
			}
			if len(out) != wantFrames {
				t.Fatalf("expected display-metadata item move to emit %d frames, got %d", wantFrames, len(out))
			}

			frameIndex := 0
			if tc.name == "zero-count full merge" {
				del, err := itemproto.DecodeDel(decodeSingleFrame(t, out[frameIndex]))
				if err != nil {
					t.Fatalf("decode display-metadata item-move source delete: %v", err)
				}
				if del.Position != itemproto.InventoryPosition(5) {
					t.Fatalf("unexpected display-metadata item-move source delete: %+v", del)
				}
				frameIndex++
			}
			...
			for _, wantUpdate := range tc.wantUpdates {
				update, err := itemproto.DecodeUpdate(decodeSingleFrame(t, out[frameIndex]))
				if err != nil {
					t.Fatalf("decode display-metadata item-move update: %v", err)
				}
				if update.Position != wantUpdate.Position || update.Count != wantUpdate.Count {
					t.Fatalf("unexpected display-metadata item-move update identity/count: got %+v want %+v", update, wantUpdate)
				}
				if update.Sockets != wantSockets {
					t.Fatalf("expected template-authored item-move update sockets %+v, got %+v", wantSockets, update.Sockets)
				}
				if update.Attributes != wantAttributes {
					t.Fatalf("expected template-authored item-move update attributes %+v, got %+v", wantAttributes, update.Attributes)
				}
				frameIndex++
			}
			if tc.wantSet != nil {
				set, err := itemproto.DecodeSet(decodeSingleFrame(t, out[frameIndex]))
				if err != nil {
					t.Fatalf("decode display-metadata item-move destination set: %v", err)
				}
				if set.Position != tc.wantSet.Position || set.Vnum != tc.wantSet.Vnum || set.Count != tc.wantSet.Count {
					t.Fatalf("unexpected display-metadata item-move set identity/count: got %+v want %+v", set, *tc.wantSet)
				}
				if set.Sockets != wantSockets {
					t.Fatalf("expected template-authored item-move set sockets %+v, got %+v", wantSockets, set.Sockets)
				}
				if set.Attributes != wantAttributes {
					t.Fatalf("expected template-authored item-move set attributes %+v, got %+v", wantAttributes, set.Attributes)
				}
			}
		})
	}
...
func TestGameRuntimeItemMoveEquipReplacesOccupiedWearSlotWithEffectTransition(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	itemStore := itemcatalog.NewFileStore(filepath.Join(t.TempDir(), "item-templates.json"))
	worn := itemcatalog.Template{Vnum: 11520, Name: "Worn Test Armor", Stackable: false, MaxCount: 1, EquipSlot: inventory.EquipmentSlotBody.String(), EquipEffect: &itemcatalog.PointEffect{PointType: 1, PointIndex: 1, PointDelta: 20}}
	...
	if len(out) < 5 {
		t.Fatalf("expected occupied wear swap refresh frames, got %d", len(out))
	}
	sourceSet, err := itemproto.DecodeSet(decodeSingleFrame(t, out[0]))
	if err != nil {
		t.Fatalf("decode occupied wear swap source set: %v", err)
	}
	if sourceSet.Vnum != worn.Vnum || sourceSet.Count != 1 || sourceSet.Position != itemproto.InventoryPosition(5) {
		t.Fatalf("unexpected occupied wear swap source set: %+v", sourceSet)
	}
	equipSet, err := itemproto.DecodeSet(decodeSingleFrame(t, out[1]))
	if err != nil {
		t.Fatalf("decode occupied wear swap equipment set: %v", err)
	}
	if equipSet.Vnum != carried.Vnum || equipSet.Count != 1 || equipSet.Position != bodyPosition {
		t.Fatalf("unexpected occupied wear swap equipment set: %+v", equipSet)
	}
	removedPoint, err := worldproto.DecodePlayerPointChange(decodeSingleFrame(t, out[2]))
	if err != nil {
		t.Fatalf("decode occupied wear swap removed point change: %v", err)
	}
	if removedPoint.Type != 1 || removedPoint.Amount != -20 || removedPoint.Value != 730 {
		t.Fatalf("unexpected occupied wear swap removed point change: %+v", removedPoint)
	}
	appliedPoint, err := worldproto.DecodePlayerPointChange(decodeSingleFrame(t, out[3]))
	if err != nil {
		t.Fatalf("decode occupied wear swap applied point change: %v", err)
	}
	if appliedPoint.Type != 1 || appliedPoint.Amount != 50 || appliedPoint.Value != 780 {
		t.Fatalf("unexpected occupied wear swap applied point change: %+v", appliedPoint)
	}
	if _, err := worldproto.DecodeCharacterUpdate(decodeSingleFrame(t, out[4])); err != nil {
		t.Fatalf("decode occupied wear swap character update: %v", err)
	}
	if queued := flushServerFrames(t, flow); len(queued) != 0 {
		t.Fatalf("expected no queued frames after occupied wear swap, got %d", len(queued))
	}
	account, err := accounts.Load("occupied-wear-equip")
	if err != nil {
		t.Fatalf("load occupied wear equip account: %v", err)
	}
	wantInventory := []inventory.ItemInstance{{ID: 5800, Vnum: worn.Vnum, Count: 1, Slot: 5}}
	wantEquipment := []inventory.ItemInstance{{ID: 5801, Vnum: carried.Vnum, Count: 1, Equipped: true, EquipSlot: inventory.EquipmentSlotBody}}
	if !reflect.DeepEqual(account.Characters[0].Inventory, wantInventory) {
		t.Fatalf("occupied wear swap persisted unexpected inventory: got %#v want %#v", account.Characters[0].Inventory, wantInventory)
	}
	if !reflect.DeepEqual(account.Characters[0].Equipment, wantEquipment) {
		t.Fatalf("occupied wear swap persisted unexpected equipment: got %#v want %#v", account.Characters[0].Equipment, wantEquipment)
	}
	if account.Characters[0].Points[1] != 780 {
		t.Fatalf("occupied wear swap persisted unexpected points: got %d want 780", account.Characters[0].Points[1])
	}
}

func TestGameRuntimeSlashEquipReplacesOccupiedWearSlot(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	itemStore := itemcatalog.NewFileStore(filepath.Join(t.TempDir(), "item-templates.json"))
	...
	if len(out) < 3 {
		t.Fatalf("expected slash occupied wear swap refresh frames, got %d", len(out))
	}
	sourceSet, err := itemproto.DecodeSet(decodeSingleFrame(t, out[0]))
	if err != nil {
		t.Fatalf("decode slash occupied wear swap source set: %v", err)
	}
	if sourceSet.Vnum != worn.Vnum || sourceSet.Position != itemproto.InventoryPosition(5) {
		t.Fatalf("unexpected slash occupied wear swap source set: %+v", sourceSet)
	}
	equipSet, err := itemproto.DecodeSet(decodeSingleFrame(t, out[1]))
	if err != nil {
		t.Fatalf("decode slash occupied wear swap equipment set: %v", err)
	}
	bodyPosition, err := itemproto.EquipmentPosition(0)
	if err != nil {
		t.Fatalf("build body equipment position: %v", err)
	}
	if equipSet.Vnum != carried.Vnum || equipSet.Position != bodyPosition {
		t.Fatalf("unexpected slash occupied wear swap equipment set: %+v", equipSet)
	}
	account, err := accounts.Load("slash-occupied-wear-equip")
	if err != nil {
		t.Fatalf("load slash occupied wear equip account: %v", err)
	}
	wantInventory := []inventory.ItemInstance{{ID: 5802, Vnum: worn.Vnum, Count: 1, Slot: 5}}
	wantEquipment := []inventory.ItemInstance{{ID: 5803, Vnum: carried.Vnum, Count: 1, Equipped: true, EquipSlot: inventory.EquipmentSlotBody}}
	if !reflect.DeepEqual(account.Characters[0].Inventory, wantInventory) || !reflect.DeepEqual(account.Characters[0].Equipment, wantEquipment) {
		t.Fatalf("slash occupied wear swap persisted unexpected state: inventory %#v equipment %#v", account.Characters[0].Inventory, account.Characters[0].Equipment)
	}
}

func TestGameRuntimeItemMoveEquipOccupiedReplaceClosesActiveExchangeShell(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	itemStore := itemcatalog.NewFileStore(filepath.Join(t.TempDir(), "item-templates.json"))
	...
	if len(out) < 4 {
		t.Fatalf("expected exchange occupied wear swap to emit END plus refresh frames, got %d", len(out))
	}
	assertExchangeEndFrame(t, out[0], "occupied wear exchange owner close")
	sourceSet, err := itemproto.DecodeSet(decodeSingleFrame(t, out[1]))
	if err != nil {
		t.Fatalf("decode exchange occupied wear swap source set: %v", err)
	}
	if sourceSet.Vnum != worn.Vnum || sourceSet.Position != itemproto.InventoryPosition(5) {
		t.Fatalf("unexpected exchange occupied wear swap source set: %+v", sourceSet)
	}
	queuedClose := flushServerFrames(t, peerFlow)
	if len(queuedClose) != 2 {
		t.Fatalf("expected occupied wear peer to receive END plus appearance update, got %d", len(queuedClose))
	}
	...
	if err != nil {
		t.Fatalf("decode occupied wear exchange peer appearance update: %v", err)
	}
	if peerUpdate.VID != owner.VID {
		t.Fatalf("unexpected occupied wear exchange peer appearance update: %+v", peerUpdate)
	}
	account, err := accounts.Load("ex-occupied-wear-equip")
	if err != nil {
		t.Fatalf("load exchange occupied wear equip account: %v", err)
	}
	wantInventory := []inventory.ItemInstance{{ID: 5804, Vnum: worn.Vnum, Count: 1, Slot: 5}}
	wantEquipment := []inventory.ItemInstance{{ID: 5805, Vnum: carried.Vnum, Count: 1, Equipped: true, EquipSlot: inventory.EquipmentSlotBody}}
	if !reflect.DeepEqual(account.Characters[0].Inventory, wantInventory) || !reflect.DeepEqual(account.Characters[0].Equipment, wantEquipment) {
		t.Fatalf("exchange occupied wear swap persisted unexpected owner state: inventory %#v equipment %#v", account.Characters[0].Inventory, account.Characters[0].Equipment)
	}
	assertExchangeAccountUnchanged(t, accounts, "ex-occupied-wear-peer", peer, "peer exchange occupied wear equip")
}

func TestGameRuntimeItemMoveEquipRejectsLockedOccupiedWearSourceWithoutMutation(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	itemStore := itemcatalog.NewFileStore(filepath.Join(t.TempDir(), "item-templates.json"))
	worn := itemcatalog.Template{Vnum: 11526, Name: "Locked Worn Test Armor", Stackable: false, MaxCount: 1, EquipSlot: inventory.EquipmentSlotBody.String()}
	carried := itemcatalog.Template{Vnum: 11527, Name: "Locked Carried Test Armor", Stackable: false, MaxCount: 1, EquipSlot: inventory.EquipmentSlotBody.String()}
	if err := itemStore.Save(itemcatalog.Snapshot{Templates: []itemcatalog.Template{worn, carried}}); err != nil {
		t.Fatalf("seed locked occupied wear equip templates: %v", err)
	}
	owner := peerVisibilityCharacter("LockedOccupiedWearEquip", 0x01030264, 0x02040264, 1300, 2300, 0, 101, 201)
	owner.Inventory = []inventory.ItemInstance{{ID: 5807, Vnum: carried.Vnum, Count: 1, Slot: 5, Locked: true}}
	owner.Equipment = []inventory.ItemInstance{{ID: 5808, Vnum: worn.Vnum, Count: 1, Equipped: true, EquipSlot: inventory.EquipmentSlotBody}}
	issuePeerTicket(t, ticketStore, "locked-occupied-wear-equip", 0x64646464, owner)
	if err := accounts.Save(accountstore.Account{Login: "locked-occupied-wear-equip", Empire: owner.Empire, Characters: cloneCharacters([]loginticket.Character{owner})}); err != nil {
		t.Fatalf("seed locked occupied wear equip account: %v", err)
	}

	runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, ticketStore, accounts, nil, nil, itemStore, nil)
	if err != nil {
		t.Fatalf("unexpected locked occupied wear equip runtime error: %v", err)
	}
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "locked-occupied-wear-equip", 0x64646464)
	defer closeSessionFlow(t, flow)
	bodyPosition, err := itemproto.EquipmentPosition(0)
	if err != nil {
		t.Fatalf("build body equipment position: %v", err)
	...
	if err != nil {
		t.Fatalf("unexpected locked occupied wear equip error: %v", err)
	}
	if len(out) != 1 {
		t.Fatalf("expected locked occupied wear equip to emit one info-chat frame, got %d", len(out))
	}
	delivery, err := chatproto.DecodeChatDelivery(decodeSingleFrame(t, out[0]))
	if err != nil {
		t.Fatalf("decode locked occupied wear equip info chat: %v", err)
	}
	if delivery.Type != chatproto.ChatTypeInfo || delivery.VID != 0 || delivery.Message != itemEquipOccupiedWearSlotInfoMessage {
		t.Fatalf("unexpected locked occupied wear equip chat: %+v", delivery)
	}
	account, err := accounts.Load("locked-occupied-wear-equip")
	if err != nil {
		t.Fatalf("load locked occupied wear equip account: %v", err)
	}
	if !reflect.DeepEqual(account.Characters[0].Inventory, owner.Inventory) || !reflect.DeepEqual(account.Characters[0].Equipment, owner.Equipment) {
		t.Fatalf("locked occupied wear equip mutated persisted state: inventory %#v equipment %#v", account.Characters[0].Inventory, account.Characters[0].Equipment)
	}
}
...
func TestGameRuntimeItemUseToItemPartialMergeRefreshCarriesTemplateDisplayMetadata(t *testing.T) {
	ticketStore := loginticket.NewFileStore(t.TempDir())
	accounts := accountstore.NewFileStore(t.TempDir())
	itemStore := itemcatalog.NewFileStore(filepath.Join(t.TempDir(), "item-templates.json"))
	if err := itemStore.Save(itemcatalog.Snapshot{Templates: []itemcatalog.Template{{
		Vnum:      27005,
		Name:      "Socketed Drag Potion",
		Stackable: true,
		MaxCount:  10,
		Sockets:   itemcatalog.SocketValues{11, -22, 33},
		Attributes: itemcatalog.AttributeValues{
		...
		t.Fatalf("seed display-metadata use-to-item template: %v", err)
	}
	owner := peerVisibilityCharacter("UseToItemAttrs", 0x01030214, 0x02040214, 1300, 2300, 0, 101, 201)
	owner.Inventory = []inventory.ItemInstance{
		{ID: 1411, Vnum: 27005, Count: 7, Slot: 5},
		{ID: 1412, Vnum: 27005, Count: 8, Slot: 6},
	}
	issuePeerTicket(t, ticketStore, "use-to-item-attrs", 0x63636364, owner)
	if err := accounts.Save(accountstore.Account{Login: "use-to-item-attrs", Empire: owner.Empire, Characters: cloneCharacters([]loginticket.Character{owner})}); err != nil {
		t.Fatalf("seed display-metadata use-to-item owner account: %v", err)
	}

	runtime, err := newGameRuntimeWithStoresAndTransferTriggersAndItemStore(config.Service{LegacyAddr: ":13000", PublicAddr: "127.0.0.1"}, ticketStore, accounts, nil, nil, itemStore, nil)
	if err != nil {
		t.Fatalf("unexpected display-metadata use-to-item runtime error: %v", err)
	}
	flow, _ := enterGameWithLoginTicket(t, runtime.SessionFactory(), "use-to-item-attrs", 0x63636364)
	defer closeSessionFlow(t, flow)
	...
	if err != nil {
		t.Fatalf("unexpected display-metadata use-to-item error: %v", err)
	}
	if len(out) != 2 {
		t.Fatalf("expected display-metadata use-to-item to emit source and target item updates only, got %d", len(out))
	}
	sourceUpdate, err := itemproto.DecodeUpdate(decodeSingleFrame(t, out[0]))
	if err != nil {
		t.Fatalf("decode display-metadata use-to-item source update: %v", err)
	}
	if sourceUpdate.Position != itemproto.InventoryPosition(5) || sourceUpdate.Count != 5 {
		t.Fatalf("unexpected display-metadata use-to-item source update: %+v", sourceUpdate)
	}
	targetUpdate, err := itemproto.DecodeUpdate(decodeSingleFrame(t, out[1]))
	if err != nil {
		t.Fatalf("decode display-metadata use-to-item target update: %v", err)
	}
	if targetUpdate.Position != itemproto.InventoryPosition(6) || targetUpdate.Count != 10 {
		t.Fatalf("unexpected display-metadata use-to-item target update: %+v", targetUpdate)
	}
	wantSockets := [itemproto.ItemSocketCount]int32{11, -22, 33}
	if sourceUpdate.Sockets != wantSockets || targetUpdate.Sockets != wantSockets {
		t.Fatalf("expected template-authored use-to-item update sockets %+v, got source %+v target %+v", wantSockets, sourceUpdate.Sockets, targetUpdate.Sockets)
	}
	wantAttributes := [itemproto.ItemAttributeCount]itemproto.Attribute{{Type: 4, Value: 55}, {Type: 9, Value: -7}}
	if sourceUpdate.Attributes != wantAttributes || targetUpdate.Attributes != wantAttributes {
		t.Fatalf("expected template-authored use-to-item update attributes %+v, got source %+v target %+v", wantAttributes, sourceUpdate.Attributes, targetUpdate.Attributes)
	}
}
...
func TestGameSessionFlowPracticeMobItemMoveFailsClosedAfterImmediateRetaliationReachesOwnerHPFloor(t *testing.T) {
	...
	if len(attackOut) != 5 {
		t.Fatalf("expected immediate retaliation floor attack to emit 5 frames before immediate zero-HP item-move denial, got %d", len(attackOut))
	}
