...
func TestGameSessionFlowPostFloorSyncPositionFailsClosed(t *testing.T) {
	...
	if queued := flushServerFrames(t, peerFlow); len(queued) != 1 {
		t.Fatalf("expected peer DEAD fanout after owner floor before sync denial, got %d", len(queued))
	}
...
func TestGameSessionFlowPostFloorSyncPositionFailsClosedBeforeRestartTown(t *testing.T) {
	...
	if queued := flushServerFrames(t, sourceFlow); len(queued) != 1 {
		t.Fatalf("expected source peer DEAD fanout after owner floor before town sync denial, got %d", len(queued))
	}
