...
func (r *gameRuntime) scheduleSpawnGroupReturnStep(entityID uint64) {
	...
	delay := bootstrapSpawnGroupReturnStepDelay
	if actor, ok := r.SpawnGroup(entityID); ok {
		delay = worldruntime.EffectiveStaticActorSpawnReturnDelay(actor.CombatProfile)
	}
	if delay <= 0 {
		return
	}
	...
	r.spawnReturnStepDueAt[entityID] = now.Add(delay)
}
...
func (r *gameRuntime) scheduleSpawnGroupHomewardStep(entityID uint64) {
	...
	delay := bootstrapSpawnGroupHomewardStepDelay
	if actor, ok := r.SpawnGroup(entityID); ok {
		delay = worldruntime.EffectiveStaticActorSpawnHomewardDelay(actor.CombatProfile)
	}
	if delay <= 0 {
		return
	}
	...
	r.spawnHomewardStepDueAt[entityID] = now.Add(delay)
}
...
func newGameRuntimeWithStoresAndTransferTriggersAndItemAndQuestStore(cfg config.Service, store loginticket.Store, accounts accountstore.Store, staticActors staticstore.Store, interactions interactionstore.Store, items itemcatalog.Store, questState queststate.Store, transferTriggers []bootstrapTransferTrigger) (*gameRuntime, error) {
	...
	runtime.sessionFactory = func() service.SessionFlow {
		...
		clearActiveMyShopOpen := func() {
			...
			if joinedSharedWorld && sharedWorld != nil && sharedWorldID != 0 {
				sharedWorld.SetMyShopWindowOpen(sharedWorldID, false, "", nil)
			}
		}
		setActiveMyShopOpen := func(sign string, stock []myShopStockRow) {
		...
		executeSelectedItemUse := func(position itemproto.Position, emitUseEcho bool) gameflow.ItemUseResult {
			...
			if position.Cell >= itemproto.InventoryMaxCell {
				...
				if wearIndex != player.Unique1ItemUseWearIndex && wearIndex != player.Unique2ItemUseWearIndex {
					return gameflow.ItemUseResult{Accepted: false}
				}
			}
			slot := inventory.SlotIndex(position.Cell)
			if !player.IsOwnedWornItemUseCell(slot) {
			...
			if useResult.ItemRemoved && !player.IsOwnedWornItemUseCell(useResult.Slot) {
		...
		inner := boot.NewFlow(boot.Config{
			...
			Game: gameflow.Config{
				...
				HandleChat: func(packet chatproto.ClientChatPacket) gameflow.ChatResult {
					...
					if packet.Type == chatproto.ChatTypeTalking {
						...
						if slashCubeMakeCommand(packet.Message) {
							...
							fields := strings.Fields(strings.TrimSpace(packet.Message[1:]))
							makeAll := len(fields) == 3 && fields[2] == "all"
							if len(fields) != 2 && !makeAll {
								// Extra /cube make args stay recognized fail-closed consume.
								return gameflow.ChatResult{Accepted: true}
							}
							attemptCubeMake := func() (frames [][]byte, continueLoop bool) {
								...
								if failure := selectedPlayer.ValidateCarriedItemGrant(rewardTemplate, recipe.Reward.Count); failure != "" {
									if failure == player.CarriedItemGrantFailureNoValidPlacement {
										return [][]byte{chatproto.EncodeChatDelivery(chatproto.ChatDeliveryPacket{
											Type:    chatproto.ChatTypeInfo,
											Message: itemPickupInventoryFullInfoMessage,
										})}, false
									}
									return nil, false
								}
								...
								return committed, craftSucceeded
							}
							if !makeAll {
								frames, _ := attemptCubeMake()
								return gameflow.ChatResult{Accepted: true, Frames: frames}
							}
							var allFrames [][]byte
							for attempt := 0; attempt < int(inventory.CarriedInventorySlotCount); attempt++ {
								frames, continueLoop := attemptCubeMake()
								allFrames = append(allFrames, frames...)
								if !continueLoop {
									break
								}
							}
							return gameflow.ChatResult{Accepted: true, Frames: allFrames}
						}
				...
				HandleOnClick: func(packet combatproto.ClientOnClickPacket) gameflow.OnClickResult {
					stateMu.Lock()
					defer stateMu.Unlock()
					...
					frames, started := sharedWorld.OpenMyShopGuestBrowse(sharedWorldID, packet.VID)
					if len(frames) == 0 {
						return gameflow.OnClickResult{Accepted: false}
					}
					if started {
...
func slashCloseMyShopCommand(message string) bool {
	if !strings.HasPrefix(message, "/") {
		return false
	}
	fields := strings.Fields(strings.TrimSpace(message[1:]))
...
// (other extra args stay recognized so the handler can fail closed without talking-chat
// fallthrough). Exact one-attempt make is `fields == [cube make]`; make-all is
// `fields == [cube make all]`.
...
func itemUseResultFrames(character loginticket.Character, result player.ItemUseResult, templates map[uint32]itemcatalog.Template, emitUseEcho bool) ([][]byte, error) {
	...
	} else if player.IsOwnedWornItemUseCell(result.Slot) {
...
func itemUseResultPosition(slot inventory.SlotIndex) (itemproto.Position, error) {
	if wearIndex, ok := player.OwnedWornItemUseWearIndex(slot); ok {
		return itemproto.EquipmentPosition(wearIndex)
	}
...
func refineKeepFailureResultFrames(character loginticket.Character, result player.RefineKeepFailureResult, templates map[uint32]itemcatalog.Template, refineType uint8) ([][]byte, error) {
	...
	if result.GoldBefore < result.Gold || result.GoldBefore-result.Gold != uint64(result.Cost) || result.Gold > uint64(math.MaxInt32) || result.Cost < 0 {
		return nil, fmt.Errorf("refine keep-failure gold point-change out of range")
	}
	frames = append(frames, worldproto.EncodePlayerPointChange(worldproto.PlayerPointChangePacket{
		VID:    character.VID,
		Type:   bootstrapGoldPointType,
...
func (r *gameRuntime) resolveRuntimeUseTemplate(selectedPlayer *player.Runtime, slot inventory.SlotIndex) (itemcatalog.Template, bool) {
	if player.IsOwnedWornItemUseCell(slot) {
		return r.resolveRuntimeWornCellUseTemplate(selectedPlayer, slot)
	}
...
func (r *gameRuntime) resolveRuntimeWornCellUseTemplate(selectedPlayer *player.Runtime, slot inventory.SlotIndex) (itemcatalog.Template, bool) {
	if r == nil || selectedPlayer == nil {
		return itemcatalog.Template{}, false
	}
	equipSlot, ok := player.OwnedWornItemUseEquipSlot(slot)
	if !ok {
		return itemcatalog.Template{}, false
	}
	for _, item := range selectedPlayer.LiveEquipment() {
...
func (r *gameRuntime) replaceRegenRespawnDelayMs(delays map[string]int64) {
	if r == nil || r.sharedWorld == nil {
		return
	}
	r.sharedWorld.replaceRegenRespawnDelayMs(delays)
}

func (r *gameRuntime) regenRespawnDelayMsSnapshot() map[string]int64 {
	if r == nil || r.sharedWorld == nil {
		return nil
	}
