...
func Run(args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) int {
	...
	case "export-tree-status-status":
		return runExportTreeStatusStatus(args[1:], stdout, stderr)
	...
	case "persistence-status-status":
		return runPersistenceStatusStatus(args[1:], stdout, stderr)
...
func printUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  export-tree-status-status inspect a retained export-tree-status snapshot without walking the original tree")
	...
	fmt.Fprintln(w, "  persistence-status-status inspect a retained GET /local/persistence/status snapshot without curling the live mux")
	...
	fmt.Fprintln(w, "")
	printExportTreeStatusStatusUsage(w)
	fmt.Fprintln(w, "")
	...
	fmt.Fprintln(w, "")
	printPersistenceStatusStatusUsage(w)
	fmt.Fprintln(w, "")
