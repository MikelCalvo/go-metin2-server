...
func TestRunImportExportRejectsUsageErrorsWithoutOpeningDatabase(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			want: "--i-confirm-scoped-replace is only supported for kind account-character-roster, character-item-state, character-quest-state, character-point-state, character-safebox-state, character-myshop-unit-prices, bootstrap-ground-item-state, item-template-state, or static-actor-content-state",
		},
		{
