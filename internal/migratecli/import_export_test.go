...
func TestRunImportExportImportsEmptyExportsAgainstRegisteredDriver(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.kind, func(t *testing.T) {
			...
			// requires additive 0026 instance sockets, 0029 instance attributes, and 0030
			// ownership timers before INSERT.
			...
			if tc.kind == "bootstrap-ground-item-state" {
				ledger = []dbmigrations.LedgerEntry{
					...
					ledgerEntry(worldruntime.BootstrapGroundItemOwnershipTimerMigrationVersion),
				}
			}
...
func TestRunImportExportStaticActorContentStateScopedReplaceSetsReplaced(t *testing.T) {
	driverName := registerMigrateCLITestSQLDriver(t)
	catalog, err := dbmigrations.Catalog()
	if err != nil {
		t.Fatalf("load catalog: %v", err)
	...
	code := Run(
		[]string{
			...
			"--export", "-",
			"--driver", driverName,
			"--dsn", "memory://import-static-actor-content-state-replace",
			"--i-confirm-sql-import",
			"--i-confirm-scoped-replace",
		},
		strings.NewReader(`{"migration_version":13,"migration_name":"static_actor_combat_profile_state","entity_ids":[],"interaction_definition_keys":[],"combat_profile_names":[],"interaction_definitions":[],"merchant_catalog_entries":[],"quest_flag_reward_items":[],"quest_flag_consume_items":[],"static_actors":[],"reward_drops":[],"combat_profiles":[],"combat_profile_death_reward_drops":[]}`),
		&stdout,
		&stderr,
	)
	if code != exitOK {
	...
	if result.MigrationVersion != 13 || result.MigrationName != "static_actor_combat_profile_state" {
		t.Fatalf("unexpected migration identity: %#v", result)
	}
	if !strings.Contains(stdout.String(), `"replaced": true`) {
		t.Fatalf("expected replaced field in stdout JSON, got %q", stdout.String())
...
func TestRunImportExportAuthLoginTicketHandoffScopedReplaceSetsReplaced(t *testing.T) {
	driverName := registerMigrateCLITestSQLDriver(t)
	catalog, err := dbmigrations.Catalog()
	if err != nil {
		t.Fatalf("load catalog: %v", err)
	...
	currentMigrateCLITestDriver(t).setLedger([]dbmigrations.LedgerEntry{
		ledgerEntry(loginticket.AuthLoginTicketHandoffMigrationVersion),
	})
	...
	code := Run(
		[]string{
			...
			"--export", "-",
			"--driver", driverName,
			"--dsn", "memory://import-auth-login-ticket-handoff-replace",
			"--i-confirm-sql-import",
			"--i-confirm-scoped-replace",
		},
		strings.NewReader(`{"migration_version":7,"migration_name":"auth_login_ticket_handoff","login_keys":[],"tickets":[]}`),
		&stdout,
		&stderr,
	)
	if code != exitOK {
	...
	if result.MigrationVersion != 7 || result.MigrationName != "auth_login_ticket_handoff" {
		t.Fatalf("unexpected migration identity: %#v", result)
	}
	if !strings.Contains(stdout.String(), `"replaced": true`) {
		t.Fatalf("expected replaced field in stdout JSON, got %q", stdout.String())
