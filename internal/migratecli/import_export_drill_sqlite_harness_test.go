...
func TestImportExportDrillSQLiteHermeticPrintedScriptTwoPhaseWipeRosterReimportsSeededTipKinds(t *testing.T) {
	...
	for _, name := range []string{"export-tree-status-before.json", "export-tree-status-after.json"} {
		...
		if name == "export-tree-status-after.json" && !treeStatus.WipeImportResultOutcomesComplete {
			t.Fatalf("export-tree-status-after.json must report wipe_import_result_outcomes_complete=true, got %#v", treeStatus)
		}
		if name == "export-tree-status-after.json" && !treeStatus.WipeImportResultAllReplaced {
			t.Fatalf("export-tree-status-after.json must report wipe_import_result_all_replaced=true, got %#v", treeStatus)
		}
