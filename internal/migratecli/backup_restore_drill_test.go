...
func validBackupRestoreRuntimeConfig() string {
  ...
  "persistence": {
    ...
    "cube_recipe_store_path": "/var/metin2/cube-recipes/cube-recipes.json"
  },
  "database": {
    "configured": false,
...
func TestRunBackupRestoreDrillPrintsLabRetentionCommands(t *testing.T) {
  ...
  "build_date": "2026-08-21T15:30:45Z"
	...
	for _, want := range []string{
		...
		`mkdir -p "$BASE"/accounts "$BASE"/login-tickets "$BASE"/item-templates "$BASE"/interaction-store "$BASE"/static-actors "$BASE"/quest-state "$BASE"/ground-items "$BASE"/safebox "$BASE"/cube-recipes`,
		...
		`"$OPS/local/cube-recipe-store/validate"`,
		`"$OPS/local/cube-recipe-store/crash-temps/cleanup"`,
	...
	idxAuthdLog := strings.Index(body, `cp -p "$AUTHD_LOG" "$BASE/authd.log"`)
...
func TestRunBackupRestoreDrillPrintsCubeRecipeStoreBackupRestoreEndpoints(t *testing.T) {
	payload := `{
  "local_channel_id": 1,
  "visibility_mode": "whole_map",
  "visibility_radius": 0,
...
func TestRunBackupRestoreDrillRejectsSharedFileStoreParents(t *testing.T) {
  ...
  "persistence": {
    ...
    "cube_recipe_store_path": "/tmp/cube-recipes/cube-recipes.json"
  },
  "database": {"configured": false, "dsn_configured": false}
}`
...
func TestRunBackupRestoreDrillRejectsMalformedAndOversizedInput(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "unknown-field", payload: `{"local_channel_id":1,"visibility_mode":"whole_map","visibility_radius":0,"visibility_sector_size":0,"persistence":{"login_ticket_store_dir":"/a","account_store_dir":"/b","static_actor_store_path":"/c/s.json","interaction_store_path":"/d/i.json","item_template_store_path":"/e/t.json","quest_state_store_path":"/f/q.json","ground_item_store_path":"/g/gi.json","safebox_store_path":"/h/s.json","cube_recipe_store_path":"/i/c.json"},"database":{"configured":false,"dsn_configured":false},"extra":true}`},
...
func TestRunBackupRestoreDrillHonorsCustomDaemonLogPaths(t *testing.T) {
	buildInfoPath := writeTempJSON(t, "build-info.json", `{
  "version": "v0.1.0",
  "commit": "abcdef0123456789deadbeef",
  "build_date": "2026-08-21T15:30:45Z"
	...
	for _, want := range []string{
		...
		`cp -p "$GAMED_LOG" "$BASE/gamed.log"`,
		`cp -p "$AUTHD_LOG" "$BASE/authd.log"`,
	} {
		if !strings.Contains(body, want) {
			t.Fatalf("expected %q in stdout:\n%s", want, body)
		}
...
func TestRunBackupRestoreDrillRejectsRelativeDaemonLogPaths(t *testing.T) {
	buildInfoPath := writeTempJSON(t, "build-info.json", `{
  "version": "v0.1.0",
  "commit": "abcdef0123456789deadbeef",
  "build_date": "2026-08-21T15:30:45Z"
