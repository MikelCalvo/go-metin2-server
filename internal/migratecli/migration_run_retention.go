...
func renderMigrationRunRetentionScript(plan migrationRunRetentionPlan) string {
	...
	b.WriteString(`  --persistence-status "$RUN/persistence-status-before.json" \` + "\n")
	b.WriteString(`  > "$RUN/persistence-status-before-status.json"` + "\n")
	...
	b.WriteString(`metin2-migrate drivers \` + "\n")
	b.WriteString(`  --require-driver "$DRIVER" \` + "\n")
	b.WriteString(`  > "$RUN/sql-drivers.json"` + "\n")
	...
	b.WriteString(`  > "$RUN/persistence-status-after-status.json"` + "\n")
	b.WriteString("\n")
	...
	b.WriteString(`# Successful apply removes the lock; do not fail the runbook script on that path.` + "\n")
	b.WriteString(`if [ -e "$RUN/$LOCK_FILE" ]; then` + "\n")
