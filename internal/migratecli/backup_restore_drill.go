...
func renderBackupRestoreDrillScript(plan backupRestoreDrillPlan) string {
	...
	b.WriteString("  --require-stores-complete \\\n")
	b.WriteString("  --require-no-crash-temps \\\n")
	b.WriteString("  > \"$BASE/backup-tree-status.json\"\n")
	b.WriteString("metin2-migrate backup-tree-status-status --backup-tree-status \"$BASE/backup-tree-status.json\" \\\n")
	b.WriteString("  --require-stores-complete \\\n")
	b.WriteString("  --require-no-crash-temps \\\n")
	b.WriteString("  > \"$BASE/backup-tree-status-status.json\"\n")
	b.WriteString("\n")
