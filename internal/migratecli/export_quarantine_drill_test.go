...
func TestRunExportQuarantineDrillPrintsLabRetentionCommands(t *testing.T) {
  ...
  "build_date": "2026-08-25T12:00:00Z"
	...
	for _, want := range []string{
		...
		`mkdir -p "$BASE"/account-character-roster "$BASE"/character-item-state "$BASE"/character-point-state "$BASE"/auth-login-ticket-handoff "$BASE"/character-quest-state "$BASE"/character-safebox-state "$BASE"/item-template-state "$BASE"/static-actor-content-state "$BASE"/bootstrap-ground-item-state`,
