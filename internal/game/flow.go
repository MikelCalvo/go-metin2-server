...
type HandleUseSkillFunc func(combatproto.ClientUseSkillPacket) UseSkillResult
...
type SafeboxCheckinResult struct {
	Accepted bool
	Frames   [][]byte
}
...
func NewFlow(machine *session.StateMachine, cfg Config) *Flow {
	...
	useSkillHandler := cfg.HandleUseSkill
	if useSkillHandler == nil {
		useSkillHandler = func(combatproto.ClientUseSkillPacket) UseSkillResult { return UseSkillResult{Accepted: false} }
	}
	...
	if characterPositionHandler == nil {
		characterPositionHandler = func(combatproto.ClientCharacterPositionPacket) CharacterPositionResult {
			return CharacterPositionResult{Accepted: false}
		}
	}
	...
	safeboxCheckinHandler := cfg.HandleSafeboxCheckin
	if safeboxCheckinHandler == nil {
		safeboxCheckinHandler = func(itemproto.ClientSafeboxCheckinPacket) SafeboxCheckinResult {
...
func (f *Flow) HandleClientFrame(in frame.Frame) ([][]byte, error) {
	...
	case combatproto.HeaderClientAttack:
		...
		return result.Frames, nil
	case combatproto.HeaderClientUseSkill:
		packet, err := combatproto.DecodeClientUseSkill(in)
		if err != nil {
			return nil, err
		}
		result := f.handleUseSkill(packet)
		if !result.Accepted {
			return nil, nil
		}
		return result.Frames, nil
	...
	case combatproto.HeaderClientOnClick:
		...
		return result.Frames, nil
	case combatproto.HeaderClientCharacterPosition:
		packet, err := combatproto.DecodeClientCharacterPosition(in)
		if err != nil {
			return nil, err
		}
		result := f.handleCharacterPosition(packet)
		if !result.Accepted {
			return nil, nil
		}
		return result.Frames, nil
	...
	case itemproto.HeaderClientExchange:
		...
		return result.Frames, nil
	case itemproto.HeaderClientRefine:
		packet, err := itemproto.DecodeClientRefine(in)
		if err != nil {
			return nil, err
		}
		result := f.handleItemRefine(packet)
		if !result.Accepted {
			return nil, nil
		}
		return result.Frames, nil
	case itemproto.HeaderClientSafeboxCheckin:
		packet, err := itemproto.DecodeClientSafeboxCheckin(in)
		if err != nil {
			return nil, err
		}
		result := f.handleSafeboxCheckin(packet)
		if !result.Accepted {
			return nil, nil
		}
		return result.Frames, nil
