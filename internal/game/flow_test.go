...
func TestHandleClientFrameAcceptsSafeboxCheckoutInGameAndReturnsHandlerFrames(t *testing.T) {
	machine := session.NewStateMachineAt(session.PhaseGame)
	wantFrame := []byte("safebox-checkout")
	flow := NewFlow(machine, Config{
		HandleSafeboxCheckout: func(packet itemproto.ClientSafeboxCheckoutPacket) SafeboxCheckoutResult {
			if packet.SafeSlot != 8 || packet.Position != itemproto.InventoryPosition(6) {
				t.Fatalf("unexpected safebox checkout packet: %+v", packet)
			}
			return SafeboxCheckoutResult{Accepted: true, Frames: [][]byte{wantFrame}}
		},
	})

	out, err := flow.HandleClientFrame(decodeSingleFrame(t, itemproto.EncodeClientSafeboxCheckout(itemproto.ClientSafeboxCheckoutPacket{SafeSlot: 8, Position: itemproto.InventoryPosition(6)})))
	if err != nil {
		t.Fatalf("unexpected safebox checkout error: %v", err)
	}
	if len(out) != 1 || !bytes.Equal(out[0], wantFrame) {
		t.Fatalf("expected handler safebox-checkout frame, got %#v", out)
	}
	if machine.Current() != session.PhaseGame {
		t.Fatalf("expected phase %q, got %q", session.PhaseGame, machine.Current())
	}
...
func TestHandleClientFrameAcceptsSafeboxItemMoveInGameAndReturnsHandlerFrames(t *testing.T) {
	machine := session.NewStateMachineAt(session.PhaseGame)
	wantFrame := []byte("safebox-item-move")
	flow := NewFlow(machine, Config{
		HandleSafeboxItemMove: func(packet itemproto.ClientSafeboxItemMovePacket) SafeboxItemMoveResult {
			if packet.Source != itemproto.InventoryPosition(7) || packet.Destination != itemproto.InventoryPosition(8) || packet.Count != 3 {
				t.Fatalf("unexpected safebox item-move packet: %+v", packet)
			}
			return SafeboxItemMoveResult{Accepted: true, Frames: [][]byte{wantFrame}}
		},
	})

	out, err := flow.HandleClientFrame(decodeSingleFrame(t, itemproto.EncodeClientSafeboxItemMove(itemproto.ClientSafeboxItemMovePacket{Source: itemproto.InventoryPosition(7), Destination: itemproto.InventoryPosition(8), Count: 3})))
	if err != nil {
		t.Fatalf("unexpected safebox item-move error: %v", err)
	}
	if len(out) != 1 || !bytes.Equal(out[0], wantFrame) {
		t.Fatalf("expected handler safebox-item-move frame, got %#v", out)
	}
	if machine.Current() != session.PhaseGame {
		t.Fatalf("expected phase %q, got %q", session.PhaseGame, machine.Current())
	}
...
func TestHandleClientFrameAcceptsMallCheckoutInGameAndReturnsHandlerFrames(t *testing.T) {
	machine := session.NewStateMachineAt(session.PhaseGame)
	wantFrame := []byte("mall-checkout")
	flow := NewFlow(machine, Config{
		HandleMallCheckout: func(packet itemproto.ClientMallCheckoutPacket) MallCheckoutResult {
			if packet.MallSlot != 4 || packet.Position != itemproto.InventoryPosition(9) {
				t.Fatalf("unexpected mall checkout packet: %+v", packet)
			}
			return MallCheckoutResult{Accepted: true, Frames: [][]byte{wantFrame}}
		},
	})

	out, err := flow.HandleClientFrame(decodeSingleFrame(t, itemproto.EncodeClientMallCheckout(itemproto.ClientMallCheckoutPacket{MallSlot: 4, Position: itemproto.InventoryPosition(9)})))
	if err != nil {
		t.Fatalf("unexpected mall checkout error: %v", err)
	}
	if len(out) != 1 || !bytes.Equal(out[0], wantFrame) {
		t.Fatalf("expected handler mall-checkout frame, got %#v", out)
	}
	if machine.Current() != session.PhaseGame {
		t.Fatalf("expected phase %q, got %q", session.PhaseGame, machine.Current())
	}
...
func TestHandleClientFrameAcceptsAttackInGameAndReturnsFrames(t *testing.T) {
	machine := session.NewStateMachineAt(session.PhaseGame)
	expected := combatproto.EncodeServerTarget(combatproto.ServerTargetPacket{TargetVID: 0x02040107, HPPercent: 100})
	invoked := false
	flow := NewFlow(machine, Config{
		HandleAttack: func(packet combatproto.ClientAttackPacket) AttackResult {
			invoked = true
			if packet.AttackType != combatproto.ClientAttackTypeNormal || packet.TargetVID != 0x02040107 || packet.CRCProcPiece != 0x12 || packet.CRCFilePiece != 0x34 {
				t.Fatalf("unexpected attack packet: %+v", packet)
			}
			return AttackResult{Accepted: true, Frames: [][]byte{expected}}
		},
	})
	...
	if !invoked {
		t.Fatal("expected attack handler to be invoked")
	}
	if len(out) != 1 {
		t.Fatalf("expected 1 outgoing attack frame, got %d", len(out))
	}
	if !bytes.Equal(out[0], expected) {
		t.Fatalf("unexpected attack frames: got %x want %x", out[0], expected)
	}
	target, err := combatproto.DecodeServerTarget(decodeSingleFrame(t, out[0]))
	if err != nil {
		t.Fatalf("decode attack target-refresh frame: %v", err)
	}
	if target.TargetVID != 0x02040107 || target.HPPercent != 100 {
		t.Fatalf("unexpected attack target-refresh payload: %+v", target)
	}
	if machine.Current() != session.PhaseGame {
		t.Fatalf("expected phase %q, got %q", session.PhaseGame, machine.Current())
	}
...
func TestHandleClientFrameAcceptsCharacterPositionInGameAndReturnsHandlerFrames(t *testing.T) {
	machine := session.NewStateMachineAt(session.PhaseGame)
	wantFrame := control.EncodePing(control.PingPacket{ServerTime: 0x01020304})
	flow := NewFlow(machine, Config{
		HandleCharacterPosition: func(packet combatproto.ClientCharacterPositionPacket) CharacterPositionResult {
			if packet.Position != 1 {
				t.Fatalf("unexpected character-position packet: %+v", packet)
			}
			return CharacterPositionResult{Accepted: true, Frames: [][]byte{wantFrame}}
		},
	})

	out, err := flow.HandleClientFrame(decodeSingleFrame(t, combatproto.EncodeClientCharacterPosition(combatproto.ClientCharacterPositionPacket{Position: 1})))
	if err != nil {
		t.Fatalf("unexpected character-position error: %v", err)
	}
	if len(out) != 1 || !bytes.Equal(out[0], wantFrame) {
		t.Fatalf("expected handler character-position frame, got %#v", out)
	}
	if machine.Current() != session.PhaseGame {
		t.Fatalf("expected phase %q, got %q", session.PhaseGame, machine.Current())
	}
...
func TestHandleClientFrameCharacterPositionWithoutHandlerIsNoOp(t *testing.T) {
	machine := session.NewStateMachineAt(session.PhaseGame)
	flow := NewFlow(machine, Config{})
	out, err := flow.HandleClientFrame(decodeSingleFrame(t, combatproto.EncodeClientCharacterPosition(combatproto.ClientCharacterPositionPacket{Position: 1})))
	if err != nil {
		t.Fatalf("unexpected character-position error without handler: %v", err)
	}
	if len(out) != 0 {
		t.Fatalf("expected no outgoing character-position frames without handler, got %d", len(out))
	}
	if machine.Current() != session.PhaseGame {
		t.Fatalf("expected phase %q, got %q", session.PhaseGame, machine.Current())
	}
...
func TestHandleClientFrameRejectsMalformedAttackInGame(t *testing.T) {
	machine := session.NewStateMachineAt(session.PhaseGame)
	flow := NewFlow(machine, Config{})
...
func TestHandleClientFrameRejectsMalformedCharacterPositionInGame(t *testing.T) {
	machine := session.NewStateMachineAt(session.PhaseGame)
	flow := NewFlow(machine, Config{})
	_, err := flow.HandleClientFrame(frame.Frame{Header: combatproto.HeaderClientCharacterPosition, Length: 6, Payload: []byte{0x01, 0x02}})
	if !errors.Is(err, combatproto.ErrInvalidPayload) {
		t.Fatalf("expected combatproto.ErrInvalidPayload, got %v", err)
	}
	if machine.Current() != session.PhaseGame {
