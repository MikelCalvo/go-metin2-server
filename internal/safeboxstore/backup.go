...
const (
	BackupManifestFilename = "safebox-backup-manifest.json"
	BackupManifestFormat   = "go-metin2-safebox-backup-v1"
)

var (
	ErrBackupDirRequired      = errors.New("safebox backup dir is required")
	ErrBackupDirNotEmpty      = errors.New("safebox backup dir is not empty")
	ErrBackupDirInsideStore   = errors.New("safebox backup dir is inside safebox store")
	ErrRestoreSourceRequired  = errors.New("safebox restore source dir is required")
	ErrRestoreSourceNotFound  = errors.New("safebox restore source dir not found")
	ErrRestoreDirNotEmpty     = errors.New("safebox restore dir is not empty")
	ErrRestoreDirInsideSource = errors.New("safebox restore dir is inside safebox backup source")
	ErrBackupManifestRequired = errors.New("safebox backup manifest is required")
	ErrInvalidBackupManifest  = errors.New("invalid safebox backup manifest")
)
...
func (s *FileStore) BackupTo(dstDir string) error {
	...
	if err := ensureEmptyDir(dstDir, ErrBackupDirNotEmpty, "read safebox backup dir"); err != nil {
		return err
	}
	...
	if err := os.MkdirAll(dstDir, 0o755); err != nil {
		return fmt.Errorf("create safebox backup dir: %w", err)
	}
	...
	if hasSnapshot {
		...
		if err := backup.Save(snapshot); err != nil {
			return s.rollbackBackupFailure(dstDir, committedSnapshot, fmt.Errorf("backup safebox snapshot: %w", err))
		}
	}
	...
	if err := s.syncStoreDir(dstDir); err != nil {
		return s.rollbackBackupFailure(dstDir, committedSnapshot, fmt.Errorf("sync safebox backup dir: %w", err))
	}
	return nil
}
...
func (s *FileStore) rollbackBackupFailure(dstDir string, snapshotCommitted bool, backupErr error) error {
	...
	if snapshotCommitted {
		if err := os.Remove(filepath.Join(dstDir, filepath.Base(s.path))); err != nil && !errors.Is(err, os.ErrNotExist) {
			rollbackErrs = append(rollbackErrs, fmt.Errorf("remove backup safebox snapshot: %w", err))
		}
	}
	if err := os.Remove(filepath.Join(dstDir, BackupManifestFilename)); err != nil && !errors.Is(err, os.ErrNotExist) {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("remove safebox backup manifest: %w", err))
	}
	if err := s.syncStoreDir(dstDir); err != nil {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("sync safebox backup rollback dir: %w", err))
	}
...
func writeBackupManifest(dir string, snapshotFilename string, summary SnapshotSummary, hasSnapshot bool) error {
	...
	if hasSnapshot {
		...
		if err != nil {
			return fmt.Errorf("read safebox backup snapshot for manifest: %w", err)
		}
	...
	return writeJSONFileAtomically(dir, BackupManifestFilename, manifest, "safebox backup manifest")
}
...
func (s *FileStore) RestoreFrom(srcDir string) error {
	...
	if err := ensureEmptyDir(storeDir, ErrRestoreDirNotEmpty, "read safebox restore dir"); err != nil {
		return err
	}
	...
	if err := os.MkdirAll(storeDir, 0o755); err != nil {
		return fmt.Errorf("create safebox restore dir: %w", err)
	}

	committedSnapshot := false
	if hasSnapshot {
		if err := s.Save(snapshot); err != nil {
			return s.rollbackRestoreFailure(true, fmt.Errorf("restore safebox snapshot: %w", err))
		}
		committedSnapshot = true
	}
	...
	if err := s.syncStoreDir(storeDir); err != nil {
		return s.rollbackRestoreFailure(committedSnapshot, fmt.Errorf("sync safebox restore dir: %w", err))
	}
	return nil
}
...
func (s *FileStore) rollbackRestoreFailure(snapshotCommitted bool, restoreErr error) error {
	...
	if snapshotCommitted {
		if err := os.Remove(s.path); err != nil && !errors.Is(err, os.ErrNotExist) {
			rollbackErrs = append(rollbackErrs, fmt.Errorf("remove restored safebox snapshot: %w", err))
		}
	}
	if err := os.Remove(filepath.Join(storeDir, BackupManifestFilename)); err != nil && !errors.Is(err, os.ErrNotExist) {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("remove restored safebox backup manifest: %w", err))
	}
	if err := s.syncStoreDir(storeDir); err != nil {
		rollbackErrs = append(rollbackErrs, fmt.Errorf("sync safebox restore rollback dir: %w", err))
	}
	if len(rollbackErrs) == 0 {
		return restoreErr
	}
...
func (s *FileStore) loadBackupSnapshotForRestore(srcDir string) (SnapshotSummary, Snapshot, bool, error) {
	...
	if _, err := os.Stat(srcDir); err != nil {
		...
		return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("stat safebox restore source dir: %w", err)
	}
...
func (s *FileStore) validateActiveBackupManifest() error {
	...
	if _, err := os.Lstat(manifestPath); err != nil {
		...
		return fmt.Errorf("stat active safebox backup manifest: %w", err)
	}
	...
	if _, err := os.Stat(s.path); err == nil && !hasSnapshot {
		return fmt.Errorf("%w: active manifest omits committed safebox snapshot", ErrInvalidBackupManifest)
	} else if err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("stat active safebox snapshot for backup manifest: %w", err)
	}
	return nil
}
...
func (s *FileStore) validateBackupManifestWithCoverage(srcDir string, requireClosedDirectory bool) (SnapshotSummary, Snapshot, bool, error) {
	...
	if err := rejectBackupEntrySymlink(manifestPath, "safebox backup manifest"); err != nil {
		return SnapshotSummary{}, Snapshot{}, false, err
	}
	...
	if err != nil {
		...
		return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("read safebox backup manifest: %w", err)
	}
	...
	if len(manifest.Files) == 0 {
		...
		if file.Filename != filepath.Base(s.path) {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: manifest filename %q does not match safebox snapshot filename", ErrInvalidBackupManifest, file.Filename)
		}
		committedFiles[file.Filename] = struct{}{}
		snapshotPath := filepath.Join(srcDir, file.Filename)
		if err := rejectBackupEntrySymlink(snapshotPath, fmt.Sprintf("safebox backup snapshot %q", file.Filename)); err != nil {
			return SnapshotSummary{}, Snapshot{}, false, err
		}
		rawSnapshot, err := os.ReadFile(snapshotPath)
		if err != nil {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: read manifest safebox snapshot: %v", ErrInvalidBackupManifest, err)
		}
		if int64(len(rawSnapshot)) != file.SizeBytes {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: safebox snapshot size mismatch", ErrInvalidBackupManifest)
		}
		checksum := sha256.Sum256(rawSnapshot)
		if got := hex.EncodeToString(checksum[:]); got != file.SHA256 {
			return SnapshotSummary{}, Snapshot{}, false, fmt.Errorf("%w: safebox snapshot checksum mismatch", ErrInvalidBackupManifest)
		}
...
func validateBackupDirectoryEntries(srcDir string, manifestFiles map[string]struct{}) error {
	...
	if err != nil {
		return fmt.Errorf("read safebox backup dir for manifest coverage: %w", err)
	}
	for _, entry := range entries {
		...
		if strings.HasPrefix(name, safeboxCrashTempPrefix) && strings.HasSuffix(name, safeboxCrashTempSuffix) {
			continue
		}
...
func summaryWithoutCrashTemps(summary SnapshotSummary) SnapshotSummary {
	...
	if summary.CharacterKeys == nil {
		summary.CharacterKeys = []string{}
	}
	return summary
}
...
func snapshotSummariesEqual(a, b SnapshotSummary) bool {
	...
	if a.CharacterCount != b.CharacterCount || a.CellCount != b.CellCount || len(a.Logins) != len(b.Logins) || len(a.CharacterKeys) != len(b.CharacterKeys) {
		return false
	}
	...
	for i := range a.CharacterKeys {
		if a.CharacterKeys[i] != b.CharacterKeys[i] {
			return false
		}
	}
	return true
...
func writeJSONFileAtomically(dir, filename string, value any, context string) error {
	temp, err := os.CreateTemp(dir, safeboxCrashTempPrefix+"*"+safeboxCrashTempSuffix)
	if err != nil {
...
func rejectBackupDestinationInsideStore(storeDir string, dstDir string) error {
	return rejectPathInsideOrEqual(storeDir, dstDir, ErrBackupDirInsideStore, "safebox store", "safebox backup")
}

func rejectRestoreDestinationInsideSource(srcDir string, storeDir string) error {
	return rejectPathInsideOrEqual(srcDir, storeDir, ErrRestoreDirInsideSource, "safebox restore source", "safebox restore")
}
...
func crashTempFilesInDir(dir string, committedFilename string) ([]string, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("read safebox store crash temp files: %w", err)
	}
	files := make([]string, 0)
	for _, entry := range entries {
		name := entry.Name()
		...
		if strings.HasPrefix(name, safeboxCrashTempPrefix) && strings.HasSuffix(name, safeboxCrashTempSuffix) {
			if entry.Type()&os.ModeSymlink != 0 {
				return nil, fmt.Errorf("%w: safebox crash temp file %q is a symlink", ErrInvalidSnapshot, name)
			}
			if entry.IsDir() {
				continue
			}
