...
const (
	...
	expectedBootstrapGroundItemInstanceAttributesStatusSHA256  = "c7afca9b9e68e99926b644298df476dfb1bed88667246d0783d8b0a6f8c9df31"
)
...
func TestLocalMapOccupancySpawnGroupChaseStepsEndpointReturnsMapLocalPendingStepsForLoopbackGet(t *testing.T) {
	lookup := &stubMapSpawnGroupChaseStepsLookup{snapshots: map[uint32]any{
		42: []map[string]any{{"entity_id": uint64(55), "remaining_ms": int64(5000), "actor": map[string]any{"entity_id": uint64(55), "spawn_group_ref": "practice.chase_step"}, "step": map[string]any{"next": map[string]any{"map_index": uint32(42), "x": int32(1800)}}}},
	}}
	mux := RegisterLocalMapSpawnGroupChaseStepsEndpoint(NewPprofMux("gamed"), lookup.MapSpawnGroupChaseSteps)

	req := httptest.NewRequest(http.MethodGet, "/local/maps/42/spawn-group-chase-steps", nil)
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
	...
	if lookup.calls != 1 || lookup.lastMapIndex != 42 {
		t.Fatalf("expected map chase-step lookup for map 42, got calls=%d map=%d", lookup.calls, lookup.lastMapIndex)
	}
	if contentType := rec.Header().Get("Content-Type"); !strings.Contains(contentType, "application/json") {
		t.Fatalf("expected application/json content type, got %q", contentType)
	}
	...
	if !strings.Contains(string(body), `"entity_id":55`) || !strings.Contains(string(body), `"remaining_ms":5000`) || !strings.Contains(string(body), `"step"`) {
		t.Fatalf("unexpected JSON response body %q", string(body))
	}
}

func TestLocalMapOccupancySpawnGroupChaseStepsEndpointRejectsInvalidMapIndex(t *testing.T) {
	lookup := &stubMapSpawnGroupChaseStepsLookup{}
	mux := RegisterLocalMapSpawnGroupChaseStepsEndpoint(NewPprofMux("gamed"), lookup.MapSpawnGroupChaseSteps)

	req := httptest.NewRequest(http.MethodGet, "/local/maps/not-a-map/spawn-group-chase-steps", nil)
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
	...
	if lookup.calls != 0 {
		t.Fatalf("expected chase-step lookup not to be called for invalid map, got %d", lookup.calls)
	}
}

func TestLocalMapOccupancySpawnGroupChaseStepsEndpointRejectsNonLoopbackRemoteAddr(t *testing.T) {
	lookup := &stubMapSpawnGroupChaseStepsLookup{}
	mux := RegisterLocalMapSpawnGroupChaseStepsEndpoint(NewPprofMux("gamed"), lookup.MapSpawnGroupChaseSteps)
	...
	if lookup.calls != 0 {
		t.Fatalf("expected chase-step lookup not to be called for non-loopback, got %d", lookup.calls)
	}
}

func TestLocalMapOccupancySpawnGroupChaseStepsEndpointRejectsWrongMethod(t *testing.T) {
	lookup := &stubMapSpawnGroupChaseStepsLookup{}
	mux := RegisterLocalMapSpawnGroupChaseStepsEndpoint(NewPprofMux("gamed"), lookup.MapSpawnGroupChaseSteps)

	req := httptest.NewRequest(http.MethodPost, "/local/maps/42/spawn-group-chase-steps", strings.NewReader("ignored"))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
	...
	if lookup.calls != 0 {
		t.Fatalf("expected chase-step lookup not to be called for wrong method, got %d", lookup.calls)
	}
}

func TestLocalMapOccupancySpawnGroupChaseStepsEndpointReturnsNotFoundForMissingMap(t *testing.T) {
	lookup := &stubMapSpawnGroupChaseStepsLookup{snapshots: map[uint32]any{}}
	mux := RegisterLocalMapSpawnGroupChaseStepsEndpoint(NewPprofMux("gamed"), lookup.MapSpawnGroupChaseSteps)
	...
	if lookup.calls != 1 || lookup.lastMapIndex != 42 {
		t.Fatalf("expected map chase-step lookup for map 42 once, got calls=%d map_index=%d", lookup.calls, lookup.lastMapIndex)
	}
}
...
func TestLocalSpawnGroupChaseStepsEndpointReturnsJSONSnapshotsForLoopbackGet(t *testing.T) {
	snapshotter := &stubListSnapshotter{snapshots: []map[string]any{{"entity_id": uint64(55), "ready_at": "2026-07-25T12:00:00Z", "remaining_ms": int64(5000), "actor": map[string]any{"entity_id": uint64(55), "spawn_group_ref": "practice.chase_step"}, "step": map[string]any{"next": map[string]any{"x": int32(1800)}}}}}
	mux := RegisterLocalSpawnGroupChaseStepsEndpoint(NewPprofMux("gamed"), snapshotter.Snapshot)

	req := httptest.NewRequest(http.MethodGet, "/local/spawn-group-chase-steps", nil)
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
	...
	if snapshotter.calls != 1 {
		t.Fatalf("expected spawn-group chase-step snapshotter call, got %d calls", snapshotter.calls)
	}
	if contentType := rec.Header().Get("Content-Type"); !strings.Contains(contentType, "application/json") {
		t.Fatalf("expected application/json content type, got %q", contentType)
	}
	...
	if !strings.Contains(string(body), `"entity_id":55`) || !strings.Contains(string(body), `"remaining_ms":5000`) || !strings.Contains(string(body), `"step"`) || !strings.Contains(string(body), `"next"`) {
		t.Fatalf("unexpected JSON response body %q", string(body))
	}
}

func TestLocalSpawnGroupChaseStepsEndpointRejectsNonLoopbackRemoteAddr(t *testing.T) {
	snapshotter := &stubListSnapshotter{}
	mux := RegisterLocalSpawnGroupChaseStepsEndpoint(NewPprofMux("gamed"), snapshotter.Snapshot)
	...
	if snapshotter.calls != 0 {
		t.Fatalf("expected spawn-group chase-step snapshotter not to be called, got %d calls", snapshotter.calls)
	}
}

func TestLocalSpawnGroupChaseStepsEndpointRejectsWrongMethod(t *testing.T) {
	snapshotter := &stubListSnapshotter{}
	mux := RegisterLocalSpawnGroupChaseStepsEndpoint(NewPprofMux("gamed"), snapshotter.Snapshot)

	req := httptest.NewRequest(http.MethodPost, "/local/spawn-group-chase-steps", strings.NewReader("ignored"))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalSpawnGroupChaseStepEndpointReturnsExactSnapshotForLoopbackGet(t *testing.T) {
	snapshot := map[string]any{"entity_id": uint64(55), "ready_at": "2026-07-25T12:00:00Z", "remaining_ms": int64(5000), "actor": map[string]any{"entity_id": uint64(55), "spawn_group_ref": "practice.chase_step"}, "step": map[string]any{"next": map[string]any{"x": int32(1800)}}}
	mux := RegisterLocalSpawnGroupChaseStepSnapshotEndpoint(NewPprofMux("gamed"), func(entityID uint64) (any, bool) {
		if entityID != 55 {
			return nil, false
		}
		return snapshot, true
...
func TestLocalSpawnGroupChaseStepEndpointRejectsInvalidEntityID(t *testing.T) {
	mux := RegisterLocalSpawnGroupChaseStepSnapshotEndpoint(NewPprofMux("gamed"), func(uint64) (any, bool) {
		t.Fatal("spawn-group chase-step lookup should not be called for invalid entity IDs")
		return nil, false
	})

	req := httptest.NewRequest(http.MethodGet, "/local/spawn-group-chase-steps/not-an-id", nil)
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalSpawnGroupChaseStepSnapshotEndpointRejectsNonLoopbackRemoteAddr(t *testing.T) {
	mux := RegisterLocalSpawnGroupChaseStepSnapshotEndpoint(NewPprofMux("gamed"), func(uint64) (any, bool) {
		t.Fatal("spawn-group chase-step lookup should not be called for non-loopback callers")
		return nil, false
	})
...
func TestLocalSpawnGroupChaseStepSnapshotEndpointRejectsWrongMethod(t *testing.T) {
	mux := RegisterLocalSpawnGroupChaseStepSnapshotEndpoint(NewPprofMux("gamed"), func(uint64) (any, bool) {
		t.Fatal("spawn-group chase-step lookup should not be called for wrong methods")
		return nil, false
	})

	req := httptest.NewRequest(http.MethodPost, "/local/spawn-group-chase-steps/55", strings.NewReader("ignored"))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalSpawnGroupChaseStepSnapshotEndpointReturnsNotFoundForMissingEntityID(t *testing.T) {
	mux := RegisterLocalSpawnGroupChaseStepSnapshotEndpoint(NewPprofMux("gamed"), func(uint64) (any, bool) {
		return nil, false
	})
...
func TestLocalSpawnGroupReturnStepEndpointRejectsInvalidRequestBeforeCallback(t *testing.T) {
	...
	for _, path := range []string{"/local/spawn-groups/not-an-id/return-step?max_step=100", "/local/spawn-groups/0/return-step?max_step=100", "/local/spawn-groups/44/return-step?max_step=0", "/local/spawn-groups/44/return-step?max_step=-1", "/local/spawn-groups/44/return-step?max_step=abc"} {
		req := httptest.NewRequest(http.MethodPost, path, nil)
		req.RemoteAddr = "127.0.0.1:12345"
		rec := httptest.NewRecorder()
...
func TestLocalCombatTargetsEndpointReturnsJSONSnapshotsForLoopbackGet(t *testing.T) {
	snapshotter := &stubListSnapshotter{snapshots: []map[string]any{{"subject_entity_id": uint64(17), "subject": map[string]any{"name": "MkmkSura", "vid": uint32(99), "map_index": uint32(1)}, "target_vid": uint32(22), "snapshot_version": uint64(3), "hp_percent": uint8(80), "target_current_hp": uint8(8), "target_max_hp": uint8(10), "normal_attack_damage": uint8(1), "target_attack_value": uint16(7), "target_defense_value": uint16(2), "engaged_by_entity_id": uint64(17), "engaged_by": map[string]any{"name": "MkmkSura", "vid": uint32(99), "map_index": uint32(1)}, "retaliation_point_delta": int32(-1), "retaliation_server_origin": true, "retaliation_pending": true, "retaliation_ready_at": "2026-08-15T12:00:01Z", "retaliation_remaining_ms": int64(750)}}}
	...
	if !strings.Contains(string(body), `"target_vid":22`) || !strings.Contains(string(body), `"hp_percent":80`) || !strings.Contains(string(body), `"target_current_hp":8`) || !strings.Contains(string(body), `"target_max_hp":10`) || !strings.Contains(string(body), `"normal_attack_damage":1`) || !strings.Contains(string(body), `"target_attack_value":7`) || !strings.Contains(string(body), `"target_defense_value":2`) || !strings.Contains(string(body), `"subject"`) || !strings.Contains(string(body), `"name":"MkmkSura"`) || !strings.Contains(string(body), `"engaged_by_entity_id":17`) || !strings.Contains(string(body), `"retaliation_point_delta":-1`) || !strings.Contains(string(body), `"retaliation_server_origin":true`) || !strings.Contains(string(body), `"retaliation_pending":true`) || !strings.Contains(string(body), `"retaliation_ready_at":"2026-08-15T12:00:01Z"`) || !strings.Contains(string(body), `"retaliation_remaining_ms":750`) {
		t.Fatalf("unexpected JSON response body %q", string(body))
	}
}
...
func TestLocalMigrationStatusEndpointReturnsDryRunPlanForLoopbackGet(t *testing.T) {
	planner := &stubMigrationStatusPlanner{plan: dbmigrations.Plan{
		...
		Pending: []dbmigrations.PlanStep{
			...
			{Version: 15, Name: "character_safebox_money", Direction: dbmigrations.DirectionUp, Path: "0015_character_safebox_money.up.sql", SHA256: expectedCharacterSafeboxMoneyStatusSHA256},
			{Version: 16, Name: "static_actor_combat_profile_chase_delay", Direction: dbmigrations.DirectionUp, Path: "0016_static_actor_combat_profile_chase_delay.up.sql", SHA256: expectedStaticActorCombatProfileChaseDelayStatusSHA256},
			{Version: 17, Name: "static_actor_combat_profile_return_delay", Direction: dbmigrations.DirectionUp, Path: "0017_static_actor_combat_profile_return_delay.up.sql", SHA256: expectedStaticActorCombatProfileReturnDelayStatusSHA256},
			{Version: 18, Name: "static_actor_combat_profile_homeward_delay", Direction: dbmigrations.DirectionUp, Path: "0018_static_actor_combat_profile_homeward_delay.up.sql", SHA256: expectedStaticActorCombatProfileHomewardDelayStatusSHA256},
			{Version: 19, Name: "static_actor_combat_profile_max_step", Direction: dbmigrations.DirectionUp, Path: "0019_static_actor_combat_profile_max_step.up.sql", SHA256: expectedStaticActorCombatProfileMaxStepStatusSHA256},
			...
			{Version: 24, Name: "character_item_instance_sockets", Direction: dbmigrations.DirectionUp, Path: "0024_character_item_instance_sockets.up.sql", SHA256: expectedCharacterItemInstanceSocketsStatusSHA256},
			...
			{Version: 29, Name: "bootstrap_ground_item_instance_attributes", Direction: dbmigrations.DirectionUp, Path: "0029_bootstrap_ground_item_instance_attributes.up.sql", SHA256: expectedBootstrapGroundItemInstanceAttributesStatusSHA256},
		},
	}}
...
func TestLocalCharacterPointStateQuarantineEndpointReturnsCanonicalJSON(t *testing.T) {
	...
	for index := 0; index < 255; index++ {
		rows = append(rows, accountstore.CharacterPointRow{CharacterID: 22, PointIndex: uint8(index), Value: 1})
	}
	for index := 0; index < 255; index++ {
		rows = append(rows, accountstore.CharacterPointRow{CharacterID: 11, PointIndex: uint8(index), Value: 2})
	}
	body, err := json.Marshal(accountstore.CharacterPointStateExport{
		MigrationVersion: accountstore.CharacterPointStateMigrationVersion,
		MigrationName:    accountstore.CharacterPointStateMigrationName,
		Points:           rows,
	})
	...
	mux := RegisterLocalCharacterPointStateQuarantineEndpoint(NewPprofMux("gamed"))
	req := httptest.NewRequest(http.MethodPost, "/local/account-store/exports/character-point-state/quarantine", bytes.NewReader(body))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalCharacterPointStateQuarantineEndpointRejectsNonLoopbackRemoteAddr(t *testing.T) {
	mux := RegisterLocalCharacterPointStateQuarantineEndpoint(NewPprofMux("gamed"))
...
func TestLocalCharacterPointStateQuarantineEndpointRejectsWrongMethod(t *testing.T) {
	mux := RegisterLocalCharacterPointStateQuarantineEndpoint(NewPprofMux("gamed"))
	req := httptest.NewRequest(http.MethodGet, "/local/account-store/exports/character-point-state/quarantine", nil)
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalCharacterPointStateQuarantineEndpointRejectsInvalidExport(t *testing.T) {
	mux := RegisterLocalCharacterPointStateQuarantineEndpoint(NewPprofMux("gamed"))
	req := httptest.NewRequest(http.MethodPost, "/local/account-store/exports/character-point-state/quarantine", strings.NewReader(`{"migration_version":3,"migration_name":"character_point_state","points":[]}`))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalCharacterPointStateQuarantineEndpointRejectsMalformedJSON(t *testing.T) {
	mux := RegisterLocalCharacterPointStateQuarantineEndpoint(NewPprofMux("gamed"))
	req := httptest.NewRequest(http.MethodPost, "/local/account-store/exports/character-point-state/quarantine", strings.NewReader(`{"migration_version":11`))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalAuthLoginTicketHandoffQuarantineEndpointReturnsCanonicalJSON(t *testing.T) {
	issuedAlpha := time.Date(2026, 8, 19, 9, 30, 0, 0, time.UTC)
	issuedZeta := time.Date(2026, 8, 19, 10, 45, 0, 0, time.UTC)
	body, err := json.Marshal(loginticket.AuthLoginTicketHandoffExport{
	...
	mux := RegisterLocalAuthLoginTicketHandoffQuarantineEndpoint(NewPprofMux("gamed"))
	req := httptest.NewRequest(http.MethodPost, "/local/login-tickets/exports/auth-login-ticket-handoff/quarantine", bytes.NewReader(body))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
	...
	for _, want := range []string{
		...
		`"login_keys":[16909060,33554432]`,
		`"migration_version":7`,
		`"login":"Alpha"`,
		`"login":"zeta"`,
		`"login_normalized":"alpha"`,
	} {
		if !strings.Contains(responseBody, want) {
			t.Fatalf("expected quarantine response to contain %s, got %s", want, responseBody)
		}
...
func TestLocalAuthLoginTicketHandoffQuarantineEndpointRejectsNonLoopbackRemoteAddr(t *testing.T) {
	mux := RegisterLocalAuthLoginTicketHandoffQuarantineEndpoint(NewPprofMux("gamed"))
	req := httptest.NewRequest(http.MethodPost, "/local/login-tickets/exports/auth-login-ticket-handoff/quarantine", strings.NewReader(`{"migration_version":7,"migration_name":"auth_login_ticket_handoff","tickets":[]}`))
	req.RemoteAddr = "198.51.100.10:12345"
	rec := httptest.NewRecorder()
...
func TestLocalAuthLoginTicketHandoffQuarantineEndpointRejectsWrongMethod(t *testing.T) {
	mux := RegisterLocalAuthLoginTicketHandoffQuarantineEndpoint(NewPprofMux("gamed"))
	req := httptest.NewRequest(http.MethodGet, "/local/login-tickets/exports/auth-login-ticket-handoff/quarantine", nil)
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalAuthLoginTicketHandoffQuarantineEndpointRejectsInvalidExport(t *testing.T) {
	mux := RegisterLocalAuthLoginTicketHandoffQuarantineEndpoint(NewPprofMux("gamed"))
	req := httptest.NewRequest(http.MethodPost, "/local/login-tickets/exports/auth-login-ticket-handoff/quarantine", strings.NewReader(`{"migration_version":4,"migration_name":"auth_login_ticket_handoff","tickets":[]}`))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalAuthLoginTicketHandoffQuarantineEndpointRejectsMalformedJSON(t *testing.T) {
	mux := RegisterLocalAuthLoginTicketHandoffQuarantineEndpoint(NewPprofMux("gamed"))
	req := httptest.NewRequest(http.MethodPost, "/local/login-tickets/exports/auth-login-ticket-handoff/quarantine", strings.NewReader(`{"migration_version":7`))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestNewPprofMuxDoesNotExposeLocalAuthLoginTicketHandoffQuarantineByDefault(t *testing.T) {
	mux := NewPprofMux("authd")
	req := httptest.NewRequest(http.MethodPost, "/local/login-tickets/exports/auth-login-ticket-handoff/quarantine", strings.NewReader(`{"migration_version":7,"migration_name":"auth_login_ticket_handoff","tickets":[]}`))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalItemTemplateStateExportEndpointReturnsLoopbackJSON(t *testing.T) {
	...
	body := rec.Body.String()
	for _, want := range []string{`"migration_version":9`, `"migration_name":"item_template_refine_info"`, `"templates":[`, `"sockets":[`, `"attributes":[`, `"use_effects":[`, `"refine_infos":`, `"refine_materials":`, `"vnum":27001`, `"shop_buy_price":50`, `"point_delta":25`} {
		if !strings.Contains(body, want) {
...
type stubMapSpawnGroupChaseStepsLookup struct {
	snapshots    map[uint32]any
	calls        int
	lastMapIndex uint32
}
