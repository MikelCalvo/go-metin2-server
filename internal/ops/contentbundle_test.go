...
func TestLocalContentBundleValidateEndpointExpandsKillQuestOnlyDropTableAuthoringExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("expected status %d for kill-quest-only drop-table authoring example validation, got %d body=%s", http.StatusOK, rec.Code, rec.Body.String())
	}
	if bytes.Contains(rec.Body.Bytes(), []byte(`"drop_tables"`)) || bytes.Contains(rec.Body.Bytes(), []byte(`"reward_drop_table_ref"`)) {
		t.Fatalf("expected validation response to strip authoring-only drop-table fields, got %s", rec.Body.String())
	}
	var got contentbundle.Bundle
	if err := json.Unmarshal(rec.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode kill-quest-only drop-table validation response: %v", err)
	}
	wantSpawn := contentbundle.SpawnGroup{
	...
	if len(got.SpawnGroups) != 1 || !reflect.DeepEqual(got.SpawnGroups[0], wantSpawn) {
		t.Fatalf("expected validation response to expand kill-quest-only drop table plus require gate without combat channels, got %+v", got.SpawnGroups)
	}
	if len(got.ItemTemplates) != 0 {
		t.Fatalf("expected kill-quest-only drop-table authoring example to carry no item templates, got %+v", got.ItemTemplates)
	}
...
func TestLocalContentBundleValidateEndpointRejectsCollidingRegenMemberRefsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in colliding regen member refs example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}
...
func TestLocalContentBundleValidateEndpointRejectsOrphanQuestGateExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in orphan quest-gate example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOrphanServiceQuestGateExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in orphan service quest-gate example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsEmptyDropTableExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in empty drop-table example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsConflictingKillQuestCreditExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in conflicting kill-quest credit example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOrphanRequireQuestFromExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in orphan require_quest_from example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsPartialRequireQuestGateExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in partial require-quest-gate example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsReversePartialRequireQuestGateExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in reverse partial require-quest-gate example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsPartialKillQuestCreditExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in partial kill-quest credit example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsKillQuestFromEqualsToExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in kill-quest from-equals-to example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}
...
func TestLocalContentBundleValidateEndpointRejectsPartialServiceQuestGateExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in partial service quest-gate example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsReversePartialServiceQuestGateExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in reverse partial service quest-gate example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOrphanServiceQuestFromExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in orphan service quest_from example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOrphanServiceQuestToExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in orphan service quest_to example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsGatedServiceQuestToExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in gated service quest_to example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsDanglingInteractionRefExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in dangling interaction ref example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsUnsupportedInteractionKindExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-unsupported-interaction-kind-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid unsupported interaction kind example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in unsupported interaction kind example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsDuplicateStaticActorExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-duplicate-static-actor-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid duplicate static actor example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in duplicate static actor example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsRewardDropWithoutItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-reward-drop-without-item-templates-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid reward drop without item templates example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in reward drop without item templates example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsMerchantCatalogWithoutItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-merchant-catalog-without-item-templates-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid merchant catalog without item templates example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in merchant catalog without item templates example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsMerchantCatalogItemMissingFromItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-merchant-catalog-item-missing-from-item-templates-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid merchant catalog item missing from item templates example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
...
func TestLocalContentBundleValidateEndpointRejectsMerchantCatalogCountAboveStackLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in merchant catalog count above stack limit example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsMerchantCatalogCountAboveNonStackableLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-merchant-catalog-count-above-non-stackable-limit-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid merchant catalog count above non-stackable limit example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in merchant catalog count above non-stackable limit example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsRewardDropItemMissingFromItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-reward-drop-item-missing-from-item-templates-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid reward drop item missing from item templates example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in reward drop item missing from item templates example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsCombatProfileDeathRewardItemMissingFromItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in combat-profile death-reward item missing from item templates example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsCombatProfileDeathRewardWithoutItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in combat-profile death-reward without item templates example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsQuestFlagRewardItemWithoutItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-quest-flag-reward-item-without-item-templates-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid quest-flag reward item without item templates example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in quest-flag reward item without item templates example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsQuestFlagRewardItemMissingFromItemTemplatesExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-quest-flag-reward-item-missing-from-item-templates-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid quest-flag reward item missing from item templates example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in quest-flag reward item missing from item templates example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}
...
func TestLocalContentBundleValidateEndpointRejectsQuestFlagConsumeItemMissingFromItemTemplatesExample(t *testing.T) {
	...
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-quest-flag-consume-item-missing-from-item-templates-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid quest-flag consume item missing from item templates example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in quest-flag consume item missing from item templates example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsQuestFlagRewardItemCountAboveStackLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-quest-flag-reward-item-count-above-stack-limit-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid quest-flag reward item count above stack limit example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in quest-flag reward item count above stack limit example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsQuestFlagRewardItemCountAboveNonStackableLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-quest-flag-reward-item-count-above-non-stackable-limit-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid quest-flag reward item count above non-stackable limit example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in quest-flag reward item count above non-stackable limit example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsQuestFlagConsumeItemCountAboveStackLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-quest-flag-consume-item-count-above-stack-limit-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid quest-flag consume item count above stack limit example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in quest-flag consume item count above stack limit example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsQuestFlagConsumeItemCountAboveNonStackableLimitExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in quest-flag consume item count above non-stackable limit example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignSizeExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-size-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign size example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign size example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignTitleExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-title-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign title example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign title example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignTitleExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-title-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign title example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign title example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignCatalogExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-catalog-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign catalog example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign catalog example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignCatalogExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign catalog example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignWarpCoordsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-warp-coords-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign warp coords example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign warp coords example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignWarpCoordsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-warp-coords-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign warp coords example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign warp coords example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxOversizeSizeExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-oversize-size-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox oversize size example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox oversize size example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignRewardGoldExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-reward-gold-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign reward gold example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign reward gold example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignRewardGoldExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-reward-gold-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign reward gold example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign reward gold example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignRewardExperienceExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-reward-experience-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign reward experience example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign reward experience example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignRewardExperienceExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-reward-experience-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign reward experience example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign reward experience example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignConsumeGoldExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-consume-gold-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign consume gold example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign consume gold example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignConsumeGoldExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-consume-gold-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign consume gold example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign consume gold example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignConsumeExperienceExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-consume-experience-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign consume experience example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign consume experience example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignConsumeExperienceExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-consume-experience-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign consume experience example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign consume experience example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignRewardItemsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-reward-items-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign reward items example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign reward items example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignRewardItemsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-reward-items-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign reward items example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign reward items example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignConsumeItemsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-consume-items-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign consume items example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign consume items example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignConsumeItemsExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-consume-items-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign consume items example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign consume items example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignQuestToExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-quest-to-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign quest_to example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign quest_to example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignQuestToExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-quest-to-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign quest_to example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign quest_to example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenSafeboxForeignRewardItemVnumExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-safebox-foreign-reward-item-vnum-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_safebox foreign reward_item_vnum example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_safebox foreign reward_item_vnum example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsOpenCubeForeignRewardItemVnumExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	repoRoot := filepath.Clean(filepath.Join(filepath.Dir(filename), "..", ".."))
	raw, err := os.ReadFile(filepath.Join(repoRoot, "docs", "examples", "bootstrap-invalid-open-cube-foreign-reward-item-vnum-bundle.json"))
	if err != nil {
		t.Fatalf("read invalid open_cube foreign reward_item_vnum example bundle: %v", err)
	}
	mux := RegisterLocalContentBundleValidateEndpoint(NewPprofMux("gamed"))
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in open_cube foreign reward_item_vnum example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}

func TestLocalContentBundleValidateEndpointRejectsShopPreviewForeignRewardItemVnumExample(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate ops contentbundle test file")
	}
	...
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("expected status %d for checked-in shop_preview foreign reward_item_vnum example, got %d body=%s", http.StatusBadRequest, rec.Code, rec.Body.String())
	}
}
