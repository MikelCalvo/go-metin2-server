...
func TestLocalCharacterInteractionVisibilityEndpointReturnsExactSnapshotForLoopbackGet(t *testing.T) {
	snapshotter := &stubNamedSnapshotter{snapshots: map[string]any{"Mkmk Sura": map[string]any{"name": "Mkmk Sura", "visible_interactable_static_actors": []map[string]any{{"entity_id": uint64(7), "name": "VillageGuide", "preview": "Welcome"}}}}}
	mux := RegisterLocalCharacterInteractionVisibilityEndpoint(NewPprofMux("gamed"), snapshotter.Snapshot)

	req := httptest.NewRequest(http.MethodGet, "/local/interaction-visibility/Mkmk%20Sura", nil)
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
	...
	if snapshotter.calls != 1 || snapshotter.lastName != "Mkmk Sura" {
		t.Fatalf("expected decoded interaction visibility snapshot name Mkmk Sura, got calls=%d name=%q", snapshotter.calls, snapshotter.lastName)
	}
	body, err := io.ReadAll(rec.Body)
	if err != nil {
		t.Fatalf("read response body: %v", err)
	}
	if !strings.Contains(string(body), `"visible_interactable_static_actors"`) || !strings.Contains(string(body), `"name":"VillageGuide"`) || !strings.Contains(string(body), `"preview":"Welcome"`) {
		t.Fatalf("unexpected JSON response body %q", string(body))
	}
}

func TestLocalCharacterInteractionVisibilityEndpointRejectsNonLoopbackRemoteAddr(t *testing.T) {
	snapshotter := &stubNamedSnapshotter{}
	mux := RegisterLocalCharacterInteractionVisibilityEndpoint(NewPprofMux("gamed"), snapshotter.Snapshot)

	req := httptest.NewRequest(http.MethodGet, "/local/interaction-visibility/MkmkSura", nil)
	req.RemoteAddr = "198.51.100.10:12345"
	rec := httptest.NewRecorder()
...
func TestLocalCharacterInteractionVisibilityEndpointRejectsInvalidCharacterName(t *testing.T) {
	snapshotter := &stubNamedSnapshotter{}
	mux := RegisterLocalCharacterInteractionVisibilityEndpoint(NewPprofMux("gamed"), snapshotter.Snapshot)

	req := httptest.NewRequest(http.MethodGet, "/local/interaction-visibility/Mkmk%2FSura", nil)
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
...
func TestLocalCharacterInteractionVisibilityEndpointReturnsNotFoundForMissingCharacter(t *testing.T) {
	snapshotter := &stubNamedSnapshotter{snapshots: map[string]any{}}
	mux := RegisterLocalCharacterInteractionVisibilityEndpoint(NewPprofMux("gamed"), snapshotter.Snapshot)

	req := httptest.NewRequest(http.MethodGet, "/local/interaction-visibility/Missing", nil)
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
	...
	if snapshotter.calls != 1 || snapshotter.lastName != "Missing" {
		t.Fatalf("expected interaction visibility snapshotter to be called for Missing, got calls=%d name=%q", snapshotter.calls, snapshotter.lastName)
	}
}

func TestLocalCharacterInteractionVisibilityEndpointRejectsWrongMethod(t *testing.T) {
	snapshotter := &stubNamedSnapshotter{}
	mux := RegisterLocalCharacterInteractionVisibilityEndpoint(NewPprofMux("gamed"), snapshotter.Snapshot)

	req := httptest.NewRequest(http.MethodPost, "/local/interaction-visibility/MkmkSura", strings.NewReader("ignored"))
	req.RemoteAddr = "127.0.0.1:12345"
	rec := httptest.NewRecorder()
