...
Persistence unit tests should prove both accepted round trips and fail-closed rejection of malformed durable snapshots. For account snapshots that means testing invalid item payloads, out-of-range carried slots, duplicate equipped-slot claims, malformed quickslot tuples, duplicate quickslot positions, unknown top-level fields, trailing JSON values, symlinked committed snapshot files, duplicate non-zero character identities, reusable all-zero empty character slots, and zero-ID slots with leftover non-empty state as durable-data contracts. Login-ticket tests should cover one-shot handoff integrity as well as payload validation: issuing a ticket must not overwrite a ticket that becomes committed for the same login key after the preflight existence check, empty-login and zero-key tickets are rejected, committed ticket filenames must match the embedded login key, symlinked committed ticket files fail closed before JSON decode, malformed tickets must not be consumed/deleted, multiple empty select-screen slots must not look like duplicate character IDs, non-empty zero-ID slots must fail closed, successful consume should still sync the directory after removing the ticket, and dry-run validation should summarize pending tickets without consuming them while ignoring hidden crash-temp files and reporting the embedded select-screen character / empty-slot counts. Backup/restore tests should also prove the operator integrity boundary: restore preflight requires the deterministic backup manifest, verifies checksums/summary metadata before mutating a destination, rejects backup destinations equal to or nested under the active source store including symlink-resolved destinations, rejects restore destinations equal to or nested under the backup source including symlink-resolved destinations, rejects backup-entry symlinks, untracked non-temp files/directories, or crash-temp-shaped directories that are not named by the manifest, reports ignored crash-temp regular files in backup-validation summaries so operators can see residue before restore, and leaves backup/restore destinations empty on committed-copy/sync, missing-manifest, corrupt-manifest, symlink-entry, or closed-directory-coverage failures. When restore leaves a backup manifest in an active replacement store, normal validation and persistence-status tests should also fail closed if the manifest becomes malformed or stale against the current committed account/item-template snapshot bytes. Some runtime recovery tests still seed duplicate carried-slot or duplicate live-item-ID snapshots deliberately; keep those as narrow runtime-state tests instead of relaxing the durable stores.
