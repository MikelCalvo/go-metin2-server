...
- `metin2-migrate backup-restore-drill --runtime-config <path|-> --build-info <path|-> [--authd-ops-base-url http://127.0.0.1:6061] [--gamed-log-path /var/log/metin2/gamed.log] [--authd-log-path /var/log/metin2/authd.log]` — read-only printer that turns retained runtime-config + build-info snapshots into the path-aware curl / aside-rename script for the lab `/var/metin2/backups/YYYYMMDDTHHMMSSZ-<commit12>/` tree without executing backup or restore. The printed script retains both-daemon build-info (`gamed` via `--ops-base-url`, `authd` via `--authd-ops-base-url`), optional `/var/log/metin2/{gamed,authd}.log` copies when present (`cp -p`, missing files non-fatal), `runtime-config.json` / `persistence-status-*.json`, a `notes.md` stub, uses lab store subdirectory names (`accounts`, `interaction-store`, `safebox`, ...), and includes the optional store validate + crash-temp cleanup triage before backup; if an operator runs that printed section, `crash-temps/cleanup` mutates only hidden crash-temp residue after validate. Hermetic `/bin/sh` execution of that printed script against drained loopback `gamed`/`authd` ops muxes is owned by [hermetic backup/restore drill HTTP execution proof](../plans/2026-08-24-hermetic-backup-restore-drill-http-execution-proof.md); the CLI still never auto-runs the drill. The eight-store validate / crash-temp / backup / restore routes plus `runtime-config` / `persistence-status` are registered through one shared helper (`minimal.RegisterGamedFileStorePersistenceOps`) used by both `cmd/gamed` and that hermetic proof so the drill surface cannot drift — see [gamed file-store persistence ops registration helper](../plans/2026-08-24-gamed-file-store-persistence-ops-registration-helper.md). After backup/validate the printed script redirects `backup-tree-status --require-stores-complete --require-no-crash-temps` to `$BASE/backup-tree-status.json` and `backup-tree-status-status --require-stores-complete --require-no-crash-temps` to `$BASE/backup-tree-status-status.json` before aside-rename / restore. Re-inspect a retained tree later with `metin2-migrate backup-tree-status --backup-tree <absolute-path> [--require-stores-complete] [--require-no-crash-temps]` — see [CLI backup-tree-status](../plans/2026-09-06-cli-backup-tree-status-contract-freeze.md). Re-inspect a retained `backup-tree-status.json` later with `metin2-migrate backup-tree-status-status --backup-tree-status <path> [--require-stores-complete] [--require-no-crash-temps]` — see [CLI backup-tree-status-status](../plans/2026-09-06-cli-backup-tree-status-status-contract-freeze.md) and [CLI backup-tree-status no-crash-temps require-gate](../plans/2026-09-06-cli-backup-tree-status-no-crash-temps-require-gate-contract-freeze.md). After retaining `$BASE/persistence-status-before.json` the printed script also redirects ungated `persistence-status-status` to `$BASE/persistence-status-before-status.json`; after `$BASE/persistence-status-after.json` it redirects gated `persistence-status-status --require-ok --require-drained` to `$BASE/persistence-status-after-status.json`. Re-inspect a retained `persistence-status-after.json` later with `metin2-migrate persistence-status-status --persistence-status <path> --require-ok --require-drained` — see [CLI persistence-status-status](../plans/2026-09-07-cli-persistence-status-status-contract-freeze.md).
