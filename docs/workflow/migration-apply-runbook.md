...
- stock CLI/daemon binaries do not register or select a production DB engine/driver (a build-tagged SQLite harness proves live ledger apply/read/snapshot under `go test -tags=sqlite_harness ./db/migrations -run SQLiteHarness`, programmatic `0002` roster SQL import under `go test -tags=sqlite_harness ./internal/accountstore -run SQLiteHarnessRosterImport`, programmatic `0003` item-state SQL import (including additive `0024` presence-aware instance sockets and additive `0027` presence-aware instance attributes) under `go test -tags=sqlite_harness ./internal/accountstore -run SQLiteHarnessItemStateImport`, programmatic `0004` quest-state SQL import under `go test -tags=sqlite_harness ./internal/queststate -run SQLiteHarnessQuestStateImport`, programmatic `0011` point-state SQL import under `go test -tags=sqlite_harness ./internal/accountstore -run SQLiteHarnessPointStateImport`, programmatic `0023` myshop unit-prices SQL import under `go test -tags=sqlite_harness ./internal/accountstore -run SQLiteHarnessMyShopUnitPricesImport`, programmatic `0015` safebox-state SQL import (including additive `0025` presence-aware safebox cell instance sockets and additive `0028` presence-aware safebox cell instance attributes) under `go test -tags=sqlite_harness ./internal/safeboxstore -run SQLiteHarnessSafeboxStateImport`, programmatic `0010` ground-item-state SQL import (including additive `0026` presence-aware instance sockets and additive `0029` presence-aware instance attributes) under `go test -tags=sqlite_harness ./internal/worldruntime -run SQLiteHarnessGroundItemStateImport`, and programmatic `0009` item-template-state SQL import under `go test -tags=sqlite_harness ./internal/itemstore -run SQLiteHarnessItemTemplateStateImport`, programmatic `0007` auth-login-ticket-handoff SQL import under `go test -tags=sqlite_harness ./internal/loginticket -run SQLiteHarnessAuthLoginTicketHandoffImport`, and programmatic tip-`0013` static-actor content-state SQL import under `go test -tags=sqlite_harness ./internal/staticstore -run SQLiteHarnessStaticActorContentStateImport`; see [SQLite schema_migrations driver-backed harness](../plans/2026-08-25-sqlite-schema-migrations-driver-harness.md), [account/character roster SQL import/backfill](../plans/2026-08-25-account-character-roster-sql-import-backfill.md), [character item-state SQL import/backfill](../plans/2026-08-26-character-item-state-sql-import-backfill.md), [seeded item instance-sockets tip sync](../plans/2026-08-30-seeded-item-instance-sockets-import-export-drill.md), [character item instance-attributes SQL additive](../plans/2026-08-30-character-item-instance-attributes-sql-additive.md), [seeded item instance-attributes tip sync](../plans/2026-08-31-seeded-item-instance-attributes-import-export-drill.md), [character quest-state SQL import/backfill](../plans/2026-08-26-character-quest-state-sql-import-backfill.md), [character point-state SQL import/backfill](../plans/2026-08-26-character-point-state-sql-import-backfill.md), [character myshop unit-prices migration](../plans/2026-08-29-character-myshop-unit-prices-migration.md), [character safebox-state SQL import/backfill](../plans/2026-08-26-character-safebox-state-sql-import-backfill.md), [seeded safebox cell instance-sockets tip sync](../plans/2026-08-30-seeded-safebox-cell-instance-sockets-import-export-drill.md), [safebox cell instance-attributes SQL additive](../plans/2026-08-30-character-safebox-item-instance-attributes-sql-additive.md), [seeded safebox cell instance-attributes tip sync](../plans/2026-08-31-seeded-safebox-cell-instance-attributes-import-export-drill.md), [bootstrap ground-item-state SQL import/backfill](../plans/2026-08-26-bootstrap-ground-item-state-sql-import-backfill.md), [seeded ground-item instance-sockets tip sync](../plans/2026-08-30-seeded-ground-item-instance-sockets-import-export-drill.md), [bootstrap ground-item instance-attributes SQL additive](../plans/2026-08-31-bootstrap-ground-item-instance-attributes-sql-additive.md), [item-template-state SQL import/backfill](../plans/2026-08-26-item-template-state-sql-import-backfill.md), and [auth-login-ticket-handoff SQL import/backfill](../plans/2026-08-26-auth-login-ticket-handoff-sql-import-backfill.md), [static-actor content-state SQL import/backfill](../plans/2026-08-27-static-actor-content-state-sql-import-backfill.md));
...
- `migration-catalog-status.json` — optional `go-metin2-migration-catalog-status-v1` output from re-validating a retained catalog without opening a database — see [CLI catalog-status](../plans/2026-09-05-cli-catalog-status-contract-freeze.md);
...
Printed scripts also emit gated `status-status` companions immediately after
...
After schema migrations are applied, operators can backfill retained migration-shaped exports with `metin2-migrate import-export --kind <kind> --export <path> --driver <driver> --dsn <dsn> --i-confirm-sql-import`. This reuses the programmatic `Import*` seams (insert-only by default; no silent upsert) and still does not register a stock production driver. Every landed tip kind also accepts opt-in `--i-confirm-scoped-replace` to delete-then-insert only that export's scope inside one transaction: tip-`0002` `account-character-roster`, tip-`0003` `character-item-state`, tip-`0004` `character-quest-state`, tip-`0011` `character-point-state`, tip-`0015` `character-safebox-state`, tip-`0023` `character-myshop-unit-prices`, tip-`0010` `bootstrap-ground-item-state`, tip-`0009` `item-template-state`, tip-`0013` `static-actor-content-state`, and tip-`0007` `auth-login-ticket-handoff`. See the matching GREEN plans under `docs/plans/2026-08-31` through `2026-09-03` and [CLI import-export](../plans/2026-08-27-cli-import-export.md).

For a retained `export-quarantine-drill` tree, a later GREEN will keep gated `$BASE/export-tree-status.json` plus `$BASE/export-tree-status-status.json` after the last `quarantine-export` line (`--require-quarantine-complete` only) so operators can fail closed before this import walk — freeze only, see [CLI export-quarantine-drill export-tree-status contract freeze](../plans/2026-09-08-cli-export-quarantine-drill-export-tree-status-contract-freeze.md). Until that GREEN lands, inspect the tree by walking each tip-kind `quarantine.json` or by running `export-tree-status` by hand. Then print the reviewable import walk without embedding a DSN via:
...
The printer never executes the imports, never opens a database, and never embeds the DSN value; each printed `import-export` line still requires `--i-confirm-sql-import` at execution time. Before expanding the named DSN environment variable or walking the retained tree, the generated `set -eu` script runs `metin2-migrate drivers --require-driver "$DRIVER" > /dev/null`. That checks the exact importing CLI binary and stops an unlinked driver before target credentials or export-tree writes; it is not a retained driver artifact and does not prove target health. Default output stays insert-only; pass `--i-confirm-print-scoped-replace` to print `--i-confirm-scoped-replace` on every tip-kind line for lab re-backfill after tip rebuild — see [import-export-drill driver preflight](../plans/2026-09-08-cli-import-export-drill-driver-preflight-contract-freeze.md) and [import-export-drill opt-in scoped-replace printer](../plans/2026-09-03-import-export-drill-opt-in-scoped-replace.md). For seeded tip-`0002` inclusive re-backfill, pass `--i-confirm-print-two-phase-wipe-roster-reimport` instead so the printed script synthesizes wipe-scope exports, inspects each retained `wipe-quarantine.json` with `synthesize-wipe-export-status`, wipes character-FK tip kinds, replaces tip-`0002` roster, then reimports every non-roster tip kind; every printer mode also retains before/after `export-tree-status` snapshots beside the tree plus matching ungated/gated `export-tree-status-status` redirects (before stays ungated; after requires quarantine+import-result artifact completeness plus import-result outcomes-complete, and two-phase after also requires all-replaced plus two-phase wipe + wipe-import completeness plus wipe-import outcome completeness and wipe-import all-replaced) — see [import-export-drill two-phase wipe→roster→reimport](../plans/2026-09-03-import-export-drill-two-phase-wipe-roster-reimport.md), [CLI synthesize wipe-export status](../plans/2026-09-03-cli-synthesize-wipe-export-status.md), [CLI export-tree status](../plans/2026-09-04-cli-export-tree-status.md) / [import-result completeness aggregates](../plans/2026-09-04-cli-export-tree-status-import-result-completeness.md), [cutover-artifact gate](../plans/2026-09-04-cli-export-tree-status-cutover-artifact-gate-contract-freeze.md), [import-outcome aggregates](../plans/2026-09-04-cli-export-tree-status-import-outcome-aggregates-contract-freeze.md), [import-outcome require-gate](../plans/2026-09-04-cli-export-tree-status-import-outcome-require-gate-contract-freeze.md), [wipe-import outcome aggregates](../plans/2026-09-05-cli-export-tree-status-wipe-import-outcome-aggregates-contract-freeze.md), [wipe-import outcome require-gate](../plans/2026-09-05-cli-export-tree-status-wipe-import-outcome-require-gate-contract-freeze.md), and [export-tree-status-status](../plans/2026-09-05-cli-export-tree-status-status-contract-freeze.md). The tree-owned print-only helper under `contrib/lab-retention-gc/` can optionally dump the same script as `import-export-drill.sh` when `METIN2_IMPORT_EXPORT_TREE` and `METIN2_IMPORT_DRIVER` are set, forwards the scoped-replace opt-in only when `METIN2_IMPORT_PRINT_SCOPED_REPLACE=YES`, and forwards the two-phase opt-in only when `METIN2_IMPORT_PRINT_TWO_PHASE_WIPE_ROSTER=YES` — see [contrib import-export drill print helper](../plans/2026-08-27-contrib-import-export-drill-print-helper.md) and [lab retention / GC print-only unit samples](lab-retention-gc-unit-samples.md). Hermetic `/bin/sh` execution against build-tagged SQLite is owned by [hermetic import-export drill SQLite execution proof](../plans/2026-08-27-hermetic-import-export-drill-sqlite-execution-proof.md), including the seeded non-empty retained-tree twin in [hermetic import-export drill SQLite seeded tree](../plans/2026-08-27-hermetic-import-export-drill-sqlite-seeded-tree.md), the opt-in scoped-replace twin in [hermetic import-export-drill opt-in scoped-replace SQLite](../plans/2026-09-03-hermetic-import-export-drill-opt-in-scoped-replace-sqlite.md) (empty-tree full replace + seeded omit-roster re-backfill; FK-safe print order puts tip-`0002` last), and the two-phase wipe→roster→reimport twin owned by this plan's hermetic proof (tip-`0003`+`0027` / tip-`0015`+`0028` / tip-`0010`+`0029` attributes already seeded — see [ops docs tip sync after catalog tips 0027/0028/0029](../plans/2026-08-31-ops-docs-0027-0028-0029-attributes-tip-sync.md) and [seeded ground-item instance-attributes tip sync](../plans/2026-08-31-seeded-ground-item-instance-attributes-import-export-drill.md)). See also [CLI import-export](../plans/2026-08-27-cli-import-export.md).
