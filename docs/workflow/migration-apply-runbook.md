...
- treating `apply-lock-status` or `manual_clear_candidate=true` as authorization to delete an existing lock; it is an inspection helper only, and lab recovery uses aside-rename after operator judgment.
