...
After schema migrations are applied, operators can backfill retained migration-shaped exports with `metin2-migrate import-export --kind <kind> --export <path> --driver <driver> --dsn <dsn> --i-confirm-sql-import`. This reuses the programmatic `Import*` seams (insert-only by default; no silent upsert) and still does not register a stock production driver. Tip-`0003` `character-item-state` may additionally pass `--i-confirm-scoped-replace` to delete-then-insert only the export's character scope (inventory / equipment / quickslots) inside one transaction — see [character item-state import scoped replace GREEN](../plans/2026-08-31-character-item-state-import-scoped-replace-green.md). Tip-`0004` `character-quest-state` may likewise pass `--i-confirm-scoped-replace` to delete-then-insert only the export's character scope (`character_quest_flags`) — see [character quest-state import scoped replace GREEN](../plans/2026-09-01-character-quest-state-import-scoped-replace-green.md) and the prior freeze [character quest-state import replace/upsert contract freeze](../plans/2026-09-01-character-quest-state-import-replace-upsert-contract-freeze.md). Tip-`0015` `character-safebox-state` may likewise pass `--i-confirm-scoped-replace` to delete-then-insert only the export's character scope (`character_safebox_passwords` / `character_safebox_items`, including additive `0025` sockets + `0028` attributes) — see [character safebox-state import scoped replace GREEN](../plans/2026-09-01-character-safebox-state-import-scoped-replace-green.md) and the prior freeze [character safebox-state import replace/upsert contract freeze](../plans/2026-09-01-character-safebox-state-import-replace-upsert-contract-freeze.md). Tip-`0011` `character-point-state` may likewise pass `--i-confirm-scoped-replace` to delete-then-insert only the export's character scope (`character_points`, including fixed-width wipe-to-empty via declared `character_ids`) — see [character point-state import scoped replace GREEN](../plans/2026-09-01-character-point-state-import-scoped-replace-green.md) and the prior freeze [character point-state import replace/upsert contract freeze](../plans/2026-09-01-character-point-state-import-replace-upsert-contract-freeze.md). Tip-`0023` `character-myshop-unit-prices` may likewise pass `--i-confirm-scoped-replace` to delete-then-insert only the export's character scope (`character_myshop_unit_prices`, including wipe-to-empty via declared `character_ids`) — see [character myshop unit-prices import scoped replace GREEN](../plans/2026-09-01-character-myshop-unit-prices-import-scoped-replace-green.md) and the prior freeze [character myshop unit-prices import replace/upsert contract freeze](../plans/2026-09-01-character-myshop-unit-prices-import-replace-upsert-contract-freeze.md). Tip-`0002` `account-character-roster` may likewise pass `--i-confirm-scoped-replace` to delete-then-insert only the export's account scope (`accounts` / `characters`, including wipe-to-empty via declared `account_ids`; child tip domains fail closed rather than cascade) — see [account/character roster import scoped replace GREEN](../plans/2026-09-02-account-character-roster-import-scoped-replace-green.md) and the prior freeze [account/character roster import replace/upsert contract freeze](../plans/2026-09-02-account-character-roster-import-replace-upsert-contract-freeze.md). Tip-`0010` `bootstrap-ground-item-state` may likewise pass `--i-confirm-scoped-replace` to delete-then-insert only the export's VID scope (`bootstrap_ground_items`, including additive `0026` sockets + `0029` attributes and wipe-to-empty via declared `vids`) — see [bootstrap ground-item-state import scoped replace GREEN](../plans/2026-09-02-bootstrap-ground-item-state-import-scoped-replace-green.md) and the prior freeze [bootstrap ground-item-state import replace/upsert contract freeze](../plans/2026-09-02-bootstrap-ground-item-state-import-replace-upsert-contract-freeze.md). Tip-`0009` `item-template-state` may likewise pass `--i-confirm-scoped-replace` to delete-then-insert only the export's vnum scope (`item_templates` plus child socket / attribute / use-effect / equip-effect / refine-info / refine-material tables, including additive `0021` `keep_on_fail` + `0022` `fail_result_vnum` and wipe-to-empty via declared `vnums`) — see [item-template-state import scoped replace GREEN](../plans/2026-09-02-item-template-state-import-scoped-replace-green.md) and the prior freeze [item-template-state import replace/upsert contract freeze](../plans/2026-09-02-item-template-state-import-replace-upsert-contract-freeze.md). Tip-`0013` `static-actor-content-state` may likewise pass `--i-confirm-scoped-replace` to delete-then-insert only the export's entity / interaction-definition / combat-profile scope (`interaction_definitions` + merchant / quest-flag children, `static_actors` / `static_actor_reward_drops`, and `static_actor_combat_profiles` / death-reward drops, including additive `0016`–`0020`) — see [static-actor content-state import scoped replace GREEN](../plans/2026-09-02-static-actor-content-state-import-scoped-replace-green.md) and the prior freeze [static-actor content-state import replace/upsert contract freeze](../plans/2026-09-02-static-actor-content-state-import-replace-upsert-contract-freeze.md). Tip-`0007` `auth-login-ticket-handoff` may likewise pass `--i-confirm-scoped-replace` to delete-then-insert only the export's login-key scope (`auth_login_tickets`, including optional consumed history) — see [auth-login-ticket-handoff import scoped replace GREEN](../plans/2026-09-03-auth-login-ticket-handoff-import-scoped-replace-green.md) and the prior freeze [auth-login-ticket-handoff import replace/upsert contract freeze](../plans/2026-09-02-auth-login-ticket-handoff-import-replace-upsert-contract-freeze.md). Other kinds stay insert-only. See [CLI import-export](../plans/2026-08-27-cli-import-export.md).
...
The printer never executes the imports, never opens a database, and never embeds the DSN value; each printed `import-export` line still requires `--i-confirm-sql-import` at execution time. The tree-owned print-only helper under `contrib/lab-retention-gc/` can optionally dump the same script as `import-export-drill.sh` when `METIN2_IMPORT_EXPORT_TREE` and `METIN2_IMPORT_DRIVER` are set — see [contrib import-export drill print helper](../plans/2026-08-27-contrib-import-export-drill-print-helper.md) and [lab retention / GC print-only unit samples](lab-retention-gc-unit-samples.md). Hermetic `/bin/sh` execution against build-tagged SQLite is owned by [hermetic import-export drill SQLite execution proof](../plans/2026-08-27-hermetic-import-export-drill-sqlite-execution-proof.md), including the seeded non-empty retained-tree twin in [hermetic import-export drill SQLite seeded tree](../plans/2026-08-27-hermetic-import-export-drill-sqlite-seeded-tree.md) (tip-`0003`+`0027` / tip-`0015`+`0028` / tip-`0010`+`0029` attributes already seeded — see [ops docs tip sync after catalog tips 0027/0028/0029](../plans/2026-08-31-ops-docs-0027-0028-0029-attributes-tip-sync.md) and [seeded ground-item instance-attributes tip sync](../plans/2026-08-31-seeded-ground-item-instance-attributes-import-export-drill.md)). See also [CLI import-export drill printer](../plans/2026-08-27-cli-import-export-drill.md).
