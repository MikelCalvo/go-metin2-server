...
- `migration-apply.lock` — an operator-chosen local lock path that should not already exist;
...
- If `--lock-file` already exists, assume another operator or interrupted run owns the migration window. Do not delete it blindly; inspect process ownership and deployment notes first.
