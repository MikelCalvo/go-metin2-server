...
For a retained `export-quarantine-drill` tree, the printer keeps gated `$BASE/export-tree-status.json` plus `$BASE/export-tree-status-status.json` after the last `quarantine-export` line (`--require-quarantine-complete` only) so operators can fail closed before this import walk — see [CLI export-quarantine-drill export-tree-status contract freeze](../plans/2026-09-08-cli-export-quarantine-drill-export-tree-status-contract-freeze.md). Then print the reviewable import walk without embedding a DSN via:
