...
The printer never executes the imports, never opens a database, and never embeds the DSN value; each printed `import-export` line still requires `--i-confirm-sql-import` at execution time. Default output stays insert-only; pass `--i-confirm-print-scoped-replace` to print `--i-confirm-scoped-replace` on every tip-kind line for lab re-backfill after tip rebuild — see [import-export-drill opt-in scoped-replace printer](../plans/2026-09-03-import-export-drill-opt-in-scoped-replace.md). The tree-owned print-only helper under `contrib/lab-retention-gc/` can optionally dump the same script as `import-export-drill.sh` when `METIN2_IMPORT_EXPORT_TREE` and `METIN2_IMPORT_DRIVER` are set, and forwards the scoped-replace opt-in only when `METIN2_IMPORT_PRINT_SCOPED_REPLACE=YES` — see [contrib import-export drill print helper](../plans/2026-08-27-contrib-import-export-drill-print-helper.md) and [lab retention / GC print-only unit samples](lab-retention-gc-unit-samples.md). Hermetic `/bin/sh` execution against build-tagged SQLite is owned by [hermetic import-export drill SQLite execution proof](../plans/2026-08-27-hermetic-import-export-drill-sqlite-execution-proof.md), including the seeded non-empty retained-tree twin in [hermetic import-export drill SQLite seeded tree](../plans/2026-08-27-hermetic-import-export-drill-sqlite-seeded-tree.md) and the opt-in scoped-replace twin in [hermetic import-export-drill opt-in scoped-replace SQLite](../plans/2026-09-03-hermetic-import-export-drill-opt-in-scoped-replace-sqlite.md) (empty-tree full replace + seeded omit-roster re-backfill; FK-safe print order puts tip-`0002` last) (tip-`0003`+`0027` / tip-`0015`+`0028` / tip-`0010`+`0029` attributes already seeded — see [ops docs tip sync after catalog tips 0027/0028/0029](../plans/2026-08-31-ops-docs-0027-0028-0029-attributes-tip-sync.md) and [seeded ground-item instance-attributes tip sync](../plans/2026-08-31-seeded-ground-item-instance-attributes-import-export-drill.md)). See also [CLI import-export drill printer](../plans/2026-08-27-cli-import-export-drill.md).
