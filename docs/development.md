...
`apply-lock-status` is the read-only lock-triage companion for `apply --lock-file`: it reads a non-symlink regular metadata-only `go-metin2-migration-apply-lock-v1` file with a 16 KiB cap, returns `present: false` when the path is absent, and returns `go-metin2-migration-apply-lock-status-v1` JSON for valid locks without opening the database target, deleting the lock file, or exposing DSNs / executable SQL.
