...
For safer on-box checks before a backup, restore, login-handoff investigation, or migration runbook, the file stores expose read-only validation summaries. `accountstore.FileStore.Validate()` returns account count, character count, and deterministic login list through loopback-only `POST /local/account-store/validate`. `loginticket.FileStore.Validate()` returns pending ticket count, deterministic login list, and matching login-key list through loopback-only `POST /local/login-tickets/validate`, without consuming or deleting handoff tickets. Both endpoints are shipped on `gamed`, return `409` when committed store state fails validation, and are intentionally operator/debug surfaces, not gameplay or remote admin APIs.
