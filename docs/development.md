...
`cmd/metin2-migrate` is the first shipped migration CLI. Its catalog, status, empty-ledger-snapshot, ledger-snapshot, plan, plan-artifact, apply-preflight, and apply-lock-status commands are read-only/preflight surfaces:
...
go run ./cmd/metin2-migrate apply-preflight --ledger-snapshot snapshot.json --target-version latest --plan-artifact plan-artifact.json
