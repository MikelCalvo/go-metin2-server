...
`backup-restore-drill` is the offline companion to [file-store backup/restore drill](workflow/file-store-backup-restore-drill.md): it reads a retained `GET /local/runtime-config` JSON snapshot (file or stdin), requires absolute non-empty `persistence.*` paths, fails closed when file-path stores share a parent directory, and prints a path-aware shell script of preflight/backup/validate/aside-rename/restore curls. Optional `--ops-base-url` defaults to `http://127.0.0.1:6060` and `--backup-base` defaults to `/var/metin2/backups/drill`. The command never executes HTTP, opens a database, writes files, or mutates live stores.
