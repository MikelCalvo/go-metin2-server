...
`backup-restore-drill` is the offline companion to [file-store backup/restore drill](workflow/file-store-backup-restore-drill.md): it reads a retained `GET /local/runtime-config` JSON snapshot (file or stdin), requires absolute non-empty `persistence.*` paths, fails closed when file-path stores share a parent directory, and prints a path-aware shell script of aggregate preflight, optional store validate + crash-temp cleanup triage, backup, backup/validate, aside-rename, and restore curls. Optional `--ops-base-url` defaults to `http://127.0.0.1:6060` and `--backup-base` defaults to `/var/metin2/backups/drill`. The command never executes HTTP, opens a database, writes files, or mutates live stores; the printed `crash-temps/cleanup` curls are operator-run triage only and mutate hidden crash-temp residue after validate when an operator chooses to execute that section.
