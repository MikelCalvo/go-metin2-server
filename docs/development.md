...
`cmd/metin2-migrate` is the first shipped migration CLI. Its catalog, status, empty-ledger-snapshot, ledger-snapshot, plan, plan-artifact, apply-preflight, apply-preflight-status, apply-lock-status, quarantine-export, and backup-restore-drill commands are read-only/preflight surfaces:
