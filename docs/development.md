...
`db/migrations` now owns the first validated migration catalog skeleton for future DB-backed stores. The embedded catalog is intentionally small: `0001_bootstrap_schema_migrations` creates only the schema migration ledger, and `0002_account_character_roster` freezes the first schema-only account/character roster boundary. These migrations do not imply that accounts, characters, items, content, or runtime state are DB-backed yet.

The `schema_migrations` ledger stores `version`, `name`, `up_sha256`, and `applied_at` so a future migrator can verify that an applied historical migration still matches the project-owned SQL body. The account/character roster migration defines `accounts` and `characters` tables for normalized logins, explicit account IDs, four select-screen slots, normalized character names, bootstrap appearance/stat/location/guild/gold fields, and timestamps; inventory, equipment, quickslots, item instances, quest state, login tickets, authored content, and world runtime state remain deliberately out of scope. `internal/accountstore` can now project committed JSON snapshots into that roster shape through `ExportAccountCharacterRoster()` and the loopback-only `GET /local/account-store/exports/account-character-roster` endpoint. The projection is deterministic, schema-shaped, metadata-bearing (`migration_version` / `migration_name`), and read-only: it does not open a database, emit SQL, apply migrations, or mutate the account store, and it fails closed if a committed snapshot would violate the roster contract. The `db/migrations` package validates migration file naming, up/down pairing, contiguous versions from `0001`, non-empty UTF-8 SQL bodies, deterministic ordering, project-owned header comments, and the mandatory `migrations.manifest.json` checksum/path manifest. It also exposes read-only dry-run planners: `PlanUpToLatest` / `PlanCatalogUpToLatest` compare already-read ledger rows with the catalog, `PlanToVersion` / `PlanCatalogToVersion` can preview an explicit target version including rollback-to-zero, and `ReadSQLLedgerEntries` / `PlanUpToLatestFromSQLLedger` / `PlanToVersionFromSQLLedger` add a narrow `database/sql`-compatible reader seam that queries only `version`, `name`, and `up_sha256` from `schema_migrations`, closes rows, and fails closed on query/scan/iteration/close errors. These planners return pending migration metadata without applying SQL or exposing SQL as the plan payload. `gamed` wires the metadata planners through loopback-only `GET /local/db/migrations/status` for latest-version status and `GET /local/db/migrations/plan?target_version=N` for explicit target previews: without DB config they use an empty applied ledger, and with both DB driver and DSN configured they read only the ledger rows through `database/sql`. The migration-plan endpoints and roster export are metadata/projection-only surfaces and do not imply that runtime stores are DB-backed. Run `go test ./db/migrations ./internal/accountstore` before adding or changing migration files or roster projection behavior so manifest/checksum drift and schema-shape drift are caught locally before CI.
