...
`cmd/metin2-migrate` is the first shipped migration CLI. Its catalog, status, empty-ledger-snapshot, ledger-snapshot, plan, plan-artifact, apply-preflight, apply-preflight-status, and apply-lock-status commands are read-only/preflight surfaces:
