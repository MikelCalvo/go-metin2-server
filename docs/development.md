...
`apply` requires an operator-supplied `database/sql` driver name, DSN, strict offline ledger snapshot, and target version. It reuses `db/migrations.ApplyToVersion`, so pending up/down migration statements run inside one transaction and the transaction-local `schema_migrations` ledger is re-verified before commit when applicable. Successful output is metadata-only `ApplyResult` JSON; executable SQL and DSNs are not printed, and the supplied DSN is redacted from apply errors. Operators may pass `--plan-sha256 <hex>` to require that the metadata-only dry-run `Plan` recomputed from the supplied ledger snapshot and target matches a previously inspected `metin2-migrate plan` JSON artifact before the CLI opens the database. Operators may also pass `--audit-file <path>` to write a metadata-only `go-metin2-migration-apply-audit-v1` JSON artifact with the applied result, resolved target, configured driver name, DSN-present boolean, and SHA-256 of the exact offline ledger snapshot; the audit file is created exclusively, never overwrites an existing file, rejects no-op apply plans, and is removed if the apply attempt fails after reservation. The command is not a daemon endpoint, not an auto-migration hook, not a backup/restore mechanism, and not proof that runtime stores are DB-backed. Production use still needs an explicit driver/engine choice in the build, backup/restore runbook, and deployment coordination before real rollout.
