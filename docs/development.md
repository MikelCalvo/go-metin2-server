...
For safer on-box checks before a backup, restore, login-handoff investigation, or migration runbook, the file stores expose read-only validation summaries. `accountstore.FileStore.Validate()` returns account count, character count, deterministic login list, and any same-directory `.account-*.json` crash-temp residue through loopback-only `POST /local/account-store/validate`. `loginticket.FileStore.Validate()` returns pending ticket count, deterministic login list, matching login-key list, and any `.ticket-*.json` crash-temp residue through loopback-only `POST /local/login-tickets/validate`, without consuming or deleting handoff tickets. Both endpoints are shipped on `gamed`, return `409` when committed store state fails validation, and are intentionally operator/debug surfaces, not gameplay or remote admin APIs.

Authored item-template snapshots now follow the same validation posture. `itemstore.FileStore.Validate()` strictly loads the committed `item-templates.json` snapshot when present, treats a missing snapshot as an empty authored-template store that will fall back to built-in bootstrap templates at runtime, reports deterministic template counts and vnums, and includes same-directory `.item-templates-*.json` crash-temp residue while excluding the committed snapshot itself. `gamed` exposes this through loopback-only `POST /local/item-templates/validate`; malformed committed snapshots, unknown fields, trailing JSON, duplicate vnums, or invalid template policy return `409` instead of letting operators mistake corrupt authored content for the built-in fallback path.
