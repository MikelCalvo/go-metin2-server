...
- [ ] Confirm the pending-frame chase executor can now apply one retained engaged chase step after an accepted hit or proximity acquisition: wait through the owned `5s` chase delay while still engaged/in-leash and confirm the retained owner sees the ordinary delete/add/info/update refresh toward the owner without clearing the selected combat target; if the actor becomes `return_required`, confirm chase loses to the return-step path. Proximity acquisition alone may arm chase and, after the same `5s` delay, apply that delete/readd chase step, and may also arm the delayed server-origin retaliation cadence, without inventing selected-target ownership or an immediate retaliation piggyback. Once the frozen chase `MOVE` runtime consumer lands, the retained-viewer path should instead queue one server-driven `MOVE` replication for the actor VID at the planned next coordinates while remove/add visibility cases keep `CHARACTER_DEL` / add-info-update. If loopback ops access is available while that chase deadline is armed, query `GET /local/spawn-group-chase-steps`, `GET /local/spawn-group-chase-steps/{entity_id}`, and `GET /local/maps/{map_index}/spawn-group-chase-steps` and confirm the pending row exposes the same `entity_id`, current `actor`, planned `step.next`, `ready_at`, and `remaining_ms` countdown without mutating position or engagement; map-local lookup should include only pending chase steps for that actor's current effective map and should return an empty array for known maps with no due rows. Until that MOVE consumer lands, chase remains delete/readd visibility only.
