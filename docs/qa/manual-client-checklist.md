...
- that owner-side retaliation point-loss now clamps at `0` HP too; once the floor is reached the current slice stops further retaliation point-loss and later same-owner `ATTACK` attempts fail closed too, without yet claiming broader player-death behavior
