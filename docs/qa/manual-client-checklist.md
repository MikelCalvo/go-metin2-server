...
- [ ] Confirm the selected session receives self-only refreshes for both slots: source stack remains in `A` with the reduced count and the split stack appears in `B`; when the source stack carries presence-aware instance sockets/attributes, confirm both `ITEM_SET` frames keep that presence (including explicit zero), the destination identity is fresh/independent, and the source remainder also keeps an independent clone of source presence rather than aliasing the pre-split live inventory pointer (`docs/plans/2026-09-06-item-move-partial-split-remainder-preserve-instance-sockets-attributes.md`; focused remainder proofs stay the next GREEN twin)
...
- empty-destination partial splits plus compatible occupied-destination partial, exact, and zero-count merges are accepted and persisted; partial empty-destination splits mint a fresh destination identity and clone presence-aware instance sockets/attributes independently of the source remainder while the source remainder must also keep an independent clone of source presence (`docs/plans/2026-09-06-item-move-partial-split-remainder-preserve-instance-sockets-attributes.md`; focused remainder proofs stay the next GREEN twin); compatible partial merges keep an independent source-remainder presence clone while destination presence wins on the merged cell; full-stack incompatible occupied destinations swap and persist instead of failing closed
