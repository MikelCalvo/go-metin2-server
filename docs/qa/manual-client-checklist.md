...
- that owner-side retaliation point-loss now clamps at `0` HP too; once the floor is reached the current slice emits self-only `DEAD(owner_vid)` plus self-only `TARGET(0, 0)`, and later same-owner combat `TARGET` / `ATTACK` attempts fail closed too, without yet claiming broader player-death behavior
...
- once retaliation has already driven the owning character to `0` HP, later owner-side combat `TARGET` / `ATTACK`, owner `MOVE` / `SYNC_POSITION`, owner static-actor `INTERACT`, owner merchant-buy, and owner slash `/use_item` attempts fail closed too, without yet claiming broader chat or revive policy
