...
- if the target has only partial room, both source and target counts refresh, the source item quickslot remains, and item quickslots bound to the refreshed target cell are cleared while non-item quickslots with the same byte slot remain
