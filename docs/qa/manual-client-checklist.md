...
- [ ] If using `docs/examples/bootstrap-npc-service-bundle.json`, interact with `QuestHunter` after that kill credit advances `killed_qa_mob`; confirm one self-only info chat acknowledging the turn-in, one self-only `PLAYER_POINT_CHANGE(POINT_GOLD)` for authored `reward_gold = 100`, one self-only `PLAYER_POINT_CHANGE(POINT_EXP)` for authored `reward_experience = 50`, carried-inventory SET/UPDATE frames for authored `reward_items` (`27001` x1 then `11200` x1), and that `GET /local/quest-state/characters/{character}` no longer reports `killed_qa_mob` while reconnect/`GET /local/currency/{character}` / `GET /local/points/{character}` / `GET /local/inventory/{character}` keep the updated gold, experience, potion, and sword; interacting with `QuestHunter` before the kill (or after the turn-in clears the flag) must return `Quest requirements are not met.` without mutating quest-state, gold, experience, or inventory
