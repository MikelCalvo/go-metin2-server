...
- [ ] After the first accepted live owner hit, stop sending `ATTACK` for at least the owned `1s` retaliation delay and confirm one queued self-only HP `POINT_CHANGE` follow-up beat plus one self plain `DAMAGE_INFO(owner_vid, abs(delta))` arrives without a second client attack
- [ ] Wait one more owned `1s` delay without another accepted hit and confirm a second queued self-only HP `POINT_CHANGE` follow-up beat plus another owner self plain `DAMAGE_INFO` arrives while the mob stays alive and engaged
