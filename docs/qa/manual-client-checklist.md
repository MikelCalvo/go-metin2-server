...
- this is still not a completed safebox or mall feature: money / mall / client `SAFEBOX_CHANGE_PASSWORD` packets remain deferred; warehouse password challenge + durable optional password + `/safebox_change_password` + same-account cell rematerialize are owned for check-in/out/move
