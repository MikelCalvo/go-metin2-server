...
- [ ] While still at `0` HP and before `/restart_here`, try fresh busy-shell opens: `/open_safebox`, `/open_cube`, host-only `CG::MYSHOP`, a refineable `REFINE` preview, and `EXCHANGE START` against a living visible peer; also have that living peer try `EXCHANGE START` against the dead owner
