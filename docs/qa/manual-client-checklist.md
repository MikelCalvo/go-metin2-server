...
- Open private shop host movement deny (contract freeze): with an accepted private shop open, host MOVE / SyncPosition must stay fail-closed with no position change and the shop must stay open until empty-sign close (no walk-away auto-close / no reject chat).
