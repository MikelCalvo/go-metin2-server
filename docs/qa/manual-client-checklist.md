...
- if the target has only partial room, both source and target counts refresh as count-only `ITEM_UPDATE` frames that keep destination presence-aware instance sockets/attributes authoritative (including explicit zero; source presence discarded; omitted destination keeps template-fallback encode), source and target item quickslots remain stable, and non-item quickslots with the same byte slot remain unchanged
