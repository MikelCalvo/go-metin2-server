...
- [ ] When that immediate or delayed retaliation floor reaches `0` HP, confirm the client also receives one self-only `TARGET(0, 0)` clear instead of keeping the stale engaged practice mob selected
...
- that owner-side retaliation point-loss now clamps at `0` HP too; once the floor is reached the current slice clears the stale target with self-only `TARGET(0, 0)` and later same-owner combat `TARGET` / `ATTACK` attempts fail closed too, without yet claiming broader player-death behavior
