...
- selected characters at the bootstrap zero-HP floor cannot pick up visible ground items; `ITEM_PICKUP` fails closed with no item pickup notice, inventory/gold mutation, or ground-handle removal; after `/restart_here` / `/restart_town` the same owner-side pickup succeeds normally again (`TestGameSessionFlowPostFloorItemPickupFailsClosed`, `TestGameSessionFlowPostFloorItemPickupFailsClosedBeforeRestartTown`)
...
- post-floor `ITEM_PICKUP` stays silent and non-mutating while at `0` HP and leaves the pending ground handle registered; after `/restart_here` the same pickup succeeds with ordinary `GROUND_DEL` + `ITEM_SET` + `ITEM_GET`, and after `/restart_town` a town-local drop then pickup succeeds at the empire town-return position (`TestGameSessionFlowPostFloorItemPickupFailsClosed`, `TestGameSessionFlowPostFloorItemPickupFailsClosedBeforeRestartTown`)
