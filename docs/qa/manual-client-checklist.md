...
Expected result:
- ordinary unsupported storage-facing packets (`SAFEBOX_CHECKOUT`, `SAFEBOX_ITEM_MOVE`, `MALL_CHECKOUT`) are parsed by the game socket but remain fail-closed in the shipped bootstrap runtime: no storage-transfer response frames (`SAFEBOX_DEL`, `SAFEBOX_WRONG_PASSWORD`, `SAFEBOX_MONEY_CHANGE`, `MALL_OPEN`, `MALL_SET`, or `MALL_DEL`) are visible for those paths, no carried inventory/equipment/quickslot/point/gold state changes, no ground actor appears, no exchange/merchant peer frames are queued, and reconnect/operator inspection shows the selected-character snapshot unchanged
