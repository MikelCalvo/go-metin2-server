...
- [ ] After satisfying that gate (for example via the owned quest-flag NPC path), interact with `CubeMaster` again and confirm optional self-only info chat `The craftsman lights the forge.` plus command chat `cube open 20022`. Operators can also inspect `GET /local/content-bundle/open-cube-routes/CubeMaster` or `GET /local/content-bundle/maps/1/open-cube-routes` for the authored race_num/gate placement without opening cube in-game; a second interact after the 1s cooldown while still open must emit `The Build window is already open.` without a second open
