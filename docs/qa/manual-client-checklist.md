...
- [ ] After a successful `probability = 100` refine preview, confirm the dialog with a matching `REFINE` (`same pos` / `same type`) that has enough gold and materials, then reconnect and inspect the resulting source `vnum`, gold, and material stacks
- [ ] After a successful refine preview, send `REFINE` with `type = 255` and confirm the dialog cancels with no mutation
- [ ] After a successful refine preview, attempt confirm while a merchant / exchange / safebox presentation is open and confirm the attempt stays fail-closed with no refine mutation
...
- after a successful preview for `probability = 100`, a matching confirm may consume gold/materials, replace the source carried `vnum` with `result_vnum` in the same cell, persist, and emit self-only material refreshes + result `ITEM_SET` + gold `PLAYER_POINT_CHANGE`; `type = 255` cancels the open dialog with no mutation; busy merchant/exchange/safebox windows, insufficient gold/materials, stale source identity, and probability below `100` stay fail-closed for confirm
...
- the client may see one self-only `GC::SAFEBOX_SIZE` presentation frame from `/open_safebox` and the same-socket character becomes busy for exchange `START` under the owned requester/partner busy-window chat strings; `/close_safebox` clears that busy presentation without storage mutation; out-of-range `/open_safebox 4` (and other sizes outside `1..3`) emit no frames, do not fall through as ordinary talking chat, and do not set the busy presentation flag
...
- [ ] If packet tooling allows it, have one side display a carried item, have that side send `ACCEPT`, arrange the paired receiver so the displayed item cannot fit into carried inventory under the loaded template (full inventory with no compatible stack room), then have the receiver send the second `ACCEPT`
...
- [ ] While an exchange shell is still open and showing one carried item, attempt to drop a template-guarded carried item whose loaded template authors `drop_reject_message`
- [ ] While an exchange shell is still open, equip one carried item and unequip one worn item with same-socket equipment movement or the `/equip_item` / `/unequip_item` harness
- [ ] While an exchange shell is still open, complete one same-socket merchant `SHOP BUY` and one merchant `SHOP SELL` / `SELL2` from a previously opened merchant window
...
Run this when the target build has authored QA `spawn_groups` practice-mob content loaded with a non-zero bootstrap death-reward descriptor. The repository example bundle at `docs/examples/bootstrap-npc-service-bundle.json` now includes `practice.qa_reward_mob` with EXP, gold, one fixed drop-vnum reward descriptor, and kill-quest credit that advances `quest:first_steps.killed_qa_mob` for the selected killer after the accepted death edge. Separate authoring-only fixtures at `docs/examples/bootstrap-drop-table-authoring-bundle.json` and `docs/examples/bootstrap-regen-authoring-bundle.json` can be used to validate fixed `drop_tables` expansion and one-count `regen_spawns` expansion for EXP, gold, and drop-vnum channels before importing the canonicalized spawn-group reward descriptor. The formula-profile fixture in 5.10.1 also carries profile-default reward metadata and may be reused when validating reward frames after a formula kill. The narrower kill-quest-only fixture at `docs/examples/bootstrap-kill-quest-credit-bundle.json` remains available when QA wants that credit seam without the broader NPC service content.
...
- [ ] If using `docs/examples/bootstrap-npc-service-bundle.json`, interact with `QuestHunter` after that kill credit advances `killed_qa_mob`; confirm one self-only info chat acknowledging the turn-in and that `GET /local/quest-state/characters/{character}` no longer reports `killed_qa_mob`; interacting with `QuestHunter` before the kill (or after the turn-in clears the flag) must return `Quest requirements are not met.` without mutating quest-state
...
- kill-quest credit is a separate spawn-group content seam from the EXP/gold/drop descriptor; the combined NPC service QA fixture now authors both, and `docs/examples/bootstrap-kill-quest-credit-bundle.json` remains the narrow credit-only example (see `quest-state-bootstrap.md`)
...
- [ ] On the owning client, confirm each accepted live content-loaded hit now returns the usual target-refresh, one immediate self-only HP `POINT_CHANGE` decrement, and then one self plain `DAMAGE_INFO(target_vid, damage)` hit-effect companion while the mob remains alive; with a second visible live client, confirm that peer receives exactly one queued matching `DAMAGE_INFO` hit-effect frame without also receiving the owner's self-only target refresh or retaliation point-change
...
- its runtime combat loop still reuses the owned built-in practice-profile semantics for HP, death, timed respawn, retaliation, and the first fixed same-target `250ms` normal-attack cadence gate, while current spawn-backed `DAMAGE_INFO` hit-effect emission now covers the owner and currently visible live peers without forwarding the owner's self-only target refresh or retaliation point-change
