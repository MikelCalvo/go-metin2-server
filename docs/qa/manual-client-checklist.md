...
- [ ] Lower the owning character's HP near `0` if you can, then confirm the immediate or delayed retaliation tick clamps at `0` HP instead of going negative, that the floor beat emits self `PLAYER_POINT_CHANGE(value=0)` → `DEAD(owner_vid)` → `TARGET(0, 0)` → `DAMAGE_INFO(owner_vid, abs(delta))`, that a visible live peer receives `DEAD(owner_vid)` then that same owner `DAMAGE_INFO`, and that no further delayed follow-up beat arrives once that floor is reached
