...
- invalid, malformed carried-item, anti-sell, equipped, runtime-locked, explicit zero-count `SELL2`, or zero-HP owner attempts fail closed and leave both live and persisted inventory/gold unchanged
...
- [ ] Immediately after `/restart_town`, try one same-target normal `ATTACK` without reselecting and confirm it still fails closed on the live socket with no response frame; if the same practice mob later remains alive and becomes visible again, send a fresh `TARGET` and confirm the mob resumes from its current runtime-owned HP instead of resetting because of the owner's recovery
