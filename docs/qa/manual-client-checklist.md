...
- [ ] If using that same QA bundle, keep the QA merchant window open after `QuestGuide` unlocks it, then INTERACT `QuestResetGuide` (`quest:first_steps_reset`): confirm that INTERACT prepends one merchant-family `GC::SHOP END` before the authored clear chat `Quest cleared: first_steps.met_guide = 0.`, gold/inventory stay unchanged, a later packet `SHOP BUY` against the already-closed window is silent/no-frame, and gated merchant/talk INTERACT now mismatch until `QuestGuide` writes `met_guide = 1` again. The composed PvE vertical gameplay proof (`TestPveVerticalAuthoringBundleClosesGuideUnlockKillCreditAndTurnIn`) now INTERACTs that reset actor on the authoring-form import path after `QuestHunter` turn-in. If clearing through `POST /local/quest-state/transition` instead, keep the merchant window open and confirm the next packet `SHOP BUY` / `SHOP SELL` / `SHOP SELL2` or `/shop_buy` auto-closes that stale window with one merchant-family `GC::SHOP END` and leaves gold/inventory unchanged until the merchant is opened again
