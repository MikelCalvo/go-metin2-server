...
- [ ] If that fixture reports `status = return_required`, send `TARGET` for that visible mob and confirm it fails closed with no target acknowledgement; this is a temporary stationary-mob gate until a later return/chase slice moves the actor back inside leash
