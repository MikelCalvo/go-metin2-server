...
- [ ] Note for QA tooling: commit-time busy-window / display / precondition drift after a second-accept finalize plan is built is covered by the shared-world `CommitExchangeFinalize` unit proof (`SharedWorldCommitExchangeFinalizeRejectsBusyWindowOpenedAfterAcceptPlan`) rather than a live mid-commit packet race; busy-window / gold-carrier drift returns the same self-only START/ACCEPT busy or gold-carrier info-chat strings to the commit requester and leaves the shell cancellable; Check/Space/gold-overflow/Other display/precondition drift emits the dual-sided info-chat strings named in the exchange bootstrap contract then self/peer `GC::EXCHANGE END` auto-cancel (`docs/plans/2026-08-28-exchange-finalize-reject-auto-cancel.md`)
- [ ] Note for next slice: busy-window / gold-carrier-cap `ACCEPT` and commit-time rejects still leave the shell cancellable after their owned self-only info-chat; the frozen companion `docs/plans/2026-08-28-exchange-busy-gold-carrier-reject-auto-cancel.md` owns chat-then-self/peer `GC::EXCHANGE END` auto-cancel for those paths before RED (`LESS_GOLD` / persist-fail stay cancellable)
