...
- [ ] Confirm there is still no live automatic chase executor or chase packet fanout yet: engaged practice mobs remain stationary in client-visible gameplay while `PlanStaticActorSpawnChaseStep` is covered by focused `internal/worldruntime` unit tests only
