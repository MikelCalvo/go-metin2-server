...
- a same-socket `/quit`, `/logout`, or `/phase_select` also counts as an immediate owner-disappearance boundary for that cadence in the current slice: the owner leaves shared-world visibility right away, the pending delayed follow-up beat is cancelled, and the still-live practice mob becomes targetable again at its current runtime-owned HP without waiting for disconnect, close, or later fresh bootstrap completion
