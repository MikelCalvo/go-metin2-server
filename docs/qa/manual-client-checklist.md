...
- equipping onto an already-occupied matching wear cell fails closed with exactly one self-only `CHAT_TYPE_INFO` `You are already wearing equipment.`; no item refresh, quickslot change, point change, appearance update, carried/equipment mutation, or persistence change is visible; occupied-wear swap/replace remains out of scope
...
- [ ] If authored QA NPC content is loaded, interact with the visible `Warehouse` `open_safebox` actor and confirm optional self-only info chat plus `CHAT_TYPE_COMMAND` `ShowMeSafeboxPassword` without `SAFEBOX_SIZE` / busy exchange state; then `/safebox_password 000000` (or the durable custom password) opens with `SAFEBOX_SIZE` (+ rematerialized `SAFEBOX_SET` when present) and becomes busy for exchange `START`; wrong password yields header-only `SAFEBOX_WRONG_PASSWORD` and leaves presentation closed; with the quest gate unmet, expect `Quest requirements are not met.` and no password prompt
...
- this is still not a completed safebox or mall feature: money / mall / change-password remain deferred; warehouse password challenge + durable optional password + same-account cell rematerialize are owned for check-in/out/move
