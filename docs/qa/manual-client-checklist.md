...
- once retaliation has already driven the owning character to `0` HP, later owner-side combat `TARGET` / `ATTACK`, owner `MOVE` / `SYNC_POSITION`, owner static-actor `INTERACT`, owner merchant-buy, owner slash `/use_item`, and owner peer-facing `CHAT` / `WHISPER` attempts fail closed too, while `/quit`, `/logout`, and `/phase_select` keep their current separate command behavior
