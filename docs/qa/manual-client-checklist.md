...
- [ ] While still at `0` HP and before `/restart_here`, try fresh busy-shell opens: `/open_safebox`, a pending warehouse `/safebox_password` after `ShowMeSafeboxPassword`, `/open_cube`, host-only `CG::MYSHOP`, a refineable `REFINE` preview, and `EXCHANGE START` against a living visible peer; also have that living peer try `EXCHANGE START` against the dead owner
- [ ] Confirm those post-floor open attempts stay silent and non-mutating: no `SAFEBOX_SIZE` / `cube open` / `SHOP_SIGN` / `REFINE_INFORMATION_NEW` / exchange start frames, no busy flags, no inventory/gold changes (`TestGameSessionFlowPostFloorOpenSafeboxFailsClosed`, `TestGameSessionFlowPostFloorSafeboxPasswordOpenFailsClosed`, `TestGameSessionFlowPostFloorOpenCubeFailsClosed`, `TestGameSessionFlowPostFloorMyShopOpenFailsClosed`, `TestGameSessionFlowPostFloorRefinePreviewOpenFailsClosed`, `TestGameSessionFlowPostFloorExchangeStartFailsClosed`, `TestGameSessionFlowPostFloorExchangeStartAgainstDeadPartnerFailsClosed`); after `/restart_here`, a stale pre-death `/safebox_password` still fails closed until a fresh warehouse interact re-arms the challenge
