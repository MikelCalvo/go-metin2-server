...
Expected result:
- the playable QA loop uses authored profile formula damage / max HP, authored acquire/leash radii, and authored chase/return/homeward/reaction delays plus `max_step`, not the built-in `practice_mob` one-point / `200`/`400` / `5s`/`1s`/`100` defaults
