...
- `quest_flag` applies exactly one selected-character compare-and-set transition, persists it to the quest-state store, and returns one self-only info acknowledgement only on success; repeated same-actor attempts that no longer match the authored `quest_from` fail closed without inventing quest UI
