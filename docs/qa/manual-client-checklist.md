...
- exclusive ownership timers, public ownership release, the bootstrap `300`-second destroy deadline, FileStore rematerialize with absolute timers, and process-local exclusive `OwnerID` rebind on matching owner rejoin are now owned for the bootstrap path; real party membership and graceful Leave→FileStore deletion semantics remain deferred
