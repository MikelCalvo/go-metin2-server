...
- [ ] If using `docs/examples/bootstrap-npc-service-bundle.json`, interact with `QuestHunter` after that kill credit advances `killed_qa_mob`; confirm one self-only info chat acknowledging the turn-in, one self-only `PLAYER_POINT_CHANGE(POINT_GOLD)` for authored `reward_gold = 100`, one carried-inventory SET/UPDATE for authored `reward_item_vnum = 27001` / `reward_item_count = 1`, and that `GET /local/quest-state/characters/{character}` no longer reports `killed_qa_mob` while reconnect/`GET /local/currency/{character}` / `GET /local/inventory/{character}` keep the updated gold and potion; interacting with `QuestHunter` before the kill (or after the turn-in clears the flag) must return `Quest requirements are not met.` without mutating quest-state, gold, or inventory
