...
- after `/open_safebox`, a non-`anti_safebox` `SAFEBOX_CHECKIN` into an empty in-range `safe_pos` removes the carried item, syncs source item quickslots, stores the item in same-session in-memory safebox state, persists the inventory/quickslot account snapshot, and emits self-only inventory removal plus `GC::SAFEBOX_SET`; `/close_safebox` clears only the busy presentation flag, and a later same-session `/open_safebox` re-emits `SAFEBOX_SIZE` plus remembered `SAFEBOX_SET` rows; reconnect/restart/logout still discard those in-memory contents until persistence exists; if a merchant window is open and the check-in would otherwise succeed, the client first sees self `GC::SHOP END`
