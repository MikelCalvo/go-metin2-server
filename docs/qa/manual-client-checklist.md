...
- import/adapt `docs/examples/bootstrap-pve-vertical-authoring-bundle.json` when validating the authoring-form regen/drop + quest loop (validate/import expands it into the same owned runtime content, including the gated `Warehouse` `open_safebox` actor and gated `CubeMaster` `open_cube` actor; the checked-in expanded twin is `docs/examples/bootstrap-pve-vertical-canonical-bundle.json`), or
