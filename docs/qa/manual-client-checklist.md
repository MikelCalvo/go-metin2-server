...
- [ ] If packet logging is available, confirm the initial `ITEM_SET` for template-authored fixtures carries the owned item `flags` bits for `stackable` / `sell_count_per_gold` / `slow_query` / `rare` / `unique` / `confirm_when_use` / `quest_use` / `quest_use_multiple` / `log` / `applicable`, carries the owned `anti_flags` bits for `anti_get`, transfer/job/sex/empire guards, projects authored socket/attribute display arrays and the template `highlight` byte, and leaves unowned bits zero
