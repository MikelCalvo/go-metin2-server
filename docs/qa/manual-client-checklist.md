...
- [ ] After a successful preview for authored `probability` in `1..99`, confirm with a matching `REFINE` that has enough gold and materials; production draws one `crypto/rand` roll so manual QA cannot force an exact outcome — observe either the owned success burst (`RefineSuceeded <type>`, result `ITEM_SET`) or the owned failure burst (`RefineFailed <type>`). Without `keep_on_fail`, failure destroys the source (`ITEM_DEL` + source quickslot sync). With authored `keep_on_fail`, failure keeps the source while still consuming materials/gold. Automated tests inject fixed rolls via `QueueRefineConfirmRollForTest`.
...
- item-template validation rejects `refine_info` without `refineable`, with a zero result `vnum`, negative cost, probability outside `0..100`, more than five materials, material rows with zero `vnum` / non-positive count, or `keep_on_fail` when `probability` is `0` or `100`
...
- this is still not a completed refine system: catalysts, downgrade/`fail_result_vnum` failure variants, and broader window choreography remain deferred beyond the confirm-after-preview success seam, the deterministic `probability = 0` destroy + `RefineFailed` companion, the `1..99` injected-roll confirm seam, and the template-authored `keep_on_fail` keep-failure companion
