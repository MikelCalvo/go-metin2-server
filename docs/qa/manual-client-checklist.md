...
- [ ] If slash harness tooling is available, run `/open_safebox` and confirm one self-only `GC::SAFEBOX_SIZE` plus one self-only `GC::SAFEBOX_MONEY_CHANGE` (default `0` when no durable warehouse gold) appear without inventory/carried-gold mutation. The composed PvE vertical gameplay proof now uses that lab opener after `QuestGuide` re-unlock so `SAFEBOX_CHECKIN` / `SAFEBOX_CHECKOUT` of turn-in `11200` can run without waiting out the 10-second warehouse password reopen cooldown
