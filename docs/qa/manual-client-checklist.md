...
- [ ] While still at `0` HP and before `/restart_here`, try fresh busy-shell opens: `/open_safebox`, `/open_cube`, host-only `CG::MYSHOP`, and `EXCHANGE START` against a living visible peer; also have that living peer try `EXCHANGE START` against the dead owner
- [ ] Confirm those post-floor open attempts stay silent and non-mutating: no `SAFEBOX_SIZE` / `cube open` / `SHOP_SIGN` / exchange start frames, no busy flags, no inventory/gold changes (`TestGameSessionFlowPostFloorOpenSafeboxFailsClosed`, `TestGameSessionFlowPostFloorOpenCubeFailsClosed`, `TestGameSessionFlowPostFloorMyShopOpenFailsClosed`, `TestGameSessionFlowPostFloorExchangeStartFailsClosed`, `TestGameSessionFlowPostFloorExchangeStartAgainstDeadPartnerFailsClosed`)
