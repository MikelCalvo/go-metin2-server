...
- finalization/result semantics beyond the first mutual-accept mutation + shell close remain unsupported in the shipped bootstrap runtime; apart from the current guard feedback, display-only item-add/item-del/gold-add/first-side accept paths, and the frozen mutual-accept finalize contract, unsupported exchange requests emit no exchange result frames and do not mutate state
...
- this is an exchange-window shell plus the first frozen mutual-accept finalize contract, not a completed exchange, trade, safebox, or player-shop feature
...
- [ ] Confirm there is still no live automatic chase executor or chase packet fanout yet: engaged practice mobs remain stationary in client-visible gameplay while `PlanStaticActorSpawnChaseStep` is covered by focused `internal/worldruntime` unit tests and the pending-frame chase executor contract in `spec/protocol/spawn-leash-bootstrap.md` stays ahead of implementation; once that executor lands, revisit this checklist for one-second engaged chase steps that preserve engagement, lose to `return_required`, and preflight before EnterGame/transfer visibility
