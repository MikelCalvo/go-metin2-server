...
- [ ] If using `docs/examples/bootstrap-npc-service-bundle.json`, pick up the practice-mob drop (`27001`) after kill credit advances `killed_qa_mob`, then interact with `QuestHunter`; confirm one self-only info chat acknowledging the turn-in, one self-only `PLAYER_POINT_CHANGE(POINT_GOLD)` for authored `reward_gold = 100`, one self-only `PLAYER_POINT_CHANGE(POINT_EXP)` for authored `reward_experience = 50`, carried-inventory DEL/UPDATE frames for authored `consume_items` (`27001` x1), carried-inventory SET/UPDATE frames for authored `reward_items` (`11200` x1), and that `GET /local/quest-state/characters/{character}` shows `killed_qa_mob` cleared. Interacting without the required carried potion must return `Quest requirements are not met.` without mutating gold/inventory/quest state
