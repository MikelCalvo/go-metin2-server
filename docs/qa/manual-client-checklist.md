...
- [ ] Quest-state work is currently a server-side persistence primitive only; use `POST /local/quest-state/validate` to preflight the configured snapshot and `POST /local/quest-state/crash-temps/cleanup` only for disposable interrupted `.quest-state-*.json` temp writes; no manual client quest UI, NPC quest progression, or quest reward behavior should be expected until a later client-visible slice wires it into runtime content
