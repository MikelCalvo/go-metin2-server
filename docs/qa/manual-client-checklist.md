...
- [ ] Carry a consumable item before retaliation reaches the owner's `0` HP floor, then after that floor is reached send `/use_item <slot>` and confirm it fails closed with no `GC PLAYER_POINT_CHANGE`, no `GC ITEM_SET`, no info chat, and no inventory / point mutation in loopback runtime snapshots
...
- a same-socket `/quit` or `/logout` also counts as an immediate owner-disappearance boundary for that cadence in the current slice: the owner leaves shared-world visibility right away, the pending delayed follow-up beat is cancelled, and the still-live practice mob becomes targetable again at its current runtime-owned HP without waiting for client disconnect or socket close to finish
...
- when either the immediate retaliation tick or the delayed follow-up beat reaches that same owner floor, currently visible peers also receive one queued `GC DEAD(owner_vid)` while broader corpse/respawn choreography still stays out of scope
