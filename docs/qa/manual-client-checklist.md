...
- `CG::MYSHOP` (`0x0802`) encode/decode is codec-owned only in this bootstrap: GAME deny-no-response dispatch is the next frozen seam (valid packets should stop being unexpected disconnects and instead decode + deny with no frames); there is still no accepted private-shop open/close/browse/buy path and no `GC::SHOP_SIGN` companion to exercise in manual client QA; keep treating personal shops as out of scope for playable PvE loop checks
