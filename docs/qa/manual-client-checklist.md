...
- [ ] If loopback ops access is available for the same return-required live mob, call `POST /local/spawn-groups/{entity_id}/return-home` and confirm the response reports `status = at_home`, the actor's `current` position equals its preserved `home`, old-position-only viewers receive `CHARACTER_DEL`, home-position viewers receive the normal add/info/update burst, and a fresh `TARGET` works again while HP/death/reward profile state is otherwise unchanged; if the mob was already selected/engaged when a successful return-home trigger runs, confirm the owning client also receives one self-only `TARGET(0, 0)` clear and must reselect before attacking again; if the trigger ran while the actor was already at home after an accepted hit, confirm any pre-trigger delayed retaliation beat does not arrive and only a fresh post-reselect/post-hit engagement arms a new delayed beat
