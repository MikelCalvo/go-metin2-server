...
- `quest_flag` turn-in `reward_items` that resolve to `anti_get` or selected-character-restricted templates fail closed before the quest transition: no chat/point/item frames and no quest-state/gold/experience/inventory mutation
