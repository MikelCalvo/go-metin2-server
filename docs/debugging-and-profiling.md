...
The quarantine contract requires `migration_version = 3`, `migration_name = "character_item_state"`, present (possibly empty) `inventory_items` / `equipment_items` / `quickslots` arrays, `character_id > 0`, globally unique item ids across inventory and equipment, unique `(character_id, slot)` and `(character_id, equip_slot)` pairs, unique `(character_id, position)` quickslots, migration-valid item/quickslot bounds, and presence-aware optional instance sockets (`has_sockets` false requires zero sockets; `has_sockets` true including all-zero is authoritative). Use this after retaining an export artifact and before any future DB backfill/import tool. It is not a repository write path and never emits SQL or DSNs. SQL import still requires tip-`0003` plus additive ledger `24` / `character_item_instance_sockets` before INSERT — see [seeded item instance-sockets tip sync](plans/2026-08-30-seeded-item-instance-sockets-import-export-drill.md).
