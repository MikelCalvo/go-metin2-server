...
The quarantine contract requires `migration_version = 10`, `migration_name = "bootstrap_ground_item_state"`, a present (possibly empty) `ground_items` array, and rows that satisfy the same fail-closed `0010` projection rules already owned by the live export path (unique positive `vid`, owner identity bounds, map/pickup bounds, exclusive item-count vs gold-amount shapes with gold using `vnum = 1`, and presence-aware optional instance sockets on item-shaped rows: `has_sockets` false requires zero sockets; `has_sockets` true including all-zero is authoritative; gold-shaped rows stay socket-less). Use this after retaining an export artifact and before any future DB backfill/import or crash-recovery tool. It is not a repository write path and never emits SQL or DSNs. SQL import still requires tip-`0010` plus additive ledger `26` / `bootstrap_ground_item_instance_sockets` before INSERT — see [seeded ground-item instance-sockets tip sync](plans/2026-08-30-seeded-ground-item-instance-sockets-import-export-drill.md).
