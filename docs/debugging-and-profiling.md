...
- `POST /local/content-bundle/import-preview` compares a candidate replacement against the live exported bundle and returns no-mutation `current` / `candidate` summaries plus count/amount `deltas`, including per-interaction-kind reference deltas, per-map before/after/signed deltas for changed authored map counts, and spawn reward EXP/gold totals
