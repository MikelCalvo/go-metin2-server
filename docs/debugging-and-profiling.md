...
Validates the durable pending ground-item FileStore without mutating live handles. This endpoint is available only on `gamed`, is loopback-only, rejects non-`POST` methods with `405`, rejects non-empty request bodies with `400`, rejects request bodies over 4 KiB with `413`, and returns `409` if the committed `ground-items.json` snapshot is a symlink, is malformed, has unknown/trailing JSON, duplicates a VID, or violates the durable restart contract. Successful responses are the deterministic ground-item summary (`ground_item_count`, `item_shaped_count`, `gold_shaped_count`, sorted `vids`, plus optional `crash_temp_count` / `crash_temp_files`). A missing committed snapshot is reported as an empty valid store. Routes use the `/local/ground-item-store/...` prefix so they do not collide with live `GET /local/ground-items` inspection routes.

### `POST /local/ground-item-store/crash-temps/cleanup`

Removes same-directory `.ground-items-*.json` crash-temp residue after first validating the committed snapshot through the same strict loader used by `/local/ground-item-store/validate` and `/local/persistence/status`. Loopback-only `POST`, empty body required, returns the post-cleanup summary or `409` on validation/cleanup failure.

### `POST /local/ground-item-store/backup`

Copies the durable pending ground-item snapshot into an operator-supplied empty destination directory and returns the validation summary of the copied snapshot set. Loopback-only `POST` with `{"dst_dir":"..."}`. A successful backup writes `ground-item-backup-manifest.json` (`go-metin2-ground-item-backup-v1`) with deterministic summary, filename, size, and SHA-256 checksum. Missing committed snapshots backup as an empty store with no synthetic snapshot file. Hidden `.ground-items-*.json` crash leftovers are ignored as payload.

### `POST /local/ground-item-store/backup/validate`

Dry-runs an operator-supplied ground-item backup source through the same strict manifest and snapshot checks used by backup, without mutating the active store. Loopback-only `POST` with `{"src_dir":"..."}`.

### `POST /local/ground-item-store/restore`

Restores the durable pending ground-item FileStore from an operator-supplied manifested backup into an empty destination and rematerializes the pending set into the live shared world (clearing any previous pending handles first). Loopback-only `POST` with `{"src_dir":"..."}`. Restore refuses live selected-character sessions, non-empty destinations, and destinations nested under the backup source. After FileStore replacement it applies the same despawn/exclusive-ownership filter used at process restart, then rewrites a fresh active backup manifest against the filtered live set.

### `POST /local/safebox-store/validate`

Validates the durable same-account safebox FileStore without mutating live presentation windows. This endpoint is available only on `gamed`, is loopback-only, rejects non-`POST` methods with `405`, rejects non-empty request bodies with `400`, rejects request bodies over 4 KiB with `413`, and returns `409` if the committed `safebox.json` snapshot is a symlink, is malformed, has unknown/trailing JSON, duplicates a character key, or violates the durable cell contract. Successful responses are the deterministic safebox summary (`character_count`, `cell_count`, sorted `logins`, sorted `character_keys` as `login:character_id`, plus optional `crash_temp_count` / `crash_temp_files` for `.safebox-*.json`). A missing committed snapshot is reported as an empty valid store. Routes use the `/local/safebox-store/...` prefix so they stay separate from live open-safebox content-bundle inspection routes.

### `POST /local/safebox-store/crash-temps/cleanup`

Removes same-directory `.safebox-*.json` crash-temp residue after first validating the committed snapshot through the same strict loader used by `/local/safebox-store/validate` and `/local/persistence/status`. Loopback-only `POST`, empty body required, returns the post-cleanup summary or `409` on validation/cleanup failure.

### `POST /local/safebox-store/backup`

Copies the durable safebox snapshot into an operator-supplied empty destination directory and returns the validation summary of the copied snapshot set. Loopback-only `POST` with `{"dst_dir":"..."}`. A successful backup writes `safebox-backup-manifest.json` (`go-metin2-safebox-backup-v1`) with deterministic summary, filename, size, and SHA-256 checksum. Missing committed snapshots backup as an empty store with no synthetic snapshot file. Hidden `.safebox-*.json` crash leftovers are ignored as payload.

### `POST /local/safebox-store/backup/validate`

Dry-runs an operator-supplied safebox backup source through the same strict manifest and snapshot checks used by backup, without mutating the active store. Loopback-only `POST` with `{"src_dir":"..."}`.
...
### `GET /local/db/drivers`

Returns the metadata-only `go-metin2-sql-drivers-v1` envelope with a deterministic `drivers` array of `database/sql` driver names linked into the running process. It is registered only on `gamed`, is loopback-only, rejects non-`GET` methods with `405`, rejects non-loopback callers with `403`, and returns `200` with `"drivers": []` when no stock driver is linked. It never opens a DSN, queries `schema_migrations`, includes configured runtime DB fields, or mutates state. `authd` and the default ops mux do not expose this route. The CLI equivalent is `metin2-migrate drivers [--require-driver <name>]`; the require gate fails closed before migration-run retention can open a target.

### `GET /local/db/migrations/status`

Returns a loopback-only, read-only migration dry-run plan from the validated `db/migrations` catalog to the latest embedded migration. When DB preflight config is disabled, `gamed` plans against an empty applied ledger. When both `METIN2_*_DB_DRIVER` and `METIN2_*_DB_DSN` are configured, it reads only `version`, `name`, and `up_sha256` from `schema_migrations` through `database/sql`, validates those rows against the embedded catalog, and returns `409` on query, ledger, checksum, or catalog drift. The response is metadata-only (`current_version`, `latest_version`, `up_to_date`, and pending step `version` / `name` / `direction` / `path` / `sha256`) and never includes executable SQL. Re-inspect a retained copy of this Plan JSON (`post-apply-status.json` / `post-rollback-status.json`) with CLI `status-status` — see [CLI status-status](plans/2026-09-07-cli-status-status-contract-freeze.md). Live `GET /local/db/migrations/status` stays no-format and this still does not add a loopback `GET /local/db/migrations/status-status` endpoint. `$RUN/daemon-migrations-status.json` stays an ungated live capture; the hermetic SQLite curl stub now emits a valid empty-ledger Plan for that optional retain. Read-only linked-driver discovery is available through `metin2-migrate drivers` and gamed-only `GET /local/db/drivers`; see [CLI sql-drivers contract freeze](plans/2026-09-07-cli-sql-drivers-contract-freeze.md). This does not register a stock driver.
...
Successful responses include `migration_version`, `migration_name`, deterministic `inventory_items`, deterministic `equipment_items`, and deterministic `quickslots`. Inventory rows are ordered by account/character/slot and expose item id, character id, carried slot, vnum, count, lock flag, and presence-aware additive `0024` instance sockets (`has_sockets` / `socket0` / `socket1` / `socket2`: omit / `has_sockets=false` means nil instance sockets / template fallback; `has_sockets=true` including all-zero is authoritative) plus presence-aware additive `0027` instance attributes (`has_attributes` / `attr0_type`/`attr0_value` … `attr6_type`/`attr6_value`: omit / `has_attributes=false` means nil instance attributes / template fallback; `has_attributes=true` including all-zero / type-zero is authoritative). Equipment rows expose item id, character id, named equipment slot, vnum, count, lock flag, and the same presence-aware socket and attribute fields. Quickslot rows expose character id, quickslot position, type, and slot. Export identity stays tip-`0003` / `character_item_state` even when additive `0024`/`0027` columns are projected. The response deliberately omits roster account rows, executable SQL, item-template definitions, quest state, login tickets, authored content, and runtime world state, and it does not apply migrations or mutate the account store. Use it as a second-stage operator/backfill preflight after the account/character roster export.

### `POST /local/account-store/exports/character-item-state/quarantine`

Validates and canonicalizes a retained `0003_character_item_state` export without opening a database or mutating account snapshots. This endpoint is registered only on `gamed`, is loopback-only, rejects non-`POST` methods with `405`, rejects non-loopback callers with `403`, rejects oversized bodies over 1 MiB with `413`, rejects malformed/invalid-UTF-8/`null` JSON with `400`, and returns `409` when the payload fails the quarantine contract.

Successful responses include:

- `summary.character_count`
- `summary.inventory_item_count`
- `summary.equipment_item_count`
- `summary.quickslot_count`
- deterministic sorted `summary.character_ids`
- a canonicalized `export` whose inventory rows are ordered by ascending `character_id` / `slot` / `id`, equipment rows by ascending `character_id` / owned equipment-slot order / `id`, and quickslots by ascending `character_id` / `position` / `type` / `slot`

The quarantine contract requires `migration_version = 3`, `migration_name = "character_item_state"`, present (possibly empty) `inventory_items` / `equipment_items` / `quickslots` arrays, `character_id > 0`, globally unique item ids across inventory and equipment, unique `(character_id, slot)` and `(character_id, equip_slot)` pairs, unique `(character_id, position)` quickslots, migration-valid item/quickslot bounds, and presence-aware optional instance sockets (`has_sockets` false requires zero sockets; `has_sockets` true including all-zero is authoritative) plus presence-aware optional instance attributes (`has_attributes` false requires zero attr types/values; `has_attributes` true including all-zero / type-zero is authoritative). Use this after retaining an export artifact and before any future DB backfill/import tool. It is not a repository write path and never emits SQL or DSNs. SQL import still requires tip-`0003` plus additive ledger `24` / `character_item_instance_sockets` and `27` / `character_item_instance_attributes` before INSERT — see [character item instance-attributes SQL additive](plans/2026-08-30-character-item-instance-attributes-sql-additive.md), [seeded item instance-sockets tip sync](plans/2026-08-30-seeded-item-instance-sockets-import-export-drill.md), and [seeded item instance-attributes tip sync](plans/2026-08-31-seeded-item-instance-attributes-import-export-drill.md).
...
The quarantine contract requires `migration_version = 9`, `migration_name = "item_template_refine_info"`, present (possibly empty) `templates` / `sockets` / `attributes` / `use_effects` / `equip_effects` / `refine_infos` / `refine_materials` arrays, unique template vnums, child rows that reference exported templates, migration-valid positions/bounds, contiguous refine-material positions from `0`, and reconstructed templates that satisfy the authored bootstrap validation rules (including safebox-reject requiring `anti_safebox`, and refine `keep_on_fail` / `fail_result_vnum` bounds matching additive `0021`/`0022`). Use this after retaining an export artifact and before any future DB backfill/import tool. It is not a repository write path and never emits SQL or DSNs. SQL import still requires tip-`0009` plus additive ledger `21` / `item_template_refine_keep_on_fail` and `22` / `item_template_refine_fail_result_vnum` before INSERT — see [seeded item-template refine fields tip sync](plans/2026-08-30-seeded-item-template-refine-fields-import-export-drill.md).

### `GET /local/static-actors/exports/static-actor-content-state`

Returns a loopback-only, read-only JSON projection of the committed authored static-actor and interaction-definition snapshots onto the `0013_static_actor_combat_profile_state` migration boundary (after the historical `0008` / `0012` static-actor content tips). This endpoint is registered only on `gamed`, rejects non-`GET` methods with `405`, rejects non-loopback callers with `403`, and returns `409` if either committed content store cannot be loaded, if any actor/definition/catalog/reward/quest-flag-item/combat-profile row would violate the schema shape, or if an interaction kind outside the owned set (`info` / `talk` / `warp` / `shop_preview` / `open_safebox` / `quest_flag`) appears in the committed stores.

Successful responses include `migration_version`, `migration_name`, deterministic `interaction_definitions`, `merchant_catalog_entries`, `quest_flag_reward_items`, `quest_flag_consume_items`, `static_actors`, `reward_drops`, `combat_profiles`, and `combat_profile_death_reward_drops`. Missing committed static-actor or interaction-definition snapshots are exported as empty migration-shaped collections. Actor rows keep the current authored content boundary (placement, race, optional interaction ref, optional spawn home/group/combat profile, reward scalar fields, and optional kill-quest / require-gate metadata), while ordered reward drops, quest-flag item tables, and portable combat-profile / death-reward-drop rows are emitted as child collections. The response deliberately omits executable SQL, account/item-instance rows, live runtime-only actor HP/respawn timers/combat targets, content-bundle import previews, and any database apply output; it does not mutate the JSON stores.

### `POST /local/static-actors/exports/static-actor-content-state/quarantine`

Validates and canonicalizes a retained `0013_static_actor_combat_profile_state` export without opening a database or mutating static-actor/interaction snapshots. This endpoint is registered only on `gamed`, is loopback-only, rejects non-`POST` methods with `405`, rejects non-loopback callers with `403`, rejects oversized bodies over 1 MiB with `413`, rejects malformed/invalid-UTF-8/`null` JSON with `400`, and returns `409` when the payload fails the quarantine contract.
...
- `summary.combat_profile_count`
- `summary.combat_profile_death_reward_drop_count`
- deterministic sorted `summary.entity_ids`
- deterministic sorted `summary.interaction_kinds`
- deterministic sorted `summary.combat_profiles`
- a canonicalized `export` whose definitions are ordered by kind/ref and whose child rows keep stable definition/position / profile/position ordering

The quarantine contract requires `migration_version = 13`, `migration_name = "static_actor_combat_profile_state"`, present (possibly empty) `interaction_definitions` / `merchant_catalog_entries` / `quest_flag_reward_items` / `quest_flag_consume_items` / `static_actors` / `reward_drops` / `combat_profiles` / `combat_profile_death_reward_drops` arrays, unique definition keys and actor entity ids, child rows that reference exported parents, contiguous positions from `0`, and reconstructed snapshots that satisfy the bootstrap static-actor / interaction / portable combat-profile validation rules (including complete `quest_flag` / `open_safebox` shapes and non-built-in combat-profile rows). Retained `migration_version = 8` / `static_actor_content_state` and `migration_version = 12` / `static_actor_pve_interaction_state` artifacts fail closed; re-export from the live stores. Use this after retaining an export artifact and before any future DB backfill/import tool. It is not a repository write path and never emits SQL or DSNs.
...
Successful responses include `migration_version`, `migration_name`, and deterministic `ground_items` rows sorted by visible ground `vid`. Item-shaped rows expose `item_count` plus presence-aware additive `0026` instance sockets (`has_sockets` / `socket0` / `socket1` / `socket2`: omit / `has_sockets=false` means nil instance sockets / template fallback; `has_sockets=true` including all-zero is authoritative) and presence-aware additive `0029` instance attributes (`has_attributes` / `attrN_type` / `attrN_value`: omit / `has_attributes=false` means nil instance attributes / template fallback; `has_attributes=true` including all-zero / type-zero is authoritative); gold-shaped rows expose `gold_amount`, use the current bootstrap gold marker `vnum = 1`, and stay socket-/attribute-less; both shapes expose owner identity, map position, and `pickup_range`. Export identity stays tip-`0010` / `bootstrap_ground_item_state` even when additive `0026`/`0029` columns are projected. This projection prefers the live shared-world pending set (and can be adapted through `BootstrapGroundItemStateExporter`); it deliberately omits FileStore-only restart extras such as absolute ownership/despawn timers, omits executable SQL and database apply output, and is not itself the restart-durability path. Use it as an operator/backfill preflight for the `0010` boundary. Pending ground-item / ground-gold restart rematerialize and manifested backup/restore are owned by the dedicated ground-item FileStore and `/local/ground-item-store/*` endpoints (see [ground-item process-restart durability](plans/2026-08-22-ground-item-process-restart-durability.md) and [ground-item file-store backup/restore](plans/2026-08-22-ground-item-file-store-backup-restore.md)).
...
The quarantine contract requires `migration_version = 10`, `migration_name = "bootstrap_ground_item_state"`, a present (possibly empty) `ground_items` array, and rows that satisfy the same fail-closed `0010` projection rules already owned by the live export path (unique positive `vid`, owner identity bounds, map/pickup bounds, exclusive item-count vs gold-amount shapes with gold using `vnum = 1`, presence-aware optional instance sockets on item-shaped rows: `has_sockets` false requires zero sockets; `has_sockets` true including all-zero is authoritative; and presence-aware optional instance attributes on item-shaped rows: `has_attributes` false requires zero attr types/values; `has_attributes` true including all-zero / type-zero is authoritative; gold-shaped rows stay socket-/attribute-less). Use this after retaining an export artifact and before any future DB backfill/import or crash-recovery tool. It is not a repository write path and never emits SQL or DSNs. SQL import still requires tip-`0010` plus additive ledger `26` / `bootstrap_ground_item_instance_sockets` plus additive ledger `29` / `bootstrap_ground_item_instance_attributes` before INSERT — see [bootstrap ground-item instance-attributes SQL additive](plans/2026-08-31-bootstrap-ground-item-instance-attributes-sql-additive.md), [seeded ground-item instance-sockets tip sync](plans/2026-08-30-seeded-ground-item-instance-sockets-import-export-drill.md), and [seeded ground-item instance-attributes tip sync](plans/2026-08-31-seeded-ground-item-instance-attributes-import-export-drill.md).

### `GET /local/safebox-store/exports/character-safebox-state`

Returns a loopback-only, read-only JSON projection of the committed durable safebox FileStore onto the `0028_character_safebox_item_instance_attributes` migration tip. Presence-aware additive `0025` `has_sockets` / `socket0`/`socket1`/`socket2` and additive `0028` `has_attributes` / `attr0_type`/`attr0_value` … `attr6_type`/`attr6_value` fields on items use the same semantics as tip-`0003`+`0024`+`0027` (export identity is tip-`0028`; schema preflight still requires distinct ledger `15`+`25`+`28` so `0015` money is not dropped). This endpoint is registered only on `gamed`, rejects non-`GET` methods with `405`, rejects non-loopback callers with `403`, and returns `409` if the committed safebox snapshot is invalid or cannot be projected onto the schema shape.

Successful responses include `migration_version`, `migration_name`, deterministic `passwords` rows (one per durable character key, with empty `password` meaning bootstrap default `000000`), and deterministic `items` rows for durable cells `0..14`. Item rows expose item id, character id, login, cell, vnum, count, lock flag, presence-aware additive `0025` instance sockets (`has_sockets` / `socket0` / `socket1` / `socket2`: omit / `has_sockets=false` means nil instance sockets / template fallback; `has_sockets=true` including all-zero is authoritative), and presence-aware additive `0028` instance attributes (`has_attributes` / `attrN_type` / `attrN_value`: omit / `has_attributes=false` means nil instance attributes / template fallback; `has_attributes=true` including all-zero / type-zero is authoritative). Export identity is tip-`0028` / `character_safebox_item_instance_attributes` while additive `0025`/`0028` columns stay projected and schema preflight still requires distinct ledger `15` / `character_safebox_money` plus `25` plus `28`. A missing safebox snapshot returns an empty migration-shaped export, matching `/local/safebox-store/validate`. The response deliberately omits executable SQL, account roster rows, live presentation windows, money/mall state, and runtime world state, and it does not apply migrations or mutate the safebox store. Use it as an operator/backfill preflight beside the FileStore rematerialize and `/local/safebox-store/*` backup/restore drill path.

### `POST /local/safebox-store/exports/character-safebox-state/quarantine`

Validates and canonicalizes a retained `0028_character_safebox_item_instance_attributes` export without opening a database or mutating safebox snapshots. Presence-aware additive `0025` sockets and additive `0028` attributes on items keep tip-`0028` export identity; quarantine rejects non-zero sockets without `has_sockets` and non-zero attributes without `has_attributes` (same semantics as tip-`0003`+`0024`+`0027`). Retained tip-`0015` payloads fail closed until re-exported. This endpoint is registered only on `gamed`, is loopback-only, rejects non-`POST` methods with `405`, rejects non-loopback callers with `403`, rejects oversized bodies over 1 MiB with `413`, rejects malformed/invalid-UTF-8/`null` JSON with `400`, and returns `409` when the payload fails the quarantine contract.
...
The quarantine contract requires `migration_version = 28`, `migration_name = "character_safebox_item_instance_attributes"`, present (possibly empty) `passwords` / `items` arrays, `character_id > 0`, bootstrap-valid `login` / optional alphanumeric password (`<= 6`) / optional warehouse `money` in `[0, 2147483647]`, unique password `character_id` keys, unique item ids, unique `(character_id, cell)` pairs with `cell < 15`, positive `vnum` / `count`, a stable `character_id` ↔ `login` mapping with every item referencing a password row, presence-aware optional instance sockets (`has_sockets` false requires zero sockets; `has_sockets` true including all-zero is authoritative), and presence-aware optional instance attributes (`has_attributes` false requires zero attr types/values; `has_attributes` true including all-zero / type-zero is authoritative). Use this after retaining an export artifact and before any future DB backfill/import tool. It is not a repository write path and never emits SQL or DSNs. SQL import still requires distinct ledger `15` / `character_safebox_money` plus additive ledger `25` / `character_safebox_item_instance_sockets` plus additive ledger `28` / `character_safebox_item_instance_attributes` before INSERT so `0015` money is not dropped — see [safebox cell instance-attributes SQL additive](plans/2026-08-31-safebox-cell-instance-attributes-sql-additive.md), [seeded safebox cell instance-sockets tip sync](plans/2026-08-30-seeded-safebox-cell-instance-sockets-import-export-drill.md), and [seeded safebox cell instance-attributes tip sync](plans/2026-08-31-seeded-safebox-cell-instance-attributes-import-export-drill.md).
...
- `persistence.ground_item_store_path`
- `persistence.safebox_store_path`
- `database.configured`
...
The `persistence` block reports the active bootstrap JSON store locations selected from `METIN2_*_STORE_*` / service-specific environment overrides. Use it before running local backup, restore, validation, or stale-ticket cleanup endpoints so the operator confirms the daemon is pointing at the intended account, login-ticket, static-actor, interaction, item-template, quest-state, pending ground-item, durable safebox, and authored cube-recipe stores. A running `gamed` has already rejected empty, overlapping, or shared-parent persistence paths at startup: account and login-ticket stores must be separate directory trees that are not existing regular files and not symlink roots, file-backed content / quest-state / ground-item / safebox / cube-recipe stores must be separate files that are not existing directories, no two file-backed stores may share a parent directory (restore empties `filepath.Dir(snapshotPath)`), and no file-backed store may resolve into or be lexically placed inside either persistence directory. Pending ground item/gold handles now rematerialize from `METIN2_GROUND_ITEM_STORE_PATH` / `METIN2_GAMED_GROUND_ITEM_STORE_PATH` (default `$TMP/go-metin2-server-ground-items/ground-items.json`) across `gamed` process restart, and the combined backup/restore drill covers their FileStore BackupTo / ValidateBackupFrom / RestoreFrom surface through `/local/ground-item-store/*` (restore also rematerializes the live shared-world pending set). Durable safebox cells rematerialize from `METIN2_SAFEBOX_STORE_PATH` / `METIN2_GAMED_SAFEBOX_STORE_PATH` (default `$TMP/go-metin2-server-safebox/safebox.json`) across restart/reconnect, and the same drill covers `/local/safebox-store/*` (see [safebox file-store backup/restore drill fold-in](plans/2026-08-23-safebox-file-store-backup-restore-drill.md)). Authored cube recipes rematerialize from `METIN2_CUBE_RECIPE_STORE_PATH` / `METIN2_GAMED_CUBE_RECIPE_STORE_PATH` (default `$TMP/go-metin2-server-cube-recipes/cube-recipes.json`) across restart (`docs/plans/2026-09-08-pve-vertical-authored-cube-recipe-filestore.md`); the drill decoder accepts that optional path so retained runtime-config JSON cannot fail unknown-field rejection, but it does not yet print cube-recipe backup/restore endpoints. Retained backup trees re-inspection is owned by read-only `backup-tree-status` — see [CLI backup-tree-status](plans/2026-09-06-cli-backup-tree-status-contract-freeze.md). Re-inspect a retained `backup-tree-status.json` with read-only `backup-tree-status-status` — see [CLI backup-tree-status-status](plans/2026-09-06-cli-backup-tree-status-status-contract-freeze.md). Opt-in `--require-no-crash-temps` on those inspectors fails closed on leftover crash-temp residue — see [CLI backup-tree-status no-crash-temps require-gate](plans/2026-09-06-cli-backup-tree-status-no-crash-temps-require-gate-contract-freeze.md). Re-inspect retained `persistence-status-before.json` / `persistence-status-after.json` with read-only `persistence-status-status` — see [CLI persistence-status-status](plans/2026-09-07-cli-persistence-status-status-contract-freeze.md). Matching `migration-run-retention` printer + hermetic curl-stub wiring is now GREEN — see [CLI migration-run-retention persistence-status-status](plans/2026-09-07-cli-migration-run-retention-persistence-status-status-contract-freeze.md).
...
- `ground_item_store`
  - `path`
  - `valid`
  - `summary` with `ground_item_count`, `item_shaped_count`, `gold_shaped_count`, deterministic `vids`, and optional crash-temp fields for `.ground-items-*.json`
  - `backup_manifest` with `present`, `path`, `format`, `file_count`, total declared `snapshot_size_bytes`, actual `manifest_size_bytes`, and actual `manifest_sha256` when a restored/backup metadata file is currently present next to the active ground-item snapshot
  - `restore_blocked_by_live_sessions` — `true` when a live selected-character session would make `/local/ground-item-store/restore` fail closed
  - optional `error` when validation fails
- `safebox_store`
  - `path`
  - `valid`
  - `summary` with `character_count`, `cell_count`, deterministic `logins`, deterministic `character_keys` (`login:character_id`), and optional crash-temp fields for `.safebox-*.json`
  - `backup_manifest` with `present`, `path`, `format`, `file_count`, total declared `snapshot_size_bytes`, actual `manifest_size_bytes`, and actual `manifest_sha256` when a restored/backup metadata file is currently present next to the active safebox snapshot
  - `restore_blocked_by_live_sessions` — `true` when a live selected-character session would make `/local/safebox-store/restore` fail closed
  - optional `error` when validation fails

Use this endpoint as the first read-only persistence triage check before choosing a narrower validate, crash-temp cleanup, stale-ticket cleanup, backup, restore, or DB-migration status endpoint. It deliberately keeps checking the remaining stores after one store fails, so a corrupt account snapshot does not hide healthy login-ticket, item-template, static-actor, interaction-definition, quest-state, ground-item, or safebox stores. Authored content, quest-state, ground-item, and safebox stores that have no committed snapshot are reported as valid empty stores, while corrupt committed snapshots fail closed and still let operators inspect the other persistence surfaces in the same response. The live-session fields make the existing restore guard visible before an operator attempts a replacement restore: account-store, login-ticket, item-template, quest-state, static-actor, interaction-definition, ground-item, and safebox restores are blocked while selected-character sessions are registered, because replacing durable account snapshots, pending auth handoffs, authored item templates, quest flags, authored NPC/spawn content, authored interaction definitions, pending ground handles, or durable safebox cells underneath live player state would diverge from in-memory runtime ownership. Account snapshots, login-ticket snapshots, item-template snapshots, static-actor snapshots, interaction-definition snapshots, quest-state snapshots, ground-item snapshots, and safebox snapshots that are symlinks now fail closed before JSON decoding, so active committed state cannot transparently point outside the configured store boundary. Crash-temp-shaped symlinks also fail closed across the account, login-ticket, item-template, static-actor, interaction-definition, quest-state, ground-item, and safebox stores instead of being followed, removed, or reported as disposable temp residue. Account and login-ticket character lists also reject duplicate non-zero VIDs in addition to duplicate character IDs and case-folded names, preventing restore/consume paths from admitting ambiguous visible-player identities. Static-actor and interaction-definition store files now also reject a JSON `null` document root or `null` root collection (`static_actors` / `definitions`) instead of treating those lossy snapshots as empty authored content. Login-ticket summaries now expose the embedded select-screen character count and empty-slot count just like account summaries, which helps operators compare pending one-shot authd handoffs against durable account snapshots before consuming or cleaning stale keys. Account, login-ticket, item-template, static-actor, interaction-definition, quest-state, ground-item, and safebox stores that still carry a restored backup manifest now report that manifest explicitly under `backup_manifest`, and validation still verifies that active manifest against the current committed snapshot bytes; malformed manifests, stale checksum/summary data, or item-template manifests that omit an existing committed snapshot make the affected store invalid so operators can detect post-restore drift before treating a replacement store as an exact backup copy. If that manifest path is a symlink, the status snapshot reports only `present` plus the in-store `path` and deliberately does not follow the link, decode the target, hash the target bytes, or expose target metadata; the matching validation error still marks the store invalid. It is an operator/debugging surface, not a gameplay API and not a remote admin API. Re-inspect retained `persistence-status-before.json` / `persistence-status-after.json` with read-only `persistence-status-status` — see [CLI persistence-status-status](plans/2026-09-07-cli-persistence-status-status-contract-freeze.md). Matching `migration-run-retention` printer + hermetic curl-stub wiring is now GREEN — see [CLI migration-run-retention persistence-status-status](plans/2026-09-07-cli-migration-run-retention-persistence-status-status-contract-freeze.md).
...
Use this as an on-box dry-run check before applying a larger content bundle or before committing updates to deterministic example bundles. The repository-owned `docs/examples/bootstrap-npc-service-bundle.json` fixture is required to stay byte-for-byte canonical under this same validation path, so operators can paste it directly into `/local/content-bundle/validate` or `/local/content-bundle` without hidden normalization drift. Authoring-oriented fixtures such as `docs/examples/bootstrap-drop-table-authoring-bundle.json`, `docs/examples/bootstrap-regen-authoring-bundle.json`, and `docs/examples/bootstrap-combat-profile-formula-bundle.json` intentionally expand under validation: the formula fixture derives `damage_per_normal_attack` / default `level` and copies profile-default rewards onto `practice.qa_formula_mob` before returning the portable canonical shape.
...
Current previews cover self-only `info` / `talk`, state-aware `quest_flag` acknowledgement-or-mismatch text, structured merchant `shop_preview` catalog summaries, compact `warp` destination summaries, compact warehouse `open_safebox` size summaries, and compact craftsman `open_cube` summaries. `quest_flag` previews use the same read-only compare-and-set evaluator as `/local/quest-state/transition-preview`, so a selected character that no longer satisfies the authored `quest_from` value sees `Quest requirements are not met.` in the preview without mutating the quest-state store. Gated `open_safebox` / `open_cube` mismatch previews reuse that same requirement-mismatch text without mutating quest-state.
