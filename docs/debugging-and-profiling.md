...
The quarantine contract requires `migration_version = 15`, `migration_name = "character_safebox_money"`, present (possibly empty) `passwords` / `items` arrays, `character_id > 0`, bootstrap-valid `login` / optional alphanumeric password (`<= 6`) / optional warehouse `money` in `[0, 2147483647]`, unique password `character_id` keys, unique item ids, unique `(character_id, cell)` pairs with `cell < 15`, positive `vnum` / `count`, a stable `character_id` ↔ `login` mapping with every item referencing a password row, presence-aware optional instance sockets (`has_sockets` false requires zero sockets; `has_sockets` true including all-zero is authoritative), and presence-aware optional instance attributes (`has_attributes` false requires zero attr types/values; `has_attributes` true including all-zero / type-zero is authoritative). Use this after retaining an export artifact and before any future DB backfill/import tool. It is not a repository write path and never emits SQL or DSNs. SQL import still requires tip-`0015` plus additive ledger `25` / `character_safebox_item_instance_sockets` plus additive ledger `28` / `character_safebox_item_instance_attributes` before INSERT — see [safebox cell instance-attributes SQL additive](plans/2026-08-30-character-safebox-item-instance-attributes-sql-additive.md), [seeded safebox cell instance-sockets tip sync](plans/2026-08-30-seeded-safebox-cell-instance-sockets-import-export-drill.md), and [seeded safebox cell instance-attributes tip sync](plans/2026-08-31-seeded-safebox-cell-instance-attributes-import-export-drill.md).
