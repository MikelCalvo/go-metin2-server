...
Successful responses include `migration_version`, `migration_name`, and deterministic `ground_items` rows sorted by visible ground `vid`. Item-shaped rows expose `item_count`; gold-shaped rows expose `gold_amount` and use the current bootstrap gold marker `vnum = 1`; both shapes expose owner identity, map position, and `pickup_range`. This projection reads live in-memory runtime state rather than a committed JSON store, deliberately omits executable SQL and database apply output, and does not make pending ground handles durable across restart. Use it as an operator/backfill preflight for the `0010` boundary before any future import or crash-recovery tool. The named `worldruntime.BootstrapGroundItemStateExporter` seam now lets hermetic tests and future adapters project the same migration shape without requiring live shared-world registration, but pending ground handles are still not durable across process restart.
