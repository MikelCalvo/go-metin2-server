...
Successful responses return the canonical bundle JSON that would be accepted by import. Invalid JSON, unknown fields, JSON `null` roots, dangling refs, invalid static actors/spawn groups/combat profiles, missing merchant or reward-drop item templates, and other bundle validation failures return `400`; non-loopback callers return `403`; methods other than `POST` return `405`.
