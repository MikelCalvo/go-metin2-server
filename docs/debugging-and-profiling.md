...
The quarantine contract requires `migration_version = 3`, `migration_name = "character_item_state"`, present (possibly empty) `inventory_items` / `equipment_items` / `quickslots` arrays, `character_id > 0`, globally unique item ids across inventory and equipment, unique `(character_id, slot)` and `(character_id, equip_slot)` pairs, unique `(character_id, position)` quickslots, and migration-valid item/quickslot bounds. Use this after retaining an export artifact and before any future DB backfill/import tool. It is not a repository write path and never emits SQL or DSNs.
