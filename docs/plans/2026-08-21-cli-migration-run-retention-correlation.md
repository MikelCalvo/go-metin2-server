...
2. ~~Keep automatic artifact GC / lifecycle jobs deferred.~~ Done for the offline aside-rename printer: see [CLI artifact-retention GC printer](2026-08-22-cli-artifact-retention-gc-printer.md). Automatic / scheduled deletion remains deferred.
...
7. ~~Follow-up owned separately: freeze read-only `status-status` for retained `$RUN/post-apply-status.json` / `$RUN/post-rollback-status.json`.~~ Done — see [CLI status-status contract freeze](2026-09-07-cli-status-status-contract-freeze.md). Printed scripts now emit gated `$RUN/post-apply-status-status.json` / `$RUN/post-rollback-status-status.json` companions.
8. ~~Follow-up owned separately: freeze production-engine selection / operator runbook hardening before any stock driver registration.~~ Done for docs/spec freeze — see [CLI sql-drivers / loopback GET /local/db/drivers contract freeze](2026-09-07-cli-sql-drivers-contract-freeze.md). GREEN stays follow-on.
