...
7. Follow-up owned separately: freeze read-only `status-status` for retained `$RUN/post-apply-status.json` / `$RUN/post-rollback-status.json` — see [CLI status-status contract freeze](2026-09-07-cli-status-status-contract-freeze.md). GREEN stays follow-on; printed scripts still do not emit `post-*-status-status.json`.
