...
4. Spec/QA name this shared ownership/public-release contract for reward drops beside player drops; kill-reward FileStore rematerialize with absolute ownership/despawn timers is now owned beside player drops (`TestGameRuntimePracticeMobKillRewardDropRematerializesAcrossDaemonRestart`).
...
- `go test ./internal/minimal -run 'RewardDropPublicReleaseAllowsLivingCollectorPickup|ItemPickupAllowsPublicCollectorAfterOwnershipRelease|PracticeMobKillRewardDropRematerializesAcrossDaemonRestart' -count=1`
- `gofmt` on touched Go files
- `git diff --check`

## Follow-up options

1. Keep SQL import/backfill from quarantined `0010` exports deferred until a driver-backed harness exists.
