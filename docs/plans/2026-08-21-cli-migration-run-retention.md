...
6. ~~Read-only `catalog-status` for retained `migration-catalog.json`.~~ Done — see [CLI catalog-status contract freeze](2026-09-05-cli-catalog-status-contract-freeze.md). Printed `migration-run-retention` scripts now emit `catalog-status --require-matches-embedded` beside the catalog retain line.
7. ~~Read-only `apply-lock-aside-status` for retained `apply-lock-aside.json`.~~ Done — see [CLI apply-lock-aside-status contract freeze](2026-09-05-cli-apply-lock-aside-status-contract-freeze.md). Printed leftover-lock triage now echoes a matching `--aside` inspect hint beside the leftover-lock aside-rename echo and still does not auto-run aside-rename under `set -eu`.
8. ~~Follow-up owned separately: freeze `persistence-status-status` printer + hermetic curl-stub expansion for retained `$RUN/persistence-status-*.json`.~~ Done — see [CLI migration-run-retention persistence-status-status](2026-09-07-cli-migration-run-retention-persistence-status-status-contract-freeze.md). Printed scripts now emit ungated `persistence-status-status` companions beside `$RUN/persistence-status-before.json` / `$RUN/persistence-status-after.json`.
9. ~~Follow-up owned separately: freeze read-only `status-status` for retained `$RUN/post-apply-status.json` / `$RUN/post-rollback-status.json`.~~ Done — see [CLI status-status contract freeze](2026-09-07-cli-status-status-contract-freeze.md). Printed scripts now emit gated `$RUN/post-apply-status-status.json` / `$RUN/post-rollback-status-status.json` companions. ~~Next: expand the hermetic `GET /local/db/migrations/status` curl stub to a valid empty-ledger Plan (still no gated daemon inspect).~~ Done — tagged SQLite proofs now retain an inspectable empty-ledger `$RUN/daemon-migrations-status.json` from `PlanUpToLatest(nil)`. ~~Next: freeze production-engine selection / operator runbook hardening before any stock driver registration.~~ Done for docs/spec freeze — see [CLI sql-drivers / loopback GET /local/db/drivers contract freeze](2026-09-07-cli-sql-drivers-contract-freeze.md). Next: GREEN read-only `metin2-migrate drivers --require-driver "$DRIVER"` immediately after the printed DRIVER/DSN require and before `ledger-snapshot` (`$RUN/sql-drivers.json`).
