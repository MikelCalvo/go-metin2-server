...
2. ~~Choose and document a production DB driver/engine only when repository or migrator work needs it.~~ Done for linked-driver inspection: `metin2-migrate drivers` and gamed-only `GET /local/db/drivers` now distinguish configured from linked names — see [CLI sql-drivers / loopback GET /local/db/drivers contract freeze](2026-09-07-cli-sql-drivers-contract-freeze.md). Choosing a stock production engine remains deferred.
