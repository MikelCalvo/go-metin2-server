...
2. ~~Choose and document a production DB driver/engine only when repository or migrator work needs it.~~ Operator-visible linked-driver discovery is frozen first — see [CLI sql-drivers / loopback GET /local/db/drivers contract freeze](2026-09-07-cli-sql-drivers-contract-freeze.md). GREEN stays follow-on. Choosing a stock production engine remains deferred until that inspect path exists.
