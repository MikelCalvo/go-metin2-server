...
Freeze then GREEN the next warehouse persistence-safety seam after durable
ground-item instance attributes: same-account safebox FileStore cells must
round-trip the same presence-aware instance attributes (including explicit
all-zero / type-zero) so check-in → `gamed` restart → reopen / checkout does not
silently fall back to template attributes.
...
- Durable `safeboxstore.Cell` previously serialized only sockets beside
  `cell` / `id` / `vnum` / `count` / `locked`. Restore rebuilt
  `ItemInstance{ID,Vnum,Count,Slot,Locked,Sockets?}` with `Attributes: nil`.
- That dropped authoritative instance attributes across restart even though
  carried FileStore, live encode preference, and ground-item FileStore already
  owned the same shape.
...
1. Extend `safeboxstore.Cell` with presence-aware optional attributes that mirror
   ...
   - `has_attributes=true` including all-zero / type-zero → authoritative
     `Attributes`
2. `ReplaceCharacterCells` / `cellFromItemInstance` persist instance attributes
   from the live presentation map.
3. `CharacterCells` / restore helpers rebuild the same `ItemInstance.Attributes`
   pointer semantics before open-burst `SAFEBOX_SET` / checkout / item-move.
...
6. Migration-shaped tip-`0015` safebox export identity stays tip `15`; this slice
   is FileStore rematerialize only.
