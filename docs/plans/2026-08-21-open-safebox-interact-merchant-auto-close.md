...
1. When a successful `open_safebox` `INTERACT` applies and the same socket still has an active bootstrap merchant buy window, the runtime emits self-only `GC::SHOP END` first, clears that merchant presentation, then emits any optional authored warehouse info chat plus `GC::SAFEBOX_SIZE` and remembered `SAFEBOX_SET` rows.
...
- durable safebox persistence / money / mall
