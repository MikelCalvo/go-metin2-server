...
4. On success the runtime removes the carried item from inventory, syncs source item quickslots with the already-owned removal path (`GC::QUICKSLOT_DEL` when the cell is fully removed), stores the item in the same-session in-memory safebox slot, and emits self-only `GC::ITEM_DEL` (or inventory refresh) plus `GC::SAFEBOX_SET` for that safebox cell. No gold/money/mall frames are introduced.
5. Spec/QA name this as session-scoped in-memory safebox contents: `/close_safebox` clears only the busy presentation flag; reopening with `/open_safebox` re-emits `SAFEBOX_SIZE` and may re-emit remembered in-memory `SAFEBOX_SET` rows for the still-open session. Reconnect / process restart / logout still lose those contents until a later persistence slice owns durable safebox state.
...
2. Keep checkout / move / money / password / durable persistence deferred until after the first check-in vertical is green.
