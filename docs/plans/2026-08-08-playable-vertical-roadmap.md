...
7. Next: profile-authored optional `aggro_radius` on portable `combat_profiles` (contract frozen in `content-spawn-groups-bootstrap.md`). Effective acquire / leave-radius / suppress seeding must honor the authored radius when present, otherwise keep bootstrap `DefaultSpawnAggroRadius = 200`; values above `DefaultSpawnLeashRadius` fail closed. Runtime still hard-codes the bootstrap default until that RED/GREEN lands.
