...
3. ~~Extend remaining item-template restriction/feedback edges only where a client-visible gap remains after the owned class/sex/level/anti-flag/equip-slot guards.~~ Done for the direct `ITEM_USE` / `/use_item` seam: authored `use_reject_message` now freezes self-only info-chat feedback for transfer guards (`anti_stack` / `anti_get` / `anti_drop` / `anti_give` / `anti_sell`), selected-character job/sex/empire/`min_level` restrictions, and the already-owned confirm/quest/applicable guards; omitted text stays silent/no-frame. Broader remaining gaps (for example exchange `ITEM_ADD` feedback beyond `anti_give`, or restart-restored ownership/despawn timers) stay deferred.
