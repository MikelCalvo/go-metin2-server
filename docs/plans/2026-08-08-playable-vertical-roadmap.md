...
8. Next: profile-authored optional `leash_radius` on portable `combat_profiles` (contract frozen in `content-spawn-groups-bootstrap.md` / `spawn-leash-bootstrap.md`). Effective leash classification, return-step / chase clamp, and target_return_required gating must honor the authored radius when present, otherwise keep bootstrap `DefaultSpawnLeashRadius = 400`; values below the profile's effective aggro radius fail closed. Runtime still hard-codes the bootstrap leash default until that RED/GREEN lands. Cross-map return MOVE / warp choreography remains deferred behind the packet freeze.
