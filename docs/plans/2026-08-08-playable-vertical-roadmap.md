...
3. Add aggro-lite reset/cleanup boundaries for disconnect, transfer, death, and respawn; `/restart_here` now also has focused coverage for due return-step preflight while a zero-HP owner skipped lifecycle frames; EnterGame reclaim that drops stale practice-mob engagement now also clears pending chase-step deadlines (matching leave/transfer).
