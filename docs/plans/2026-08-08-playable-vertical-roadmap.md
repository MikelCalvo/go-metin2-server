...
18. ~~Next: freeze daemon-restart proximity-suppress rematerialization before RED.~~ Done for docs-first freeze: `content-spawn-groups-bootstrap.md` now owns durable character-VID + authored `spawn_group_ref` suppress keys on the static-actor snapshot (`proximity_suppress_vids`), restore into the existing Leave→Join VID park map on `loadPersistedStaticActors`, and fail-closed engagement across restart. See [proximity suppress daemon-restart rematerialize contract freeze](2026-08-24-proximity-suppress-daemon-restart-rematerialize-contract-freeze.md). Next honest Track A follow-on: RED then GREEN `TestGameRuntimeProximityAggroSuppressRematerializesAcrossDaemonRestart`.
