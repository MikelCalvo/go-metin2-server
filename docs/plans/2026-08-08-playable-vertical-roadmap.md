...
3. ~~Write backup/restore runbooks backed by local validation or preflight tests.~~ Done for the seven manifested stores (six bootstrap JSON stores plus the durable pending ground-item FileStore) plus the read-only drill printer: status/runtime-config reporting, BackupTo/RestoreFrom, crash-temp triage, and `metin2-migrate backup-restore-drill` curls are owned through `/local/ground-item-store/*` beside the older store routes.
