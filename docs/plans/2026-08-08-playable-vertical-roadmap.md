...
6. ~~Harden multi-map and reconnect behavior so mobs do not duplicate, leak, or resurrect incorrectly.~~ Done for bootstrap scope: content-loaded still-dead EnterGame/reconnect now mirrors the `training_dummy` trailing-`DEAD` proof, due-respawn EnterGame preflight stays owned, one-ref/one-actor lookup fails closed on duplicates, import visibility stays map/AOI scoped, cross-map return-home remains delete/readd without dual-map occupancy, and Leave/transfer ownership cleanup remains owned. Broader daemon-restart dead-timer persistence and cross-map return MOVE remain out of scope.
