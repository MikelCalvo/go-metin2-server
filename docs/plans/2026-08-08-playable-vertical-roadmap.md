...
17. ~~Next: hermetic MemoryStores for proximity aggro suppress shared-world suites.~~ Done for bootstrap scope: the six live proximity suppress proofs in `proximity_aggro_suppress_test.go` now inject `staticstore.NewMemoryStore()` / `interactionstore.NewMemoryStore()` instead of disposable content FileStores; login-ticket / account FileStores remain intentional. See [hermetic proximity aggro suppress MemoryStore tests](2026-08-24-hermetic-proximity-aggro-suppress-memorystore-tests.md). Next honest Track A follow-on: freeze then implement daemon-restart proximity-suppress rematerialization (engagement stays fail-closed across restart).
