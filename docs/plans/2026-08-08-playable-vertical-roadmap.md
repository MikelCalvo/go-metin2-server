...
9. ~~Next: unengaged `within_radius` homeward-step after chase/engagement release.~~ Done for bootstrap scope: pure `PlanStaticActorSpawnLeashHomewardStep`, pending-frame `1s` / `max_step=100` homeward executor with same-map retained-viewer `MOVE`, engagement-release arming, chase mutual exclusion, and EnterGame/transfer/restart preflight flush after `return_required` return-steps / before chase. Operator `return-step` still no-ops `within_radius`; exact-home remains `return-home`. Pending homeward inspection endpoints and cross-map return MOVE stay deferred.
