...
4. Keep partner-side open player-shop / cube busy-window rejection text deferred until those presentation seams exist. Open refine-dialog exchange `START` busy rejects are now owned beside merchant/safebox, and exchange `ACCEPT` / second-accept mutual-accept finalize now also fail closed when either paired side has an open merchant/safebox/refine busy presentation.
