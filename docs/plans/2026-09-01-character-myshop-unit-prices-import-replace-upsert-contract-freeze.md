# Character myshop unit-prices SQL import replace/upsert contract freeze — 2026-09-01

## Objective

Freeze the next fail-closed **scoped replace** policy for quarantined tip-`0023`
(`character_myshop_unit_prices`) before opening RED, so operators can re-backfill
a retained silk-bag remembered unit-price export without hitting insert-only
primary-key conflicts.

This freeze does **not** invent a stock production driver, live DB myshop
repository, catalog tip `0030`, GD/DB `MYSHOP_PRICELIST_*` surfaces, remote admin,
or silent row-merge semantics.

## Why docs-first

Track E tip-`0003` / tip-`0015` / tip-`0004` / tip-`0011` scoped replace paths are
...
`accountstore.ImportCharacterMyShopUnitPrices` is still insert-only and fails
closed on a second import of the same `(character_id, vnum)` primary keys
(explicitly “does not invent upsert / merge policy”). That is safe for first-time
backfill of silk-bag remembered prices, but it blocks the honest operator
re-backfill path after a retained tip-`0023` export is corrected or a lab DB is
rebuilt to tip and re-seeded.

MyShop unit prices sit on the playable economy vertical (silk-bag open /
`OPEN_PRIVATE_SHOP` remembered price list that already rematerializes through
FileStore). Owning the re-backfill contract next keeps that durable state
...
1. `ImportCharacterMyShopUnitPrices(...)` without an explicit replace option keeps
...
### B. Opt-in scoped replace for tip-`0023` only (this freeze)

1. A new programmatic option (name TBD in GREEN; treat as
   `ImportCharacterMyShopUnitPricesOptions{Replace: true}` mirroring tip-`0003` /
   tip-`0004` / tip-`0011` / tip-`0015`) performs **scoped replace** for the
   character ids present in the quarantined export summary.
2. Scope is exactly the tip-`0023` table already owned by
   `ImportCharacterMyShopUnitPrices`:
   - `character_myshop_unit_prices`
3. For each `character_id` listed in the canonicalized export summary /
   declared `CharacterIDs`:
   - delete existing rows for that character in `character_myshop_unit_prices`,
   - insert the canonicalized export rows for that character,
   - all inside **one** transaction after the existing quarantine + schema
     preflight (ledger entry for version `23` / `character_myshop_unit_prices`).
...
6. Export tip identity stays `0023_character_myshop_unit_prices`; do **not** retip
   export / quarantine / import-result identity.

### C. Declared character scope + empty semantics

1. Tip-`0023` exports gain an optional `character_ids` field (same merge rules as
   tip-`0003` / tip-`0004` / tip-`0011` / tip-`0015`): declared ids merge with
   unit-price row-derived ids so a listed character with zero price rows can
   wipe-to-empty.
2. Quarantine continues to reject zero `character_id`, zero `vnum`, duplicate
   `(character_id, vnum)`, and per-character row counts above
   `loginticket.MyShopUnitPriceMax`. Those fail closed before any DELETE /
   INSERT.
3. A listed character with zero unit-price rows is allowed only as an explicit
   wipe-to-empty scope entry (via declared `character_ids`).
4. An export with an empty `character_ids` list and empty `unit_prices` is a
...
6. Replace of a listed character replaces that character's entire
   `character_myshop_unit_prices` set for the export; there is no per-`vnum`
   half-replace / silent merge mode in this freeze.

### D. CLI confirmation shape (tip-`0023` added beside owned replace kinds)
...
1. Replace stays off unless the operator passes an explicit confirmation flag
   ...
   `--kind character-myshop-unit-prices` (still reject every other kind that has
   not frozen+GREEN'd its own replace path).
2. Successful stdout remains metadata-only
   `CharacterMyShopUnitPricesImportResult` JSON (no DSN, no SQL text, no price
   payloads). GREEN should add `replaced: true` (omitempty) mirroring tip-`0003`
   / tip-`0004` / tip-`0011` / tip-`0015`.
...
- live DB myshop repository replacing FileStore rematerialize
- catalog tip `0030` / retip of tip-`0023` / `0003` / `0004` / `0011` / `0015` /
  `0010` export identities
- upsert / replace for roster (`0002`), ground (`0010`), templates (`0009`),
  tickets (`0007`), or static actors (`0013`) in this freeze (tip-`0003` /
  tip-`0004` / tip-`0011` / tip-`0015` replace stay owned)
...
- GD/DB `MYSHOP_PRICELIST_*`, remote admin, secrets in git, README churn,
  gameplay changes
- another crash/restart rematerialize twin (FileStore myshop unit-price
...
1. SQLite harness: first insert-only import succeeds; second insert-only import
   ...
   and leaves exactly the canonical price rows.
2. Scoped wipe: character A in export, character B only in DB beforehand →
   replace updates A and leaves B untouched.
3. Empty wipe: listed character with zero unit-price rows via declared
   `character_ids` → that character's `character_myshop_unit_prices` rows become
   empty inside the transaction.
4. CLI: insert-only confirmation alone cannot replace;
   `--i-confirm-scoped-replace` is accepted for `character-myshop-unit-prices`
   (and still for already-owned replace kinds); other unfrozen kinds still
   reject it.
5. Negatives: missing schema tip `23`, bad quarantine (duplicate vnum / zero
   vnum), nil executor, and FK-missing parent characters still fail closed
   before commit.

## Likely files to change (later GREEN, not this freeze)

- `internal/accountstore/myshop_unit_prices_export.go` /
  `myshop_unit_prices_quarantine.go` (optional `character_ids` merge +
  wipe-to-empty exception)
- `internal/accountstore/myshop_unit_prices_import.go` (+ unit / sqlite harness
  tests)
