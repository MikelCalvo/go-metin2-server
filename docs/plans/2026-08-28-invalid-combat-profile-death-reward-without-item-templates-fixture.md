...
2. ~~Add further checked-in negatives only when a later reject case still forces
   ...
   ~~Also: `quest_flag` turn-in `reward_items` that omit the top-level
   `item_templates` collection entirely.~~ Done for structured quest-flag
   reward items without bundled templates:
   ...
   ~~Also: `quest_flag` turn-in `consume_items` that omit the top-level
   `item_templates` collection entirely.~~ Done for structured quest-flag
   consume items without bundled templates:
