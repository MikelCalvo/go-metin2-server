...
- roaming beyond one opt-in idle wander step around authored home when `combat_profiles.roam_delay_ms` is positive (omitted or zero stays stationary); pathing beyond the owned grid and line pack offsets stays deferred, and authored patrol routes stay with WORLD-PATROL
