...
Close the remaining drag-to-item stack-merge recovery twin after packet
...
same restart paths restore a usable live owner so a full compatible stack merge
...
1. Drive the owner to the retaliation HP floor with a content-loaded practice mob
   ...
   quickslots on the source cell and an item quickslot on the target cell.
...
3. After `/restart_here` restores live HP, the same `ITEM_USE_TO_ITEM` emits the
   ordinary full-merge burst (`ITEM_DEL` source + `ITEM_UPDATE` target + source
   item `QUICKSLOT_DEL`), persists the merged target stack, deletes only the
   source item quickslot, and leaves recovered MaxHP unchanged (merge is not a
   consumable use effect).
4. After `/restart_town` restores live HP at the owned empire town position, the
   same `ITEM_USE_TO_ITEM` likewise succeeds and persists the merged stack beside
...
```bash
go test ./internal/minimal -run 'TestGameSessionFlowPostFloorItemUseToItemFailsClosed' -count=1
