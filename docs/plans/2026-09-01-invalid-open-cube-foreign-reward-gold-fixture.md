# Invalid Open-Cube Foreign Reward Gold Fixture — 2026-09-01

## Objective

Close the next optional negative dry-run gap after the owned `open_safebox`
foreign-`reward_gold` and `open_cube` foreign-`size` / foreign-`title` /
foreign-`catalog` / foreign-warp-coords fixtures: check in a deterministic
fixture for an `open_cube` definition that illegally authors turn-in
`reward_gold`, so operators do not improvise that reject during
`/local/content-bundle/validate`.

## Why now

- Track D bootstrap quest / NPC / regen / drop authoring is otherwise closed.
- Unit tests already reject foreign `reward_gold` on `open_cube` via inline Go
...
- Spec language already says foreign reward/consume gold/experience fields fail
  ...
  only covered foreign `size` / `title` / `catalog` / warp coords — not the
  turn-in economy field case.
- Manual QA still invents throwaway JSON (or confuses `/local/interactions`
  non-`quest_flag` `reward_gold` rejects with authored craftsman validation)
  when confirming craftsman definitions cannot author turn-in `reward_gold`,
  which drifts from the owned NPC service examples and from the other
  checked-in negatives.
- Foreign `reward_gold` is the highest-confusion remaining craftsman field after
  foreign placement fields: it is legal on `quest_flag` turn-ins and illegal on
  `open_cube` (symmetric to the landed warehouse foreign-reward-gold twin).

## Contract owned by this slice

1. `docs/examples/bootstrap-invalid-open-cube-foreign-reward-gold-bundle.json`
...
4. Spec / QA / prior plan docs name the fixture beside the existing open-cube
   ...
   open-safebox foreign-reward-gold / oversize-size negatives.
...
go test ./internal/contentbundle ./internal/ops -run 'Test(CanonicalizeRejectsCheckedInOpenCubeForeignRewardGoldExample|LocalContentBundleValidateEndpointRejectsOpenCubeForeignRewardGoldExample)$' -count=1
git diff --check
```
...
3. ~~Add further checked-in negatives only when a later reject case still forces
   QA to invent JSON.~~ Done for `open_cube` foreign turn-in `reward_experience`:
