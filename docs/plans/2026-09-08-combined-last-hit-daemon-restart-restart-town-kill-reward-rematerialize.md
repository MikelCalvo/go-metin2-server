...
4. Crash/restart rematerializes the pending exclusive handle with
   `OwnerID = 0`, absolute `ownership_expires_at` / `despawn_at` intact, and
   durable owner identity intact. Matching owner `Join` rebinds process-local
   `OwnerID` without rewriting those FileStore timers. Living peers stay
   fail-closed while exclusive ownership is active. Construction-time restore
   publicizes a still-pending handle whose absolute ownership deadline has
   already passed, so a living collector can pick it up.
...
That same proof now pins the FileStore rematerialize contract this slice
claimed but previously left implicit: exclusive rows keep the pre-crash
absolute `ownership_expires_at` / `despawn_at`, restore parks
process-local `OwnerID = 0` until matching owner Join rebinds it, and a
copied FileStore whose ownership deadline is already due publicizes at
construction so a living collector can pick the handle up.
