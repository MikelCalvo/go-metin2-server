...
4. Add explicit migrations for richer item/economy domains, ownership-timer / public-release policy beyond the FileStore extras, or broader world runtime state only after the landed account/character/item/item-template/login-ticket/static-content/ground-handle/safebox repository seams stay stable; FileStore restart rematerialize for pending ground handles and durable safebox cells is already owned, while SQL import/backfill from quarantined `0010` / `0014` / `0015` exports remains deferred until operators choose a production engine on top of the harness gate.
5. Add a production apply/rollback command only after the dry-run status boundary, the programmatic apply primitive, and ledger validation behavior are exercised against an actual driver-backed test database, with explicit backup/restore policy. The programmatic apply primitive and CLI `apply` already exist; the remaining gap is production-engine selection / operator runbook hardening beyond the SQLite harness proof, not inventing apply from scratch.
