...
- `go test ./internal/config ./internal/minimal ./internal/service ./internal/ops` validates optional DB config loading, startup fail-closed behavior for partial config, no-DSN runtime-config exposure, the configured-driver migration-status boundary, loopback-only explicit migration-plan previews including the ledger-snapshot POST preflight, and the local account/character roster, character item-state, auth login-ticket handoff, character quest-state, and item-template-state export endpoints.
