...
The tenth migration is `0010_bootstrap_ground_item_state`. It freezes the first schema boundary for pending item-shaped and gold-shaped ground handles already visible through the bootstrap runtime and local debug snapshots. It is still schema-only; runtime ground handles remain in-memory and are not restored across process restart yet.
...
4. Add explicit migrations for richer item/economy domains, item ownership timers, combat-profile defaults, or broader world runtime state only after the landed account/character/item/item-template/login-ticket/static-content/ground-handle repository seams stay stable and any restart-recovery policy for `0010` is decided deliberately.
