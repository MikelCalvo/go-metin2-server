...
3. Next combat-lane candidates remain deferred skill/ranged/PvP codecs after the owned PvE baseline, or engagement cleanup only where a later RED proves a still-missing invariant. Proximity-armed owner death-floor without hit/target is covered by `TestGameRuntimeProximityAggroDelayedRetaliationReachesOwnerDeathFloorWithoutHitOrTarget`. Death-floor → `/restart_here` still-inside proximity suppress is covered by `TestGameRuntimeProximityAggroSuppressesReacquireUntilLeaveAndReenterAfterOwnerDeathFloorRestartHere`. Subject-release suppress when the owner's shared-world snapshot is already at the `0`-HP floor is covered by `TestSharedWorldRegistrySubjectReleaseSeedsProximitySuppressWhenOwnerAlreadyAtHPFloor`.
