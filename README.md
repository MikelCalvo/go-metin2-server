...
| NPCs / shops | [~] | Bootstrap static actors already support interaction metadata plus self-only `info` / `talk` responses, `shop_preview` now renders a structured merchant title+catalog preview from authored entries backed by item templates, visible `warp` actors can now transfer the interacting player through the existing self-session rebootstrap path, a fixed `1s` per-session per-target cooldown now suppresses repeated spam against the same NPC, the first buy-only merchant transaction state path now exists behind a temporary local `/shop_buy <catalog_slot>` harness, and the first owned client-visible merchant packet contract now freezes `INTERACT`-opened `GC::SHOP START` plus planned client `SHOP BUY` / `SHOP END` before codec/runtime binding, while sell-back, quest logic, and richer NPC UI are still not started. |
