...
- consumable item use, including template-authored signed `use_effect.point_delta` values so authored negative consumables can emit self-only point decreases, optional `use_effect.consume_count` values so multi-count consumables decrement authored stack units instead of a runtime-hardcoded single item, and optional `use_effect.info_message` text for the self-only info-chat placeholder; signed point overflow/underflow, overdrawn consume-count requests, and `confirm_when_use`, `quest_use`, `quest_use_multiple`, or `applicable` consumables fail closed,
