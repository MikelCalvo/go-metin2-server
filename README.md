...
| Inventory | [~] | The first protocol-owned self bootstrap contract is now frozen in `spec/protocol/inventory-equipment-bootstrap.md`, `internal/inventory` now owns stable carried-slot identity plus a first `ItemInstance` validation seam, both `internal/accountstore` plus `internal/loginticket` now persist explicit `gold`, `inventory`, and `equipment` snapshot fields with backwards-compatible JSON normalization, and `internal/player` now owns live carried-item move/equip/unequip seams without silently mutating persisted snapshots, but client-visible packets are still ahead. |
| Equipment | [~] | The first protocol-owned worn-slot surface is now frozen in docs, `internal/inventory` now owns deterministic named `EquipmentSlot` value objects, persisted character snapshots now keep explicit equipped-item state instead of leaving worn slots implicit, and `internal/player` now owns live equip/unequip state plus deterministic equipment ordering, but self refresh packets and appearance integration are still ahead. |
...
| M3 — Character systems | [~] | The first inventory/equipment bootstrap contract is now frozen in project docs and packet matrix, `internal/inventory` owns the first slot/equipment/item value objects, persisted account/ticket snapshots now carry explicit `gold`, `inventory`, and `equipment` state with backwards-compatible JSON normalization, and `internal/player` now owns the first live currency/inventory/equipment mutation boundary with explicit save-back, but packets and client refresh flows are still ahead before the milestone is usable. |
