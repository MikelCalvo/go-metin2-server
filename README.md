...
The first authored `spawn_groups` seam is now live too: content-bundle import/export can round-trip one stationary practice mob whose runtime death/respawn loop is owned by the existing `training_dummy` combat profile, whose first accepted hit establishes a tiny aggro-lite gate against fresh third-party `TARGET` attempts until reset, whose accepted owner-side live hits now also append a deterministic immediate self-only HP retaliation tick, and whose first accepted live hit now starts a one-beat-at-a-time delayed self-only server-origin follow-up cadence while that engagement remains live.
...
| M5 — Content runtime | [~] | NPCs, mobs, spawn groups, shops, and the first quest/script runtime become available; `spawn_groups` + `combat_profile` can already load one stationary practice mob, whose first accepted hit now owns a tiny aggro-lite target gate, an immediate self-only retaliation tick on accepted live hits, and one sustained delayed self-only server-origin follow-up cadence at a time while broader authored content runtime is still pending. |
