...
- [~] First combat slice: live target selection for a visible `training_dummy`, exact `ATTACK` ownership, and the first deterministic selected-target HP refresh loop are now live
...
| M4 — Combat vertical slice | [~] | Target selection is live, the exact `ATTACK` codec is owned, and the first selected-target normal-attack loop now decrements bootstrap dummy HP while reusing `GC TARGET`; death, clear-target end-to-end, and respawn still need the next slices. |
