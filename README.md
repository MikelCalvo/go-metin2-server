...
- client packet ownership for `ITEM_GIVE`, `EXCHANGE`, `REFINE`, and first safebox/mall storage requests, including visible-target-gated `ITEM_GIVE` anti-give feedback, the first visible-peer exchange open/cancel/busy-target shell plus active-shell display-only exchange item-add/item-del/gold-add/accept frames with duplicate display-slot/source-item suppression, active-shell anti-give reject text, accept-marker reset on later display changes, `/quit` / `/logout` / `/phase_select` exchange-window teardown, and successful carried-item-use / carried item-move / slash inventory-move / drag-to-item stack-consolidation / drop / merchant buy/sell exchange teardown while accepted trade finalization, refine-result semantics, and storage mutations remain intentionally narrow or fail-closed.
...
- content bundle import/export with preview deltas for static actors, spawn groups, combat profiles, reward drops, NPC routes, warp destinations, and focused portable quest-state overview/flag readers,
...
- loopback spawn-leash tooling for materialized spawn groups: read-only inspection (`/local/spawn-groups/{entity_id}/leash?radius=...`), a controlled one-step return trigger (`POST /local/spawn-groups/{entity_id}/return-step?max_step=...`) that applies one capped planned step for `return_required` live spawn-backed actors through the existing persistence and visibility rebuild path while resetting selected combat-target/engagement ownership when the actor actually moves, a first pending-frame server-owned return-step executor for out-of-leash spawn-backed actors — including live persisted actors restored already outside leash at startup — using the same stepped visibility/target-reset path and re-arming only while the actor stays return-required, a controlled exact-home trigger (`POST /local/spawn-groups/{entity_id}/return-home`) that moves a live spawn-backed actor back to preserved authored home from either `within_radius` drift or `return_required` displacement while resetting selected combat-target/engagement ownership, and a first fail-closed combat gate with an explicit `target_return_required` runtime reason for spawn actors outside their current owned leash,
