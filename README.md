...
- [~] First content-loaded combatant: `spawn_groups` can now materialize one stationary practice mob through an authored `combat_profile`, the first post-hit aggro-lite gate blocks fresh third-party `TARGET` attempts until death / respawn resets engagement, accepted owner-side hits now also carry a deterministic immediate self-only HP retaliation tick, and the same engagement now owns both a fixed same-target `250ms` normal-attack cadence gate and a one-beat-at-a-time delayed self-only server-origin follow-up cadence that clamps retaliation point-loss at `0` HP, emits one self-only `DEAD(owner_vid)` plus target clear at that floor, fail-closes later owner combat `TARGET` / `ATTACK`, owner `MOVE` / `SYNC_POSITION`, owner static-actor `INTERACT`, owner merchant-buy attempts, owner slash `/use_item` attempts, owner slash `/inventory_move` attempts, owner slash `/equip_item` / `/unequip_item` attempts, and owner peer-facing `CHAT` / `WHISPER` attempts there, while broader mob/AI and player-death systems stay out of scope
...
The first authored `spawn_groups` seam is now live too: content-bundle import/export can round-trip one stationary practice mob whose runtime death/respawn loop is owned by the existing `training_dummy` combat profile, whose first accepted hit establishes a tiny aggro-lite gate against fresh third-party `TARGET` attempts until reset, whose accepted owner-side live hits now also append a deterministic immediate self-only HP retaliation tick, and whose same live selected-target engagement now owns both a fixed same-target `250ms` normal-attack cadence gate and a one-beat-at-a-time delayed self-only server-origin follow-up cadence that clamps retaliation point-loss at `0` HP, emits one self-only `DEAD(owner_vid)` plus target clear at that floor, and fail-closes later owner combat `TARGET` / `ATTACK`, owner `MOVE` / `SYNC_POSITION`, owner static-actor `INTERACT`, owner merchant-buy attempts, owner slash `/use_item` attempts, owner slash `/inventory_move` attempts, owner slash `/equip_item` / `/unequip_item` attempts, and owner peer-facing `CHAT` / `WHISPER` attempts there while that engagement remains live.
...
| M5 — Content runtime | [~] | NPCs, mobs, spawn groups, shops, and the first quest/script runtime become available; `spawn_groups` + `combat_profile` can already load one stationary practice mob, whose first accepted hit now owns a tiny aggro-lite target gate, an immediate self-only retaliation tick on accepted live hits, the same-target `250ms` attack cadence gate, and one sustained delayed self-only server-origin follow-up cadence at a time with retaliation point-loss clamped at `0` HP, one self-only `DEAD(owner_vid)` plus stale-target clear at that floor, and later owner combat `TARGET` / `ATTACK`, owner `MOVE` / `SYNC_POSITION`, owner static-actor `INTERACT`, owner merchant-buy attempts, and owner slash `/use_item` attempts fail-closed there while broader authored content runtime is still pending. |
