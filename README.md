...
| NPCs / shops | [~] | Bootstrap static actors already support interaction metadata plus self-only `info` / `talk` responses, `shop_preview` now renders a structured merchant title+catalog preview from authored entries backed by item templates, visible `warp` actors can now transfer the interacting player through the existing self-session rebootstrap path, a fixed `1s` per-session per-target cooldown now suppresses repeated spam against the same NPC, and the first buy-only merchant transaction gate is now frozen in docs while real transaction implementation, sell-back, quest logic, and richer NPC UI are still not started. |
