...
- `ITEM_GIVE` packet codec and `GAME` dispatch ownership with the shipped runtime intentionally fail-closed until recipient transfer/exchange semantics are frozen, plus a template-authored `anti_give` / `give_reject_message` self-only info-chat guard that explains bound-item rejection without mutating inventory, quickslots, peers, ground handles, or persistence,
