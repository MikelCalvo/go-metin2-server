...
| Bootstrap item templates | [~] | A deterministic file-backed store now exists under `internal/itemstore` for stable per-`vnum` template metadata (`name`, `stackable`, `max_count`, optional `equip_slot`, and the first optional `use_effect` point/effect payload), that metadata now anchors both the first frozen carried-stack/merchant-grant placement contract in `spec/protocol/item-stack-bootstrap.md` and the first owned template-backed `/use_item <slot>` point/effect resolution path, and `gamed` now loads the catalog at boot to validate and render structured merchant previews while also driving the first live merchant-grant full-merge / multi-stack fan-out / multi-stack-plus-fresh-slot / fresh-slot decision in runtime. |
...
- `internal/itemstore` — deterministic file-backed item-template catalog keyed by stable `vnum`, carrying the first owned `name` / `stackable` / `max_count` / optional `equip_slot` metadata plus the first optional template-authored `use_effect` payload now used to validate and render structured merchant previews and drive the first owned `/use_item <slot>` point/effect path
