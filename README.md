...
- `[~]` **M2 — Character, inventory, equipment, and economy bootstrap**
  - Inventory/equipment replay, item move/split/merge/use/drop/pickup, quickslots, merchant buy/sell, gold mutation, a first exchange open/cancel shell plus mutual-accept finalize with fail-closed origin/partner persistence rollback, refine reject/preview plus confirm-after-preview success (`probability = 100`) and destroy-failure (`probability = 0` + `RefineFailed`) paths, authored item-template guards, and persistence validation exist. Bootstrap ground-item exclusive ownership timers (30s owner-only, then blank public ownership) are owned for the in-memory path. Storage, item sockets/bonuses, full restrictions, broader refine `1..99`/catalyst outcomes, restart-restored ownership timer state, and DB-backed item persistence remain future work.
...
- accepted refine `1..99`/catalyst/keep-grade outcomes beyond the owned `probability = 100` success and `probability = 0` destroy-failure confirm seams,
