...
- client packet ownership for `ITEM_GIVE`, `EXCHANGE`, and `REFINE`, including the first visible-peer exchange open/cancel/busy-target shell plus display-only exchange item-add/item-del frames while gold/accept/refine-result semantics remain intentionally narrow or fail-closed.
