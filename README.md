...
| Inventory | [~] | The first protocol-owned self bootstrap contract is now frozen in `spec/protocol/inventory-equipment-bootstrap.md`, `internal/inventory` now owns stable carried-slot identity plus a first `ItemInstance` validation seam, and both `internal/accountstore` plus `internal/loginticket` now persist explicit `gold`, `inventory`, and `equipment` snapshot fields with backwards-compatible JSON normalization, but client-visible packets and runtime mutation flows are still ahead. |
| Equipment | [~] | The first protocol-owned worn-slot surface is now frozen in docs, `internal/inventory` now owns deterministic named `EquipmentSlot` value objects, and persisted character snapshots now keep explicit equipped-item state instead of leaving worn slots implicit, but equip/unequip runtime behavior and self refresh packets are still ahead. |
...
| M3 — Character systems | [~] | The first inventory/equipment bootstrap contract is now frozen in project docs and packet matrix, `internal/inventory` owns the first slot/equipment/item value objects, and persisted account/ticket snapshots now carry explicit `gold`, `inventory`, and `equipment` state with backwards-compatible JSON normalization, but runtime wiring, packets, and mutations are still ahead before the milestone is usable. |
