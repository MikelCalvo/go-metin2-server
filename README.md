...
| Item use / consumables | [~] | The first owned bootstrap consumable vertical is now implemented in `spec/protocol/item-use-bootstrap.md` and the minimal runtime: `/use_item <slot>` accepts carried `vnum = 27001`, applies a fixed self-only `PLAYER_POINT_CHANGE`, refreshes the consumed slot with deterministic `ITEM_SET`/`ITEM_DEL`, emits one temporary `CHAT_TYPE_INFO` placeholder effect, persists the selected character snapshot atomically through the existing rollback boundary while the socket still owns live shared-world state, and after reclaim now stays self-local/non-authoritative against persisted points/inventory plus the replacement live owner's loopback inventory view, while a later fresh reconnect after that stale socket closes still reloads the authoritative persisted points/inventory snapshot instead of the stale local divergence; it now sits beside both a first `internal/itemstore` template seam plus the newly frozen `spec/protocol/item-stack-bootstrap.md` carried-stack contract that later slices can reuse for template-driven grants and effects. |
...
| Bootstrap item templates | [~] | A deterministic file-backed store now exists under `internal/itemstore` for stable per-`vnum` template metadata (`name`, `stackable`, `max_count`, optional `equip_slot`), that metadata now also anchors the first frozen carried-stack/merchant-grant placement contract in `spec/protocol/item-stack-bootstrap.md`, and `gamed` now loads the catalog at boot to validate and render structured merchant previews while also driving the first live merchant-grant full-merge / multi-stack fan-out / multi-stack-plus-fresh-slot / fresh-slot decision in runtime. |
