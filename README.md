...
- carried item drop and counted drop with template-authored transfer guards, including `anti_stack`, plus fail-closed deterministic ground-`VID` collision guards before item or persistence mutation,
...
- merchant preview/catalog/open/close/buy/sell slices, including `GC::SHOP START` catalog entries backed by template-authored display sockets/attributes, refresh-only merchant-buy success bursts shared by packet `SHOP BUY` and the local `/shop_buy` debug harness, template-authored `anti_stack` and over-template-max fail-closed sell-back rejection, and partial-stack sell `ITEM_UPDATE` refreshes that preserve authored display sockets/attributes,
...
- self and visible-peer `DAMAGE_INFO` hit-effect emission for standalone bootstrap combat-profile non-lethal hits plus the first owner self-only spawn-backed practice-mob hit-effect, while spawn-backed peer/broader emission policy remains narrow,
