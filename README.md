...
| Inventory | [~] | The first protocol-owned self bootstrap contract is now frozen in `spec/protocol/inventory-equipment-bootstrap.md`, `internal/proto/item` now owns the first `ITEM_SET` / `ITEM_DEL` golden packet slice for that bootstrap family, `internal/inventory` now owns stable carried-slot identity plus a first `ItemInstance` validation seam, both `internal/accountstore` plus `internal/loginticket` now persist explicit `gold`, `inventory`, and `equipment` snapshot fields with backwards-compatible JSON normalization, `internal/player` now owns live carried-item move/equip/unequip seams without silently mutating persisted snapshots, and loopback-only local introspection endpoints now expose that live runtime state by exact character name before broader client refresh slices land. |
...
| M3 — Character systems | [~] | The first inventory/equipment bootstrap contract is now frozen in project docs and packet matrix, `internal/proto/item` now owns the first exact `ITEM_SET` / `ITEM_DEL` golden slice, `internal/inventory` owns the first slot/equipment/item value objects, persisted account/ticket snapshots now carry explicit `gold`, `inventory`, and `equipment` state with backwards-compatible JSON normalization, `internal/player` now owns the first live currency/inventory/equipment mutation boundary with explicit save-back, and loopback-only exact-name introspection endpoints now make that live M3 state observable before broader client refresh flows land, but packets and client refresh flows are still ahead before the milestone is usable. |
