...
- `[~]` **M2 — World/entity runtime foundation**
  ...
  - Latest world-runtime map-index hardening also suppresses ambiguous map-only player/static remnants when every authoritative primary/directory index has already been lost and the remaining buckets collide by shared entity ID or client-visible `VID`, so `/local/maps`-style snapshots fail closed instead of inventing a winning actor.
