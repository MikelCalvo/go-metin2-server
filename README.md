...
| NPCs / shops | [~] | Bootstrap static actors already support interaction metadata plus self-only `info` / `talk` responses, and visible `warp` actors can now transfer the interacting player through the existing self-session rebootstrap path; read-only `shop_preview`, real shop transactions, quest logic, and richer NPC UI are still not started. |
