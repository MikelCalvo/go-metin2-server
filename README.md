...
- A first non-player runtime scaffolding in `internal/worldruntime`: static actors can now be registered with owned entity identity and map presence, can now also be updated in-place across runtime directories/map buckets, and `gamed` now exposes the first local-only operator seed/snapshot/update/remove path for those runtime actors without claiming client-visible spawn/update behavior yet.
