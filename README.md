...
- `[~]` **M2 — Character, inventory, equipment, and economy bootstrap**
  - Inventory/equipment replay, item move/split/merge/use/drop/pickup, quickslots, merchant buy/sell, gold mutation, a first exchange open/cancel shell plus mutual-accept finalize with fail-closed origin/partner persistence rollback, refine reject/preview plus the first confirm-after-preview success path (`probability = 100` only), authored item-template guards, and persistence validation exist. Bootstrap ground-item exclusive ownership timers (30s owner-only, then blank public ownership) are owned for the in-memory path. Storage, item sockets/bonuses, full restrictions, broader refine failure/catalyst outcomes, restart-restored ownership timer state, and DB-backed item persistence remain future work.
...
- accepted refine failure/destroy/catalyst outcomes beyond the first `probability = 100` confirm-after-preview success seam,
