...
- authored item-template metadata for selected display/guard behavior, including template-backed refine-dialog preview metadata that now stays fail-closed when selected-character restrictions or transfer guards disallow the carried item and projects into the current migration-shaped item-template export, plus content-bundle summary projection of refine guard metadata (`refineable` / `refine_reject_message`) before import,
...
- read-only loopback combat-target snapshots that mirror current visibility/range/leash/aggro/death gates and expose resolved current HP, profile max HP, and normal-hit damage without mutating stale target state,
