...
| Bootstrap interaction definitions | [~] | A deterministic file-backed store now exists under `internal/interactionstore` for minimal `info` / `talk` content keyed by `kind + ref`, `gamed` now loads that catalog at boot and uses it to fail closed on missing static-actor interaction refs, and `info` interactions now produce a self-only authored chat delivery, but loopback authoring plus `talk` responses are still ahead. |
