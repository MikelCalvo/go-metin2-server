...
- `[~]` **M2 — Character, inventory, equipment, and economy bootstrap**
  - Inventory/equipment replay, item move/split/merge/use/drop/pickup, quickslots, merchant buy/sell, gold mutation, a first exchange open/cancel shell plus mutual-accept finalize, refine fail-closed path, authored item-template guards, and persistence validation exist. Bootstrap ground-item exclusive ownership timers (30s owner-only, then blank public ownership) are owned for the in-memory path. Storage, item sockets/bonuses, full restrictions, refine success, restart-restored ownership timer state, and DB-backed item persistence remain future work.
