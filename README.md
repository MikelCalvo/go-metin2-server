...
- read-only loopback combat-target snapshots that mirror current visibility/range/leash/aggro/death gates and expose resolved current HP, profile max HP, normal-hit damage, and any currently armed delayed retaliation timer without mutating stale target state,
