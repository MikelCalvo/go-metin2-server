...
- local operator/debug endpoints for runtime inspection, controlled bootstrap actions, static-actor respawn timer plus spawn-group snapshot/map-occupancy/transfer visibility inspection, and manifest-backed item-template/account-store backup/restore preflights.
...
- `[~]` **M2 — World/entity runtime foundation**
  - The repo has topology, map indexing, AOI/radius-style visibility, player/session directories, entity registries, non-player directories, static actors, spawn groups, runtime scopes, and operator snapshots. Player-directory secondary identity indexes, map-index player/static lookup and occupancy snapshot repair, and static-actor visibility-`VID` indexes now prune orphaned and non-canonical stale aliases instead of letting ghost lookup entries survive partial teardown or repair; player-directory register/update conflict checks also reclaim non-canonical aliases before allowing a new canonical owner. Map-index readers now also repair same-kind player/static presence whose stored snapshot belongs to a different effective map bucket, and map-index lookup/removal/occupancy snapshot paths preserve the surviving canonical or remembered effective-map bucket when duplicate map-only player/static buckets remain after primary-index loss, so `/local/maps`-style occupancy cannot keep reporting a player or static actor on an old map after partial repair or tolerant teardown. Entity-registry player and static-actor lookups rebuild missing directory entries from surviving map-index presence, and connected-player snapshot/scope reads now also repair map-index-live players whose player-directory entry was lost. Direct map readers and map-occupancy snapshots now repair the reverse map-index-only loss from surviving player/static directories while still refusing to shadow opposite-kind map remnants. Lookup repair still fails closed when a directory-only opposite-kind entity or visible-`VID` remnant would be shadowed, and all-static/scope snapshots now skip directory-only static actors whose projected visible `VID` is still owned by surviving map-index player presence. Player registration/update now fails closed when surviving map-only player presence still owns the requested client-visible `VID` or exact name, map-index player/static registration and update paths enforce the same cross-kind visible-ID namespace against surviving opposite-kind primary entries or map buckets, and cross-kind entity-ID guards now also reject directory-only player/static remnants before they can be overwritten after partial map-index teardown. Static-actor entity IDs are validated against the same `uint32` visibility-`VID` carrier used by client-visible actor packets before runtime registration or snapshot restore. The next work is depth: richer lifecycle, better spawn policy, stronger transfer/reconnect edges, and long-running production behavior.
...
- carried item drop and counted drop with template-authored transfer guards, including `anti_stack`, template-authored drop-rejection info messages, plus fail-closed deterministic ground-`VID` collision guards before item or persistence mutation,
...
- content bundle import/export with commit-only live visibility replay for static-actor replacement and import-preview deltas that include top-level counts, per-map static actor/spawn-group rows, portable combat-profile rows, grouped reward-drop rows, and NPC service route rows,
- loopback-only local endpoints for static actors, interactions, visibility, and content bundles, including compact summaries with interaction-definition previews, shop catalog details, warp destination details, and import-preview deltas for portable combat-profile snapshots plus grouped reward-drop changes,
