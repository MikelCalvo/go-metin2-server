...
- authored item-template metadata for selected display/guard behavior, including content-bundle summary projection of refine metadata (`refineable` / `refine_reject_message`) before import,
...
- client packet ownership for `ITEM_GIVE`, `EXCHANGE`, `REFINE`, and first safebox/mall storage requests plus codec ownership for safebox/mall responses and server refine-information frames, including visible-target-gated `ITEM_GIVE` anti-give feedback, the first visible-peer exchange open/cancel/busy-target shell plus active-shell display-only exchange item-add/item-del/gold-add/accept frames with duplicate display-slot/source-item suppression, active-shell anti-give reject text, accept-marker reset on later display changes, `/quit` / `/logout` / `/phase_select` exchange-window teardown, successful carried-item-use / carried item-move / slash inventory-move / drag-to-item stack-consolidation / drop / merchant buy/sell exchange teardown, and codec-only `REFINE_INFORMATION` / `REFINE_INFORMATION_NEW` layouts while accepted trade finalization, refine-window/result runtime semantics, and storage mutations remain intentionally narrow or fail-closed.
