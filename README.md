...
That same retaliation-driven `0`-HP floor now also removes the still-connected dead owner from later peer-entry visibility delivery: other live recipients still receive the newcomer’s ordinary `CHARACTER_ADD` / `CHAR_ADDITIONAL_INFO` / `CHARACTER_UPDATE` burst on fresh joins, and live movers/syncing peers still receive their ordinary origin-side peer-entry burst when crossing into visibility later, but the zero-HP owner receives no queued peer-entry frames from those current join / `MOVE` / `SYNC_POSITION` visibility-entry paths.
