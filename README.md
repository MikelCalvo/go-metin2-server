...
- template-backed equip/unequip point-effect application and removal guards, including signed equip-effect penalties, authored `irremovable` equipment unequip rejection with template-authored `unequip_reject_message` info-chat feedback, fail-closed rejection when live equipment-source counts exceed authored template `max_count`, and visible appearance projection for equipped `body`, `weapon`, `head`, and `hair` items using template-authored `appearance_vnum` when present while preserving the real item `vnum` in `ITEM_SET` and persistence,
