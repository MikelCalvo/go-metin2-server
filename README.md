...
- [~] First content-loaded combatant: `spawn_groups` can now materialize one stationary practice mob through an authored `combat_profile` while broader mob/AI systems stay out of scope
...
The first authored `spawn_groups` seam is now live too: content-bundle import/export can round-trip one stationary practice mob whose runtime death/respawn loop is owned by the existing `training_dummy` combat profile.
...
| M5 — Content runtime | [~] | NPCs, mobs, spawn groups, shops, and the first quest/script runtime become available; `spawn_groups` + `combat_profile` can already load one stationary practice mob, while broader authored content runtime is still pending. |
