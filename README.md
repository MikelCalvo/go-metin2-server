...
| Item use / consumables | [~] | The first docs-only bootstrap contract is now frozen in `spec/protocol/item-use-bootstrap.md` for exactly one self-only consumable path (`/use_item <slot>` on carried `vnum = 27001`, fixed `PLAYER_POINT_CHANGE`, deterministic `ITEM_SET`/`ITEM_DEL`, and one temporary `CHAT_TYPE_INFO` placeholder effect), but the runtime implementation has not landed yet. |
