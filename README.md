...
| Entity runtime beyond players | [~] | A first generic entity registry, dedicated player directory, and topology-aware map index now exist in `internal/worldruntime`; the implementation is still player-only, and session-directory extraction, richer AOI, and non-player entities are still ahead. |
...
| M2 — Entity/world runtime foundation | [~] | Live player runtime, the first generic entity registry, and the first owned map index now exist; session-directory extraction, final transfer rebootstrap, richer AOI, and non-player entities still need to replace the remaining bootstrap shortcuts. |
...
- `internal/worldruntime` — bootstrap topology, visibility helpers, and the first generic entity/runtime seams (entity registry + player directory + map index today; session-directory boundary next)
