...
| NPCs / shops | [~] | Bootstrap static actors already support interaction metadata plus self-only `info` / `talk` responses, `shop_preview` now renders a structured merchant title+catalog preview from authored entries backed by item templates, valid merchant interactions now open a real `GC::SHOP START` window, active merchant sessions now close through client `SHOP END` plus server `GC::SHOP END`, visible `warp` actors can now transfer the interacting player through the existing self-session rebootstrap path, a fixed `1s` per-session per-target cooldown now suppresses repeated spam against the same NPC, the bootstrap buy-only merchant transaction state path still lives behind a temporary local `/shop_buy <catalog_slot>` harness until the next slice binds real `SHOP BUY`, and sell-back, quest logic, and richer NPC UI are still not started. |
