...
| Bootstrap item templates | [~] | A deterministic file-backed store now exists under `internal/itemstore` for stable per-`vnum` template metadata (`name`, `stackable`, `max_count`, optional `equip_slot`), that metadata now also anchors the first frozen carried-stack/merchant-grant placement contract in `spec/protocol/item-stack-bootstrap.md`, and `gamed` now loads the catalog at boot to validate and render structured merchant previews while also driving the first live merchant-grant merge/new-slot decision in runtime. |
