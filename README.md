...
| Combat loop | [ ] | No targeting, attacks, damage, or death loop is implemented yet, but the first visible `training_dummy` target-selection contract is now frozen in `spec/protocol/combat-training-dummy-bootstrap.md` to prepare the next combat RED. |
...
| M4 — Combat vertical slice | [ ] | Targeting, attacks, damage, death, and respawn work for at least one minimal content path; the current repo only freezes the first visible `training_dummy` target-selection contract in docs. |
