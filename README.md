...
- A deterministic file-backed interaction-definition store now exists in `internal/interactionstore` for authored interaction content keyed by stable `kind + ref`; it now serves loopback HTTP authoring for `info`, `talk`, structured `shop_preview`, and the first `warp` payload shape (`map_index`, `x`, `y`, optional `text`), `gamed` now loads that catalog at boot when present, static actors that point at missing definitions now fail closed at boot plus on runtime create/update paths, visible static actors whose metadata resolves to `interaction_kind = "info"` or `interaction_kind = "talk"` now answer the interacting player with a self-only chat-backed authored delivery, visible static actors whose metadata resolves to `interaction_kind = "shop_preview"` now render a deterministic structured merchant title+catalog preview backed by item templates, visible static actors whose metadata resolves to `interaction_kind = "warp"` can now transfer the interacting player through the existing self-session rebootstrap path, known bootstrap interaction failures now also answer the player with one deterministic self-only `CHAT_TYPE_INFO` message instead of silently disappearing, a fixed `1s` per-session per-target interaction cooldown now suppresses repeated spam against the same NPC with a deliberate no-op, loopback-only `GET`/`POST /local/interactions` plus `PATCH`/`PUT`/`DELETE /local/interactions/{kind}/{ref}` now expose that widened authoring shape, loopback-only `GET /local/interaction-visibility` now exposes compact resolved previews for the interactable actors each connected player can currently see, loopback-only `GET`/`POST /local/content-bundle` now exports/imports one deterministic authored-content bundle for static actors plus interaction definitions, the first buy-only merchant transaction state path is now implemented behind a temporary local `/shop_buy <catalog_slot>` harness, valid `shop_preview` merchant interactions now open a real `GC::SHOP START` window and client `SHOP END` now closes that live merchant context, the first owned merchant window contract (`INTERACT`-opened `GC::SHOP START`, client `SHOP BUY` / `SHOP END`) is now frozen in docs, and `internal/proto/shop` now owns the first exact merchant packet-family codec slice for client `BUY` / `END` plus server `START` / `END`.
...
- A first protocol-owned M3 inventory/equipment bootstrap contract is now frozen in docs and packet matrix: the selected character now receives self-only slot-addressed carried/equipped `ITEM_SET` bootstrap frames immediately after `PLAYER_POINT_CHANGE`, the first `internal/proto/item` codec slice owns `ITEM_SET` / `ITEM_DEL` golden fixtures for that legacy wire family, bootstrap `GAME` chat now also exposes self-only `/inventory_move <from> <to>`, `/equip_item <from> <equip_slot>`, `/unequip_item <equip_slot> <to>`, `/use_item <slot>`, and a temporary local-only merchant debug harness `/shop_buy <catalog_slot>` that persist carried/worn snapshot changes plus the first consumable point delta and buy-only merchant mutation path while later `SHOP` packet slices land, and loopback-only `GET /local/inventory/{name}`, `GET /local/equipment/{name}`, plus `GET /local/currency/{name}` expose the selected character's live M3 runtime state for local debugging while later appearance/stat slices land.
- A first `internal/inventory` package now owns slot identity, deterministic equipment-slot naming, carried/equipped item-instance shape, and basic validation rules before persistence/runtime wiring lands, and a first deterministic file-backed `internal/itemstore` seam now owns stable per-`vnum` template metadata for future merchant/catalog slices.
...
| Operator/admin surface | [~] | Loopback-only `POST /local/notice`, `POST /local/relocate`, `POST /local/relocate-preview`, `POST /local/transfer`, `GET /local/runtime-config`, `GET /local/players`, `GET /local/visibility`, `GET /local/interaction-visibility`, `GET /local/inventory/{name}`, `GET /local/equipment/{name}`, `GET /local/currency/{name}`, `GET /local/maps`, `GET`/`POST /local/content-bundle`, `GET`/`POST /local/interactions`, `PATCH`/`PUT`/`DELETE /local/interactions/{kind}/{ref}`, plus `GET`/`POST /local/static-actors`, `PATCH`/`PUT /local/static-actors/{entity_id}`, and `DELETE /local/static-actors/{entity_id}` for bootstrap runtime introspection and authoring, exist on `gamed`; `/local/runtime-config` now reports the active local channel and visibility-policy selection, `/local/visibility` now also reports `visible_static_actors` per connected player under the active topology/AOI policy, `/local/inventory/{name}` + `/local/equipment/{name}` + `/local/currency/{name}` now expose the selected character's live M3 runtime state by exact name for loopback-only debugging, `/local/interaction-visibility` now reports only the currently visible interactable static actors per connected player together with compact resolved previews or fail-closed resolution markers, `/local/interactions` now authors the deterministic `info` / `talk` catalog with stable `kind + ref` identity while rejecting deletes for definitions still referenced by bootstrap static actors, `/local/content-bundle` now exports/imports one deterministic authored-content artifact covering both bootstrap static actors and interaction definitions with full-replace import semantics, `/local/maps` now reports static actors alongside connected players in each effective-map snapshot, newly seeded static actors now immediately enqueue their visibility burst to already-visible online players, operator-driven static-actor deletes now immediately enqueue `CHARACTER_DEL` to those same already-visible sessions, PATCH/PUT updates now refresh retained viewers with delete-plus-rebootstrap and also emit delete/add visibility deltas when the actor crosses map/AOI boundaries, and the structured relocate-preview/transfer responses now include full before/after map-occupancy snapshots plus explicit static-actor visibility diffs beside the delta counts; broader admin/auth tooling does not. |

### Character systems and gameplay

| Area | Status | Notes |
| --- | --- | --- |
| Character snapshots / bootstrap stats | [~] | Enough for selection, spawn, movement, chat, and bootstrap transfer-trigger slices; a first live `internal/player` runtime model now exists separately from persisted snapshots, the default bootstrap `mkmk` account now seeds `MkmkWar` at the legacy Shinsoo Yongan start while auto-migrating the untouched old fake `(1000,2000)` snapshot on load, fresh character creation now uses legacy empire-specific create positions instead of fake slot-relative coordinates, and save-back paths now explicitly merge the selected runtime snapshot instead of silently relying on stale persisted character copies. |
...
| Inventory | [~] | The first protocol-owned self bootstrap contract is now frozen in `spec/protocol/inventory-equipment-bootstrap.md`, the first carried stack/merge rules are now also frozen in `spec/protocol/item-stack-bootstrap.md`, `internal/proto/item` now owns the first `ITEM_SET` / `ITEM_DEL` golden packet slice for that bootstrap family, `internal/inventory` now owns stable carried-slot identity plus a first `ItemInstance` validation seam, both `internal/accountstore` plus `internal/loginticket` now persist explicit `gold`, `inventory`, and `equipment` snapshot fields with backwards-compatible JSON normalization, `internal/player` now owns live carried-item move/equip/unequip seams without silently mutating persisted snapshots, and stale post-reclaim `/equip_item` / `/unequip_item` can no longer overwrite the authoritative persisted or loopback live inventory/equipment views while stale post-reclaim `/use_item` can no longer overwrite persisted points/inventory or the authoritative loopback live inventory view and stale post-reclaim merchant `SHOP BUY` / `/shop_buy` can no longer overwrite persisted gold/inventory or the authoritative loopback live inventory/currency view, while a later fresh reconnect after that stale merchant-buy socket closes still reloads the authoritative persisted gold/inventory snapshot instead of the stale local divergence; the first template-driven merchant-grant path can now prefer a compatible carried-stack merge, fully fan out across several compatible carried stacks when they alone can absorb the grant, consume one or more compatible carried stacks before placing the final remainder into one fresh slot, or claim a fresh slot deterministically, bootstrap slash-command seams now persist writeback for carried-slot move/swap plus worn/carried equip transitions and emit deterministic self-only `ITEM_DEL` / `ITEM_SET` refresh packets, successful equip/unequip seams now also append one self-only `CHARACTER_UPDATE` carrying the current projected appearance, already-visible stable shared-world peers now also receive one queued `CHARACTER_UPDATE` carrying that same projected appearance, late-joining visible peers now also see that same latest projected appearance through the normal peer bootstrap burst after the runtime mutation already happened, configured radius-AOI move-driven peer-entry bursts now also reuse that same latest projected appearance after the runtime mutation already happened, transfer-driven peer-entry bursts now also reuse that same latest projected appearance after the runtime mutation already happened, reconnect-driven peer-entry bursts now also reuse that same latest projected appearance after the runtime mutation already happened, duplicate-live retry-`ENTERGAME` peer-entry bursts now also reuse that same latest projected appearance after the runtime mutation already happened, and radius-AOI move-into-range peer-entry bursts now also reuse that same latest projected appearance after the runtime mutation already happened while loopback-only local introspection endpoints continue to expose that live runtime state by exact character name before broader client refresh slices land. |
| Equipment | [~] | The first protocol-owned worn-slot surface is now frozen in docs, `internal/inventory` now owns deterministic named `EquipmentSlot` value objects, persisted character snapshots now keep explicit equipped-item state instead of leaving worn slots implicit, `internal/player` now owns live equip/unequip state plus deterministic equipment ordering, and stale post-reclaim `/equip_item` / `/unequip_item` can no longer replace the authoritative persisted or loopback live equipment state; bootstrap slash-command seams now also refresh the client with self-only `ITEM_DEL` / `ITEM_SET` packets for equip/unequip over the frozen legacy combined inventory/equipment cell namespace, successful equip/unequip seams now also append one self-only `CHARACTER_UPDATE` carrying the same projected `body` / `weapon` / `head` appearance used by bootstrap visibility, already-visible stable shared-world peers now also receive one queued `CHARACTER_UPDATE` carrying that same projected appearance, late-joining visible peers now also receive the current projected appearance through the normal peer bootstrap burst after prior runtime equip/unequip mutations, configured radius-AOI move-driven peer-entry bursts now also reuse that current projected appearance after those prior runtime equip/unequip mutations, transfer-driven peer-entry bursts now also reuse that current projected appearance after those prior runtime equip/unequip mutations, reconnect-driven peer-entry bursts now also reuse that current projected appearance after those prior runtime equip/unequip mutations, duplicate-live retry-`ENTERGAME` peer-entry bursts now also reuse that current projected appearance after those prior runtime equip/unequip mutations, the first bootstrap appearance link is now frozen in `spec/protocol/equipment-appearance-bootstrap.md` so visible `CHAR_ADDITIONAL_INFO` / `CHARACTER_UPDATE` parts can project equipped `body` / `weapon` / `head` items during bootstrap and peer entry, and loopback-only `GET /local/equipment/{name}` now exposes the selected character's live worn-state snapshot for local debugging while broader non-frozen appearance choreography is still ahead. |
| Item use / consumables | [~] | The first owned bootstrap consumable vertical is now implemented in `spec/protocol/item-use-bootstrap.md` and the minimal runtime: `/use_item <slot>` resolves carried consumables through file-backed `internal/itemstore` template metadata, applies the template-authored self-only `PLAYER_POINT_CHANGE`, refreshes the consumed slot with deterministic `ITEM_SET`/`ITEM_DEL`, emits one temporary template-authored `CHAT_TYPE_INFO` placeholder effect, persists the selected character snapshot atomically through the existing rollback boundary while the socket still owns live shared-world state, and after reclaim now stays self-local/non-authoritative against persisted points/inventory plus the replacement live owner's loopback inventory view, while a later fresh reconnect after that stale socket closes still reloads the authoritative persisted points/inventory snapshot instead of the stale local divergence; the current seeded bootstrap consumable template still maps to carried `vnum = 27001`, `type = 1`, `amount = 50`, and `message = consume:27001:+50`, and the first narrow template-backed equip/unequip follow-up now also reuses `PLAYER_POINT_CHANGE` self-only through item-template `equip_effect` metadata for the seeded practice blade `vnum = 12200` with `amount = +/-10`, while the slice continues to sit beside both the first `internal/itemstore` template seam plus the newly frozen `spec/protocol/item-stack-bootstrap.md` carried-stack contract that later slices can reuse for broader template-driven grants and effects. |
| Derived stats / combat formulas | [~] | The first narrow self-only template-backed equip/unequip point delta now exists through `PLAYER_POINT_CHANGE` and item-template `equip_effect` metadata, but broader derived-stat recomputation and combat formulas are still not started. |
| Combat loop | [~] | The first self-only client-visible combat target slice now exists: client `TARGET (0x0A01)` can select one visible in-range `training_dummy` by `vid` through the shared-world targetability seam and returns one self-only `GC TARGET (0x0A10)` acknowledgement carrying `target_vid + hp_percent`, but attacks, damage, aggro, and death handling are still not started. |
| Respawn / death handling | [ ] | Not started. |
| NPCs / shops | [~] | Bootstrap static actors already support interaction metadata plus self-only `info` / `talk` responses, `shop_preview` now renders a structured merchant title+catalog preview from authored entries backed by item templates, valid merchant interactions now open a real `GC::SHOP START` window, active merchant sessions now close through client `SHOP END` plus server `GC::SHOP END`, visible `warp` actors can now transfer the interacting player through the existing self-session rebootstrap path, a fixed `1s` per-session per-target cooldown now suppresses repeated spam against the same NPC, the bootstrap buy-only merchant transaction state path now accepts real client `SHOP BUY` requests against the active merchant context while stackable buys can fully merge into one compatible carried stack, fully fan out across several compatible carried stacks when no fresh slot is needed, or fill one or more compatible carried stacks before placing the final remainder into one fresh slot, insufficient-gold and no-placement rejections now surface deterministic self-only placeholder info chat instead of failing silently, and `/shop_buy <catalog_slot>` remains as a temporary local debug harness while sell-back, quest logic, and richer NPC UI are still not started. |
...
| Bootstrap interaction definitions | [~] | A deterministic file-backed store now exists under `internal/interactionstore` for authored interaction content keyed by `kind + ref`; `gamed` now loads that catalog at boot, loopback-only CRUD endpoints now author `info`, `talk`, structured `shop_preview`, and the first `warp` payload shape without raw JSON edits, deletes now fail closed while bootstrap static actors still reference a definition, loopback-only interaction-visibility snapshots now preview what visible interactables would resolve to including structured merchant previews and compact warp destination summaries, `GET`/`POST /local/content-bundle` now exports/imports one deterministic authored-content artifact spanning both stores, `info`, `talk`, plus `shop_preview` interactions now produce self-only authored chat deliveries while `warp` reuses the transfer path, valid merchant interactions now also bind a live `GC::SHOP START` / `SHOP END` open-close session boundary, the bootstrap buy-only merchant transaction state path now accepts real client `SHOP BUY` requests against the active merchant context while `/shop_buy <catalog_slot>` remains as a temporary local debug harness, and the first client-visible merchant packet family is now partly live in runtime rather than docs-only. |
| Bootstrap item templates | [~] | A deterministic file-backed store now exists under `internal/itemstore` for stable per-`vnum` template metadata (`name`, `stackable`, `max_count`, optional `equip_slot`, the first optional `use_effect` point/effect payload, and the first optional `equip_effect` point payload), that metadata now anchors both the first frozen carried-stack/merchant-grant placement contract in `spec/protocol/item-stack-bootstrap.md`, the first owned template-backed `/use_item <slot>` point/effect resolution path, and the first narrow self-only template-backed equip/unequip point refresh path, while `gamed` continues to load the catalog at boot to validate and render structured merchant previews and drive the first live merchant-grant full-merge / multi-stack fan-out / multi-stack-plus-fresh-slot / fresh-slot decision in runtime. |
...
| M3 — Character systems | [~] | The first inventory/equipment bootstrap contract is now frozen in project docs and packet matrix, the first carried stack/merge contract is now also frozen in `spec/protocol/item-stack-bootstrap.md`, `internal/proto/item` now owns the first exact `ITEM_SET` / `ITEM_DEL` golden slice, `internal/inventory` owns the first slot/equipment/item value objects, a first deterministic file-backed `internal/itemstore` seam now owns stable per-`vnum` template metadata for later merchant/catalog work, persisted account/ticket snapshots now carry explicit `gold`, `inventory`, and `equipment` state with backwards-compatible JSON normalization, `internal/player` now owns the first live currency/inventory/equipment mutation boundary with explicit save-back including the first template-driven merchant stack placement path for full merges, multi-stack fan-out, multi-stack-plus-fresh-slot placement, and fresh-slot fallbacks, bootstrap slash-command seams now persist carried-slot move/swap plus equip/unequip writeback and refresh the client with deterministic self-only `ITEM_DEL` / `ITEM_SET` packets plus the first self-only equip/unequip `CHARACTER_UPDATE` appearance refresh, already-visible stable shared-world peers now also receive the first queued peer-visible equip/unequip `CHARACTER_UPDATE` appearance refresh, late-joining visible peers now also see the current projected appearance through the normal peer bootstrap burst after prior runtime equip/unequip mutations, configured radius-AOI move-driven peer-entry bursts now also reuse that same current projected appearance after prior runtime equip/unequip mutations, transfer-driven peer-entry bursts now also reuse that same current projected appearance after prior runtime equip/unequip mutations, reconnect-driven peer-entry bursts now also reuse that same current projected appearance after prior runtime equip/unequip mutations, duplicate-live retry-`ENTERGAME` peer-entry bursts now also reuse that same current projected appearance after prior runtime equip/unequip mutations, the first docs-only consumable-use contract is now also frozen in `spec/protocol/item-use-bootstrap.md`, and loopback-only exact-name introspection endpoints now make that live M3 state observable before broader peer appearance/stat/item-use flows land. |
| M4 — Combat vertical slice | [~] | The first minimal targeting vertical now exists for one visible `training_dummy`: docs, shared-world targetability seam, exact client/server `TARGET` codecs, and the first self-only request/ack path are owned, while attacks, damage, death, and respawn still remain ahead. |
...
- `internal/itemstore` — deterministic file-backed item-template catalog keyed by stable `vnum`, carrying the first owned `name` / `stackable` / `max_count` / optional `equip_slot` metadata plus the first optional template-authored `use_effect` and `equip_effect` payloads now used to validate and render structured merchant previews, drive the first owned `/use_item <slot>` point/effect path, and drive the first narrow self-only equip/unequip point refresh path
...
- `internal/proto/shop` — exact bootstrap merchant packet codec for client `SHOP BUY` / `SHOP END` plus server `GC::SHOP START` / `GC::SHOP END`
- `internal/proto/combat` — exact bootstrap target-selection codec for client `TARGET` plus the current self-only server `GC TARGET` acknowledgement
...
- `docs/plans/2026-04-29-post-bootstrap-merchant-next-twenty-five-slices.md`
...
- `spec/protocol/npc-shop-open-close-bootstrap.md`
...
curl -X POST http://127.0.0.1:6060/local/content-bundle \
  -H 'Content-Type: application/json' \
  --data @bundle.json
