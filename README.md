...
- [~] First combat slice: live target selection for a visible `training_dummy`, exact `ATTACK` ownership, and the first selected-target normal-attack refresh loop are now live
...
| M4 — Combat vertical slice | [~] | Target selection is live, the exact `ATTACK` codec is owned, and the first selected-target normal-attack refresh loop now reuses `GC TARGET`; hit, damage, death, and respawn still need the first end-to-end slice. |
