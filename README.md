...
| Inventory | [~] | The first protocol-owned self bootstrap contract is now frozen in `spec/protocol/inventory-equipment-bootstrap.md`, `internal/proto/item` now owns the first `ITEM_SET` / `ITEM_DEL` golden packet slice for that bootstrap family, `internal/inventory` now owns stable carried-slot identity plus a first `ItemInstance` validation seam, both `internal/accountstore` plus `internal/loginticket` now persist explicit `gold`, `inventory`, and `equipment` snapshot fields with backwards-compatible JSON normalization, `internal/player` now owns live carried-item move/equip/unequip seams without silently mutating persisted snapshots, the first carried-slot move/swap mutation loop now persists writeback and emits deterministic self-only `ITEM_DEL` / `ITEM_SET` refresh packets through the bootstrap slash-command seam, and loopback-only local introspection endpoints now expose that live runtime state by exact character name before broader client refresh slices land. |
| Equipment | [~] | The first protocol-owned worn-slot surface is now frozen in docs, `internal/inventory` now owns deterministic named `EquipmentSlot` value objects, persisted character snapshots now keep explicit equipped-item state instead of leaving worn slots implicit, `internal/player` now owns live equip/unequip state plus deterministic equipment ordering, and loopback-only `GET /local/equipment/{name}` now exposes the selected character's live worn-state snapshot for local debugging, but self refresh packets and appearance integration are still ahead. |
...
| M3 — Character systems | [~] | The first inventory/equipment bootstrap contract is now frozen in project docs and packet matrix, `internal/proto/item` now owns the first exact `ITEM_SET` / `ITEM_DEL` golden slice, `internal/inventory` owns the first slot/equipment/item value objects, persisted account/ticket snapshots now carry explicit `gold`, `inventory`, and `equipment` state with backwards-compatible JSON normalization, `internal/player` now owns the first live currency/inventory/equipment mutation boundary with explicit save-back, the first carried-slot move/swap loop now persists writeback and refreshes the client with deterministic self-only `ITEM_DEL` / `ITEM_SET` packets through the bootstrap slash-command seam, and loopback-only exact-name introspection endpoints now make that live M3 state observable before broader equip/use client flows land. |
