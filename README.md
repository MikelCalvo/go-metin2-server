...
- [~] First combat slice: live target selection for a visible `training_dummy`, exact `ATTACK` ownership, and the first deterministic selected-target HP refresh loop are now live; this dummy HP is still runtime-owned only, not a persistence contract yet
...
| M4 — Combat vertical slice | [~] | Target selection is live, the exact `ATTACK` codec is owned, and the first selected-target normal-attack loop now decrements bootstrap dummy HP while reusing `GC TARGET`; that HP is still shared-world runtime state only, while death, clear-target end-to-end, and respawn remain for the next slices. |
