...
- A deterministic file-backed interaction-definition store now exists in `internal/interactionstore` for minimal `info` / `talk` content keyed by stable `kind + ref`, `gamed` now loads that catalog at boot when present, and static actors that point at missing definitions now fail closed at boot plus on runtime create/update paths.
...
| Bootstrap interaction definitions | [~] | A deterministic file-backed store now exists under `internal/interactionstore` for minimal `info` / `talk` content keyed by `kind + ref`, and `gamed` now loads that catalog at boot and uses it to fail closed on missing static-actor interaction refs, but live interaction responses and operator authoring are still ahead. |
