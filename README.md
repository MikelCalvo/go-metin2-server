...
| Movement replication | [~] | `MOVE` and `SYNC_POSITION` fan out to already-visible peers on the same bootstrap `MapIndex`; when opt-in radius AOI is configured, `MOVE` now also rebuilds peer visibility with add/delete transitions if the move crosses the visible-range boundary. |
