...
The first consumable item-use vertical is now no longer slash-only: the bootstrap runtime also owns one tiny carried-slot `ITEM_USE` client packet ingress that reuses the same template-backed self-only point/item/info response path as `/use_item <slot>`. The packet `SHOP BUY` failure path is now a little closer to the live merchant family too: insufficient-gold and no-valid-placement packet buys return bare self-only `GC::SHOP NOT_ENOUGH_MONEY` / `GC::SHOP INVENTORY_FULL`, while the temporary local `/shop_buy <slot>` debug harness still keeps its placeholder info-chat failures.
