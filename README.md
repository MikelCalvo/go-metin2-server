...
- A transport-only `internal/worldruntime` session directory boundary for queued frame sinks and relocate callbacks, mirrored from the bootstrap shared-world runtime ahead of full routing through that directory.
...
- `internal/worldruntime` — bootstrap topology, visibility helpers, and the first generic entity/runtime seams (entity registry + player directory + map index + transport-only session directory today; shared-world routing through that directory next)
