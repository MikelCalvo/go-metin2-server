...
| Inventory | [~] | The first protocol-owned self bootstrap contract is now frozen in `spec/protocol/inventory-equipment-bootstrap.md`, the first carried stack/merge rules are now also frozen in `spec/protocol/item-stack-bootstrap.md`, `internal/proto/item` now owns the first `ITEM_SET` / `ITEM_DEL` golden packet slice for that bootstrap family, `internal/inventory` now owns stable carried-slot identity plus a first `ItemInstance` validation seam, both `internal/accountstore` plus `internal/loginticket` now persist explicit `gold`, `inventory`, and `equipment` snapshot fields with backwards-compatible JSON normalization, `internal/player` now owns live carried-item move/equip/unequip seams without silently mutating persisted snapshots, the first template-driven merchant-grant path can now prefer a compatible carried-stack merge, optionally split one stackable grant across that existing stack plus one fresh slot, or claim a fresh slot deterministically, bootstrap slash-command seams now persist writeback for carried-slot move/swap plus worn/carried equip transitions and emit deterministic self-only `ITEM_DEL` / `ITEM_SET` refresh packets, and loopback-only local introspection endpoints now expose that live runtime state by exact character name before broader client refresh slices land. |
...
| Bootstrap item templates | [~] | A deterministic file-backed store now exists under `internal/itemstore` for stable per-`vnum` template metadata (`name`, `stackable`, `max_count`, optional `equip_slot`), that metadata now also anchors the first frozen carried-stack/merchant-grant placement contract in `spec/protocol/item-stack-bootstrap.md`, and `gamed` now loads the catalog at boot to validate and render structured merchant previews while also driving the first live merchant-grant full-merge / split-merge / fresh-slot decision in runtime. |
