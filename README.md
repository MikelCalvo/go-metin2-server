...
That same retaliation-driven `0`-HP floor now also removes the still-connected dead owner from later fresh peer-join visibility delivery: other live recipients still receive the newcomer’s ordinary `CHARACTER_ADD` / `CHAR_ADDITIONAL_INFO` / `CHARACTER_UPDATE` burst, but the zero-HP owner receives no queued peer-entry frames from that join path.
