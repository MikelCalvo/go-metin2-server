...
The same floor now also removes that still-connected owner from later local `CHAT_TYPE_TALKING` recipient fanout: visible peers still keep their ordinary self echo, but the zero-HP owner receives no queued local-chat delivery.
