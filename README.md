...
- client packet ownership for `ITEM_GIVE`, `EXCHANGE`, and `REFINE`, including the first visible-peer exchange open/cancel/busy-target shell and display-only exchange item-add frames while item removal/gold/accept/refine-result semantics remain intentionally narrow or fail-closed.
