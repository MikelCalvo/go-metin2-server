...
- A first documented bootstrap reconnect/teardown runtime contract: close/disconnect tears down session-directory hooks, entity ownership, and map occupancy idempotently, reconnect rebuilds fresh live runtime state from persisted snapshots instead of stale in-memory ownership, and a duplicate fresh `ENTERGAME` for the same still-live selected character is now rejected instead of creating a ghost `GAME` session with no shared-world registration.
