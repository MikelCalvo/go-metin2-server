-- go-metin2 migration: 0012 static_actor_pve_interaction_state down
DROP TABLE interaction_quest_flag_consume_items;
DROP TABLE interaction_quest_flag_reward_items;

CREATE TABLE interaction_definitions_mig8 (
    kind TEXT NOT NULL,
    ref TEXT NOT NULL,
...
INSERT INTO interaction_definitions_mig8 (
...
WHERE kind IN ('info', 'talk', 'warp', 'shop_preview');

CREATE TABLE interaction_merchant_catalog_entries_mig8 (
    ...
    FOREIGN KEY (definition_kind, definition_ref) REFERENCES interaction_definitions_mig8(kind, ref),
...
INSERT INTO interaction_merchant_catalog_entries_mig8 (
...
WHERE definition_kind = 'shop_preview'
  AND EXISTS (
      SELECT 1
      FROM interaction_definitions_mig8 defs
      WHERE defs.kind = interaction_merchant_catalog_entries.definition_kind
        AND defs.ref = interaction_merchant_catalog_entries.definition_ref
  );

CREATE TABLE static_actors_mig8 (
    ...
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (interaction_kind, interaction_ref) REFERENCES interaction_definitions_mig8(kind, ref),
    CHECK (entity_id > 0),
    CHECK (name <> ''),
    CHECK (map_index > 0),
...
INSERT INTO static_actors_mig8 (
...
SELECT
    ...
    combat_profile,
    CASE
        WHEN interaction_kind IN ('info', 'talk', 'warp', 'shop_preview') THEN interaction_kind
        ELSE NULL
    END,
    CASE
        WHEN interaction_kind IN ('info', 'talk', 'warp', 'shop_preview') THEN interaction_ref
        ELSE NULL
    END,
    spawn_group_ref,
    reward_experience, reward_gold, created_at, updated_at
...
CREATE TABLE static_actor_reward_drops_mig8 (
    ...
    FOREIGN KEY (entity_id) REFERENCES static_actors_mig8(entity_id),
    CHECK (position >= 0 AND position < 255),
    CHECK (item_vnum > 0 AND item_vnum <= 4294967295)
);

INSERT INTO static_actor_reward_drops_mig8 (
...
WHERE EXISTS (
    SELECT 1
    FROM static_actors_mig8 actors
    WHERE actors.entity_id = static_actor_reward_drops.entity_id
);
...
ALTER TABLE interaction_definitions_mig8 RENAME TO interaction_definitions;
ALTER TABLE interaction_merchant_catalog_entries_mig8 RENAME TO interaction_merchant_catalog_entries;
ALTER TABLE static_actors_mig8 RENAME TO static_actors;
ALTER TABLE static_actor_reward_drops_mig8 RENAME TO static_actor_reward_drops;
